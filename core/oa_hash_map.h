...
/**
 * OAHashMap - Open Addressing HashMap with Robin Hood Hashing
 * 
 * A high-performance HashMap implementation using open addressing with Robin Hood hashing.
 * This data structure is optimized for both insertion and lookup operations.
 * 
 * Key Features:
 * - Robin Hood Hashing: Balances probe distances by allowing swaps during insertion,
 *   resulting in more uniform distribution and faster average lookups
 * - Backward Shift Deletion: Improves deletion performance and prevents infinite loops
 * - In-place Storage: Keys and values are stored directly in arrays, maximizing cache efficiency
 * 
 * Template Parameters:
 * @tparam TKey The type of keys stored in the map
 * @tparam TValue The type of values associated with each key
 * @tparam Hasher Hash function class (default: HashMapHasherDefault)
 * @tparam Comparator Key comparison class (default: HashMapComparatorDefault<TKey>)
 * 
 * Performance Characteristics:
 * - Average insertion: O(1)
 * - Average lookup: O(1)
 * - Worst-case operations: O(n)
 * - Automatic resizing occurs when load factor exceeds 0.9
 * 
 * Thread Safety: Not thread-safe. External synchronization required for concurrent access.
 * 
 * Copyability: Non-copyable and non-assignable to prevent dangling pointers.
 */
