...
/**
 * @class Math
 * @brief Collection of static inline mathematical utility functions.
 *
 * Provides thin wrappers over standard trigonometric, exponential, logarithmic,
 * and power functions for both single- and double-precision types, along with
 * helpers for angle conversions, interpolation, easing, wrapping, snapping, and
 * approximate comparisons. Includes utilities for random number generation,
 * special value checks (NaN/Inf), fast absolute value conversions, half-float
 * conversions, and optimized float-to-int conversion for cross-platform use.
 */
#ifndef MATH_FUNCS_H
#define MATH_FUNCS_H
...
// use an inline implementation of isinf as a workaround for problematic
// libstdc++ versions from gcc 5.x era
#elif defined(__GNUC__) && __GNUC__ < 6
    union {
      uint64_t u;
      double f;
...
// libstdc++ versions from gcc 5.x era
  ...
  static _ALWAYS_INLINE_ double lerp(double p_from, double p_to,
                                     double p_weight) {
  ...
  static _ALWAYS_INLINE_ double lerp_angle(double p_from, double p_to,
                                           double p_weight) {
    double difference = fmod(p_to - p_from, Math_TAU);
    double distance = fmod(2.0 * difference, Math_TAU) - difference;
    return p_from + distance * p_weight;
  }
  static _ALWAYS_INLINE_ float lerp_angle(float p_from, float p_to,
                                          float p_weight) {
  ...
  static _ALWAYS_INLINE_ double inverse_lerp(double p_from, double p_to,
                                             double p_value) {
    return (p_value - p_from) / (p_to - p_from);
  }
  static _ALWAYS_INLINE_ float inverse_lerp(float p_from, float p_to,
                                            float p_value) {
    return (p_value - p_from) / (p_to - p_from);
  }

  static _ALWAYS_INLINE_ double range_lerp(double p_value, double p_istart,
                                           double p_istop, double p_ostart,
                                           double p_ostop) {
    return Math::lerp(p_ostart, p_ostop,
                      Math::inverse_lerp(p_istart, p_istop, p_value));
  }
  static _ALWAYS_INLINE_ float range_lerp(float p_value, float p_istart,
                                          float p_istop, float p_ostart,
                                          float p_ostop) {
    return Math::lerp(p_ostart, p_ostop,
                      Math::inverse_lerp(p_istart, p_istop, p_value));
  }

  static _ALWAYS_INLINE_ double smoothstep(double p_from, double p_to,
                                           double p_weight) {
  ...
  static _ALWAYS_INLINE_ float smoothstep(float p_from, float p_to,
                                          float p_weight) {
    if (is_equal_approx(p_from, p_to))
      return p_from;
  ...
  static _ALWAYS_INLINE_ double move_toward(double p_from, double p_to,
                                            double p_delta) {
    return abs(p_to - p_from) <= p_delta
               ? p_to
               : p_from + SGN(p_to - p_from) * p_delta;
  }
  static _ALWAYS_INLINE_ float move_toward(float p_from, float p_to,
                                           float p_delta) {
    return abs(p_to - p_from) <= p_delta
               ? p_to
               : p_from + SGN(p_to - p_from) * p_delta;
  ...
  static _ALWAYS_INLINE_ int64_t wrapi(int64_t value, int64_t min,
                                       int64_t max) {
  ...
  // double only, as these functions are mainly used by the editor and not
  ...
  static _ALWAYS_INLINE_ bool
  is_equal_approx_ratio(real_t a, real_t b, real_t epsilon = CMP_EPSILON,
                        real_t min_epsilon = CMP_EPSILON) {
    // this is an approximate way to check that numbers are close, as a ratio of
    // their average size helps compare approximate numbers that may be very big
    // or very small
    real_t diff = abs(a - b);
  ...
  // this function should be as fast as possible and rounding mode should not
  // matter
  static _ALWAYS_INLINE_ int fast_ftoi(float a) {
    ...
    WINAPI_FAMILY == WINAPI_FAMILY_PHONE_APP // windows 8 phone?
...
*/
  ...
  static _ALWAYS_INLINE_ float snap_scalar(float p_offset, float p_step,
                                           float p_target) {
    return p_step != 0 ? Math::stepify(p_target - p_offset, p_step) + p_offset
                       : p_target;
  }

  static _ALWAYS_INLINE_ float snap_scalar_separation(float p_offset,
                                                      float p_step,
                                                      float p_target,
                                                      float p_separation) {
    if (p_step != 0) {
      float a =
          Math::stepify(p_target - p_offset, p_step + p_separation) + p_offset;
      float b = a;
      if (p_target >= 0)
        b -= p_separation;
      else
