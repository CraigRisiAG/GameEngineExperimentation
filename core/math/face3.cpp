...
/**
 * @file face3.cpp
 * @brief Implements geometric operations for a triangular face.
 *
 * This file provides the `Face3` methods for splitting a triangle by a plane,
 * testing intersections with rays, segments, and AABBs, computing area, normal
 * orientation, median and random points, projecting ranges for collision tests,
 * support mapping for physics engines, and finding the closest point on the
 * triangle to an arbitrary point. It also includes helpers to classify the
 * triangle relative to another face or plane and to check for degeneracy.
 */
#include "face3.h"

#include "core/math/geometry.h"

int Face3::split_by_plane(const Plane &p_plane, Face3 p_res[3],
                          bool p_is_point_over[3]) const {
...
bool Face3::intersects_ray(const Vector3 &p_from, const Vector3 &p_dir,
                           Vector3 *p_intersection) const {

  return Geometry::ray_intersects_triangle(p_from, p_dir, vertex[0], vertex[1],
                                           vertex[2], p_intersection);
}

bool Face3::intersects_segment(const Vector3 &p_from, const Vector3 &p_dir,
                               Vector3 *p_intersection) const {

  return Geometry::segment_intersects_triangle(
      p_from, p_dir, vertex[0], vertex[1], vertex[2], p_intersection);
}
...
Face3::Side Face3::get_side_of(const Face3 &p_face,
                               ClockDirection p_clock_dir) const {
...
void Face3::project_range(const Vector3 &p_normal, const Transform &p_transform,
...
void Face3::get_support(const Vector3 &p_normal, const Transform &p_transform,
                        Vector3 *p_vertices, int *p_count, int p_max) const {
