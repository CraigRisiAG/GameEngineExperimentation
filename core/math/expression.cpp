...
/*
 * This file implements the Expression class, providing parsing and execution of
 * dynamically defined expressions supporting built-in math, logic, type
 * conversion, text utilities, and object referencing functions. It includes a
 * tokenizer and recursive-descent parser to build an expression tree,
 * validation of argument counts, and runtime evaluation against provided inputs
 * and an optional base instance. Errors during parsing or execution are
 * captured with descriptive messages to aid debugging. Public APIs include
 * parsing expressions, executing them with inputs, and querying execution
...
void Expression::exec_func(BuiltinFunc p_func, const Variant **p_inputs,
  ...
  case MATH_RANGE_LERP: {
    ...
    VALIDATE_ARG_NUM(3);
    VALIDATE_ARG_NUM(4);
    *r_return = Math::range_lerp((double)*p_inputs[0], (double)*p_inputs[1],
  ...
  case MATH_DECTIME: {
    ...
    VALIDATE_ARG_NUM(1);
    VALIDATE_ARG_NUM(2);
    *r_return = Math::dectime((double)*p_inputs[0], (double)*p_inputs[1],
                              (double)*p_inputs[2]);
  } break;
  case MATH_RANDOMIZE: {
    Math::randomize();
...
#define GET_CHAR() (str_ofs >= expression.length() ? 0 : expression[str_ofs++])
    ...
    default: {
      ...
      CharType next_char =
          (str_ofs >= expression.length()) ? 0 : expression[str_ofs];
...
const char *Expression::token_name[TK_MAX] = {"CURLY BRACKET OPEN",
                                              "CURLY BRACKET CLOSE",
                                              "BRACKET OPEN",
                                              "BRACKET CLOSE",
                                              "PARENTHESIS OPEN",
...
Expression::ENode *Expression::_parse_expression() {
  ...
  while (true) {
    ...
    case TK_BUILTIN_FUNC: {
      ...
      if (bifunc->arguments.size() != expected_args) {
        _set_error("Builtin func '" + get_func_name(bifunc->func) +
                   "' expects " + itos(expected_args) + " arguments.");
      }
  ...
  while (expression.size() > 1) {
    ...
    if (is_unary) {
      ...
      op->nodes[1] =
          expression[next_op + 1].node; // next expression goes as right
...
bool Expression::_execute(const Array &p_inputs, Object *p_instance,
                          Expression::ENode *p_node, Variant &r_ret,
                          String &r_error_str) {

  switch (p_node->type) {
  case Expression::ENode::TYPE_INPUT: {

    const Expression::InputNode *in =
        static_cast<const Expression::InputNode *>(p_node);
    if (in->index < 0 || in->index >= p_inputs.size()) {
      r_error_str = vformat(RTR("Invalid input %i (not passed) in expression"),
                            in->index);
      return true;
    }
  ...
  case Expression::ENode::TYPE_CONSTANT: {
    ...
    const Expression::ConstantNode *c =
        static_cast<const Expression::ConstantNode *>(p_node);
    r_ret = c->value;
  ...
  case Expression::ENode::TYPE_SELF: {
    ...
    if (!p_instance) {
      r_error_str =
          RTR("self can't be used because instance is null (not passed)");
      return true;
    }
    r_ret = p_instance;
  } break;
  case Expression::ENode::TYPE_OPERATOR: {
    ...
    const Expression::OperatorNode *op =
        static_cast<const Expression::OperatorNode *>(p_node);
    ...
    if (!valid) {
      r_error_str = vformat(RTR("Invalid operands to operator %s, %s and %s."),
  ...
  case Expression::ENode::TYPE_INDEX: {
    ...
    const Expression::IndexNode *index =
        static_cast<const Expression::IndexNode *>(p_node);
    ...
    if (!valid) {
      r_error_str = vformat(RTR("Invalid index of type %s for base type %s"),
                            Variant::get_type_name(idx.get_type()),
                            Variant::get_type_name(base.get_type()));
      return true;
    }
  ...
  case Expression::ENode::TYPE_NAMED_INDEX: {
    ...
    const Expression::NamedIndexNode *index =
        static_cast<const Expression::NamedIndexNode *>(p_node);
    ...
    if (!valid) {
      r_error_str =
          vformat(RTR("Invalid named index '%s' for base type %s"),
                  String(index->name), Variant::get_type_name(base.get_type()));
      return true;
    }

  } break;
  case Expression::ENode::TYPE_ARRAY: {
    const Expression::ArrayNode *array =
        static_cast<const Expression::ArrayNode *>(p_node);
    ...
    for (int i = 0; i < array->array.size(); i++) {
      ...
      Variant value;
      bool ret =
          _execute(p_inputs, p_instance, array->array[i], value, r_error_str);
  ...
  case Expression::ENode::TYPE_DICTIONARY: {
    const Expression::DictionaryNode *dictionary =
        static_cast<const Expression::DictionaryNode *>(p_node);

    Dictionary d;
    for (int i = 0; i < dictionary->dict.size(); i += 2) {

      Variant key;
      bool ret = _execute(p_inputs, p_instance, dictionary->dict[i + 0], key,
                          r_error_str);
      ...
      Variant value;
      ret = _execute(p_inputs, p_instance, dictionary->dict[i + 1], value,
                     r_error_str);
      if (ret)
        return true;
  ...
  case Expression::ENode::TYPE_CONSTRUCTOR: {
    ...
    const Expression::ConstructorNode *constructor =
        static_cast<const Expression::ConstructorNode *>(p_node);
    ...
    for (int i = 0; i < constructor->arguments.size(); i++) {
      ...
      Variant value;
      bool ret = _execute(p_inputs, p_instance, constructor->arguments[i],
                          value, r_error_str);
    ...
    r_ret = Variant::construct(constructor->data_type,
                               (const Variant **)argp.ptr(), argp.size(), ce);

    if (ce.error != Callable::CallError::CALL_OK) {
      r_error_str = vformat(RTR("Invalid arguments to construct '%s'"),
  ...
  case Expression::ENode::TYPE_BUILTIN_FUNC: {
    ...
    const Expression::BuiltinFuncNode *bifunc =
        static_cast<const Expression::BuiltinFuncNode *>(p_node);
    ...
    for (int i = 0; i < bifunc->arguments.size(); i++) {
      ...
      Variant value;
      bool ret = _execute(p_inputs, p_instance, bifunc->arguments[i], value,
                          r_error_str);
      if (ret)
        return true;
      arr.write[i] = value;
    ...
    Callable::CallError ce;
    exec_func(bifunc->func, (const Variant **)argp.ptr(), &r_ret, ce,
              r_error_str);
  ...
  case Expression::ENode::TYPE_CALL: {
    ...
    const Expression::CallNode *call =
        static_cast<const Expression::CallNode *>(p_node);
    ...
    for (int i = 0; i < call->arguments.size(); i++) {
      ...
      Variant value;
      ret = _execute(p_inputs, p_instance, call->arguments[i], value,
                     r_error_str);
    ...
    Callable::CallError ce;
    r_ret =
        base.call(call->method, (const Variant **)argp.ptr(), argp.size(), ce);
...
Error Expression::parse(const String &p_expression,
                        const Vector<String> &p_input_names) {
...
Variant Expression::execute(Array p_inputs, Object *p_base, bool p_show_error) {
  ...
  ERR_FAIL_COND_V_MSG(error_set, Variant(),
                      "There was previously a parse error: " + error_str + ".");
...
void Expression::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("parse", "expression", "input_names"),
                       &Expression::parse, DEFVAL(Vector<String>()));
  ClassDB::bind_method(
      D_METHOD("execute", "inputs", "base_instance", "show_error"),
      &Expression::execute, DEFVAL(Array()), DEFVAL(Variant()), DEFVAL(true));
  ClassDB::bind_method(D_METHOD("has_execute_failed"),
                       &Expression::has_execute_failed);
  ClassDB::bind_method(D_METHOD("get_error_text"), &Expression::get_error_text);
}

Expression::Expression()
    : output_type(Variant::NIL), sequenced(false), error_set(true), root(NULL),
      nodes(NULL), execution_error(false) {}
