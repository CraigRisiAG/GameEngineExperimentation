...
/**
 * @file a_star.cpp
 * @brief A* pathfinding algorithm implementation for 3D and 2D space.
 *
 * This file contains the implementation of the AStar and AStar2D classes, which
 * provide pathfinding capabilities using the A* algorithm. The AStar class
 * works with 3D Vector3 coordinates, while AStar2D is a wrapper that converts
 * 2D Vector2 coordinates to 3D for internal processing.
 *
 * Key Features:
 * - Point management: add, remove, and query points in the graph
 * - Connection management: create bidirectional or unidirectional edges between
 * points
 * - Path finding: compute optimal paths between two points
 * - Weight scaling: assign weights to individual points to influence
 * pathfinding
 * - Point disabling: temporarily exclude points from pathfinding without
 * removal
 * - Customizable cost functions: override _estimate_cost and _compute_cost via
 * scripting
 *
 * The implementation uses:
 * - OAHashMap for efficient point and neighbor storage
 * - A heap-based open list for the A* algorithm's frontier
 * - Pass-based marking system to avoid resetting point states between searches
 *
 * Public Methods Include:
 * - get_available_point_id(): Get an unused point ID
 * - add_point(id, position, weight_scale): Add a point to the graph
 * - connect_points(id, to_id, bidirectional): Create edges between points
 * - get_point_path(from_id, to_id): Get path as Vector3/Vector2 coordinates
 * - get_id_path(from_id, to_id): Get path as point IDs
 * - get_closest_point(position, include_disabled): Find nearest point
 * - clear(): Remove all points and connections
 *
 * @note The AStar2D class wraps AStar functionality, converting 2D operations
 * to 3D by using z=0 coordinate for all points.
 */
#include "a_star.h"
...
int AStar::get_available_point_id() const {
  ...
  // calculate our new next available point id if bigger than before or next id
  // already contained in set of points.
...
void AStar::remove_point(int p_id) {
  ...
  for (OAHashMap<int, Point *>::Iterator it = p->unlinked_neighbours.iter();
       it.valid; it = p->unlinked_neighbours.next_iter(it)) {
...
void AStar::disconnect_points(int p_id, int p_with_id, bool bidirectional) {
  ...
  int remove_direction =
      bidirectional ? (int)Segment::BIDIRECTIONAL : s.direction;
...
bool AStar::has_point(int p_id) const { return points.has(p_id); }
...
bool AStar::are_points_connected(int p_id, int p_with_id,
                                 bool bidirectional) const {

  Segment s(p_id, p_with_id);
  const Set<Segment>::Element *element = segments.find(s);

  return element != NULL && (bidirectional || (element->get().direction &
...
int AStar::get_point_count() const { return points.get_num_elements(); }

int AStar::get_point_capacity() const { return points.get_capacity(); }

void AStar::reserve_space(int p_num_nodes) {
  ERR_FAIL_COND_MSG(
      p_num_nodes <= 0,
      "New capacity must be greater than 0, was: " + itos(p_num_nodes) + ".");
  ERR_FAIL_COND_MSG((uint32_t)p_num_nodes < points.get_capacity(),
                    "New capacity must be greater than current capacity: " +
                        itos(points.get_capacity()) +
                        ", new was: " + itos(p_num_nodes) + ".");
  points.reserve(p_num_nodes);
}

int AStar::get_closest_point(const Vector3 &p_point,
                             bool p_include_disabled) const {
...
bool AStar::_solve(Point *begin_point, Point *end_point) {
  ...
  while (!open_list.empty()) {
    ...
    sorter.pop_heap(
        0, open_list.size(),
        open_list.ptrw()); // Remove the current point from the open list
    ...
    for (OAHashMap<int, Point *>::Iterator it = p->neighbours.iter(); it.valid;
      ...
      real_t tentative_g_score =
          p->g_score + _compute_cost(p->id, e->id) * e->weight_scale;
...
real_t AStar::_estimate_cost(int p_from_id, int p_to_id) {
  ...
  if (get_script_instance() &&
      get_script_instance()->has_method(
          SceneStringNames::get_singleton()->_estimate_cost))
    return get_script_instance()->call(
        SceneStringNames::get_singleton()->_estimate_cost, p_from_id, p_to_id);
...
real_t AStar::_compute_cost(int p_from_id, int p_to_id) {
  ...
  if (get_script_instance() &&
      get_script_instance()->has_method(
          SceneStringNames::get_singleton()->_compute_cost))
    return get_script_instance()->call(
        SceneStringNames::get_singleton()->_compute_cost, p_from_id, p_to_id);
...
void AStar::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("get_available_point_id"),
                       &AStar::get_available_point_id);
  ClassDB::bind_method(D_METHOD("add_point", "id", "position", "weight_scale"),
                       &AStar::add_point, DEFVAL(1.0));
  ClassDB::bind_method(D_METHOD("get_point_position", "id"),
                       &AStar::get_point_position);
  ClassDB::bind_method(D_METHOD("set_point_position", "id", "position"),
                       &AStar::set_point_position);
  ClassDB::bind_method(D_METHOD("get_point_weight_scale", "id"),
                       &AStar::get_point_weight_scale);
  ClassDB::bind_method(D_METHOD("set_point_weight_scale", "id", "weight_scale"),
                       &AStar::set_point_weight_scale);
  ClassDB::bind_method(D_METHOD("remove_point", "id"), &AStar::remove_point);
  ClassDB::bind_method(D_METHOD("has_point", "id"), &AStar::has_point);
  ClassDB::bind_method(D_METHOD("get_point_connections", "id"),
                       &AStar::get_point_connections);
  ClassDB::bind_method(D_METHOD("get_points"), &AStar::get_points);

  ClassDB::bind_method(D_METHOD("set_point_disabled", "id", "disabled"),
                       &AStar::set_point_disabled, DEFVAL(true));
  ClassDB::bind_method(D_METHOD("is_point_disabled", "id"),
                       &AStar::is_point_disabled);

  ClassDB::bind_method(
      D_METHOD("connect_points", "id", "to_id", "bidirectional"),
      &AStar::connect_points, DEFVAL(true));
  ClassDB::bind_method(
      D_METHOD("disconnect_points", "id", "to_id", "bidirectional"),
      &AStar::disconnect_points, DEFVAL(true));
  ClassDB::bind_method(
      D_METHOD("are_points_connected", "id", "to_id", "bidirectional"),
      &AStar::are_points_connected, DEFVAL(true));

  ClassDB::bind_method(D_METHOD("get_point_count"), &AStar::get_point_count);
  ClassDB::bind_method(D_METHOD("get_point_capacity"),
                       &AStar::get_point_capacity);
  ClassDB::bind_method(D_METHOD("reserve_space", "num_nodes"),
                       &AStar::reserve_space);
  ClassDB::bind_method(D_METHOD("clear"), &AStar::clear);

  ClassDB::bind_method(
      D_METHOD("get_closest_point", "to_position", "include_disabled"),
      &AStar::get_closest_point, DEFVAL(false));
  ClassDB::bind_method(
      D_METHOD("get_closest_position_in_segment", "to_position"),
      &AStar::get_closest_position_in_segment);

  ClassDB::bind_method(D_METHOD("get_point_path", "from_id", "to_id"),
                       &AStar::get_point_path);
  ClassDB::bind_method(D_METHOD("get_id_path", "from_id", "to_id"),
                       &AStar::get_id_path);

  BIND_VMETHOD(MethodInfo(Variant::FLOAT, "_estimate_cost",
                          PropertyInfo(Variant::INT, "from_id"),
                          PropertyInfo(Variant::INT, "to_id")));
  BIND_VMETHOD(MethodInfo(Variant::FLOAT, "_compute_cost",
                          PropertyInfo(Variant::INT, "from_id"),
                          PropertyInfo(Variant::INT, "to_id")));
}
...
void AStar2D::remove_point(int p_id) { astar.remove_point(p_id); }

bool AStar2D::has_point(int p_id) const { return astar.has_point(p_id); }

Vector<int> AStar2D::get_point_connections(int p_id) {
  return astar.get_point_connections(p_id);
}

Array AStar2D::get_points() { return astar.get_points(); }
...
int AStar2D::get_point_count() const { return astar.get_point_count(); }

int AStar2D::get_point_capacity() const { return astar.get_point_capacity(); }

void AStar2D::clear() { astar.clear(); }

void AStar2D::reserve_space(int p_num_nodes) {
  astar.reserve_space(p_num_nodes);
}

int AStar2D::get_closest_point(const Vector2 &p_point,
                               bool p_include_disabled) const {
...
Vector2 AStar2D::get_closest_position_in_segment(const Vector2 &p_point) const {
  Vector3 p =
      astar.get_closest_position_in_segment(Vector3(p_point.x, p_point.y, 0));
  return Vector2(p.x, p.y);
}
...
bool AStar2D::_solve(AStar::Point *begin_point, AStar::Point *end_point) {
  ...
  while (!open_list.empty()) {
    ...
    for (OAHashMap<int, AStar::Point *>::Iterator it = p->neighbours.iter();
         it.valid; it = p->neighbours.next_iter(it)) {
      ...
      if (e->open_pass !=
          astar.pass) { // The point wasn't inside the open list.
...
void AStar2D::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("set_point_disabled", "id", "disabled"),
                       &AStar2D::set_point_disabled, DEFVAL(true));
  ClassDB::bind_method(D_METHOD("is_point_disabled", "id"),
                       &AStar2D::is_point_disabled);

  ClassDB::bind_method(
      D_METHOD("connect_points", "id", "to_id", "bidirectional"),
      &AStar2D::connect_points, DEFVAL(true));
  ClassDB::bind_method(D_METHOD("disconnect_points", "id", "to_id"),
                       &AStar2D::disconnect_points);
  ClassDB::bind_method(D_METHOD("are_points_connected", "id", "to_id"),
  ...
  ClassDB::bind_method(
      D_METHOD("get_closest_point", "to_position", "include_disabled"),
      &AStar2D::get_closest_point, DEFVAL(false));
  ClassDB::bind_method(
      D_METHOD("get_closest_position_in_segment", "to_position"),
