...
/**
 * @file geometry.cpp
 * @brief Provides geometry utilities for mesh processing, voxel wrapping,
 * polygon operations, and shape construction.
 *
 * This module includes:
 * - MeshData optimization, face connectivity classification, and separation of
 * disconnected objects.
 * - Voxelization-based geometry wrapping to generate enclosing faces around 3D
 * meshes.
 * - Convex decomposition, polygon boolean/offset operations (via Clipper), and
 * atlas packing.
 * - Construction of convex meshes and primitive plane sets for boxes,
 * cylinders, spheres, and capsules.
 *
 * All functions rely on Godot's math primitives (Vector2, Vector3, Plane,
 * Face3) and helper utilities, offering robust computational geometry support
 * for collision, rendering, and editor tooling.
 */
#include "geometry.h"
...
// This implementation is very inefficient, commenting unless bugs happen. See
// the other one.
/*
bool Geometry::is_point_in_polygon(const Vector2 &p_point, const Vector<Vector2>
&p_polygon) {

        Vector<int> indices = Geometry::triangulate_polygon(p_polygon);
        for (int j = 0; j + 3 <= indices.size(); j += 3) {
                int i1 = indices[j], i2 = indices[j + 1], i3 = indices[j + 2];
                if (Geometry::is_point_in_triangle(p_point, p_polygon[i1],
p_polygon[i2], p_polygon[i3])) return true;
        }
        return false;
}
*/
...
static bool _connect_faces(_FaceClassify *p_faces, int len, int p_group) {
  // Connect faces, error will occur if an edge is shared between more than 2
  // faces. Clear connections.
...
static bool _group_face(_FaceClassify *p_faces, int len, int p_index,
                        int p_group) {
...
static inline void _plot_face(uint8_t ***p_cell_status, int x, int y, int z,
                              int len_x, int len_y, int len_z,
                              const Vector3 &voxelsize, const Face3 &p_face) {
  ...
  for (int i = 0; i < div_x; i++) {
    ...
    for (int j = 0; j < div_y; j++) {
      ...
      for (int k = 0; k < div_z; k++) {
        ...
        _plot_face(p_cell_status, new_x, new_y, new_z, new_len_x, new_len_y,
                   new_len_z, voxelsize, p_face);
      }
    }
  }
}

static inline void _mark_outside(uint8_t ***p_cell_status, int x, int y, int z,
                                 int len_x, int len_y, int len_z) {
...
static inline void _build_faces(uint8_t ***p_cell_status, int x, int y, int z,
                                int len_x, int len_y, int len_z,
                                Vector<Face3> &p_faces) {
...
#define vert(m_idx) Vector3(((m_idx) & 4) >> 2, ((m_idx) & 2) >> 1, (m_idx) & 1)
  ...
  for (int i = 0; i < 6; i++) {
    ...
    p_faces.push_back(Face3(face_points[0], face_points[1], face_points[2]));

    p_faces.push_back(Face3(face_points[2], face_points[3], face_points[0]));
  }
}
...
#define _MAX_LENGTH 20
  ...
  // Determine which cells connect to the outside by traversing the outside and
  // recursively flood-fill marking.
...
Geometry::decompose_polygon_in_convex(Vector<Point2> polygon) {
...
Vector<Plane> Geometry::build_cylinder_planes(real_t p_radius, real_t p_height,
                                              int p_sides,
                                              Vector3::Axis p_axis) {
...
Vector<Plane> Geometry::build_sphere_planes(real_t p_radius, int p_lats,
                                            int p_lons, Vector3::Axis p_axis) {
  ...
  for (int i = 0; i < p_lons; i++) {
    ...
    for (int j = 1; j <= p_lats; j++) {
      ...
      Vector3 angle =
          normal.linear_interpolate(axis, j / (real_t)p_lats).normalized();
...
Vector<Plane> Geometry::build_capsule_planes(real_t p_radius, real_t p_height,
                                             int p_sides, int p_lats,
                                             Vector3::Axis p_axis) {
...
void Geometry::make_atlas(const Vector<Size2i> &p_rects,
                          Vector<Point2i> &r_result, Size2i &r_size) {

  // Super simple, almost brute force scanline stacking fitter.
  // It's pretty basic for now, but it tries to make sure that the aspect ratio
  // of the resulting atlas is somehow square. This is necessary because video
  // cards have limits. On texture size (usually 2048 or 4096), so the more
  // square a texture, the more chances. It will work in every hardware. For
  // example, it will prioritize a 1024x1024 atlas (works everywhere) instead of
  // a 256x8192 atlas (won't work anywhere).
  ...
  for (int i = 0; i <= 12; i++) {
    ...
    for (int j = 0; j < wrects.size(); j++) {
      ...
      if (ofs == 0 ||
          end_h > limit_h) // While h limit not reached, keep stacking.
...
Vector<Vector<Point2>> Geometry::_polypaths_do_operation(
    PolyBooleanOperation p_op, const Vector<Point2> &p_polypath_a,
    const Vector<Point2> &p_polypath_b, bool is_a_open) {
  ...
  for (int i = 0; i != p_polypath_a.size(); ++i) {
    path_a << IntPoint(p_polypath_a[i].x * SCALE_FACTOR,
                       p_polypath_a[i].y * SCALE_FACTOR);
  }
  for (int i = 0; i != p_polypath_b.size(); ++i) {
    path_b << IntPoint(p_polypath_b[i].x * SCALE_FACTOR,
                       p_polypath_b[i].y * SCALE_FACTOR);
  }
  Clipper clp;
  clp.AddPath(path_a, ptSubject,
              !is_a_open);           // Forward compatible with Clipper 10.0.0.
  ...
  for (Paths::size_type i = 0; i < paths.size(); ++i) {
    ...
    for (Paths::size_type j = 0; j < scaled_path.size(); ++j) {
      polypath.push_back(
          Point2(static_cast<real_t>(scaled_path[j].X) / SCALE_FACTOR,
...
Geometry::_polypath_offset(const Vector<Point2> &p_polypath, real_t p_delta,
                           PolyJoinType p_join_type, PolyEndType p_end_type) {
  ...
  for (int i = 0; i != p_polypath.size(); ++i) {
    path << IntPoint(p_polypath[i].x * SCALE_FACTOR,
                     p_polypath[i].y * SCALE_FACTOR);
  }
