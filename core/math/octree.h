...
/**
 * @brief A generic octree spatial partitioning container with optional pairing
 * support.
 *
 * This templated class stores elements associated with axis-aligned bounding
 * boxes (AABB) and organizes them in a hierarchical octree for efficient
 * spatial queries and collision checks.
 *
 * Features:
 * - Dynamic insertion, removal, and movement of elements while maintaining
 * octree consistency.
 * - Optional pairing system (`use_pairs`) to track intersecting element pairs
 * and invoke user-provided callbacks on pair/unpair events.
 * - Spatial queries (cull) for convex shapes, AABBs, line segments, and points,
 * returning matching userdata pointers.
 * - Configurable pairability per element, including type and mask filtering.
 *
 * Key Operations:
 * - create(): Insert a new element with userdata, bounding box, and pairing
 * configuration.
 * - move(): Update an element’s bounding box, reinserting it as needed.
 * - erase(): Remove an element from the octree.
 * - set_pairable(): Toggle or configure an element’s pairability.
 * - cull_convex(), cull_aabb(), cull_segment(), cull_point(): Collect elements
 * intersecting given shapes.
 *
 * Callbacks:
 * - set_pair_callback(): Called when a pair starts intersecting.
 * - set_unpair_callback(): Called when a pair stops intersecting.
 *
 * Notes:
 * - AABB validity is checked (limits and NaN) in debug builds.
 * - The octree expands dynamically up to a defined size limit to ensure all
 * elements are enclosed.
 * - Pair checks and unpairing are optimized using pass markers to avoid
 * redundant processing.
 *
 * Template Parameters:
 * - T: User-defined payload type.
 * - use_pairs: Enable/disable pairing system (default: false).
 * - AL: Allocator type (default: DefaultAllocator).
 */
#ifndef OCTREE_H
#define OCTREE_H
...
class Octree {
public:
  typedef void *(*PairCallback)(void *, OctreeElementID, T *, int,
                                OctreeElementID, T *, int);
  typedef void (*UnpairCallback)(void *, OctreeElementID, T *, int,
                                 OctreeElementID, T *, int, void *);
...
private:
  ...
  struct Octant {
    ...
    int children_count; // cache for amount of childrens (fast check for
                        // removal)
  ...
  typedef Map<OctreeElementID, Element, Comparator<OctreeElementID>, AL>
      ElementMap;
  ...
  _FORCE_INLINE_ void _pair_check(PairData *p_pair) {
    ...
    if (intersect != p_pair->intersect) {
      ...
      if (intersect) {
        ...
        if (pair_callback) {
          p_pair->ud = pair_callback(pair_callback_userdata, p_pair->A->_id,
                                     p_pair->A->userdata, p_pair->A->subindex,
                                     p_pair->B->_id, p_pair->B->userdata,
                                     p_pair->B->subindex);
        }
        pair_count++;
      } else {
        ...
        if (unpair_callback) {
          unpair_callback(pair_callback_userdata, p_pair->A->_id,
                          p_pair->A->userdata, p_pair->A->subindex,
                          p_pair->B->_id, p_pair->B->userdata,
                          p_pair->B->subindex, p_pair->ud);
        }
        pair_count--;
      }
  ...
  _FORCE_INLINE_ void _pair_unreference(Element *p_A, Element *p_B) {
    ...
    if (E->get().refcount == 0) {
      ...
      if (E->get().intersect) {
        if (unpair_callback) {
          unpair_callback(pair_callback_userdata, p_A->_id, p_A->userdata,
                          p_A->subindex, p_B->_id, p_B->userdata, p_B->subindex,
                          E->get().ud);
        }
  ...
  _FORCE_INLINE_ void _optimize() {
    ...
    while (root && root->children_count < 2 && !root->elements.size() &&
           !(use_pairs && root->pairable_elements.size())) {
  ...
  bool _remove_element_from_octant(Element *p_element, Octant *p_octant,
                                   Octant *p_limit = NULL);
  ...
  void _cull_aabb(Octant *p_octant, const AABB &p_aabb, T **p_result_array,
                  int *p_result_idx, int p_result_max, int *p_subindex_array,
                  uint32_t p_mask);
  void _cull_segment(Octant *p_octant, const Vector3 &p_from,
                     const Vector3 &p_to, T **p_result_array, int *p_result_idx,
                     int p_result_max, int *p_subindex_array, uint32_t p_mask);
  void _cull_point(Octant *p_octant, const Vector3 &p_point, T **p_result_array,
                   int *p_result_idx, int p_result_max, int *p_subindex_array,
                   uint32_t p_mask);
...
public:
  OctreeElementID create(T *p_userdata, const AABB &p_aabb = AABB(),
                         int p_subindex = 0, bool p_pairable = false,
                         uint32_t p_pairable_type = 0,
                         uint32_t pairable_mask = 1);
  void move(OctreeElementID p_id, const AABB &p_aabb);
  void set_pairable(OctreeElementID p_id, bool p_pairable = false,
                    uint32_t p_pairable_type = 0, uint32_t pairable_mask = 1);
  ...
  int cull_convex(const Vector<Plane> &p_convex, T **p_result_array,
  ...
  int cull_segment(const Vector3 &p_from, const Vector3 &p_to,
                   T **p_result_array, int p_result_max,
                   int *p_subindex_array = NULL, uint32_t p_mask = 0xFFFFFFFF);

  int cull_point(const Vector3 &p_point, T **p_result_array, int p_result_max,
                 int *p_subindex_array = NULL, uint32_t p_mask = 0xFFFFFFFF);
...
template <class T, bool use_pairs, class AL>
void Octree<T, use_pairs, AL>::_insert_element(Element *p_element,
                                               Octant *p_octant) {

  real_t element_size =
      p_element->aabb.get_longest_axis_size() * 1.01; // avoid precision issues
  ...
  if (use_pairs) {
    ...
    typename List<Element *, AL>::Element *E =
        p_octant->pairable_elements.front();
...
void Octree<T, use_pairs, AL>::_ensure_valid_root(const AABB &p_aabb) {
  ...
  if (!root) {
    ...
    while (!base.encloses(p_aabb)) {
      ...
      ERR_FAIL_COND_MSG(base.size.x > OCTREE_SIZE_LIMIT,
                        "Octree upper size limit reached, does the AABB "
                        "supplied contain NAN?");
      ...
      if (ABS(base.position.x + base.size.x) <= ABS(base.position.x)) {
        ...
        base.size *= 2.0;
        gp->aabb = base;
        gp->children[(1 << 0) | (1 << 1) | (1 << 2)] =
            root; // add at all-positive
...
template <class T, bool use_pairs, class AL>
bool Octree<T, use_pairs, AL>::_remove_element_from_octant(Element *p_element,
                                                           Octant *p_octant,
                                                           Octant *p_limit) {
  ...
  while (true) {
    ...
    if (p_octant->children_count == 0 && p_octant->elements.empty() &&
        p_octant->pairable_elements.empty()) {
    ...
    if (!removed && !unpaired)
      return octant_removed; // no reason to keep going up anymore! was already
                             // visited and was not removed
...
template <class T, bool use_pairs, class AL>
void Octree<T, use_pairs, AL>::_unpair_element(Element *p_element,
                                               Octant *p_octant) {
...
template <class T, bool use_pairs, class AL>
void Octree<T, use_pairs, AL>::_pair_element(Element *p_element,
                                             Octant *p_octant) {
...
void Octree<T, use_pairs, AL>::_remove_element(Element *p_element) {
  ...
  typename List<typename Element::OctantOwner, AL>::Element *I =
      p_element->octant_owners.front();
...
template <class T, bool use_pairs, class AL>
OctreeElementID Octree<T, use_pairs, AL>::create(
    T *p_userdata, const AABB &p_aabb, int p_subindex, bool p_pairable,
...
#endif
  typename ElementMap::Element *E =
      element_map.insert(last_element_id++, Element());
  Element &e = E->get();
...
void Octree<T, use_pairs, AL>::move(OctreeElementID p_id, const AABB &p_aabb) {
  ...
  List<typename Element::OctantOwner, AL> owners =
      e.octant_owners; // save the octant owners
  ...
  for (typename List<typename Element::OctantOwner, AL>::Element *F =
           owners.front();
       F;) {
...
template <class T, bool use_pairs, class AL>
void Octree<T, use_pairs, AL>::set_pairable(OctreeElementID p_id,
                                            bool p_pairable,
  ...
  if (p_pairable == e.pairable && e.pairable_type == p_pairable_type &&
      e.pairable_mask == p_pairable_mask)
...
template <class T, bool use_pairs, class AL>
void Octree<T, use_pairs, AL>::_cull_convex(Octant *p_octant,
                                            _CullConvexData *p_cull) {
  ...
  if (!p_octant->elements.empty()) {
    ...
    for (; I; I = I->next()) {
      ...
      if (e->last_pass == pass ||
          (use_pairs && !(e->pairable_type & p_cull->mask)))
        continue;
      e->last_pass = pass;
  ...
  for (int i = 0; i < 8; i++) {
    ...
    if (p_octant->children[i] &&
        p_octant->children[i]->aabb.intersects_convex_shape(
            p_cull->planes, p_cull->plane_count)) {
...
template <class T, bool use_pairs, class AL>
void Octree<T, use_pairs, AL>::_cull_aabb(Octant *p_octant, const AABB &p_aabb,
                                          T **p_result_array, int *p_result_idx,
                                          int p_result_max,
  ...
  for (int i = 0; i < 8; i++) {
    ...
    if (p_octant->children[i] &&
        p_octant->children[i]->aabb.intersects_inclusive(p_aabb)) {
      _cull_aabb(p_octant->children[i], p_aabb, p_result_array, p_result_idx,
                 p_result_max, p_subindex_array, p_mask);
    }
  }
}

template <class T, bool use_pairs, class AL>
void Octree<T, use_pairs, AL>::_cull_segment(
    Octant *p_octant, const Vector3 &p_from, const Vector3 &p_to,
    T **p_result_array, int *p_result_idx, int p_result_max,
  ...
  for (int i = 0; i < 8; i++) {
    ...
    if (p_octant->children[i] &&
        p_octant->children[i]->aabb.intersects_segment(p_from, p_to)) {
      _cull_segment(p_octant->children[i], p_from, p_to, p_result_array,
                    p_result_idx, p_result_max, p_subindex_array, p_mask);
    }
  }
}

template <class T, bool use_pairs, class AL>
void Octree<T, use_pairs, AL>::_cull_point(Octant *p_octant,
                                           const Vector3 &p_point,
                                           T **p_result_array,
                                           int *p_result_idx, int p_result_max,
                                           int *p_subindex_array,
                                           uint32_t p_mask) {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    // could be optimized..
    if (p_octant->children[i] &&
        p_octant->children[i]->aabb.has_point(p_point)) {
      _cull_point(p_octant->children[i], p_point, p_result_array, p_result_idx,
                  p_result_max, p_subindex_array, p_mask);
    }
  }
}
...
template <class T, bool use_pairs, class AL>
int Octree<T, use_pairs, AL>::cull_convex(const Vector<Plane> &p_convex,
                                          T **p_result_array, int p_result_max,
                                          uint32_t p_mask) {
...
template <class T, bool use_pairs, class AL>
int Octree<T, use_pairs, AL>::cull_aabb(const AABB &p_aabb, T **p_result_array,
                                        int p_result_max, int *p_subindex_array,
                                        uint32_t p_mask) {
  ...
  int result_count = 0;
  pass++;
  _cull_aabb(root, p_aabb, p_result_array, &result_count, p_result_max,
             p_subindex_array, p_mask);

  return result_count;
}

template <class T, bool use_pairs, class AL>
int Octree<T, use_pairs, AL>::cull_segment(const Vector3 &p_from,
                                           const Vector3 &p_to,
                                           T **p_result_array, int p_result_max,
                                           int *p_subindex_array,
                                           uint32_t p_mask) {
  ...
  int result_count = 0;
  pass++;
  _cull_segment(root, p_from, p_to, p_result_array, &result_count, p_result_max,
                p_subindex_array, p_mask);
...
template <class T, bool use_pairs, class AL>
int Octree<T, use_pairs, AL>::cull_point(const Vector3 &p_point,
                                         T **p_result_array, int p_result_max,
                                         int *p_subindex_array,
                                         uint32_t p_mask) {
  ...
  int result_count = 0;
  pass++;
  _cull_point(root, p_point, p_result_array, &result_count, p_result_max,
              p_subindex_array, p_mask);
...
template <class T, bool use_pairs, class AL>
void Octree<T, use_pairs, AL>::set_pair_callback(PairCallback p_callback,
                                                 void *p_userdata) {
...
template <class T, bool use_pairs, class AL>
void Octree<T, use_pairs, AL>::set_unpair_callback(UnpairCallback p_callback,
                                                   void *p_userdata) {
