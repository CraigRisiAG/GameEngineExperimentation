...
/**
 * @file marshalls.h
 * @brief Miscellaneous helpers for marshalling data types and endian-independent encoding/decoding
 * 
 * This header provides utilities for converting between various data types (integers, floats, doubles, strings)
 * and byte arrays in a platform-independent manner. It includes:
 * - Union types for reinterpreting floating-point and double-precision values as integers
 * - Encoding functions to serialize primitive types into byte arrays
 * - Decoding functions to deserialize byte arrays back into primitive types
 * - Support for variant encoding/decoding with optional object serialization
 * 
 * All encoding/decoding operations use little-endian byte order for consistency across platforms.
 */
#ifndef MARSHALLS_H
#define MARSHALLS_H
