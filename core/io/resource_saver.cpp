...
/// @brief Resource saving system implementation
/// 
/// This file implements the ResourceSaver and ResourceFormatSaver classes, which form the core
/// of the engine's resource persistence layer. It provides functionality for saving resources
/// to disk in various formats, managing multiple format savers, and supporting custom script-based
/// savers.
///
/// Key Components:
/// - ResourceFormatSaver: Abstract base class for format-specific savers
/// - ResourceSaver: Manager class that coordinates multiple savers
///
/// Features:
/// - Multiple format saver registration and management
/// - Script-based custom resource savers via global class names
/// - File extension matching and format recognition
/// - Resource path localization
/// - Save callbacks for tracking saved resources
/// - Optional file modification timestamp tracking (tools only)
///
/// The system uses a priority-based saver list where savers can be inserted at the front
/// for higher priority or at the back for lower priority. When saving a resource, savers
/// are queried in order of priority until one recognizes the resource and file extension.
