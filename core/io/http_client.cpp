 ...
 * @brief HTTP client implementation for handling HTTP/HTTPS requests and responses.
 *
 * This file contains the implementation of the HTTPClient class, which provides
 * functionality for making HTTP requests, managing connections, handling SSL/TLS,
 * and processing responses including chunked transfer encoding.
 *
 * Key features:
 * - HTTP method support (GET, HEAD, POST, PUT, DELETE, OPTIONS, TRACE, CONNECT, PATCH)
 ...
 * - Custom request headers and response parsing
 * - Keep-alive connection management
 *
 * The implementation uses a state machine pattern to manage connection states
 * (RESOLVING, CONNECTING, CONNECTED, REQUESTING, BODY, etc.) and handles
 * asynchronous operations through the poll() method.
 */
