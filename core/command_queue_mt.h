 ...
 * @brief Multi-threaded command queue for asynchronous command execution.
 *
 * This header implements a thread-safe command queue using a ring buffer
 * allocation scheme. It supports three modes of command execution:
 * - Asynchronous push: Fire and forget command execution
 * - Push and return: Execute command and retrieve return value synchronously
 * - Push and sync: Execute command synchronously without return value
 *
 * The implementation uses preprocessor metaprogramming to generate command
 * structures and push methods for functions with 0 to 15 parameters.
 *
 * @details
 * - Ring buffer management with manual deallocation tracking
 * - Mutex-protected concurrent access
 * - Semaphore-based synchronization for sync commands
 * - Memory pooling with 256KB command buffer
 * - Support for up to 8 concurrent synchronous operations
 *
 * @note Uses macro expansion to generate template specializations for varying
 * parameter counts. The COMMA, COMMA_SEP_LIST, SEMIC_SEP_LIST, and
 * SPACE_SEP_LIST macros facilitate generating parameter lists with proper
 * delimiters.
 */
...
#define DECL_CMD(N)                                                            \
  ...
  struct Command##N : public CommandBase {                                     \
    ...
    virtual void call() { (instance->*method)(COMMA_SEP_LIST(ARG, N)); }       \
  };
...
#define DECL_PUSH_AND_RET(N)                                                   \
  template <class T, class M, COMMA_SEP_LIST(TYPE_PARAM, N) COMMA(N) class R>  \
  void push_and_ret(T *p_instance, M p_method,                                 \
...
#define DECL_PUSH_AND_SYNC(N)                                                  \
  template <class T, class M COMMA(N) COMMA_SEP_LIST(TYPE_PARAM, N)>           \
  void push_and_sync(T *p_instance,                                            \
...
class CommandQueueMT {
  ...
  struct SyncCommand : public CommandBase {
    ...
    virtual void post() { sync_sem->sem.post(); }
  };
  ...
  template <class T> T *allocate_and_lock() {
