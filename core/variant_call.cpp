 ...
 * @brief Variant method calling and construction system implementation
 *
 * This file implements the core variant method invocation infrastructure for
 * the Godot engine. It provides:
 *
 * - Variant method calls: Enables calling methods on variant types with
 * argument validation
 * - Variant constructors: Handles construction of variants from various
 * argument combinations
 * - Type function registration: Manages the registration of all callable
 * methods per variant type
 * - Constants: Registers type-specific constants (e.g., Vector3.ZERO,
 * Transform.IDENTITY)
 *
 * The system uses a callback-based approach with the _VariantCall struct to:
 * - Store method metadata (argument types, counts, defaults, return types)
 * - Validate arguments before method execution
 * - Handle both const and non-const methods
 * - Support optional default arguments
 *
 * Key components:
 * - FuncData: Stores metadata about a single callable method
 * - TypeFunc: Maps method names to FuncData for a specific variant type
 * - ConstructData: Metadata for variant constructors
 * - VCALL_* macros: Generate method wrapper functions with different argument
 * counts
 * - ADDFUNC_* macros: Register methods with proper type information
 *
 * Supported variant types include:
 * - Primitive: String, bool, int, float
 * - Math: Vector2/3, Rect2, Plane, Quat, Color, Transform, Basis, AABB
 * - Collections: Array, Dictionary, PackedByteArray, PackedVector3Array, etc.
 * - Special: Object references, Callable, Signal, NodePath, RID
 */
#include "variant.h"
...
typedef void (*VariantFunc)(Variant &r_ret, Variant &p_self,
                            const Variant **p_args);
typedef void (*VariantConstructFunc)(Variant &r_ret, const Variant **p_args);

struct _VariantCall {

  static void Vector3_dot(Variant &r_ret, Variant &p_self,
                          const Variant **p_args) {

    r_ret =
        reinterpret_cast<Vector3 *>(p_self._data._mem)
            ->dot(*reinterpret_cast<const Vector3 *>(p_args[0]->_data._mem));
  }
  ...
  struct FuncData {
    ...
    _FORCE_INLINE_ bool verify_arguments(const Variant **p_args,
    ...
    _FORCE_INLINE_ void call(Variant &r_ret, Variant &p_self,
  ...
  // void addfunc(Variant::Type p_type, const StringName& p_name,VariantFunc
  // p_func);

  static void make_func_return_variant(Variant::Type p_type,
  ...
  static void
  addfunc(bool p_const, Variant::Type p_type, Variant::Type p_return,
          bool p_has_return, const StringName &p_name, VariantFunc p_func,
          const Vector<Variant> &p_defaultarg, const Arg &p_argtype1 = Arg(),
          const Arg &p_argtype2 = Arg(), const Arg &p_argtype3 = Arg(),
          const Arg &p_argtype4 = Arg(), const Arg &p_argtype5 = Arg()) {
...
#define VCALL_LOCALMEM0(m_type, m_method)                                      \
  static void _call_##m_type##_##m_method(Variant &r_ret, Variant &p_self,     \
                                          const Variant **p_args) {            \
...
#define VCALL_LOCALMEM2(m_type, m_method)                                      \
  static void _call_##m_type##_##m_method(Variant &r_ret, Variant &p_self,     \
                                          const Variant **p_args) {            \
    reinterpret_cast<m_type *>(p_self._data._mem)                              \
        ->m_method(*p_args[0], *p_args[1]);                                    \
  }
#define VCALL_LOCALMEM2R(m_type, m_method)                                     \
  static void _call_##m_type##_##m_method(Variant &r_ret, Variant &p_self,     \
                                          const Variant **p_args) {            \
    r_ret = reinterpret_cast<m_type *>(p_self._data._mem)                      \
                ->m_method(*p_args[0], *p_args[1]);                            \
  }
#define VCALL_LOCALMEM3(m_type, m_method)                                      \
  static void _call_##m_type##_##m_method(Variant &r_ret, Variant &p_self,     \
                                          const Variant **p_args) {            \
    reinterpret_cast<m_type *>(p_self._data._mem)                              \
        ->m_method(*p_args[0], *p_args[1], *p_args[2]);                        \
  }
...
#define VCALL_LOCALMEM4(m_type, m_method)                                      \
  static void _call_##m_type##_##m_method(Variant &r_ret, Variant &p_self,     \
                                          const Variant **p_args) {            \
    reinterpret_cast<m_type *>(p_self._data._mem)                              \
        ->m_method(*p_args[0], *p_args[1], *p_args[2], *p_args[3]);            \
  }
...
#define VCALL_LOCALMEM5R(m_type, m_method)                                     \
  ...
  static void _call_String_to_ascii(Variant &r_ret, Variant &p_self,
                                    const Variant **p_args) {
  ...
  static void _call_String_to_utf8(Variant &r_ret, Variant &p_self,
                                   const Variant **p_args) {
  ...
  static void _call_Plane_intersect_3(Variant &r_ret, Variant &p_self,
                                      const Variant **p_args) {
    Vector3 result;
    if (reinterpret_cast<Plane *>(p_self._data._mem)
            ->intersect_3(*p_args[0], *p_args[1], &result))
      r_ret = result;
    else
      r_ret = Variant();
  }

  static void _call_Plane_intersects_ray(Variant &r_ret, Variant &p_self,
                                         const Variant **p_args) {
    Vector3 result;
    if (reinterpret_cast<Plane *>(p_self._data._mem)
            ->intersects_ray(*p_args[0], *p_args[1], &result))
      r_ret = result;
    else
      r_ret = Variant();
  }

  static void _call_Plane_intersects_segment(Variant &r_ret, Variant &p_self,
                                             const Variant **p_args) {
    Vector3 result;
    if (reinterpret_cast<Plane *>(p_self._data._mem)
            ->intersects_segment(*p_args[0], *p_args[1], &result))
      r_ret = result;
    else
      r_ret = Variant();
  }
  ...
  static void
  _call_PackedByteArray_get_string_from_ascii(Variant &r_ret, Variant &p_self,
                                              const Variant **p_args) {

    PackedByteArray *ba =
        reinterpret_cast<PackedByteArray *>(p_self._data._mem);
    String s;
    if (ba->size() > 0) {
  ...
  static void
  _call_PackedByteArray_get_string_from_utf8(Variant &r_ret, Variant &p_self,
                                             const Variant **p_args) {
  ...
  static void _call_PackedByteArray_compress(Variant &r_ret, Variant &p_self,
                                             const Variant **p_args) {
    ...
    if (ba->size() > 0) {
      ...
      compressed.resize(
          Compression::get_max_compressed_buffer_size(ba->size(), mode));
      int result =
          Compression::compress(compressed.ptrw(), ba->ptr(), ba->size(), mode);
  ...
  static void _call_PackedByteArray_decompress(Variant &r_ret, Variant &p_self,
                                               const Variant **p_args) {
    ...
    int result = Compression::decompress(decompressed.ptrw(), buffer_size,
                                         ba->ptr(), ba->size(), mode);
  ...
  static void _call_PackedByteArray_hex_encode(Variant &r_ret, Variant &p_self,
                                               const Variant **p_args) {
    PackedByteArray *ba =
        reinterpret_cast<PackedByteArray *>(p_self._data._mem);
    if (ba->size() == 0) {
...
#define VCALL_PARRMEM0(m_type, m_elemtype, m_method)                           \
  static void _call_##m_type##_##m_method(Variant &r_ret, Variant &p_self,     \
                                          const Variant **p_args) {            \
    Variant::PackedArrayRef<m_elemtype>::get_array_ptr(                        \
...
#define VCALL_PARRMEM0R(m_type, m_elemtype, m_method)                          \
  static void _call_##m_type##_##m_method(Variant &r_ret, Variant &p_self,     \
                                          const Variant **p_args) {            \
    r_ret = Variant::PackedArrayRef<m_elemtype>::get_array_ptr(                \
                p_self._data.packed_array)                                     \
                ->m_method();                                                  \
  }
...
#define VCALL_PTR2(m_type, m_method)                                           \
  static void _call_##m_type##_##m_method(Variant &r_ret, Variant &p_self,     \
                                          const Variant **p_args) {            \
    reinterpret_cast<m_type *>(p_self._data._ptr)                              \
        ->m_method(*p_args[0], *p_args[1]);                                    \
  }
#define VCALL_PTR2R(m_type, m_method)                                          \
  static void _call_##m_type##_##m_method(Variant &r_ret, Variant &p_self,     \
                                          const Variant **p_args) {            \
    r_ret = reinterpret_cast<m_type *>(p_self._data._ptr)                      \
                ->m_method(*p_args[0], *p_args[1]);                            \
  }
...
#define VCALL_PTR5R(m_type, m_method)                                          \
  ...
  static void _call_Transform2D_xform(Variant &r_ret, Variant &p_self,
                                      const Variant **p_args) {

    switch (p_args[0]->type) {

    case Variant::VECTOR2:
      r_ret = reinterpret_cast<Transform2D *>(p_self._data._ptr)
                  ->xform(p_args[0]->operator Vector2());
      return;
    case Variant::RECT2:
      r_ret = reinterpret_cast<Transform2D *>(p_self._data._ptr)
                  ->xform(p_args[0]->operator Rect2());
      return;
    case Variant::PACKED_VECTOR2_ARRAY:
      r_ret = reinterpret_cast<Transform2D *>(p_self._data._ptr)
                  ->xform(p_args[0]->operator PackedVector2Array());
      return;
    default:
      r_ret = Variant();
    }
  ...
  static void _call_Transform2D_xform_inv(Variant &r_ret, Variant &p_self,
                                          const Variant **p_args) {
    ...
    case Variant::VECTOR2:
      r_ret = reinterpret_cast<Transform2D *>(p_self._data._ptr)
                  ->xform_inv(p_args[0]->operator Vector2());
      return;
    case Variant::RECT2:
      r_ret = reinterpret_cast<Transform2D *>(p_self._data._ptr)
                  ->xform_inv(p_args[0]->operator Rect2());
      return;
    case Variant::PACKED_VECTOR2_ARRAY:
      r_ret = reinterpret_cast<Transform2D *>(p_self._data._ptr)
                  ->xform_inv(p_args[0]->operator PackedVector2Array());
      return;
    default:
      r_ret = Variant();
    }
  ...
  static void _call_Transform2D_basis_xform(Variant &r_ret, Variant &p_self,
                                            const Variant **p_args) {
    ...
    case Variant::VECTOR2:
      r_ret = reinterpret_cast<Transform2D *>(p_self._data._ptr)
                  ->basis_xform(p_args[0]->operator Vector2());
      return;
    default:
      r_ret = Variant();
    }
  ...
  static void _call_Transform2D_basis_xform_inv(Variant &r_ret, Variant &p_self,
                                                const Variant **p_args) {
    ...
    case Variant::VECTOR2:
      r_ret = reinterpret_cast<Transform2D *>(p_self._data._ptr)
                  ->basis_xform_inv(p_args[0]->operator Vector2());
      return;
    default:
      r_ret = Variant();
    }
  ...
  VCALL_PTR2R(
      Basis,
      is_equal_approx); // TODO: Break compatibility in 4.0 to change this to an
                        // instance method (a.is_equal_approx(b) as VCALL_PTR1R)
                        // for consistency.
  ...
  static void _call_Transform_xform(Variant &r_ret, Variant &p_self,
                                    const Variant **p_args) {

    switch (p_args[0]->type) {

    case Variant::VECTOR3:
      r_ret = reinterpret_cast<Transform *>(p_self._data._ptr)
                  ->xform(p_args[0]->operator Vector3());
      return;
    case Variant::PLANE:
      r_ret = reinterpret_cast<Transform *>(p_self._data._ptr)
                  ->xform(p_args[0]->operator Plane());
      return;
    case Variant::AABB:
      r_ret = reinterpret_cast<Transform *>(p_self._data._ptr)
                  ->xform(p_args[0]->operator ::AABB());
      return;
    case Variant::PACKED_VECTOR3_ARRAY:
      r_ret = reinterpret_cast<Transform *>(p_self._data._ptr)
                  ->xform(p_args[0]->operator ::PackedVector3Array());
      return;
    default:
      r_ret = Variant();
    }
  ...
  static void _call_Transform_xform_inv(Variant &r_ret, Variant &p_self,
                                        const Variant **p_args) {
    ...
    case Variant::VECTOR3:
      r_ret = reinterpret_cast<Transform *>(p_self._data._ptr)
                  ->xform_inv(p_args[0]->operator Vector3());
      return;
    case Variant::PLANE:
      r_ret = reinterpret_cast<Transform *>(p_self._data._ptr)
                  ->xform_inv(p_args[0]->operator Plane());
      return;
    case Variant::AABB:
      r_ret = reinterpret_cast<Transform *>(p_self._data._ptr)
                  ->xform_inv(p_args[0]->operator ::AABB());
      return;
    case Variant::PACKED_VECTOR3_ARRAY:
      r_ret = reinterpret_cast<Transform *>(p_self._data._ptr)
                  ->xform_inv(p_args[0]->operator ::PackedVector3Array());
      return;
    default:
      r_ret = Variant();
    }
  ...
  static void Transform_init2(Variant &r_ret, const Variant **p_args) {
    ...
    r_ret =
        Transform(p_args[0]->operator Basis(), p_args[1]->operator Vector3());
  }

  static void Callable_init2(Variant &r_ret, const Variant **p_args) {

    r_ret =
        Callable(p_args[0]->operator ObjectID(), p_args[1]->operator String());
  }

  static void Signal_init2(Variant &r_ret, const Variant **p_args) {

    r_ret =
        Signal(p_args[0]->operator ObjectID(), p_args[1]->operator String());
  }

  static void add_constructor(
      VariantConstructFunc p_func, const Variant::Type p_type,
  ...
  static void add_constant(int p_type, StringName p_constant_name,
  ...
  static void add_variant_constant(int p_type, StringName p_constant_name,
                                   const Variant &p_constant_value) {
...
Variant Variant::call(const StringName &p_method, const Variant **p_args,
...
void Variant::call_ptr(const StringName &p_method, const Variant **p_args,
  ...
  if (type == Variant::OBJECT) {
...
#ifdef DEBUG_ENABLED
    if (EngineDebugger::is_active() && !_get_obj().id.is_reference() &&
        ObjectDB::get_instance(_get_obj().id) == nullptr) {
      r_error.error = Callable::CallError::CALL_ERROR_INSTANCE_IS_NULL;
      return;
    }
    ...
    Map<StringName, _VariantCall::FuncData>::Element *E =
        _VariantCall::type_funcs[type].functions.find(p_method);
    ...
    if (E) {
      ...
      if (type == CALLABLE) {
        if (p_method == CoreStringNames::get_singleton()->call) {
          ...
          reinterpret_cast<const Callable *>(_data._mem)
              ->call(p_args, p_argcount, ret, r_error);
          valid = true;
        }
        if (p_method == CoreStringNames::get_singleton()->call_deferred) {
          reinterpret_cast<const Callable *>(_data._mem)
              ->call_deferred(p_args, p_argcount);
          valid = true;
        }
      } else if (type == SIGNAL) {
        if (p_method == CoreStringNames::get_singleton()->emit) {
          ...
          reinterpret_cast<const Signal *>(_data._mem)
              ->emit(p_args, p_argcount);
          valid = true;
        }
      }
      if (!valid) {
...
Variant Variant::construct(const Variant::Type p_type, const Variant **p_args,
                           int p_argcount, Callable::CallError &r_error,
                           bool p_strict) {
  ...
  if (p_argcount == 0) { // generic construct
    ...
    // misc types
    case COLOR:
      return p_args[0]->type == Variant::STRING ? Color::html(*p_args[0])
                                                : Color::hex(*p_args[0]);
    case STRING_NAME:
    ...
    for (List<_VariantCall::ConstructData>::Element *E = c.constructors.front();
         E; E = E->next()) {
      ...
      for (int i = 0; i < cd.arg_count; i++) {
        if (!Variant::can_convert(p_args[i]->type, cd.arg_types[i])) {
          r_error.error =
              Callable::CallError::CALL_ERROR_INVALID_ARGUMENT; // no such
                                                                // constructor
          r_error.argument = i;
  ...
  r_error.error =
      Callable::CallError::CALL_ERROR_INVALID_METHOD; // no such constructor
  return Variant();
}
...
Vector<Variant::Type>
Variant::get_method_argument_types(Variant::Type p_type,
                                   const StringName &p_method) {

  const _VariantCall::TypeFunc &tf = _VariantCall::type_funcs[p_type];

  const Map<StringName, _VariantCall::FuncData>::Element *E =
      tf.functions.find(p_method);
  if (!E)
...
bool Variant::is_method_const(Variant::Type p_type,
                              const StringName &p_method) {
...
Vector<StringName>
Variant::get_method_argument_names(Variant::Type p_type,
                                   const StringName &p_method) {
...
Variant::Type Variant::get_method_return_type(Variant::Type p_type,
                                              const StringName &p_method,
                                              bool *r_has_return) {
...
Vector<Variant>
Variant::get_method_default_arguments(Variant::Type p_type,
                                      const StringName &p_method) {
...
void Variant::get_method_list(List<MethodInfo> *p_list) const {
  ...
  for (const Map<StringName, _VariantCall::FuncData>::Element *E =
           tf.functions.front();
       E; E = E->next()) {
...
void Variant::get_constructor_list(Variant::Type p_type,
                                   List<MethodInfo> *p_list) {

  ERR_FAIL_INDEX(p_type, VARIANT_MAX);

  // custom constructors
  for (const List<_VariantCall::ConstructData>::Element *E =
           _VariantCall::construct_funcs[p_type].constructors.front();
       E; E = E->next()) {
...
void Variant::get_constants_for_type(Variant::Type p_type,
...
Variant Variant::get_constant_value(Variant::Type p_type,
                                    const StringName &p_value, bool *r_valid) {
...
void register_variant_methods() {
  ...
  _VariantCall::type_funcs =
      memnew_arr(_VariantCall::TypeFunc, Variant::VARIANT_MAX);

  _VariantCall::construct_funcs =
      memnew_arr(_VariantCall::ConstructFunc, Variant::VARIANT_MAX);
  _VariantCall::constant_data =
      memnew_arr(_VariantCall::ConstantData, Variant::VARIANT_MAX);

#define ADDFUNC0R(m_vtype, m_ret, m_class, m_method, m_defarg)                 \
  _VariantCall::addfunc(true, Variant::m_vtype, Variant::m_ret, true,          \
                        _scs_create(#m_method), VCALL(m_class, m_method),      \
                        m_defarg);
#define ADDFUNC1R(m_vtype, m_ret, m_class, m_method, m_arg1, m_argname1,       \
                  m_defarg)                                                    \
  _VariantCall::addfunc(                                                       \
      true, Variant::m_vtype, Variant::m_ret, true, _scs_create(#m_method),    \
      VCALL(m_class, m_method), m_defarg,                                      \
      _VariantCall::Arg(Variant::m_arg1, _scs_create(m_argname1)));
#define ADDFUNC2R(m_vtype, m_ret, m_class, m_method, m_arg1, m_argname1,       \
                  m_arg2, m_argname2, m_defarg)                                \
  _VariantCall::addfunc(                                                       \
      true, Variant::m_vtype, Variant::m_ret, true, _scs_create(#m_method),    \
      VCALL(m_class, m_method), m_defarg,                                      \
      _VariantCall::Arg(Variant::m_arg1, _scs_create(m_argname1)),             \
      _VariantCall::Arg(Variant::m_arg2, _scs_create(m_argname2)));
#define ADDFUNC3R(m_vtype, m_ret, m_class, m_method, m_arg1, m_argname1,       \
                  m_arg2, m_argname2, m_arg3, m_argname3, m_defarg)            \
  _VariantCall::addfunc(                                                       \
      true, Variant::m_vtype, Variant::m_ret, true, _scs_create(#m_method),    \
      VCALL(m_class, m_method), m_defarg,                                      \
      _VariantCall::Arg(Variant::m_arg1, _scs_create(m_argname1)),             \
      _VariantCall::Arg(Variant::m_arg2, _scs_create(m_argname2)),             \
      _VariantCall::Arg(Variant::m_arg3, _scs_create(m_argname3)));
#define ADDFUNC4R(m_vtype, m_ret, m_class, m_method, m_arg1, m_argname1,       \
                  m_arg2, m_argname2, m_arg3, m_argname3, m_arg4, m_argname4,  \
                  m_defarg)                                                    \
  _VariantCall::addfunc(                                                       \
      true, Variant::m_vtype, Variant::m_ret, true, _scs_create(#m_method),    \
      VCALL(m_class, m_method), m_defarg,                                      \
      ...
      _VariantCall::Arg(Variant::m_arg2, _scs_create(m_argname2)),             \
      _VariantCall::Arg(Variant::m_arg3, _scs_create(m_argname3)),             \
      _VariantCall::Arg(Variant::m_arg4, _scs_create(m_argname4)));

#define ADDFUNC0RNC(m_vtype, m_ret, m_class, m_method, m_defarg)               \
  _VariantCall::addfunc(false, Variant::m_vtype, Variant::m_ret, true,         \
                        _scs_create(#m_method), VCALL(m_class, m_method),      \
                        m_defarg);
#define ADDFUNC1RNC(m_vtype, m_ret, m_class, m_method, m_arg1, m_argname1,     \
                    m_defarg)                                                  \
  _VariantCall::addfunc(                                                       \
      false, Variant::m_vtype, Variant::m_ret, true, _scs_create(#m_method),   \
      VCALL(m_class, m_method), m_defarg,                                      \
      _VariantCall::Arg(Variant::m_arg1, _scs_create(m_argname1)));
#define ADDFUNC2RNC(m_vtype, m_ret, m_class, m_method, m_arg1, m_argname1,     \
                    m_arg2, m_argname2, m_defarg)                              \
  _VariantCall::addfunc(                                                       \
      false, Variant::m_vtype, Variant::m_ret, true, _scs_create(#m_method),   \
      VCALL(m_class, m_method), m_defarg,                                      \
      _VariantCall::Arg(Variant::m_arg1, _scs_create(m_argname1)),             \
      _VariantCall::Arg(Variant::m_arg2, _scs_create(m_argname2)));
#define ADDFUNC3RNC(m_vtype, m_ret, m_class, m_method, m_arg1, m_argname1,     \
                    m_arg2, m_argname2, m_arg3, m_argname3, m_defarg)          \
  _VariantCall::addfunc(                                                       \
      false, Variant::m_vtype, Variant::m_ret, true, _scs_create(#m_method),   \
      VCALL(m_class, m_method), m_defarg,                                      \
      ...
      _VariantCall::Arg(Variant::m_arg2, _scs_create(m_argname2)),             \
      _VariantCall::Arg(Variant::m_arg3, _scs_create(m_argname3)));
#define ADDFUNC4RNC(m_vtype, m_ret, m_class, m_method, m_arg1, m_argname1,     \
                    m_arg2, m_argname2, m_arg3, m_argname3, m_arg4,            \
                    m_argname4, m_defarg)                                      \
  _VariantCall::addfunc(                                                       \
      false, Variant::m_vtype, Variant::m_ret, true, _scs_create(#m_method),   \
      VCALL(m_class, m_method), m_defarg,                                      \
...
#define ADDFUNC0(m_vtype, m_ret, m_class, m_method, m_defarg)                  \
  _VariantCall::addfunc(true, Variant::m_vtype, Variant::m_ret, false,         \
                        _scs_create(#m_method), VCALL(m_class, m_method),      \
                        m_defarg);
#define ADDFUNC1(m_vtype, m_ret, m_class, m_method, m_arg1, m_argname1,        \
                 m_defarg)                                                     \
  _VariantCall::addfunc(                                                       \
      true, Variant::m_vtype, Variant::m_ret, false, _scs_create(#m_method),   \
      VCALL(m_class, m_method), m_defarg,                                      \
      _VariantCall::Arg(Variant::m_arg1, _scs_create(m_argname1)));
#define ADDFUNC2(m_vtype, m_ret, m_class, m_method, m_arg1, m_argname1,        \
                 m_arg2, m_argname2, m_defarg)                                 \
  _VariantCall::addfunc(                                                       \
      true, Variant::m_vtype, Variant::m_ret, false, _scs_create(#m_method),   \
      VCALL(m_class, m_method), m_defarg,                                      \
      _VariantCall::Arg(Variant::m_arg1, _scs_create(m_argname1)),             \
      _VariantCall::Arg(Variant::m_arg2, _scs_create(m_argname2)));
#define ADDFUNC3(m_vtype, m_ret, m_class, m_method, m_arg1, m_argname1,        \
                 m_arg2, m_argname2, m_arg3, m_argname3, m_defarg)             \
  _VariantCall::addfunc(                                                       \
      true, Variant::m_vtype, Variant::m_ret, false, _scs_create(#m_method),   \
      VCALL(m_class, m_method), m_defarg,                                      \
      ...
      _VariantCall::Arg(Variant::m_arg2, _scs_create(m_argname2)),             \
      _VariantCall::Arg(Variant::m_arg3, _scs_create(m_argname3)));
#define ADDFUNC4(m_vtype, m_ret, m_class, m_method, m_arg1, m_argname1,        \
                 m_arg2, m_argname2, m_arg3, m_argname3, m_arg4, m_argname4,   \
                 m_defarg)                                                     \
  _VariantCall::addfunc(                                                       \
      true, Variant::m_vtype, Variant::m_ret, false, _scs_create(#m_method),   \
...
#define ADDFUNC0NC(m_vtype, m_ret, m_class, m_method, m_defarg)                \
  _VariantCall::addfunc(false, Variant::m_vtype, Variant::m_ret, false,        \
                        _scs_create(#m_method), VCALL(m_class, m_method),      \
                        m_defarg);
#define ADDFUNC1NC(m_vtype, m_ret, m_class, m_method, m_arg1, m_argname1,      \
                   m_defarg)                                                   \
  _VariantCall::addfunc(                                                       \
      false, Variant::m_vtype, Variant::m_ret, false, _scs_create(#m_method),  \
      VCALL(m_class, m_method), m_defarg,                                      \
      _VariantCall::Arg(Variant::m_arg1, _scs_create(m_argname1)));
#define ADDFUNC2NC(m_vtype, m_ret, m_class, m_method, m_arg1, m_argname1,      \
                   m_arg2, m_argname2, m_defarg)                               \
  _VariantCall::addfunc(                                                       \
      false, Variant::m_vtype, Variant::m_ret, false, _scs_create(#m_method),  \
      VCALL(m_class, m_method), m_defarg,                                      \
      _VariantCall::Arg(Variant::m_arg1, _scs_create(m_argname1)),             \
      _VariantCall::Arg(Variant::m_arg2, _scs_create(m_argname2)));
#define ADDFUNC3NC(m_vtype, m_ret, m_class, m_method, m_arg1, m_argname1,      \
                   m_arg2, m_argname2, m_arg3, m_argname3, m_defarg)           \
  _VariantCall::addfunc(                                                       \
      false, Variant::m_vtype, Variant::m_ret, false, _scs_create(#m_method),  \
      VCALL(m_class, m_method), m_defarg,                                      \
      ...
      _VariantCall::Arg(Variant::m_arg2, _scs_create(m_argname2)),             \
      _VariantCall::Arg(Variant::m_arg3, _scs_create(m_argname3)));
#define ADDFUNC4NC(m_vtype, m_ret, m_class, m_method, m_arg1, m_argname1,      \
                   m_arg2, m_argname2, m_arg3, m_argname3, m_arg4, m_argname4, \
                   m_defarg)                                                   \
  _VariantCall::addfunc(                                                       \
      false, Variant::m_vtype, Variant::m_ret, false, _scs_create(#m_method),  \
  ...
  ADDFUNC2R(STRING, STRING, String, substr, INT, "from", INT, "len",
            varray(-1));

  ADDFUNC2R(STRING, INT, String, find, STRING, "what", INT, "from", varray(0));

  ADDFUNC3R(STRING, INT, String, count, STRING, "what", INT, "from", INT, "to",
            varray(0, 0));
  ADDFUNC3R(STRING, INT, String, countn, STRING, "what", INT, "from", INT, "to",
            varray(0, 0));

  ADDFUNC1R(STRING, INT, String, find_last, STRING, "what", varray());
  ADDFUNC2R(STRING, INT, String, findn, STRING, "what", INT, "from", varray(0));
  ADDFUNC2R(STRING, INT, String, rfind, STRING, "what", INT, "from",
            varray(-1));
  ADDFUNC2R(STRING, INT, String, rfindn, STRING, "what", INT, "from",
            varray(-1));
  ...
  ADDFUNC2R(STRING, STRING, String, format, NIL, "values", STRING,
            "placeholder", varray("{_}"));
  ...
  ADDFUNC3R(STRING, PACKED_STRING_ARRAY, String, split, STRING, "delimiter",
            BOOL, "allow_empty", INT, "maxsplit", varray(true, 0));
  ADDFUNC3R(STRING, PACKED_STRING_ARRAY, String, rsplit, STRING, "delimiter",
            BOOL, "allow_empty", INT, "maxsplit", varray(true, 0));
  ADDFUNC2R(STRING, PACKED_FLOAT32_ARRAY, String, split_floats, STRING,
            "delimiter", BOOL, "allow_empty", varray(true));
  ...
  ADDFUNC2R(STRING, STRING, String, strip_edges, BOOL, "left", BOOL, "right",
            varray(true, true));
  ...
  ADDFUNC1R(STRING, BOOL, String, is_valid_hex_number, BOOL, "with_prefix",
            varray(false));
  ...
  ADDFUNC2R(VECTOR2, VECTOR2, Vector2, linear_interpolate, VECTOR2, "b", FLOAT,
            "t", varray());
  ADDFUNC2R(VECTOR2, VECTOR2, Vector2, slerp, VECTOR2, "b", FLOAT, "t",
            varray());
  ADDFUNC4R(VECTOR2, VECTOR2, Vector2, cubic_interpolate, VECTOR2, "b", VECTOR2,
            "pre_a", VECTOR2, "post_b", FLOAT, "t", varray());
  ADDFUNC2R(VECTOR2, VECTOR2, Vector2, move_toward, VECTOR2, "to", FLOAT,
            "delta", varray());
  ...
  ADDFUNC2R(RECT2, BOOL, Rect2, intersects, RECT2, "b", BOOL, "include_borders",
            varray(false));
  ...
  ADDFUNC2R(RECT2, RECT2, Rect2, grow_margin, INT, "margin", FLOAT, "by",
            varray());
  ADDFUNC4R(RECT2, RECT2, Rect2, grow_individual, FLOAT, "left", FLOAT, "top",
            FLOAT, "right", FLOAT, " bottom", varray());
  ...
  ADDFUNC2R(RECT2I, RECT2I, Rect2i, grow_margin, INT, "margin", INT, "by",
            varray());
  ADDFUNC4R(RECT2I, RECT2I, Rect2i, grow_individual, INT, "left", INT, "top",
            INT, "right", INT, " bottom", varray());
  ...
  ADDFUNC2R(VECTOR3, VECTOR3, Vector3, rotated, VECTOR3, "axis", FLOAT, "phi",
            varray());
  ADDFUNC2R(VECTOR3, VECTOR3, Vector3, linear_interpolate, VECTOR3, "b", FLOAT,
            "t", varray());
  ADDFUNC2R(VECTOR3, VECTOR3, Vector3, slerp, VECTOR3, "b", FLOAT, "t",
            varray());
  ADDFUNC4R(VECTOR3, VECTOR3, Vector3, cubic_interpolate, VECTOR3, "b", VECTOR3,
            "pre_a", VECTOR3, "post_b", FLOAT, "t", varray());
  ADDFUNC2R(VECTOR3, VECTOR3, Vector3, move_toward, VECTOR3, "to", FLOAT,
            "delta", varray());
  ...
  ADDFUNC2R(PLANE, BOOL, Plane, has_point, VECTOR3, "point", FLOAT, "epsilon",
            varray(CMP_EPSILON));
  ADDFUNC1R(PLANE, VECTOR3, Plane, project, VECTOR3, "point", varray());
  ADDFUNC2R(PLANE, VECTOR3, Plane, intersect_3, PLANE, "b", PLANE, "c",
            varray());
  ADDFUNC2R(PLANE, VECTOR3, Plane, intersects_ray, VECTOR3, "from", VECTOR3,
            "dir", varray());
  ADDFUNC2R(PLANE, VECTOR3, Plane, intersects_segment, VECTOR3, "begin",
            VECTOR3, "end", varray());
  ...
  ADDFUNC4R(QUAT, QUAT, Quat, cubic_slerp, QUAT, "b", QUAT, "pre_a", QUAT,
            "post_b", FLOAT, "t", varray());
  ...
  ADDFUNC4R(COLOR, COLOR, Color, from_hsv, FLOAT, "h", FLOAT, "s", FLOAT, "v",
            FLOAT, "a", varray(1.0));
  ...
  ADDFUNC1R(DICTIONARY, DICTIONARY, Dictionary, duplicate, BOOL, "deep",
            varray(false));
  ADDFUNC2R(DICTIONARY, NIL, Dictionary, get, NIL, "key", NIL, "default",
            varray(Variant()));
  ...
  ADDFUNC3R(SIGNAL, INT, Signal, connect, CALLABLE, "callable", ARRAY, "binds",
            INT, "flags", varray(Array(), 0));
  ...
  ADDFUNC2R(ARRAY, INT, Array, bsearch, NIL, "value", BOOL, "before",
            varray(true));
  ADDFUNC4R(ARRAY, INT, Array, bsearch_custom, NIL, "value", OBJECT, "obj",
            STRING, "func", BOOL, "before", varray(true));
  ADDFUNC0NC(ARRAY, NIL, Array, invert, varray());
  ADDFUNC1R(ARRAY, ARRAY, Array, duplicate, BOOL, "deep", varray(false));
  ADDFUNC4R(ARRAY, ARRAY, Array, slice, INT, "begin", INT, "end", INT, "step",
            BOOL, "deep", varray(1, false));
  ...
  ADDFUNC2(PACKED_BYTE_ARRAY, NIL, PackedByteArray, set, INT, "idx", INT,
           "byte", varray());
  ADDFUNC1(PACKED_BYTE_ARRAY, NIL, PackedByteArray, push_back, INT, "byte",
           varray());
  ADDFUNC1(PACKED_BYTE_ARRAY, NIL, PackedByteArray, append, INT, "byte",
           varray());
  ADDFUNC1(PACKED_BYTE_ARRAY, NIL, PackedByteArray, append_array,
           PACKED_BYTE_ARRAY, "array", varray());
  ADDFUNC1(PACKED_BYTE_ARRAY, NIL, PackedByteArray, remove, INT, "idx",
           varray());
  ADDFUNC2R(PACKED_BYTE_ARRAY, INT, PackedByteArray, insert, INT, "idx", INT,
            "byte", varray());
  ADDFUNC1(PACKED_BYTE_ARRAY, NIL, PackedByteArray, resize, INT, "idx",
           varray());
  ADDFUNC0(PACKED_BYTE_ARRAY, NIL, PackedByteArray, invert, varray());
  ADDFUNC2R(PACKED_BYTE_ARRAY, PACKED_BYTE_ARRAY, PackedByteArray, subarray,
            INT, "from", INT, "to", varray());

  ADDFUNC0R(PACKED_BYTE_ARRAY, STRING, PackedByteArray, get_string_from_ascii,
            varray());
  ADDFUNC0R(PACKED_BYTE_ARRAY, STRING, PackedByteArray, get_string_from_utf8,
            varray());
  ADDFUNC0R(PACKED_BYTE_ARRAY, STRING, PackedByteArray, hex_encode, varray());
  ADDFUNC1R(PACKED_BYTE_ARRAY, PACKED_BYTE_ARRAY, PackedByteArray, compress,
            INT, "compression_mode", varray(0));
  ADDFUNC2R(PACKED_BYTE_ARRAY, PACKED_BYTE_ARRAY, PackedByteArray, decompress,
            INT, "buffer_size", INT, "compression_mode", varray(0));
  ...
  ADDFUNC2(PACKED_INT32_ARRAY, NIL, PackedInt32Array, set, INT, "idx", INT,
           "integer", varray());
  ADDFUNC1(PACKED_INT32_ARRAY, NIL, PackedInt32Array, push_back, INT, "integer",
           varray());
  ADDFUNC1(PACKED_INT32_ARRAY, NIL, PackedInt32Array, append, INT, "integer",
           varray());
  ADDFUNC1(PACKED_INT32_ARRAY, NIL, PackedInt32Array, append_array,
           PACKED_INT32_ARRAY, "array", varray());
  ADDFUNC1(PACKED_INT32_ARRAY, NIL, PackedInt32Array, remove, INT, "idx",
           varray());
  ADDFUNC2R(PACKED_INT32_ARRAY, INT, PackedInt32Array, insert, INT, "idx", INT,
            "integer", varray());
  ADDFUNC1(PACKED_INT32_ARRAY, NIL, PackedInt32Array, resize, INT, "idx",
           varray());
  ...
  ADDFUNC2(PACKED_INT64_ARRAY, NIL, PackedInt64Array, set, INT, "idx", INT,
           "integer", varray());
  ADDFUNC1(PACKED_INT64_ARRAY, NIL, PackedInt64Array, push_back, INT, "integer",
           varray());
  ADDFUNC1(PACKED_INT64_ARRAY, NIL, PackedInt64Array, append, INT, "integer",
           varray());
  ADDFUNC1(PACKED_INT64_ARRAY, NIL, PackedInt64Array, append_array,
           PACKED_INT64_ARRAY, "array", varray());
  ADDFUNC1(PACKED_INT64_ARRAY, NIL, PackedInt64Array, remove, INT, "idx",
           varray());
  ADDFUNC2R(PACKED_INT64_ARRAY, INT, PackedInt64Array, insert, INT, "idx", INT,
            "integer", varray());
  ADDFUNC1(PACKED_INT64_ARRAY, NIL, PackedInt64Array, resize, INT, "idx",
           varray());
  ...
  ADDFUNC2(PACKED_FLOAT32_ARRAY, NIL, PackedFloat32Array, set, INT, "idx",
           FLOAT, "value", varray());
  ADDFUNC1(PACKED_FLOAT32_ARRAY, NIL, PackedFloat32Array, push_back, FLOAT,
           "value", varray());
  ADDFUNC1(PACKED_FLOAT32_ARRAY, NIL, PackedFloat32Array, append, FLOAT,
           "value", varray());
  ADDFUNC1(PACKED_FLOAT32_ARRAY, NIL, PackedFloat32Array, append_array,
           PACKED_FLOAT32_ARRAY, "array", varray());
  ADDFUNC1(PACKED_FLOAT32_ARRAY, NIL, PackedFloat32Array, remove, INT, "idx",
           varray());
  ADDFUNC2R(PACKED_FLOAT32_ARRAY, INT, PackedFloat32Array, insert, INT, "idx",
            FLOAT, "value", varray());
  ADDFUNC1(PACKED_FLOAT32_ARRAY, NIL, PackedFloat32Array, resize, INT, "idx",
           varray());
  ...
  ADDFUNC2(PACKED_FLOAT64_ARRAY, NIL, PackedFloat64Array, set, INT, "idx",
           FLOAT, "value", varray());
  ADDFUNC1(PACKED_FLOAT64_ARRAY, NIL, PackedFloat64Array, push_back, FLOAT,
           "value", varray());
  ADDFUNC1(PACKED_FLOAT64_ARRAY, NIL, PackedFloat64Array, append, FLOAT,
           "value", varray());
  ADDFUNC1(PACKED_FLOAT64_ARRAY, NIL, PackedFloat64Array, append_array,
           PACKED_FLOAT64_ARRAY, "array", varray());
  ADDFUNC1(PACKED_FLOAT64_ARRAY, NIL, PackedFloat64Array, remove, INT, "idx",
           varray());
  ADDFUNC2R(PACKED_FLOAT64_ARRAY, INT, PackedFloat64Array, insert, INT, "idx",
            FLOAT, "value", varray());
  ADDFUNC1(PACKED_FLOAT64_ARRAY, NIL, PackedFloat64Array, resize, INT, "idx",
           varray());
  ...
  ADDFUNC2(PACKED_STRING_ARRAY, NIL, PackedStringArray, set, INT, "idx", STRING,
           "string", varray());
  ADDFUNC1(PACKED_STRING_ARRAY, NIL, PackedStringArray, push_back, STRING,
           "string", varray());
  ADDFUNC1(PACKED_STRING_ARRAY, NIL, PackedStringArray, append, STRING,
           "string", varray());
  ADDFUNC1(PACKED_STRING_ARRAY, NIL, PackedStringArray, append_array,
           PACKED_STRING_ARRAY, "array", varray());
  ADDFUNC1(PACKED_STRING_ARRAY, NIL, PackedStringArray, remove, INT, "idx",
           varray());
  ADDFUNC2R(PACKED_STRING_ARRAY, INT, PackedStringArray, insert, INT, "idx",
            STRING, "string", varray());
  ADDFUNC1(PACKED_STRING_ARRAY, NIL, PackedStringArray, resize, INT, "idx",
           varray());
  ...
  ADDFUNC2(PACKED_VECTOR2_ARRAY, NIL, PackedVector2Array, set, INT, "idx",
           VECTOR2, "vector2", varray());
  ADDFUNC1(PACKED_VECTOR2_ARRAY, NIL, PackedVector2Array, push_back, VECTOR2,
           "vector2", varray());
  ADDFUNC1(PACKED_VECTOR2_ARRAY, NIL, PackedVector2Array, append, VECTOR2,
           "vector2", varray());
  ADDFUNC1(PACKED_VECTOR2_ARRAY, NIL, PackedVector2Array, append_array,
           PACKED_VECTOR2_ARRAY, "array", varray());
  ADDFUNC1(PACKED_VECTOR2_ARRAY, NIL, PackedVector2Array, remove, INT, "idx",
           varray());
  ADDFUNC2R(PACKED_VECTOR2_ARRAY, INT, PackedVector2Array, insert, INT, "idx",
            VECTOR2, "vector2", varray());
  ADDFUNC1(PACKED_VECTOR2_ARRAY, NIL, PackedVector2Array, resize, INT, "idx",
           varray());
  ...
  ADDFUNC2(PACKED_VECTOR3_ARRAY, NIL, PackedVector3Array, set, INT, "idx",
           VECTOR3, "vector3", varray());
  ADDFUNC1(PACKED_VECTOR3_ARRAY, NIL, PackedVector3Array, push_back, VECTOR3,
           "vector3", varray());
  ADDFUNC1(PACKED_VECTOR3_ARRAY, NIL, PackedVector3Array, append, VECTOR3,
           "vector3", varray());
  ADDFUNC1(PACKED_VECTOR3_ARRAY, NIL, PackedVector3Array, append_array,
           PACKED_VECTOR3_ARRAY, "array", varray());
  ADDFUNC1(PACKED_VECTOR3_ARRAY, NIL, PackedVector3Array, remove, INT, "idx",
           varray());
  ADDFUNC2R(PACKED_VECTOR3_ARRAY, INT, PackedVector3Array, insert, INT, "idx",
            VECTOR3, "vector3", varray());
  ADDFUNC1(PACKED_VECTOR3_ARRAY, NIL, PackedVector3Array, resize, INT, "idx",
           varray());
  ...
  ADDFUNC2(PACKED_COLOR_ARRAY, NIL, PackedColorArray, set, INT, "idx", COLOR,
           "color", varray());
  ADDFUNC1(PACKED_COLOR_ARRAY, NIL, PackedColorArray, push_back, COLOR, "color",
           varray());
  ADDFUNC1(PACKED_COLOR_ARRAY, NIL, PackedColorArray, append, COLOR, "color",
           varray());
  ADDFUNC1(PACKED_COLOR_ARRAY, NIL, PackedColorArray, append_array,
           PACKED_COLOR_ARRAY, "array", varray());
  ADDFUNC1(PACKED_COLOR_ARRAY, NIL, PackedColorArray, remove, INT, "idx",
           varray());
  ADDFUNC2R(PACKED_COLOR_ARRAY, INT, PackedColorArray, insert, INT, "idx",
            COLOR, "color", varray());
  ADDFUNC1(PACKED_COLOR_ARRAY, NIL, PackedColorArray, resize, INT, "idx",
           varray());
  ...
  ADDFUNC2R(AABB, BOOL, AABB, intersects_segment, VECTOR3, "from", VECTOR3,
            "to", varray());
  ...
  ADDFUNC1R(TRANSFORM2D, TRANSFORM2D, Transform2D, scaled, VECTOR2, "scale",
            varray());
  ADDFUNC1R(TRANSFORM2D, TRANSFORM2D, Transform2D, translated, VECTOR2,
            "offset", varray());
  ...
  ADDFUNC1R(TRANSFORM2D, VECTOR2, Transform2D, basis_xform_inv, VECTOR2, "v",
            varray());
  ADDFUNC2R(TRANSFORM2D, TRANSFORM2D, Transform2D, interpolate_with,
            TRANSFORM2D, "transform", FLOAT, "weight", varray());
  ADDFUNC1R(TRANSFORM2D, BOOL, Transform2D, is_equal_approx, TRANSFORM2D,
            "transform", varray());
  ...
  ADDFUNC2R(BASIS, BOOL, Basis, is_equal_approx, BASIS, "b", FLOAT, "epsilon",
            varray(CMP_EPSILON)); // TODO: Replace in 4.0, see other TODO.
  ...
  ADDFUNC2R(TRANSFORM, TRANSFORM, Transform, rotated, VECTOR3, "axis", FLOAT,
            "phi", varray());
  ...
  ADDFUNC1R(TRANSFORM, TRANSFORM, Transform, translated, VECTOR3, "offset",
            varray());
  ADDFUNC2R(TRANSFORM, TRANSFORM, Transform, looking_at, VECTOR3, "target",
            VECTOR3, "up", varray());
  ADDFUNC2R(TRANSFORM, TRANSFORM, Transform, interpolate_with, TRANSFORM,
            "transform", FLOAT, "weight", varray());
  ADDFUNC1R(TRANSFORM, BOOL, Transform, is_equal_approx, TRANSFORM, "transform",
            varray());
  ...
  _VariantCall::add_constructor(_VariantCall::Vector2_init1, Variant::VECTOR2,
                                "x", Variant::FLOAT, "y", Variant::FLOAT);
  _VariantCall::add_constructor(_VariantCall::Vector2i_init1, Variant::VECTOR2I,
                                "x", Variant::INT, "y", Variant::INT);

  _VariantCall::add_constructor(_VariantCall::Rect2_init1, Variant::RECT2,
                                "position", Variant::VECTOR2, "size",
                                Variant::VECTOR2);
  _VariantCall::add_constructor(_VariantCall::Rect2_init2, Variant::RECT2, "x",
                                Variant::FLOAT, "y", Variant::FLOAT, "width",
                                Variant::FLOAT, "height", Variant::FLOAT);

  _VariantCall::add_constructor(_VariantCall::Rect2i_init1, Variant::RECT2I,
                                "position", Variant::VECTOR2, "size",
                                Variant::VECTOR2);
  _VariantCall::add_constructor(_VariantCall::Rect2i_init2, Variant::RECT2I,
                                "x", Variant::INT, "y", Variant::INT, "width",
                                Variant::INT, "height", Variant::INT);

  _VariantCall::add_constructor(_VariantCall::Transform2D_init2,
                                Variant::TRANSFORM2D, "rotation",
                                Variant::FLOAT, "position", Variant::VECTOR2);
  _VariantCall::add_constructor(
      _VariantCall::Transform2D_init3, Variant::TRANSFORM2D, "x_axis",
      Variant::VECTOR2, "y_axis", Variant::VECTOR2, "origin", Variant::VECTOR2);

  _VariantCall::add_constructor(_VariantCall::Vector3_init1, Variant::VECTOR3,
                                "x", Variant::FLOAT, "y", Variant::FLOAT, "z",
                                Variant::FLOAT);
  _VariantCall::add_constructor(_VariantCall::Vector3i_init1, Variant::VECTOR3I,
                                "x", Variant::INT, "y", Variant::INT, "z",
                                Variant::INT);

  _VariantCall::add_constructor(_VariantCall::Plane_init1, Variant::PLANE, "a",
                                Variant::FLOAT, "b", Variant::FLOAT, "c",
                                Variant::FLOAT, "d", Variant::FLOAT);
  _VariantCall::add_constructor(_VariantCall::Plane_init2, Variant::PLANE, "v1",
                                Variant::VECTOR3, "v2", Variant::VECTOR3, "v3",
                                Variant::VECTOR3);
  _VariantCall::add_constructor(_VariantCall::Plane_init3, Variant::PLANE,
                                "normal", Variant::VECTOR3, "d",
                                Variant::FLOAT);

  _VariantCall::add_constructor(_VariantCall::Quat_init1, Variant::QUAT, "x",
                                Variant::FLOAT, "y", Variant::FLOAT, "z",
                                Variant::FLOAT, "w", Variant::FLOAT);
  _VariantCall::add_constructor(_VariantCall::Quat_init2, Variant::QUAT, "axis",
                                Variant::VECTOR3, "angle", Variant::FLOAT);
  _VariantCall::add_constructor(_VariantCall::Quat_init3, Variant::QUAT,
                                "euler", Variant::VECTOR3);

  _VariantCall::add_constructor(_VariantCall::Color_init1, Variant::COLOR, "r",
                                Variant::FLOAT, "g", Variant::FLOAT, "b",
                                Variant::FLOAT, "a", Variant::FLOAT);
  _VariantCall::add_constructor(_VariantCall::Color_init2, Variant::COLOR, "r",
                                Variant::FLOAT, "g", Variant::FLOAT, "b",
                                Variant::FLOAT);

  _VariantCall::add_constructor(_VariantCall::AABB_init1, Variant::AABB,
                                "position", Variant::VECTOR3, "size",
                                Variant::VECTOR3);

  _VariantCall::add_constructor(_VariantCall::Basis_init1, Variant::BASIS,
                                "x_axis", Variant::VECTOR3, "y_axis",
                                Variant::VECTOR3, "z_axis", Variant::VECTOR3);
  _VariantCall::add_constructor(_VariantCall::Basis_init2, Variant::BASIS,
                                "axis", Variant::VECTOR3, "phi",
                                Variant::FLOAT);

  _VariantCall::add_constructor(_VariantCall::Transform_init1,
                                Variant::TRANSFORM, "x_axis", Variant::VECTOR3,
                                "y_axis", Variant::VECTOR3, "z_axis",
                                Variant::VECTOR3, "origin", Variant::VECTOR3);
  _VariantCall::add_constructor(_VariantCall::Transform_init2,
                                Variant::TRANSFORM, "basis", Variant::BASIS,
                                "origin", Variant::VECTOR3);

  _VariantCall::add_constructor(_VariantCall::Callable_init2, Variant::CALLABLE,
                                "object", Variant::OBJECT, "method_name",
                                Variant::STRING_NAME);
  _VariantCall::add_constructor(_VariantCall::Signal_init2, Variant::SIGNAL,
                                "object", Variant::OBJECT, "signal_name",
                                Variant::STRING_NAME);
  ...
  for (Map<String, Color>::Element *color = _named_colors.front(); color;
       color = color->next()) {
    _VariantCall::add_variant_constant(Variant::COLOR, color->key(),
                                       color->value());
  }
  ...
  _VariantCall::add_variant_constant(Variant::VECTOR3, "ZERO",
                                     Vector3(0, 0, 0));
  _VariantCall::add_variant_constant(Variant::VECTOR3, "ONE", Vector3(1, 1, 1));
  _VariantCall::add_variant_constant(Variant::VECTOR3, "INF",
                                     Vector3(Math_INF, Math_INF, Math_INF));
  _VariantCall::add_variant_constant(Variant::VECTOR3, "LEFT",
                                     Vector3(-1, 0, 0));
  _VariantCall::add_variant_constant(Variant::VECTOR3, "RIGHT",
                                     Vector3(1, 0, 0));
  _VariantCall::add_variant_constant(Variant::VECTOR3, "UP", Vector3(0, 1, 0));
  _VariantCall::add_variant_constant(Variant::VECTOR3, "DOWN",
                                     Vector3(0, -1, 0));
  _VariantCall::add_variant_constant(Variant::VECTOR3, "FORWARD",
                                     Vector3(0, 0, -1));
  _VariantCall::add_variant_constant(Variant::VECTOR3, "BACK",
                                     Vector3(0, 0, 1));
  ...
  _VariantCall::add_variant_constant(Variant::VECTOR3I, "ZERO",
                                     Vector3i(0, 0, 0));
  _VariantCall::add_variant_constant(Variant::VECTOR3I, "ONE",
                                     Vector3i(1, 1, 1));
  _VariantCall::add_variant_constant(Variant::VECTOR3I, "LEFT",
                                     Vector3i(-1, 0, 0));
  _VariantCall::add_variant_constant(Variant::VECTOR3I, "RIGHT",
                                     Vector3i(1, 0, 0));
  _VariantCall::add_variant_constant(Variant::VECTOR3I, "UP",
                                     Vector3i(0, 1, 0));
  _VariantCall::add_variant_constant(Variant::VECTOR3I, "DOWN",
                                     Vector3i(0, -1, 0));
  _VariantCall::add_variant_constant(Variant::VECTOR3I, "FORWARD",
                                     Vector3i(0, 0, -1));
  _VariantCall::add_variant_constant(Variant::VECTOR3I, "BACK",
                                     Vector3i(0, 0, 1));
  ...
  _VariantCall::add_variant_constant(Variant::VECTOR2, "INF",
                                     Vector2(Math_INF, Math_INF));
  ...
  _VariantCall::add_variant_constant(Variant::VECTOR2I, "LEFT",
                                     Vector2i(-1, 0));
  _VariantCall::add_variant_constant(Variant::VECTOR2I, "RIGHT",
                                     Vector2i(1, 0));
  ...
  _VariantCall::add_variant_constant(Variant::TRANSFORM2D, "IDENTITY",
                                     Transform2D());
  _VariantCall::add_variant_constant(Variant::TRANSFORM2D, "FLIP_X",
                                     Transform2D(-1, 0, 0, 1, 0, 0));
  _VariantCall::add_variant_constant(Variant::TRANSFORM2D, "FLIP_Y",
                                     Transform2D(1, 0, 0, -1, 0, 0));
  ...
  _VariantCall::add_variant_constant(Variant::TRANSFORM, "IDENTITY",
                                     identity_transform);
  _VariantCall::add_variant_constant(Variant::TRANSFORM, "FLIP_X",
                                     flip_x_transform);
  _VariantCall::add_variant_constant(Variant::TRANSFORM, "FLIP_Y",
                                     flip_y_transform);
  _VariantCall::add_variant_constant(Variant::TRANSFORM, "FLIP_Z",
                                     flip_z_transform);
  ...
  _VariantCall::add_variant_constant(Variant::BASIS, "IDENTITY",
                                     identity_basis);
  ...
  _VariantCall::add_variant_constant(Variant::PLANE, "PLANE_YZ",
                                     Plane(Vector3(1, 0, 0), 0));
  _VariantCall::add_variant_constant(Variant::PLANE, "PLANE_XZ",
                                     Plane(Vector3(0, 1, 0), 0));
  _VariantCall::add_variant_constant(Variant::PLANE, "PLANE_XY",
                                     Plane(Vector3(0, 0, 1), 0));

  _VariantCall::add_variant_constant(Variant::QUAT, "IDENTITY",
                                     Quat(0, 0, 0, 1));
}
