 ...
 * @brief Implementation of the FileAccess class for file I/O operations.
 * 
 * This file contains the implementation of the FileAccess abstract base class,
 * which provides a unified interface for reading and writing files across different
 * access types (filesystem, resources, user data). It handles file operations including:
 * - File creation and opening with multiple access types
 * - Binary and text data reading/writing (8, 16, 32, 64-bit integers, floats, doubles)
 * - String and line operations (UTF-8 support)
 * - CSV parsing and generation
 * - File metadata (modification time, permissions)
 * - Cryptographic hashing (MD5, SHA256)
 * - Endianness handling for binary data
 * 
 * The class abstracts away platform-specific file handling and supports packed data
 * access (compressed archives) alongside regular filesystem operations.
 */
