...
/**
 * @class ObjectID
 * @brief A wrapper class for storing and managing object identifiers.
 * 
 * ObjectID encapsulates a 64-bit unsigned integer to represent unique object identifiers.
 * It provides type-safe conversion to 64-bit integer types while preventing precision loss
 * through implicit conversions to smaller integer types.
 * 
 * The most significant bit (bit 63) is reserved to indicate whether an object ID represents
 * a reference (when set) or a direct instance (when unset).
 * 
 * @note This class is designed to be compatible with Variant types and enforces explicit
 * casting to 64-bit integer types to avoid bugs related to precision loss.
 * 
 * @warning Implicit conversions are only allowed to int64_t and uint64_t types.
 */
#ifndef OBJECT_ID_H
#define OBJECT_ID_H
