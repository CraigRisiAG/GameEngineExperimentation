 ...
 * @brief Core type registration and initialization for the game engine.
 *
 * This file handles the registration and setup of core engine types, including:
 * - Resource format loaders and savers (binary, image, PO, crypto)
 * - Input event types and related classes
 * - Networking and I/O classes (TCP, UDP, DTLS, SSL)
 * - Cryptography classes (X509Certificate, CryptoKey, Crypto)
 * - Utility classes (AStar, XMLParser, ConfigFile, etc.)
 * - Global singleton instances
 *
 * @note This module must be initialized early in the engine startup sequence
 * as other subsystems depend on the types and singletons registered here.
 */

/**
 * @brief Initializes and registers all core engine types.
 *
 * Performs the following operations in order:
 * - Sets up ObjectDB, ResourceCache, and StringName systems
 * - Registers global constants and variant methods
 * - Instantiates and registers resource format loaders/savers
 * - Registers all core classes with the ClassDB
 * - Creates singleton instances for Geometry, ResourceLoader, ResourceSaver, OS, Engine, ClassDB, Marshalls, and JSON
 *
 * @note Performs a static_assert to verify Callable size constraints.
 * @see register_core_settings(), register_core_singletons(), unregister_core_types()
 */
void register_core_types();

/**
 * @brief Registers project-level configuration settings.
 *
 * Defines global project settings related to:
 * - TCP connection timeouts
 * - Packet peer stream buffer limits
 * - SSL certificate paths
 *
 * @note Must be called after register_core_types() when global state is available.
 * @see register_core_types()
 */
void register_core_settings();

/**
 * @brief Registers core singleton classes and adds them to the Engine.
 *
 * Registers the following singletons accessible throughout the engine:
 * - ProjectSettings, IP, Geometry, ResourceLoader, ResourceSaver
 * - OS, Engine, ClassDB, Marshalls, TranslationServer
 * - Input, InputMap, JSON, Expression
 *
 * @note Must be called after register_core_types() and register_core_settings().
 * @see register_core_types(), Engine::add_singleton()
 */
void register_core_singletons();

/**
 * @brief Cleans up and unregisters all core types and singletons.
 *
 * Performs cleanup in reverse order of initialization:
 * - Deletes all singleton instances
 * - Removes and unreferences all resource format handlers
 * - Cleans up ClassDB, ObjectDB, and StringName systems
 * - Unregisters global constants and variant methods
 *
 * @note Must be called during engine shutdown to prevent resource leaks.
 * @see register_core_types()
 */
