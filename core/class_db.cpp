...
/**
 * @file class_db.cpp
 * @brief Implementation of ClassDB - the class database system for the game engine.
 * 
 * This file contains the core implementation of the ClassDB system, which manages:
 * - Class registration and inheritance hierarchy
 * - Method binding and invocation
 * - Property management (getters/setters)
 * - Signals and constants
 * - API categorization (Core, Editor, Server)
 * - Class instantiation and introspection
 * 
 * Thread Safety:
 * - Uses RWLock for concurrent access to class information
 * - OBJTYPE_RLOCK: Read lock for methods that query class data
 * - OBJTYPE_WLOCK: Write lock for methods that modify class data
 * 
 * Key Components:
 * - D_METHOD(): Helper functions to define methods with argument names (DEBUG_METHODS_ENABLED only)
 * - ClassInfo: Internal structure storing metadata for each registered class
 * - Default values cache: Stores default property values for classes
 * - Compatibility class mapping: Supports remapping deprecated class names
 * 
 * Main Features:
 * - Method lookup and invocation with default arguments
 * - Property access through getters/setters
 * - Signal definition and querying
 * - Integer constant and enum management
 * - API hash computation for compatibility verification
 * - Class instantiation with API-level restrictions
 * - Virtual method tracking for override validation
 */
#include "class_db.h"
