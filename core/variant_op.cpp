...
/**
 * @file variant_op.cpp
 * @brief Variant operator implementations and type conversions.
 *
 * This file implements all binary and unary operators for the Variant type
 * system, including arithmetic, comparison, logical, and bitwise operations. It
 * provides comprehensive type coercion and operation support across all variant
 * types (numeric types, vectors, transforms, colors, strings, collections,
 * etc.).
 *
 * Key components:
 * - evaluate(): Main function that performs operations between two Variants
 * - Operator overloads for: equality, comparison, arithmetic, bitwise, and
 * logical ops
 * - Type-specific operation macros (DEFAULT_OP_*) for code generation
 * - Named and indexed property access (get_named, set_named, get, set)
 * - Collection iteration support (iter_init, iter_next, iter_get)
 * - Variant interpolation and blending for animation/tweening
 * - Property introspection (get_property_list)
 * - Membership testing (in operator)
 *
 * The implementation uses computed goto tables on GCC for performance
 * optimization, with fallback to traditional switch statements on other
 * compilers. Debug builds include division-by-zero checking and object validity
 * verification.
 *
 * All operations maintain type safety with proper error handling through the
 * r_valid output parameter, allowing callers to detect unsupported type
 * combinations.
 */
#include "variant.h"
...
#define CASES(PREFIX)                                                          \
  static const void *switch_table_##PREFIX[25][Variant::VARIANT_MAX] = {       \
...
Variant::operator bool() const { return booleanize(); }

// We consider all uninitialized or empty types to be false based on the type's
// zeroiness.
bool Variant::booleanize() const { return !is_zero(); }
...
#define DEFAULT_OP_NUM_NEG(m_prefix, m_op_name, m_name, m_type)                \
  CASE_TYPE(m_prefix, m_op_name, m_name) { _RETURN(-p_a._data.m_type); };
...
#define DEFAULT_OP_NUM_VEC(m_prefix, m_op_name, m_name, m_op, m_type)          \
  CASE_TYPE(m_prefix, m_op_name, m_name) {                                     \
    ...
    if (p_b.type == FLOAT)                                                     \
      _RETURN(p_a._data.m_type m_op p_b._data._float);                         \
    if (p_b.type == VECTOR2)                                                   \
      _RETURN(p_a._data.m_type m_op *                                          \
              reinterpret_cast<const Vector2 *>(p_b._data._mem));              \
    if (p_b.type == VECTOR3)                                                   \
      _RETURN(p_a._data.m_type m_op *                                          \
...
#define DEFAULT_OP_STR_REV(m_prefix, m_op_name, m_name, m_op, m_type)          \
  CASE_TYPE(m_prefix, m_op_name, m_name) {                                     \
    if (p_b.type == STRING)                                                    \
      _RETURN(*reinterpret_cast<const m_type *>(p_b._data._mem) m_op *         \
              reinterpret_cast<const String *>(p_a._data._mem));               \
    if (p_b.type == STRING_NAME)                                               \
      _RETURN(*reinterpret_cast<const m_type *>(p_b._data._mem) m_op *         \
              reinterpret_cast<const StringName *>(p_a._data._mem));           \
    if (p_b.type == NODE_PATH)                                                 \
      _RETURN(*reinterpret_cast<const m_type *>(p_b._data._mem) m_op *         \
              reinterpret_cast<const NodePath *>(p_a._data._mem));             \
                                                                               \
    _RETURN_FAIL                                                               \
  };
...
#define DEFAULT_OP_STR(m_prefix, m_op_name, m_name, m_op, m_type)              \
  CASE_TYPE(m_prefix, m_op_name, m_name) {                                     \
    if (p_b.type == STRING)                                                    \
      _RETURN(*reinterpret_cast<const m_type *>(p_a._data._mem) m_op *         \
              reinterpret_cast<const String *>(p_b._data._mem));               \
    if (p_b.type == STRING_NAME)                                               \
      _RETURN(*reinterpret_cast<const m_type *>(p_a._data._mem) m_op *         \
              reinterpret_cast<const StringName *>(p_b._data._mem));           \
    if (p_b.type == NODE_PATH)                                                 \
      _RETURN(*reinterpret_cast<const m_type *>(p_a._data._mem) m_op *         \
              reinterpret_cast<const NodePath *>(p_b._data._mem));             \
                                                                               \
    _RETURN_FAIL                                                               \
  };
...
#define DEFAULT_OP_LOCALMEM_REV(m_prefix, m_op_name, m_name, m_op, m_type)     \
  CASE_TYPE(m_prefix, m_op_name, m_name) {                                     \
    if (p_b.type == m_name)                                                    \
      _RETURN(*reinterpret_cast<const m_type *>(p_b._data._mem) m_op *         \
              reinterpret_cast<const m_type *>(p_a._data._mem));               \
                                                                               \
    _RETURN_FAIL                                                               \
  };
...
#define DEFAULT_OP_LOCALMEM(m_prefix, m_op_name, m_name, m_op, m_type)         \
  CASE_TYPE(m_prefix, m_op_name, m_name) {                                     \
    if (p_b.type == m_name)                                                    \
      _RETURN(*reinterpret_cast<const m_type *>(p_a._data._mem) m_op *         \
              reinterpret_cast<const m_type *>(p_b._data._mem));               \
                                                                               \
    _RETURN_FAIL                                                               \
  };
...
#define DEFAULT_OP_ARRAY_EQ(m_prefix, m_op_name, m_name, m_type)               \
  CASE_TYPE(m_prefix, m_op_name, m_name) {                                     \
    if (p_b.type == NIL)                                                       \
      _RETURN(false)                                                           \
    DEFAULT_OP_ARRAY_OP_BODY(m_prefix, m_op_name, m_name, m_type, !=, !=,      \
                             true, false, false)                               \
  }
...
#define DEFAULT_OP_ARRAY_LT(m_prefix, m_op_name, m_name, m_type)               \
  DEFAULT_OP_ARRAY_OP(m_prefix, m_op_name, m_name, m_type, <, !=, false,       \
                      a_len < array_b.size(), true)

#define DEFAULT_OP_ARRAY_GT(m_prefix, m_op_name, m_name, m_type)               \
  DEFAULT_OP_ARRAY_OP(m_prefix, m_op_name, m_name, m_type, >, !=, false,       \
                      a_len < array_b.size(), true)

#define DEFAULT_OP_ARRAY_OP(m_prefix, m_op_name, m_name, m_type, m_opa, m_opb, \
                            m_ret_def, m_ret_s, m_ret_f)                       \
  CASE_TYPE(m_prefix, m_op_name, m_name) {                                     \
    DEFAULT_OP_ARRAY_OP_BODY(m_prefix, m_op_name, m_name, m_type, m_opa,       \
                             m_opb, m_ret_def, m_ret_s, m_ret_f)               \
  }

#define DEFAULT_OP_ARRAY_OP_BODY(m_prefix, m_op_name, m_name, m_type, m_opa,   \
                                 m_opb, m_ret_def, m_ret_s, m_ret_f)           \
  if (p_a.type != p_b.type)                                                    \
    _RETURN_FAIL                                                               \
                                                                               \
  const Vector<m_type> &array_a =                                              \
      PackedArrayRef<m_type>::get_array(p_a._data.packed_array);               \
  const Vector<m_type> &array_b =                                              \
      PackedArrayRef<m_type>::get_array(p_b._data.packed_array);               \
                                                                               \
  ...
  SWITCH(math, p_op, p_a.type) {
    ...
    SWITCH_OP(math, OP_MULTIPLY, p_a.type) {
      CASE_TYPE(math, OP_MULTIPLY, TRANSFORM2D) {
        ...
        case VECTOR2: {
          _RETURN(
              p_a._data._transform2d->xform(*(const Vector2 *)p_b._data._mem));
        }
        default:
          _RETURN_FAIL;
        }
      ...
      CASE_TYPE(math, OP_MULTIPLY, TRANSFORM) {
        ...
        case VECTOR3: {
          _RETURN(
              p_a._data._transform->xform(*(const Vector3 *)p_b._data._mem));
        }
        case TRANSFORM: {
    ...
    SWITCH_OP(math, OP_BIT_NEGATE, p_a.type) {
      CASE_TYPE(math, OP_BIT_NEGATE, INT) { _RETURN(~p_a._data._int); }
...
void Variant::set_named(const StringName &p_index, const Variant &p_value,
                        bool *r_valid) {
  ...
  case OBJECT: {
    ...
    if (!_get_obj().obj) {
      break;
    } else if (EngineDebugger::is_active() &&
...
#define DEFAULT_OP_ARRAY_CMD(m_name, m_type, skip_test, cmd)                   \
  case m_name: {                                                               \
    ...
    if (p_index.get_type() == Variant::INT ||                                  \
        p_index.get_type() == Variant::FLOAT) {                                \
      int index = p_index;                                                     \
...
#define DEFAULT_OP_DVECTOR_SET(m_name, m_type, skip_cond)                      \
  case m_name: {                                                               \
    ...
    if (p_index.get_type() == Variant::INT ||                                  \
      ...
      int index = p_index;                                                     \
      Vector<m_type> *arr =                                                    \
          PackedArrayRef<m_type>::get_array_ptr(_data.packed_array);           \
                                                                               \
      if (index < 0)                                                           \
        index += arr->size();                                                  \
      if (index >= 0 && index < arr->size()) {                                 \
...
#define DEFAULT_OP_DVECTOR_GET(m_name, m_type)                                 \
  case m_name: {                                                               \
    ...
    if (p_index.get_type() == Variant::INT ||                                  \
        p_index.get_type() == Variant::FLOAT) {                                \
      int index = p_index;                                                     \
      const Vector<m_type> *arr =                                              \
          &PackedArrayRef<m_type>::get_array(_data.packed_array);              \
                                                                               \
      if (index < 0)                                                           \
        index += arr->size();                                                  \
      if (index >= 0 && index < arr->size()) {                                 \
...
void Variant::set(const Variant &p_index, const Variant &p_value,
                  bool *r_valid) {
  ...
  case OBJECT: {
    ...
    if (obj) {
      ...
      if (p_index.get_type() != Variant::STRING_NAME &&
          p_index.get_type() != Variant::STRING) {
  ...
  case DICTIONARY: {
    ...
    DEFAULT_OP_DVECTOR_SET(PACKED_BYTE_ARRAY, uint8_t,
                           p_value.type != Variant::FLOAT &&
                               p_value.type != Variant::INT)
    DEFAULT_OP_DVECTOR_SET(PACKED_INT32_ARRAY, int32_t,
                           p_value.type != Variant::FLOAT &&
                               p_value.type != Variant::INT)
    DEFAULT_OP_DVECTOR_SET(PACKED_INT64_ARRAY, int64_t,
                           p_value.type != Variant::FLOAT &&
                               p_value.type != Variant::INT)
    DEFAULT_OP_DVECTOR_SET(PACKED_FLOAT32_ARRAY, float,
                           p_value.type != Variant::FLOAT &&
                               p_value.type != Variant::INT)
    DEFAULT_OP_DVECTOR_SET(PACKED_FLOAT64_ARRAY, double,
                           p_value.type != Variant::FLOAT &&
                               p_value.type != Variant::INT)
    DEFAULT_OP_DVECTOR_SET(PACKED_STRING_ARRAY, String,
                           p_value.type != Variant::STRING)
    DEFAULT_OP_DVECTOR_SET(PACKED_VECTOR2_ARRAY, Vector2,
                           p_value.type != Variant::VECTOR2)
    DEFAULT_OP_DVECTOR_SET(PACKED_VECTOR3_ARRAY, Vector3,
                           p_value.type != Variant::VECTOR3)
    DEFAULT_OP_DVECTOR_SET(PACKED_COLOR_ARRAY, Color,
...
bool Variant::in(const Variant &p_index, bool *r_valid) const {
  ...
  case PACKED_BYTE_ARRAY: {
    if (p_index.get_type() == Variant::INT ||
      ...
      int index = p_index;
      const Vector<uint8_t> *arr =
          &PackedArrayRef<uint8_t>::get_array(_data.packed_array);
      int l = arr->size();
      if (l) {
  ...
  case PACKED_INT32_ARRAY: {
    if (p_index.get_type() == Variant::INT ||
      ...
      const Vector<int32_t> *arr =
          &PackedArrayRef<int32_t>::get_array(_data.packed_array);
      int32_t l = arr->size();
      if (l) {
  ...
  case PACKED_INT64_ARRAY: {
    if (p_index.get_type() == Variant::INT ||
      ...
      const Vector<int64_t> *arr =
          &PackedArrayRef<int64_t>::get_array(_data.packed_array);
      int64_t l = arr->size();
      if (l) {
  ...
  case PACKED_FLOAT32_ARRAY: {
    ...
    if (p_index.get_type() == Variant::INT ||
      ...
      const Vector<float> *arr =
          &PackedArrayRef<float>::get_array(_data.packed_array);
      int l = arr->size();
      if (l) {
  ...
  case PACKED_FLOAT64_ARRAY: {
    ...
    if (p_index.get_type() == Variant::INT ||
      ...
      const Vector<double> *arr =
          &PackedArrayRef<double>::get_array(_data.packed_array);
      int l = arr->size();
      if (l) {
  ...
  case PACKED_STRING_ARRAY: {
    if (p_index.get_type() == Variant::STRING) {
      ...
      const Vector<String> *arr =
          &PackedArrayRef<String>::get_array(_data.packed_array);
  ...
  case PACKED_VECTOR2_ARRAY: {
    if (p_index.get_type() == Variant::VECTOR2) {
      ...
      const Vector<Vector2> *arr =
          &PackedArrayRef<Vector2>::get_array(_data.packed_array);
  ...
  case PACKED_VECTOR3_ARRAY: {
    if (p_index.get_type() == Variant::VECTOR3) {
      ...
      const Vector<Vector3> *arr =
          &PackedArrayRef<Vector3>::get_array(_data.packed_array);
  ...
  case PACKED_COLOR_ARRAY: {
    ...
    if (p_index.get_type() == Variant::COLOR) {
      ...
      Color index = p_index;
      const Vector<Color> *arr =
          &PackedArrayRef<Color>::get_array(_data.packed_array);
...
bool Variant::iter_init(Variant &r_iter, bool &valid) const {
  ...
  case OBJECT: {
    ...
    Variant ret = _get_obj().obj->call(
        CoreStringNames::get_singleton()->_iter_init, refp, 1, ce);
...
bool Variant::iter_next(Variant &r_iter, bool &valid) const {
  ...
  case OBJECT: {
    ...
    const Variant *refp[] = {&vref};
    Variant ret = _get_obj().obj->call(
        CoreStringNames::get_singleton()->_iter_next, refp, 1, ce);
...
Variant Variant::iter_get(const Variant &r_iter, bool &r_valid) const {
  ...
  case OBJECT: {
    ...
    Variant ret = _get_obj().obj->call(
        CoreStringNames::get_singleton()->_iter_get, refp, 1, ce);
...
void Variant::blend(const Variant &a, const Variant &b, float c,
                    Variant &r_dst) {
  if (a.type != b.type) {
  ...
  case VECTOR2I: {
    ...
    r_dst =
        Vector2i(int32_t(vax + vbx * c + 0.5), int32_t(vay + vby * c + 0.5));
  }
    return;
  case RECT2: {
  ...
  case RECT2I: {
    ...
    r_dst = Rect2i(int32_t(vax + vbx * c + 0.5), int32_t(vay + vby * c + 0.5),
                   int32_t(vcx + vdx * c + 0.5), int32_t(vcy + vdy * c + 0.5));
  }
    return;
  case VECTOR3: {
  ...
  case VECTOR3I: {
    ...
    r_dst = Vector3i(int32_t(vax + vbx * c + 0.5), int32_t(vay + vby * c + 0.5),
                     int32_t(vaz + vbz * c + 0.5));
  }
    return;
  case AABB: {
...
void Variant::interpolate(const Variant &a, const Variant &b, float c,
                          Variant &r_dst) {
  ...
  case PLANE: {
    ...
    return;
  case QUAT: {
    r_dst = reinterpret_cast<const Quat *>(a._data._mem)
  ...
  case PACKED_BYTE_ARRAY: {
    ...
    return;
  case PACKED_INT32_ARRAY: {
    const Vector<int32_t> *arr_a =
        &PackedArrayRef<int32_t>::get_array(a._data.packed_array);
    const Vector<int32_t> *arr_b =
        &PackedArrayRef<int32_t>::get_array(b._data.packed_array);
    ...
    return;
  case PACKED_INT64_ARRAY: {
    const Vector<int64_t> *arr_a =
        &PackedArrayRef<int64_t>::get_array(a._data.packed_array);
    const Vector<int64_t> *arr_b =
        &PackedArrayRef<int64_t>::get_array(b._data.packed_array);
    ...
    return;
  case PACKED_FLOAT32_ARRAY: {
    const Vector<float> *arr_a =
        &PackedArrayRef<float>::get_array(a._data.packed_array);
    const Vector<float> *arr_b =
        &PackedArrayRef<float>::get_array(b._data.packed_array);
    int sz = arr_a->size();
    if (sz == 0 || arr_b->size() != sz) {
    ...
    return;
  case PACKED_FLOAT64_ARRAY: {
    const Vector<double> *arr_a =
        &PackedArrayRef<double>::get_array(a._data.packed_array);
    const Vector<double> *arr_b =
        &PackedArrayRef<double>::get_array(b._data.packed_array);
    int sz = arr_a->size();
    if (sz == 0 || arr_b->size() != sz) {
  ...
  case PACKED_STRING_ARRAY: {
    ...
    return;
  case PACKED_VECTOR2_ARRAY: {
    const Vector<Vector2> *arr_a =
        &PackedArrayRef<Vector2>::get_array(a._data.packed_array);
    const Vector<Vector2> *arr_b =
        &PackedArrayRef<Vector2>::get_array(b._data.packed_array);
    int sz = arr_a->size();
    if (sz == 0 || arr_b->size() != sz) {
  ...
  case PACKED_VECTOR3_ARRAY: {
    ...
    const Vector<Vector3> *arr_a =
        &PackedArrayRef<Vector3>::get_array(a._data.packed_array);
    const Vector<Vector3> *arr_b =
        &PackedArrayRef<Vector3>::get_array(b._data.packed_array);
    int sz = arr_a->size();
    if (sz == 0 || arr_b->size() != sz) {
    ...
    return;
  case PACKED_COLOR_ARRAY: {
    const Vector<Color> *arr_a =
        &PackedArrayRef<Color>::get_array(a._data.packed_array);
    const Vector<Color> *arr_b =
        &PackedArrayRef<Color>::get_array(b._data.packed_array);
    int sz = arr_a->size();
    if (sz == 0 || arr_b->size() != sz) {
