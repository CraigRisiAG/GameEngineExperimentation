...
/**
 * @file engine.cpp
 * @brief Core engine implementation for managing iterations, physics, FPS, time scaling, and singleton objects.
 * 
 * This file implements the Engine class, which provides:
 * - Iteration and FPS management (set/get iterations per second, target FPS, frame delay)
 * - Physics configuration (jitter fix, interpolation)
 * - Time scaling controls
 * - Version, author, copyright, donor, and license information retrieval
 * - Singleton object management for engine-wide accessible components
 * - GPU error handling configuration
 * 
 * The Engine class follows the singleton pattern and is responsible for coordinating
 * core engine parameters and providing metadata about the engine build.
 */
#include "engine.h"
