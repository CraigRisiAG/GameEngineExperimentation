 ...
 * @brief Dummy implementation of the Rasterizer system for headless or testing environments.
 *
 * This file provides stub implementations of three core rasterizer classes:
 * - RasterizerSceneDummy: Handles scene rendering operations (shadows, sky, environment, lighting)
 * - RasterizerStorageDummy: Manages resource storage (textures, meshes, materials, particles)
 * - RasterizerCanvasDummy: Provides 2D canvas rendering operations
 * - RasterizerDummy: Main rasterizer interface combining all three
 *
 * All methods are no-op implementations that return default/empty values. This is useful for:
 * - Headless server builds
 * - Unit testing without GPU resources
 * - Minimal rendering pipelines
 *
 * @note This implementation does not perform actual rendering. It's designed to satisfy
 *       the interface contract while avoiding GPU-dependent operations.
 */
