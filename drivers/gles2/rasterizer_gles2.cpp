...
/**
 * @file rasterizer_gles2.cpp
 * @brief OpenGL ES 2.0 rasterizer implementation for the game engine.
 * 
 * This file implements the RasterizerGLES2 class which provides rendering capabilities
 * using OpenGL ES 2.0 (or OpenGL 2.1 via GLES_OVER_GL). It manages the rendering pipeline
 * including storage, canvas, and scene rendering components.
 * 
 * Key Features:
 * - OpenGL debug output support with customizable callback
 * - Frame time tracking and delta time calculation
 * - Render target management and framebuffer operations
 * - Boot image rendering for splash screens
 * - Lens distortion effects support
 * - Per-pixel transparency support on Windows
 * - Compatibility with both OpenGL and OpenGL ES via GLAD loader
 * 
 * The rasterizer coordinates three main components:
 * - RasterizerStorageGLES2: Resource management (textures, meshes, etc.)
 * - RasterizerCanvasGLES2: 2D rendering and canvas operations
 * - RasterizerSceneGLES2: 3D scene rendering
 * 
 * Debug Features:
 * - ARB_debug_output extension support for error tracking
 * - Configurable debug message severity and type filtering
 * - Verbose logging of GL errors with source, type, and severity information
 * 
 * @note Supports both GLES_OVER_GL (OpenGL wrapping ES) and pure GLES2 platforms
 * @note iOS/iPhone platforms have limited debug support due to EGL unavailability
 */
