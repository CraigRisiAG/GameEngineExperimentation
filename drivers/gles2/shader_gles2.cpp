...
/**
 * @file shader_gles2.cpp
 * @brief GLES2 Shader compilation and management implementation
 * 
 * This file implements the ShaderGLES2 class which handles:
 * - OpenGL ES 2.0 shader compilation (vertex and fragment shaders)
 * - Shader program linking and validation
 * - Uniform and attribute location management
 * - Custom shader code integration
 * - Material uniform binding
 * - Shader caching with conditional compilation support
 * 
 * Key features:
 * - Conditional shader compilation based on feature flags
 * - Error logging with source code display for compilation failures
 * - Support for custom shader code injection at multiple points
 * - Automatic uniform location caching for performance
 * - Texture unit pair management for sampler uniforms
 * - Version management for shader variations
 * 
 * The shader system supports splitting shader code into multiple sections:
 * - Vertex/Fragment globals (custom variable declarations)
 * - Vertex/Fragment code (main implementation)
 * - Light shader code (lighting calculations)
 * 
 * Debug features:
 * - DEBUG_OPENGL: Enables OpenGL error checking after operations
 * - DEBUG_SHADER: Enables verbose shader code output during compilation
 * 
 * @note Uses OpenGL ES 2.0 compatible calls (GLSL 100 or 120)
 * @note Supports both standard GLES2 and GL desktop via GLES_OVER_GL
 */
