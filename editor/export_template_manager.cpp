...
/// \brief Manages the download, installation, and removal of export templates.
///
/// This class handles the export template lifecycle including:
/// - Listing installed templates from the templates directory
/// - Downloading templates from mirror servers
/// - Installing templates from ZIP files
/// - Uninstalling specific template versions
/// - Installing Android-specific build sources
///
/// The manager provides a UI for users to manage different engine versions'
/// export templates and checks version compatibility for downloads based on
/// build status (dev, alpha, beta, rc, stable).
...
#include "scene/gui/link_button.h"
  ...
  void ExportTemplateManager::_update_template_list() {
    ...
    Error err =
        d->change_dir(EditorSettings::get_singleton()->get_templates_dir());
    ...
    if (templates.has(current_version)) {
      current->add_color_override("font_color",
                                  get_color("success_color", "Editor"));

      // Only display a redownload button if it can be downloaded in the first
      // place
      if (downloads_available) {
        ...
        redownload->connect(
            "pressed",
            callable_mp(this, &ExportTemplateManager::_download_template),
            varray(current_version));
      }
      ...
      uninstall->connect(
          "pressed",
          callable_mp(this, &ExportTemplateManager::_uninstall_template),
          varray(current_version));
      ...
      if (!downloads_available) {
        ...
        redownload->set_tooltip(TTR("Official export templates aren't "
  ...
  void ExportTemplateManager::_download_template(const String &p_version) {
    ...
    request_mirror->request("https://godotengine.org/mirrorlist/" + p_version +
                            ".json");
  ...
  void ExportTemplateManager::_uninstall_template(const String &p_version) {
    ...
    remove_confirm->set_text(
        vformat(TTR("Remove template version '%s'?"), p_version));
  ...
  void ExportTemplateManager::_uninstall_template_confirm() {
    ...
    ERR_FAIL_COND_MSG(err != OK, "Could not access templates directory at '" +
                                     templates_dir + "'.");
    err = da->change_dir(to_remove);
    ERR_FAIL_COND_MSG(err != OK, "Could not access templates directory at '" +
                                     templates_dir.plus_file(to_remove) + "'.");

    err = da->erase_contents_recursive();
    ERR_FAIL_COND_MSG(err != OK, "Could not remove all templates in '" +
                                     templates_dir.plus_file(to_remove) + "'.");

    da->change_dir("..");
    err = da->remove(to_remove);
    ERR_FAIL_COND_MSG(err != OK, "Could not remove templates directory at '" +
                                     templates_dir.plus_file(to_remove) + "'.");

    _update_template_list();
  }

  bool ExportTemplateManager::_install_from_file(const String &p_file,
                                                 bool p_use_progress) {
    ...
    if (!pkg) {
      ...
      EditorNode::get_singleton()->show_warning(
          TTR("Can't open export templates zip."));
      return false;
    }
    ...
    while (ret == UNZ_OK) {
      ...
      if (file.ends_with("version.txt")) {
        ...
        // major.minor[.patch].status[.module_config] so it can in theory have 3
        // or more slices.
        if (data_str.get_slice_count(".") < 3) {
          EditorNode::get_singleton()->show_warning(
              vformat(TTR("Invalid version.txt format inside templates: %s."),
                      data_str));
          unzClose(pkg);
          return false;
        }
    ...
    if (version == String()) {
      EditorNode::get_singleton()->show_warning(
          TTR("No version.txt found inside templates."));
      unzClose(pkg);
      return false;
    }
    ...
    if (err != OK) {
      EditorNode::get_singleton()->show_warning(
          TTR("Error creating path for templates:") + "\n" + template_path);
      unzClose(pkg);
      return false;
    }
    ...
    while (ret == UNZ_OK) {
      ...
      if (base_dir != contents_dir && base_dir.begins_with(contents_dir)) {
        base_dir = base_dir.substr(contents_dir.length(), file_path.length())
                       .trim_prefix("/");
      ...
      if (!f) {
        ret = unzGoToNextFile(pkg);
        fc++;
        ERR_CONTINUE_MSG(true, "Can't open file from path '" +
                                   String(to_write) + "'.");
      }
      ...
      FileAccess::set_unix_permissions(to_write,
  ...
  void ExportTemplateManager::_http_download_mirror_completed(
      int p_status, int p_code, const PackedStringArray &headers,
    ...
    if (err != OK) {
      EditorNode::get_singleton()->show_warning(
          TTR("Error parsing JSON of mirror list. Please report this issue!"));
      return;
    }
    ...
    if (d.has("mirrors")) {
      ...
      for (int i = 0; i < mirrors.size(); i++) {
        ...
        lb->connect(
            "pressed",
            callable_mp(this, &ExportTemplateManager::_begin_template_download),
            varray(m["url"]));
  ...
  void ExportTemplateManager::_http_download_templates_completed(
      int p_status, int p_code, const PackedStringArray &headers,
      const PackedByteArray &p_data) {
    ...
    default: {
      if (p_code != 200) {
        ...
        if (ret) {
          ...
          EditorNode::get_singleton()->add_io_error(
              vformat(TTR("Templates installation failed.\nThe problematic "
  ...
  void ExportTemplateManager::_begin_template_download(const String &p_url) {
    ...
    if (err != OK) {
      EditorNode::get_singleton()->show_warning(TTR("Error requesting URL:") +
                                                " " + p_url);
      return;
    }
        ...
        if (download_templates->get_body_size() > 0) {
          status += " " +
                    String::humanize_size(
                        download_templates->get_downloaded_bytes()) +
                    "/" +
                    String::humanize_size(download_templates->get_body_size());
          template_download_progress->set_max(
              download_templates->get_body_size());
          template_download_progress->set_value(
              download_templates->get_downloaded_bytes());
        } else {
          status += " " + String::humanize_size(
                              download_templates->get_downloaded_bytes());
        }
        break;
  ...
  Error ExportTemplateManager::install_android_template() {
    ...
    ERR_FAIL_COND_V(err != OK, err);
    {
      ...
      FileAccessRef f =
          FileAccess::open("res://android/build/.gdignore", FileAccess::WRITE);
      ERR_FAIL_COND_V(!f, ERR_CANT_CREATE);
      f->store_line("");
      f->close();
    }
    ...
    ERR_FAIL_COND_V_MSG(!pkg, ERR_CANT_OPEN,
    ...
    ProgressDialog::get_singleton()->add_task(
        "uncompress_src", TTR("Uncompressing Android Build Sources"),
        total_files);
  ...
  ExportTemplateManager::ExportTemplateManager() {
    ...
    main_vb->add_margin_child(TTR("Installed Versions:"), installed_scroll,
                              true);
    ...
    remove_confirm->connect(
        "confirmed",
        callable_mp(this, &ExportTemplateManager::_uninstall_template_confirm));
    ...
    template_open->connect(
        "file_selected",
        callable_mp(this, &ExportTemplateManager::_install_from_file),
        varray(true));
    ...
    request_mirror->connect(
        "request_completed",
        callable_mp(this,
                    &ExportTemplateManager::_http_download_mirror_completed));

    download_templates = memnew(HTTPRequest);
    add_child(download_templates);
    download_templates->connect(
        "request_completed",
        callable_mp(
            this, &ExportTemplateManager::_http_download_templates_completed));
    ...
    template_downloader->connect(
        "popup_hide",
        callable_mp(
            this, &ExportTemplateManager::_window_template_downloader_closed));
    ...
    vbc->add_margin_child(
        TTR("Select mirror from list: (Shift+Click: Open in Browser)"), sc);
