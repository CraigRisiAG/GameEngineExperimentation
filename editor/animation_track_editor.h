...
class AnimationTrackEdit : public Control {
  ...
  void _play_position_draw();
  bool _is_value_key_valid(const Variant &p_key_value,
                           Variant::Type &r_valid_type) const;
...
public:
  ...
  virtual void draw_key_link(int p_index, float p_pixels_sec, int p_x,
                             int p_next_x, int p_clip_left, int p_clip_right);
  virtual void draw_key(int p_index, float p_pixels_sec, int p_x,
                        bool p_selected, int p_clip_left, int p_clip_right);
  virtual void draw_bg(int p_clip_left, int p_clip_right);
  virtual void draw_fg(int p_clip_left, int p_clip_right);

  // helper
  void draw_texture_clipped(const Ref<Texture2D> &p_texture,
                            const Vector2 &p_pos);
  void draw_texture_region_clipped(const Ref<Texture2D> &p_texture,
                                   const Rect2 &p_rect, const Rect2 &p_region);
  void draw_rect_clipped(const Rect2 &p_rect, const Color &p_color,
                         bool p_filled = true);
...
public:
  virtual AnimationTrackEdit *
  create_value_track_edit(Object *p_object, Variant::Type p_type,
                          const String &p_property, PropertyHint p_hint,
                          const String &p_hint_string, int p_usage);
...
public:
  void set_type_and_name(const Ref<Texture2D> &p_type, const String &p_name,
...
class AnimationTrackEditor : public VBoxContainer {
  ...
  int _confirm_insert(InsertData p_id, int p_last_track,
                      bool p_create_beziers = false);
  void _insert_delay();

  void _root_removed(Node *p_root);

  PropertyInfo _find_hint_for_track(int p_idx, NodePath &r_base_path,
                                    Variant *r_current_val = NULL);
...
public:
  ...
  void insert_value_key(const String &p_property, const Variant &p_value,
                        bool p_advance);
  void insert_transform_key(Spatial *p_node, const String &p_sub,
