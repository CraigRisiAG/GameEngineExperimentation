...
/**
 * @brief Registers and configures all fonts used throughout the editor.
 * 
 * This function initializes a comprehensive set of fonts for the editor UI, including:
 * - Default UI fonts (regular and bold variants)
 * - Documentation fonts (regular, bold, title, and source code)
 * - Code editor fonts (source, expressions, output)
 * - Special purpose fonts (rulers, rotation controls)
 * 
 * Font configuration includes:
 * - Support for custom user-provided fonts (main, bold, and code fonts)
 * - Fallback fonts for international text support (Arabic, Hebrew, Thai, Hindi, Japanese)
 * - Antialiasing and hinting settings based on editor preferences
 * - Dynamic scaling based on EDSCALE and user-configured font sizes
 * 
 * All fonts are registered with the theme under the "EditorFonts" category and applied
 * to the provided theme object.
 * 
 * @param p_theme A reference to the Theme object where all fonts will be registered.
 *                Must be a valid Ref<Theme> object.
 * 
 * @note Font hinting is automatically selected based on the operating system:
 *       - macOS: No hinting
 *       - Windows/Linux: Light hinting (default)
 *       User preferences can override this behavior.
 * 
 * @see DynamicFont, DynamicFontData, EditorSettings
 */
