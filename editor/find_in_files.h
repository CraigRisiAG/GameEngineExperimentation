 ...
 * @brief File search and replace functionality for the game engine editor.
 *
 * This file contains three main classes for implementing a "Find in Files"
 * feature:
 *
 * - FindInFiles: Core search engine that performs recursive directory scanning
 * and pattern matching
 * - FindInFilesDialog: UI dialog for configuring search parameters and replace
 * options
 * - FindInFilesPanel: Results display panel with replace functionality
 *
 * The system supports:
 * - Pattern-based text search with regex support
 * - Whole word and case-sensitive matching options
...
private:
  void _on_result_found(String fpath, int line_number, int begin, int end,
                        String text);
  void _on_finished();
  ...
  void apply_replaces_in_file(String fpath, const Vector<Result> &locations,
                              String new_text);
