...
/**
 * @class CreateDialog
 * @brief A dialog for creating new instances of classes or script types in the
 * editor.
 *
 * This class provides a user interface for browsing and selecting from
 * available classes and scripts to create new instances. It supports searching,
 * filtering by type hierarchy, managing favorites, and tracking recently used
 * types.
 *
 * Features:
 * - Type searching with subsequence and substring matching
 * - Favorite type management with drag-and-drop reordering
 * - Recent type history tracking
 * - Feature profile-based type filtering
 * - Support for both C++ classes and global script classes
 * - Custom type instances from editor data
 * - Replace mode for changing existing object types
 *
 * Signals:
 * - create: Emitted when a type is selected and confirmed
 * - favorites_updated: Emitted when the favorites list is modified
 *
 * @method void popup_create(bool p_dont_clear, bool p_replace_mode, const
 * String &p_select_type) Opens the dialog and initializes its state
 * @method void set_base_type(const String &p_base)
 *         Sets the base class type to filter selectable types
 * @method String get_selected_type()
 *         Returns the name of the currently selected type
 * @method Object* instance_selected()
 *         Creates and returns an instance of the selected type
 * @method void select_type(const String &p_type)
 *         Selects and expands the tree to show the specified type
 * @method void set_preferred_search_result_type(const String &p_preferred_type)
 *         Sets the preferred type for search result prioritization
 */
  ...
  if (f) {
    ...
    while (!f->eof_reached()) {
      ...
      if ((ClassDB::class_exists(name) ||
        ...
        ti->set_text(0, l);
        ti->set_icon(0,
                     EditorNode::get_singleton()->get_class_icon(l, base_type));
      }
    }
...
void CreateDialog::_text_changed(const String &p_newtext) { _update_search(); }

void CreateDialog::_sbox_input(const Ref<InputEvent> &p_ie) {

  Ref<InputEventKey> k = p_ie;
  if (k.is_valid() &&
      (k->get_keycode() == KEY_UP || k->get_keycode() == KEY_DOWN ||
       k->get_keycode() == KEY_PAGEUP || k->get_keycode() == KEY_PAGEDOWN)) {
...
void CreateDialog::add_type(const String &p_type,
  ...
  if (cpp_type) {
    ...
    if (!ScriptServer::is_global_class(p_type) ||
        !ed.script_class_is_parent(p_type, base_type))
      return;

    String script_path = ScriptServer::get_global_class_path(p_type);
    if (script_path.find("res://addons/", 0) != -1) {
      if (!EditorNode::get_singleton()->is_addon_plugin_enabled(
              script_path.get_slicec('/', 3)))
        return;
    }
  }

  String inherits = cpp_type ? ClassDB::get_parent_class(p_type)
                             : ed.script_class_get_base(p_type);
  ...
  bool can_instance = (cpp_type && ClassDB::can_instance(p_type)) ||
                      ScriptServer::is_global_class(p_type);
  ...
  if (cpp_type) {
    ...
    item->set_text(
        0, p_type + " (" +
               ScriptServer::get_global_class_path(p_type).get_file() + ")");
  }
  if (!can_instance) {
    item->set_custom_color(0, get_color("disabled_font_color", "Editor"));
    item->set_selectable(0, false);
  } else if (!(*to_select &&
               (*to_select)->get_text(0) == search_box->get_text())) {
    String search_term = search_box->get_text().to_lower();

    // if the node name matches exactly as the search, the node should be
    // selected. this also fixes when the user clicks on recent nodes.
    if (p_type.to_lower() == search_term) {
      *to_select = item;
      ...
      bool selected_type_prefered =
          *to_select
              ? _is_type_prefered((*to_select)->get_text(0).split(" ")[0])
              : false;
      ...
      if (is_subsequence_of_type && !is_selected_equal) {
        if (is_substring_of_type) {
          if (!is_substring_of_selected ||
              (current_type_prefered && !selected_type_prefered)) {
            *to_select = item;
          }
        } else {
          // substring results weigh more than subsequences, so let's make sure
          // we don't override them
          if (!is_substring_of_selected) {
            if (!is_subsequence_of_selected ||
                (current_type_prefered && !selected_type_prefered)) {
              *to_select = item;
            }
          }
  ...
  item->set_icon(
      0, EditorNode::get_singleton()->get_class_icon(p_type, base_type));
...
bool CreateDialog::_is_class_disabled_by_feature_profile(
...
void CreateDialog::_update_search() {
  ...
  for (List<StringName>::Element *I = type_list.front(); I; I = I->next()) {
    ...
    if (cpp_type) {
      ...
      for (Set<StringName>::Element *E = type_blacklist.front(); E && !skip;
           E = E->next()) {
    ...
    if (search_box->get_text() == "") {
      ...
      while (type2 != "" &&
        ...
        type2 = cpp_type ? ClassDB::get_parent_class(type2)
    ...
    if (EditorNode::get_editor_data().get_custom_types().has(type) &&
      ...
      const Vector<EditorData::CustomType> &ct =
          EditorNode::get_editor_data().get_custom_types()[type];
...
void CreateDialog::_notification(int p_what) {
  ...
  case NOTIFICATION_POPUP_HIDE: {
    EditorSettings::get_singleton()->get_project_metadata(
        "dialog_bounds", "create_new_node", get_rect());
...
String CreateDialog::get_base_type() const { return base_type; }

void CreateDialog::set_preferred_search_result_type(
    const String &p_preferred_type) {
  ...
  if (selected) {
    ...
    if (custom != String()) {
      if (ScriptServer::is_global_class(custom)) {
        Object *obj =
            EditorNode::get_editor_data().script_class_instance(custom);
...
bool CreateDialog::can_drop_data_fw(const Point2 &p_point,
...
void CreateDialog::drop_data_fw(const Point2 &p_point, const Variant &p_data,
                                Control *p_from) {
...
void CreateDialog::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("_save_and_update_favorite_list"),
...
CreateDialog::CreateDialog() {
  ...
  favorites->connect("cell_selected",
                     callable_mp(this, &CreateDialog::_favorite_selected));
  favorites->connect("item_activated",
                     callable_mp(this, &CreateDialog::_favorite_activated));
  ...
  recent->connect("cell_selected",
                  callable_mp(this, &CreateDialog::_history_selected));
  recent->connect("item_activated",
                  callable_mp(this, &CreateDialog::_history_activated));
  ...
  favorite->connect("pressed",
                    callable_mp(this, &CreateDialog::_favorite_toggled));
  vbc->add_margin_child(TTR("Search:"), search_hb);
  search_box->connect("text_changed",
                      callable_mp(this, &CreateDialog::_text_changed));
  search_box->connect("gui_input",
                      callable_mp(this, &CreateDialog::_sbox_input));
  search_options = memnew(Tree);
  vbc->add_margin_child(TTR("Matches:"), search_options, true);
  get_ok()->set_disabled(true);
  register_text_enter(search_box);
  set_hide_on_ok(false);
  search_options->connect("item_activated",
                          callable_mp(this, &CreateDialog::_confirmed));
  search_options->connect("cell_selected",
                          callable_mp(this, &CreateDialog::_item_selected));
  base_type = "Object";
  preferred_search_result_type = "";

  help_bit = memnew(EditorHelpBit);
  vbc->add_margin_child(TTR("Description:"), help_bit);
  help_bit->connect("request_hide",
                    callable_mp(this, &CreateDialog::_hide_requested));

  type_blacklist.insert(
      "PluginScript"); // PluginScript must be initialized before use, which is
                       // not possible here
  type_blacklist.insert(
      "ScriptCreateDialog"); // This is an exposed editor Node that doesn't have
                             // an Editor prefix.
}
