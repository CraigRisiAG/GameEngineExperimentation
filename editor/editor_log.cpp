...
void EditorLog::_error_handler(void *p_self, const char *p_func,
                               const char *p_file, int p_line,
                               const char *p_error, const char *p_errorexp,
...
void EditorLog::_notification(int p_what) {
  ...
  if (p_what == NOTIFICATION_ENTER_TREE) {
    ...
    log->add_font_override("normal_font",
...
void EditorLog::_copy_request() { log->selection_copy(); }

void EditorLog::clear() { _clear_request(); }

void EditorLog::copy() { _copy_request(); }
...
EditorLog::EditorLog() {
  ...
  copybutton->set_shortcut(ED_SHORTCUT(
      "editor/copy_output", TTR("Copy Selection"), KEY_MASK_CMD | KEY_C));
  copybutton->connect("pressed", callable_mp(this, &EditorLog::_copy_request));

  clearbutton = memnew(Button);
  hb->add_child(clearbutton);
  clearbutton->set_text(TTR("Clear"));
  clearbutton->set_shortcut(ED_SHORTCUT("editor/clear_output",
                                        TTR("Clear Output"),
                                        KEY_MASK_CMD | KEY_MASK_SHIFT | KEY_K));
  clearbutton->connect("pressed",
                       callable_mp(this, &EditorLog::_clear_request));
  ...
  add_message(
      VERSION_FULL_NAME
      " (c) 2007-2020 Juan Linietsky, Ariel Manzur & Godot Contributors.");
  ...
  add_constant_override("separation",
