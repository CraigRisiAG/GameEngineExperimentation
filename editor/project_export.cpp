 ...
 * @brief Dialog for managing and configuring game export presets and settings.
 * 
 * This class provides a comprehensive UI for managing export presets for different platforms.
 * It allows users to create, edit, duplicate, and delete export presets with customizable
 * settings such as export path, script encryption, resource filtering, and platform-specific options.
 * 
 * Features include:
 * - Preset management (add, edit, duplicate, delete)
 * - Export path configuration
 * - Resource and scene filtering
 * - Script export modes (text, compiled, encrypted)
 * - Custom feature tags
 * - Patch package management
 * - Drag-and-drop support for presets and patches
 * - Export validation with error reporting
 * - Batch export functionality for all presets
 * 
 * @note Patch functionality is currently hidden and not fully implemented.
 * 
 * The dialog is organized into tabs:
 * - Options: Platform-specific export parameters
 * - Resources: Resource and file filtering settings
 * - Patches: Patch package configuration (hidden)
 * - Features: Custom feature tag management
 * - Script: Script export mode and encryption settings
 */
