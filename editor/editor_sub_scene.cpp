 ...
 * EditorSubScene
 * 
 * A dialog editor for selecting and importing nodes from a sub-scene.
 * 
 * This class provides functionality to browse and select nodes from an external PackedScene file
 * for importing into the current scene. It displays a tree view of the selected scene's node hierarchy
 * and allows multi-selection of nodes to be moved to a new parent.
 * 
 * Signals:
 *   - subscene_selected: Emitted when the user confirms the node selection and import operation.
 * 
 * Key Methods:
 *   - move(Node*, Node*): Moves the selected nodes to a new parent and reassigns ownership.
 *   - clear(): Clears the path and resets the scene state.
 */
