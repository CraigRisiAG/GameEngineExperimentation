 ...
 * @brief Dependency management and orphan resource handling for the Godot
 * editor.
 *
 * This file contains implementations for several dependency-related editor
 * dialogs:
 *
 * @class DependencyEditor
 * Manages resource dependencies and allows users to find and replace missing
 * dependencies. Features:
 * - Display resource dependencies in a tree view
 * - Search and replace missing dependencies
 * - Automatic fixing of broken dependencies by scanning the file system
 *
 * @class DependencyEditorOwners
 * Shows which files depend on a given resource (reverse dependency viewer).
 * Allows opening dependent resources directly from the editor.
 *
 * @class DependencyRemoveDialog
 * Handles safe deletion of files and folders by warning users about dependent
 * resources. Features:
 * - Displays a tree of all resources that depend on files being deleted
 * - Updates project settings if critical resources are deleted
 * - Cleans up favorite directories list after deletion
 * - Moves files to trash instead of permanently deleting them
 *
 * @class DependencyErrorDialog
 * Displays load errors caused by missing dependencies.
 * Offers options to open the file anyway or automatically fix dependencies.
 *
 * @class OrphanResourcesDialog
 * Identifies and allows deletion of unused resources (orphans).
 * Features:
 * - Scans file system to find resources with no dependencies
 * - Tree view with checkboxes for selective deletion
 * - Shows dependency counts for each resource
 * - Allows viewing dependencies before deletion
 */
...
void DependencyEditor::_load_pressed(Object *p_item, int p_cell, int p_button) {
  ...
  search->set_title(TTR("Search Replacement For:") + " " +
                    replacing.get_file());
  ...
  search->popup_centered_ratio(
      0.65); // So it doesn't completely cover the dialog below it.
}
...
void DependencyEditor::_fix_all() {
  ...
  _fix_and_find(EditorFileSystem::get_singleton()->get_filesystem(),
                candidates);
...
void DependencyEditor::edit(const String &p_path) {
  ...
  popup_centered_ratio(
      0.7); // So it doesn't completely cover the dialog below it.

  if (EditorNode::get_singleton()->is_scene_open(p_path)) {
    EditorNode::get_singleton()->show_warning(
        vformat(TTR("Scene '%s' is currently being edited.\nChanges will only "
                    "take effect when reloaded."),
                p_path.get_file()));
  } else if (ResourceCache::has(p_path)) {
    EditorNode::get_singleton()->show_warning(
        vformat(TTR("Resource '%s' is in use.\nChanges will only take effect "
                    "when reloaded."),
                p_path.get_file()));
  }
}
...
DependencyEditor::DependencyEditor() {
  ...
  tree->connect("button_pressed",
                callable_mp(this, &DependencyEditor::_load_pressed));
  ...
  search->connect("file_selected",
                  callable_mp(this, &DependencyEditor::_searched));
...
/////////////////////////////////////
void DependencyEditorOwners::_list_rmb_select(int p_item,
                                              const Vector2 &p_pos) {
...
DependencyEditorOwners::DependencyEditorOwners(EditorNode *p_editor) {
  ...
  file_options->connect(
      "id_pressed", callable_mp(this, &DependencyEditorOwners::_file_option));

  owners = memnew(ItemList);
  owners->set_select_mode(ItemList::SELECT_SINGLE);
  owners->connect("item_rmb_selected",
                  callable_mp(this, &DependencyEditorOwners::_list_rmb_select));
  owners->connect("item_activated",
                  callable_mp(this, &DependencyEditorOwners::_select_file));
...
void DependencyRemoveDialog::_find_files_in_removed_folder(
  ...
  for (int i = 0; i < efsd->get_file_count(); i++) {
    ...
    ERR_FAIL_COND(all_remove_files.has(
        file)); // We are deleting a directory which is contained in a directory
                // we are deleting...
    all_remove_files[file] =
        p_folder; // Point the file to the ancestor directory we are deleting so
                  // we know what to parent it under in the tree.
  }
}
...
void DependencyRemoveDialog::_find_all_removed_dependencies(
  ...
  for (int i = 0; i < efsd->get_file_count(); i++) {
    ...
    // It doesn't matter if a file we are about to delete will have some of its
...
void DependencyRemoveDialog::show(const Vector<String> &p_folders,
  ...
  if (removed_deps.empty()) {
    owners->hide();
    text->set_text(
        TTR("Remove selected files from the project? (Can't be restored)"));
    set_size(Size2());
    popup_centered();
  } else {
    ...
    owners->show();
    text->set_text(
        TTR("The files being removed are required by other resources in order "
...
void DependencyRemoveDialog::ok_pressed() {
  ...
  for (int i = 0; i < files_to_delete.size(); ++i) {
    ...
    if (files_to_delete[i] == ProjectSettings::get_singleton()->get(
...
void DependencyErrorDialog::show(Mode p_mode, const String &p_for_file,
...
DependencyErrorDialog::DependencyErrorDialog() {
  ...
  vb->add_margin_child(TTR("Load failed due to missing dependencies:"), files,
                       true);
...
bool OrphanResourcesDialog::_fill_owners(EditorFileSystemDirectory *efsd,
  ...
  for (int i = 0; i < efsd->get_file_count(); i++) {
    ...
    if (!p_parent) {
      ...
      if (!refs.has(path)) {
        ...
        if (ds) {
          ti->add_button(1, get_icon("GuiVisibilityVisible", "EditorIcons"), -1,
                         false, TTR("Show Dependencies"));
        }
...
void OrphanResourcesDialog::_find_to_delete(TreeItem *p_item,
                                            List<String> &paths) {
...
void OrphanResourcesDialog::_button_pressed(Object *p_item, int p_column,
                                            int p_id) {
...
OrphanResourcesDialog::OrphanResourcesDialog() {
  ...
  add_child(dep_edit);
  delete_confirm->connect(
      "confirmed", callable_mp(this, &OrphanResourcesDialog::_delete_confirm));
  ...
  files->set_hide_root(true);
  vbc->add_margin_child(TTR("Resources Without Explicit Ownership:"), files,
                        true);
  files->connect("button_pressed",
                 callable_mp(this, &OrphanResourcesDialog::_button_pressed));
}
