...
/// @brief Implementation of find and replace functionality across multiple
/// files in the editor.
///
/// This file contains three main classes:
/// - FindInFiles: Core search engine that scans directories and files for
/// pattern matches
/// - FindInFilesDialog: UI dialog for configuring search/replace parameters
/// - FindInFilesPanel: Results display panel with replace functionality
///
/// The search process is asynchronous and frame-rate limited to avoid UI
/// freezing. It supports whole word matching, case sensitivity, and file type
/// filtering. Replace operations modify files directly with offset tracking for
/// multiple replacements per line.

/// @class FindInFiles
/// @brief Asynchronous file search engine that processes directories and files
/// incrementally.
///
/// Emits SIGNAL_RESULT_FOUND when matches are discovered and SIGNAL_FINISHED
/// when complete. Uses a stack-based folder traversal to build a file list
/// before scanning contents.

/// @class FindInFilesDialog
/// @brief Modal dialog for configuring search and replace parameters.
///
/// Provides UI elements for:
/// - Search pattern input
/// - Replace text (in REPLACE_MODE)
/// - Whole words and case sensitivity options
/// - Target folder selection
/// - File type filtering via checkboxes
///
/// Emits SIGNAL_FIND_REQUESTED or SIGNAL_REPLACE_REQUESTED when user confirms.

/// @class FindInFilesPanel
/// @brief Results display and replace execution panel.
///
/// Features:
/// - Tree view display of results grouped by file
/// - Custom text highlighting for matched regions
/// - Checkbox selection for selective replacement
/// - Real-time progress bar during search
/// - Batch replace with file offset tracking for successive replacements
///
/// Emits SIGNAL_RESULT_SELECTED when user clicks a result and
/// SIGNAL_FILES_MODIFIED after replacement.

/// @class ConservativeGetLine
/// @brief Helper class that reads file lines while preserving line ending
/// characters.
///
/// Unlike FileAccess::get_line(), this preserves \n and \r characters needed
/// for accurate file rewriting.
...
template <typename T> inline void pop_back(T &container) {
...
static bool find_next(const String &line, String pattern, int from,
                      bool match_case, bool whole_words, int &out_begin,
                      int &out_end) {
...
void FindInFiles::set_search_text(String p_pattern) { _pattern = p_pattern; }
...
void FindInFiles::set_folder(String folder) { _root_dir = folder; }
...
void FindInFiles::_iterate() {
  ...
  if (_folders_stack.size() != 0) {
    ...
    // Scan folders first so we can build a list of files and have progress info
    // later

    PackedStringArray &folders_to_scan =
...
void FindInFiles::_scan_file(String fpath) {
  ...
  while (!f->eof_reached()) {
    ...
    while (
        find_next(line, _pattern, end, _match_case, _whole_words, begin, end)) {
...
void FindInFiles::_bind_methods() {
  ...
  ADD_SIGNAL(MethodInfo(
      SIGNAL_RESULT_FOUND, PropertyInfo(Variant::STRING, "path"),
      PropertyInfo(Variant::INT, "line_number"),
      PropertyInfo(Variant::INT, "begin"), PropertyInfo(Variant::INT, "end"),
...
FindInFilesDialog::FindInFilesDialog() {
  ...
  _search_text_line_edit->connect(
      "text_changed",
      callable_mp(this, &FindInFilesDialog::_on_search_text_modified));
  _search_text_line_edit->connect(
      "text_entered",
      callable_mp(this, &FindInFilesDialog::_on_search_text_entered));
  ...
  _replace_text_line_edit->connect(
      "text_entered",
      callable_mp(this, &FindInFilesDialog::_on_replace_text_entered));
...

  {
    ...
    folder_button->connect(
        "pressed",
        callable_mp(this, &FindInFilesDialog::_on_folder_button_pressed));
    hbc->add_child(folder_button);

    _folder_dialog = memnew(FileDialog);
    _folder_dialog->set_mode(FileDialog::MODE_OPEN_DIR);
    _folder_dialog->connect(
        "dir_selected",
        callable_mp(this, &FindInFilesDialog::_on_folder_selected));
  ...
  filter_label->set_tooltip(
      TTR("Include the files with the following extensions. Add or remove them "
          "in ProjectSettings."));
...
FindInFilesPanel::FindInFilesPanel() {
  ...
  _finder->connect(FindInFiles::SIGNAL_RESULT_FOUND,
                   callable_mp(this, &FindInFilesPanel::_on_result_found));
  _finder->connect(FindInFiles::SIGNAL_FINISHED,
                   callable_mp(this, &FindInFilesPanel::_on_finished));
  add_child(_finder);
...

  {
    ...
    _search_text_label->add_font_override(
        "font", EditorNode::get_singleton()->get_gui_base()->get_font(
                    "source", "EditorFonts"));
    ...
    _refresh_button->connect(
        "pressed",
        callable_mp(this, &FindInFilesPanel::_on_refresh_button_clicked));
    _refresh_button->hide();
    hbc->add_child(_refresh_button);

    _cancel_button = memnew(Button);
    _cancel_button->set_text(TTR("Cancel"));
    _cancel_button->connect(
        "pressed",
        callable_mp(this, &FindInFilesPanel::_on_cancel_button_clicked));
    _cancel_button->hide();
  ...
  _results_display->connect(
      "item_selected",
      callable_mp(this, &FindInFilesPanel::_on_result_selected));
  _results_display->connect(
      "item_edited", callable_mp(this, &FindInFilesPanel::_on_item_edited));
...

  {
    ...
    _replace_line_edit->connect(
        "text_changed",
        callable_mp(this, &FindInFilesPanel::_on_replace_text_changed));
    ...
    _replace_all_button->connect(
        "pressed",
        callable_mp(this, &FindInFilesPanel::_on_replace_all_clicked));
...
void FindInFilesPanel::_on_result_found(String fpath, int line_number,
                                        int begin, int end, String text) {
  ...
  if (E == NULL) {
    ...
    // The width of this column is restrained to checkboxes, but that doesn't
    // make sense for the parent items, so we override their width so they can
  ...
  String item_text =
      vformat("%3s:    %s", line_number, text.replace("\t", "    "));
  ...
  r.begin = begin;
  r.end = end;
  r.draw_begin = (item_text_width - raw_text_width) +
                 font->get_string_size(text.left(r.begin)).x;
...
void FindInFilesPanel::apply_replaces_in_file(String fpath,
  ...
  for (int i = 0; i < locations.size(); ++i) {
    ...
    int _;
    if (!find_next(line, search_text, repl_begin, _finder->is_match_case(),
                   _finder->is_whole_words(), _, _)) {
      // Make sure the replace is still valid in case the file was tampered
      // with.
      print_verbose(String("Occurrence no longer matches, replace will be "
                           "ignored in {0}: line {1}, col {2}")
...
void FindInFilesPanel::_bind_methods() {
  ...
  ClassDB::bind_method("_draw_result_text",
                       &FindInFilesPanel::draw_result_text);

  ADD_SIGNAL(MethodInfo(
      SIGNAL_RESULT_SELECTED, PropertyInfo(Variant::STRING, "path"),
      PropertyInfo(Variant::INT, "line_number"),
      PropertyInfo(Variant::INT, "begin"), PropertyInfo(Variant::INT, "end")));

  ADD_SIGNAL(MethodInfo(SIGNAL_FILES_MODIFIED,
