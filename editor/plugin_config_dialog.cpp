 ...
 * @brief Dialog for creating and editing editor plugin configurations.
 *
 * This dialog provides a user interface for configuring editor plugins,
 * allowing users to:
 * - Create new plugins with customizable settings (name, description, author,
 * version)
 * - Select the scripting language for the plugin
 * - Edit existing plugin configurations
 * - Automatically generate plugin script templates
 *
 * The dialog supports multiple scripting languages through ScriptServer and has
 * special handling for GDScript with template generation. When creating a new
 * plugin, it:
 * - Creates the plugin directory structure
 * - Generates a plugin.cfg configuration file
 * - Creates a template script file with appropriate EditorPlugin boilerplate
 *
 * @signal plugin_ready(script_path: String, activate_name: String)
 *         Emitted when a plugin is successfully created or configured.
 *         Parameters:
 *         - script_path: Path to the generated or edited plugin script
 *         - activate_name: Subfolder name if activation was requested, empty
 * string otherwise
 *
 * @note The dialog has two modes:
 *       - Create mode: For new plugins (shows subfolder and activation options)
 *       - Edit mode: For existing plugins (hides subfolder and activation
 * options)
 *
 * @see EditorPlugin
 * @see ConfigFile
...
void PluginConfigDialog::_on_confirmed() {
  ...
  if (!_edit_mode) {
    ...
    if (lang_name == GDScriptLanguage::get_singleton()->get_name()) {
      // Hard-coded GDScript template to keep usability until we use script
      // templates.
      Ref<Script> gdscript = memnew(GDScript);
      gdscript->set_source_code("tool\n"
    ...
    emit_signal("plugin_ready", script.operator->(),
...
void PluginConfigDialog::_on_cancelled() { _clear_fields(); }

void PluginConfigDialog::_on_required_text_changed(const String &) {
  int lang_idx = script_option_edit->get_selected();
  String ext = ScriptServer::get_language(lang_idx)->get_extension();
  get_ok()->set_disabled(script_edit->get_text().get_basename().empty() ||
...
void PluginConfigDialog::_notification(int p_what) {
  ...
  case NOTIFICATION_READY: {
    ...
    get_cancel()->connect(
        "pressed", callable_mp(this, &PluginConfigDialog::_on_cancelled));
  } break;
...
void PluginConfigDialog::config(const String &p_config_path) {
  if (p_config_path.length()) {
    ...
    ERR_FAIL_COND_MSG(err != OK, "Cannot load config file from path '" +
                                     p_config_path + "'.");
...
PluginConfigDialog::PluginConfigDialog() {
  ...
  name_edit->connect(
      "text_changed",
      callable_mp(this, &PluginConfigDialog::_on_required_text_changed));
