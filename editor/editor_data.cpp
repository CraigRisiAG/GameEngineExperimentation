 ...
 * @brief Implementation of editor data management classes for the Godot Engine
 * editor.
 *
 * This file contains the implementation of three main classes:
 * - EditorHistory: Manages the navigation history of edited objects in the
 * editor
 * - EditorData: Manages editor plugins, scenes, custom types, and editor states
 * - EditorSelection: Manages the selection of nodes in the scene tree
 *
 * EditorHistory provides functionality to:
 * - Track and navigate through a history of edited objects
 * - Clean up invalid history entries
 * - Support hierarchical object properties (subproperties)
 * - Query current history state
 *
 * EditorData provides functionality to:
 * - Manage editor plugins (main editors and sub-editors)
 * - Manage edited scenes with their states
 * - Handle custom types and script classes
 * - Manage undo/redo operations
 * - Save and restore editor states
 * - Handle scene versioning and updates
 * - Copy/paste object parameters
 * - Track editor breakpoints
 *
 * EditorSelection provides functionality to:
 * - Track selected nodes in the editor
 * - Manage node metadata through editor plugins
 * - Filter out child nodes when parents are selected
...
void EditorHistory::_add_object(ObjectID p_object, const String &p_property,
                                int p_level_change, bool p_inspector_only) {
...
int EditorHistory::get_history_len() { return history.size(); }
int EditorHistory::get_history_pos() { return current; }
...
ObjectID EditorHistory::get_history_obj(int p_obj) const {
  ...
  ERR_FAIL_INDEX_V(history[p_obj].level, history[p_obj].path.size(),
                   ObjectID());
  return history[p_obj].path[history[p_obj].level].object;
}

bool EditorHistory::is_at_beginning() const { return current <= 0; }
...
EditorHistory::EditorHistory() { current = -1; }
...
UndoRedo &EditorData::get_undo_redo() { return undo_redo; }
...
void EditorData::add_custom_type(const String &p_type, const String &p_inherits,
                                 const Ref<Script> &p_script,
                                 const Ref<Texture2D> &p_icon) {

  ERR_FAIL_COND_MSG(p_script.is_null(),
...
bool EditorData::_find_updated_instances(Node *p_root, Node *p_node,
  ...
  */
  ...
  for (int i = 0; i < p_node->get_child_count(); i++) {
    ...
    bool found =
        _find_updated_instances(p_root, p_node->get_child(i), checked_paths);
    if (found)
      return true;
  }
...
bool EditorData::check_and_update_scene(int p_idx) {
  ...
  bool must_reload = _find_updated_instances(
      edited_scene[p_idx].root, edited_scene[p_idx].root, checked_scenes);
  ...
  if (must_reload) {
    ...
    for (List<Node *>::Element *E = edited_scene.write[p_idx].selection.front();
         E; E = E->next()) {
...
void EditorData::save_edited_scene_state(EditorSelection *p_selection,
...
Dictionary EditorData::restore_edited_scene_state(EditorSelection *p_selection,
                                                  EditorHistory *p_history) {
...
bool EditorData::script_class_is_parent(const String &p_class,
  ...
  while (p_inherits != base) {
    if (ClassDB::class_exists(base)) {
      ...
      return ClassDB::is_parent_class(base_script->get_instance_base_type(),
                                      p_inherits);
    } else {
      return false;
    }
...
void EditorData::script_class_set_icon_path(const String &p_class,
...
void EditorData::script_class_set_name(const String &p_path,
...
void EditorSelection::add_node(Node *p_node) {
  ...
  p_node->connect("tree_exiting",
                  callable_mp(this, &EditorSelection::_node_removed),
                  varray(p_node), CONNECT_ONESHOT);
...
void EditorSelection::remove_node(Node *p_node) {
  ...
  if (meta)
    memdelete(meta);
  selection.erase(p_node);
  p_node->disconnect("tree_exiting",
                     callable_mp(this, &EditorSelection::_node_removed));
  // emit_signal("selection_changed");
}
...
void EditorSelection::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("add_node", "node"),
                       &EditorSelection::add_node);
  ClassDB::bind_method(D_METHOD("remove_node", "node"),
                       &EditorSelection::remove_node);
  ClassDB::bind_method(D_METHOD("get_selected_nodes"),
                       &EditorSelection::get_selected_nodes);
  ClassDB::bind_method(D_METHOD("get_transformable_selected_nodes"),
                       &EditorSelection::_get_transformable_selected_nodes);
  ClassDB::bind_method(D_METHOD("_emit_change"),
