 ...
 * @brief Editor property editors for various data types in the Godot editor inspector.
 *
 * This file contains implementations of property editor classes for displaying and editing
 * different property types in the inspector panel. It includes editors for:
 * - Primitive types (bool, int, float, string)
 * - Math types (Vector2, Vector3, Transform, Basis, etc.)
 * - Resource types and object references
 * - Special types (Color, NodePath, RID, etc.)
 * - Collection types (Array, Dictionary)
 *
 * Each editor class provides:
 * - update_property(): Updates the UI to reflect the current property value
 * - Value change callbacks: Emits signals when the user modifies the property
 * - setup(): Configures editor parameters (min/max values, options, etc.)
 * - Custom UI rendering and interaction handling
 *
 * The EditorInspectorDefaultPlugin at the end creates appropriate editor instances
 * based on property type and hints, serving as the default handler for all property types.
 */
