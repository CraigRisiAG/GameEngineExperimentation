...
/// @brief Implementation of FileSystemDock, a file system browser and manager
/// for the editor.
///
/// This file contains the complete implementation of the FileSystemDock class,
/// which provides a user interface for browsing, managing, and organizing
/// project files and folders. The dock displays the file system in a tree view
/// and/or list view, supports drag-and-drop operations, favorites management,
/// file searching, and various file operations like rename, duplicate, move,
/// and delete.
///
/// Key features include:
/// - Dual display modes: tree-only view and split view (tree + file list)
/// - File and folder search functionality with real-time filtering
/// - Favorites management for quick access to frequently used files/folders
/// - Drag-and-drop support for moving files and organizing favorites
/// - Right-click context menus for various file operations
/// - Resource preview thumbnails for visual file identification
/// - File history navigation (back/forward buttons)
/// - Integration with the import dock for asset import settings
/// - Support for feature profiles to show/hide file types
/// - Editor integration for opening scenes, inheriting scenes, and instancing
///
/// The class maintains:
/// - Tree structure representing the project's file system hierarchy
/// - File list for detailed view of directory contents
/// - Navigation history for quick file/folder access
/// - Search state and filtered results
/// - Display mode preferences and layout state
///
/// Main responsibilities:
/// - Building and maintaining the file system tree UI
/// - Handling file/folder operations (move, rename, delete, duplicate)
/// - Managing favorites and file system navigation
/// - Updating resource paths and dependencies after file moves
/// - Providing drag-and-drop functionality for files and folders
/// - Integrating with the editor's resource and scene management systems
...
Ref<Texture2D>
FileSystemDock::_get_tree_item_icon(EditorFileSystemDirectory *p_dir,
                                    int p_idx) {
  ...
  if (!p_dir->get_file_import_is_valid(p_idx)) {
    ...
    file_icon = (has_icon(file_type, "EditorIcons"))
                    ? get_icon(file_type, "EditorIcons")
                    : get_icon("File", "EditorIcons");
  }
  return file_icon;
}

bool FileSystemDock::_create_tree(TreeItem *p_parent,
  ...
  subdirectory_item->set_icon_modulate(
      0, get_color("folder_icon_modulate", "FileDialog"));
  subdirectory_item->set_selectable(0, true);
  String lpath = p_dir->get_path();
  subdirectory_item->set_metadata(0, lpath);
  if (!p_select_in_favorites &&
      (path == lpath || ((display_mode == DISPLAY_MODE_SPLIT) &&
  ...
  for (int i = 0; i < p_dir->get_subdir_count(); i++)
    parent_should_expand =
        (_create_tree(subdirectory_item, p_dir->get_subdir(i),
                      uncollapsed_paths, p_select_in_favorites,
                      p_unfold_path) ||
         parent_should_expand);
  ...
  if (display_mode == DISPLAY_MODE_TREE_ONLY) {
    ...
    for (int i = 0; i < p_dir->get_file_count(); i++) {
      ...
      EditorResourcePreview::get_singleton()->queue_resource_preview(
          file_metadata, this, "_tree_thumbnail_done", udata);
    }
...
void FileSystemDock::_update_tree(const Vector<String> &p_uncollapsed_paths,
  ...
  for (int i = 0; i < favorite_paths.size(); i++) {
    ...
    if (searched_string.length() == 0 ||
      ...
      if (!fave.ends_with("/")) {
        ...
        udata.push_back(ti);
        EditorResourcePreview::get_singleton()->queue_resource_preview(
            fave, this, "_tree_thumbnail_done", udata);
      }
    }
  }
  ...
  _create_tree(root, EditorFileSystem::get_singleton()->get_filesystem(),
               uncollapsed_paths, p_select_in_favorites, p_unfold_path);
...
void FileSystemDock::_notification(int p_what) {
  ...
  case NOTIFICATION_ENTER_TREE: {
    ...
    EditorFeatureProfileManager::get_singleton()->connect(
        ...
        callable_mp(this, &FileSystemDock::_feature_profile_changed));

    EditorFileSystem::get_singleton()->connect(
        "filesystem_changed", callable_mp(this, &FileSystemDock::_fs_changed));
    EditorResourcePreview::get_singleton()->connect(
        "preview_invalidated",
        callable_mp(this, &FileSystemDock::_preview_invalidated));

    String ei = "EditorIcons";
    button_reload->set_icon(get_icon("Reload", ei));
    button_toggle_display_mode->set_icon(get_icon("Panels2", ei));
    button_file_list_display_mode->connect(
        "pressed", callable_mp(this, &FileSystemDock::_toggle_file_display));

    files->connect(
        "item_activated",
        callable_mp(this, &FileSystemDock::_file_list_activate_file));
    button_hist_next->connect("pressed",
                              callable_mp(this, &FileSystemDock::_fw_history));
    button_hist_prev->connect("pressed",
                              callable_mp(this, &FileSystemDock::_bw_history));
    ...
    file_list_popup->connect(
        "id_pressed",
        callable_mp(this, &FileSystemDock::_file_list_rmb_option));
    tree_popup->connect("id_pressed",
                        callable_mp(this, &FileSystemDock::_tree_rmb_option));

    current_path->connect("text_entered",
                          callable_mp(this, &FileSystemDock::_navigate_to_path),
                          make_binds(false));
  ...
  case EditorSettings::NOTIFICATION_EDITOR_SETTINGS_CHANGED: {
    ...
    bool new_always_show_folders = bool(EditorSettings::get_singleton()->get(
        "docks/filesystem/always_show_folders"));
...
void FileSystemDock::_tree_multi_selected(Object *p_item, int p_column,
                                          bool p_selected) {
...
String FileSystemDock::get_current_path() const { return path; }
...
void FileSystemDock::_navigate_to_path(const String &p_path,
                                       bool p_select_in_favorites) {
  if (p_path == "Favorites") {
    path = p_path;
  } else {
    String target_path = p_path;
    // If the path is a file, do not only go to the directory in the tree, also
    ...
    if (dirAccess->file_exists(p_path)) {
      ...
      memdelete(dirAccess);
      ERR_FAIL_MSG(vformat("Cannot navigate to '%s' as it has not been found "
                           "in the file system!",
                           p_path));
    }
    memdelete(dirAccess);
...
void FileSystemDock::_tree_thumbnail_done(const String &p_path,
...
void FileSystemDock::_search(EditorFileSystemDirectory *p_path,
                             List<FileInfo> *matches, int p_max_items) {
...
void FileSystemDock::_update_file_list(bool p_keep_selection) {
  ...
  if (path == "Favorites") {
    ...
    if (searched_string.length() > 0) {
      ...
      _search(EditorFileSystem::get_singleton()->get_filesystem(), &filelist,
              128);
    } else {
      if (display_mode == DISPLAY_MODE_TREE_ONLY || always_show_folders) {
        ...
        if (directory != "res://") {
          ...
          files->set_item_icon_modulate(files->get_item_count() - 1,
                                        folder_color);
        }
        ...
        for (int i = 0; i < efd->get_subdir_count(); i++) {
          ...
          files->set_item_metadata(files->get_item_count() - 1,
  ...
  for (List<FileInfo>::Element *E = filelist.front(); E; E = E->next()) {
    ...
    if (!finfo->import_broken) {
      ...
      tooltip += "\n" + TTR("Status: Import of file failed. Please fix file "
    ...
    if (fpath == main_scene) {
      files->set_item_custom_fg_color(item_index,
    ...
    if (!finfo->import_broken) {
      ...
      udata[1] = fname;
      EditorResourcePreview::get_singleton()->queue_resource_preview(
          fpath, this, "_file_list_thumbnail_done", udata);
    }
  ...
  if (file_list_display_mode == FILE_LIST_DISPLAY_THUMBNAILS &&
    ...
    for (int i = 0; i < files->get_item_count(); i++) {
      ...
      if (files->get_item_metadata(i) == p_path) {
        ...
        EditorResourcePreview::get_singleton()->queue_resource_preview(
            p_path, this, "_file_list_thumbnail_done", udata);
        break;
      }
    }
  }
...
void FileSystemDock::_get_all_items_in_dir(EditorFileSystemDirectory *efsd,
                                           Vector<String> &files,
                                           Vector<String> &folders) const {
  if (efsd == NULL)
    return;
...
void FileSystemDock::_find_remaps(EditorFileSystemDirectory *efsd,
                                  const Map<String, String> &renames,
                                  Vector<String> &to_remaps) const {
  for (int i = 0; i < efsd->get_subdir_count(); i++) {
...
void FileSystemDock::_try_move_item(const FileOrFolder &p_item,
  ...
  if (new_path == old_path) {
    ...
    // This check doesn't erroneously catch renaming to a longer name as folder
  ...
  if (err == OK) {
    ...
    for (int i = 0; i < file_changed_paths.size(); ++i) {
      ...
      print_verbose("  Remap: " + file_changed_paths[i] + " -> " +
                    p_file_renames[file_changed_paths[i]]);
      emit_signal("files_moved", file_changed_paths[i],
                  p_file_renames[file_changed_paths[i]]);
    }
    for (int i = 0; i < folder_changed_paths.size(); ++i) {
      ...
      emit_signal(
          "folder_moved", folder_changed_paths[i],
          p_folder_renames[folder_changed_paths[i]].substr(
              0, p_folder_renames[folder_changed_paths[i]].length() - 1));
    }
  } else {
    EditorNode::get_singleton()->add_io_error(TTR("Error moving:") + "\n" +
                                              old_path + "\n");
...
void FileSystemDock::_try_duplicate_item(const FileOrFolder &p_item,
                                         const String &p_new_path) const {
  // Ensure folder paths end with "/".
  String old_path = (p_item.is_file || p_item.path.ends_with("/"))
                        ? p_item.path
                        : (p_item.path + "/");
...
void FileSystemDock::_update_resource_paths_after_move(
    const Map<String, String> &p_renames) const {
...
void FileSystemDock::_update_dependencies_after_move(
  ...
  // 1) EditorFileSystem contains the old paths/folder structure from before the
  // rename/move. 2) ResourceLoader can use the new paths without needing to
  // call rescan.
  Vector<String> remaps;
  _find_remaps(EditorFileSystem::get_singleton()->get_filesystem(), p_renames,
               remaps);
  for (int i = 0; i < remaps.size(); ++i) {
    // Because we haven't called a rescan yet the found remap might still be an
    // old path itself.
...
void FileSystemDock::_update_project_settings_after_move(
  ...
  for (const Map<StringName, PropertyInfo>::Element *E = prop_info.front(); E;
    ...
    if (E->get().hint == PROPERTY_HINT_FILE) {
      ...
      if (p_renames.has(old_path)) {
        ProjectSettings::get_singleton()->set_setting(E->key(),
                                                      p_renames[old_path]);
      }
    };
  }
  ...
  for (const List<PropertyInfo>::Element *E = property_list.front(); E;
    ...
    if (E->get().name.begins_with("autoload/")) {
      // If the autoload resource paths has a leading "*", it indicates that it
      // is a Singleton, so we have to handle both cases when updating.
...
void FileSystemDock::_update_favorites_list_after_move(
    ...
    const Map<String, String> &p_folders_renames) const {
  Vector<String> favorites = EditorSettings::get_singleton()->get_favorites();
  ...
  for (int i = 0; i < to_move.size(); i++) {
    if (to_move[i].is_file &&
      ...
      EditorFileSystem::get_singleton()->move_group_file(
          to_move[i].path, p_to_path.plus_file(to_move[i].path.get_file()));
    }
  }
...
Vector<String>
FileSystemDock::_remove_self_included_paths(Vector<String> selected_strings) {
...
void FileSystemDock::_file_option(int p_option,
    ...
    if (!p_selected.empty()) {
      ...
      if (to_rename.path != "res://") {
        ...
        if (to_rename.is_file) {
          ...
          String name =
              to_rename.path.substr(0, to_rename.path.length() - 1).get_file();
  ...
  case FILE_DUPLICATE: {
    ...
    for (int i = 0; i < p_selected.size(); i++) {
      ...
      if (to_duplicate.is_file) {
        ...
        String name =
            to_duplicate.path.substr(0, to_duplicate.path.length() - 1)
                .get_file();
  ...
  case FILE_REIMPORT: {
    ...
    ERR_FAIL_COND_MSG(reimport.size() == 0,
...
void FileSystemDock::_search_changed(const String &p_text,
  ...
  switch (display_mode) {
  case DISPLAY_MODE_TREE_ONLY: {
    _update_tree(searched_string.length() == 0 ? uncollapsed_paths_before_search
                                               : Vector<String>(),
                 false, false, unfold_path);
  } break;
  case DISPLAY_MODE_SPLIT: {
    _update_file_list(false);
    _update_tree(searched_string.length() == 0 ? uncollapsed_paths_before_search
...
void FileSystemDock::focus_on_filter() { file_list_search_box->grab_focus(); }
...
Variant FileSystemDock::get_drag_data_fw(const Point2 &p_point,
                                         Control *p_from) {
...
bool FileSystemDock::can_drop_data_fw(const Point2 &p_point,
...
void FileSystemDock::drop_data_fw(const Point2 &p_point, const Variant &p_data,
                                  Control *p_from) {
...
void FileSystemDock::_get_drag_target_folder(String &target,
...
void FileSystemDock::_file_and_folders_fill_popup(
    PopupMenu *p_popup, Vector<String> p_paths,
  ...
  for (int i = 0; i < p_paths.size(); i++) {
    ...
    if (fpath.ends_with("/")) {
      ...
      all_folders = false;
      all_files_scenes &= (EditorFileSystem::get_singleton()->get_file_type(
                               fpath) == "PackedScene");
    }
  ...
  if (all_files) {
    if (all_files_scenes) {
      if (filenames.size() == 1) {
        p_popup->add_icon_item(get_icon("Load", "EditorIcons"),
                               TTR("Open Scene"), FILE_OPEN);
        p_popup->add_icon_item(get_icon("CreateNewSceneFrom", "EditorIcons"),
                               TTR("New Inherited Scene"), FILE_INHERIT);
        if (ProjectSettings::get_singleton()->get(
                "application/run/main_scene") != filenames[0]) {
          p_popup->add_icon_item(get_icon("PlayScene", "EditorIcons"),
                                 TTR("Set As Main Scene"), FILE_MAIN_SCENE);
        }
      } else {
        p_popup->add_icon_item(get_icon("Load", "EditorIcons"),
                               TTR("Open Scenes"), FILE_OPEN);
      }
      p_popup->add_icon_item(get_icon("Instance", "EditorIcons"),
                             TTR("Instance"), FILE_INSTANCE);
      p_popup->add_separator();
    } else if (filenames.size() == 1) {
      p_popup->add_icon_item(get_icon("Load", "EditorIcons"), TTR("Open"),
                             FILE_OPEN);
      p_popup->add_separator();
    }
  }
  ...
  if (p_paths.size() >= 1) {
    if (!all_favorites) {
      p_popup->add_icon_item(get_icon("Favorites", "EditorIcons"),
                             TTR("Add to Favorites"), FILE_ADD_FAVORITE);
    }
    if (!all_not_favorites) {
      p_popup->add_icon_item(get_icon("NonFavorite", "EditorIcons"),
  ...
  if (p_paths.size() == 1) {
    p_popup->add_icon_item(get_icon("ActionCopy", "EditorIcons"),
                           TTR("Copy Path"), FILE_COPY_PATH);
    if (p_paths[0] != "res://") {
      p_popup->add_icon_item(get_icon("Rename", "EditorIcons"),
                             TTR("Rename..."), FILE_RENAME);
      p_popup->add_icon_item(get_icon("Duplicate", "EditorIcons"),
                             TTR("Duplicate..."), FILE_DUPLICATE);
    }
  }

  if (p_paths.size() > 1 || p_paths[0] != "res://") {
    p_popup->add_icon_item(get_icon("MoveUp", "EditorIcons"), TTR("Move To..."),
                           FILE_MOVE);
    p_popup->add_icon_item(get_icon("Remove", "EditorIcons"), TTR("Delete"),
                           FILE_REMOVE);
  }
  ...
  if (p_paths.size() == 1) {
    ...
    if (p_display_path_dependent_options) {
      p_popup->add_icon_item(get_icon("Folder", "EditorIcons"),
                             TTR("New Folder..."), FILE_NEW_FOLDER);
      p_popup->add_icon_item(get_icon("PackedScene", "EditorIcons"),
                             TTR("New Scene..."), FILE_NEW_SCENE);
      p_popup->add_icon_item(get_icon("Script", "EditorIcons"),
                             TTR("New Script..."), FILE_NEW_SCRIPT);
      p_popup->add_icon_item(get_icon("Object", "EditorIcons"),
                             TTR("New Resource..."), FILE_NEW_RESOURCE);
      p_popup->add_separator();
    }
    ...
    p_popup->add_icon_item(get_icon("Filesystem", "EditorIcons"), item_text,
...
void FileSystemDock::_tree_rmb_select(const Vector2 &p_pos) {
  ...
  if (paths.size() == 1) {
    if (paths[0].ends_with("/")) {
      tree_popup->add_icon_item(get_icon("GuiTreeArrowDown", "EditorIcons"),
                                TTR("Expand All"), FOLDER_EXPAND_ALL);
      tree_popup->add_icon_item(get_icon("GuiTreeArrowRight", "EditorIcons"),
                                TTR("Collapse All"), FOLDER_COLLAPSE_ALL);
...
void FileSystemDock::_tree_rmb_empty(const Vector2 &p_pos) {
  ...
  tree_popup->clear();
  tree_popup->set_size(Size2(1, 1));
  tree_popup->add_icon_item(get_icon("Folder", "EditorIcons"),
                            TTR("New Folder..."), FILE_NEW_FOLDER);
  tree_popup->add_icon_item(get_icon("PackedScene", "EditorIcons"),
                            TTR("New Scene..."), FILE_NEW_SCENE);
  tree_popup->add_icon_item(get_icon("Script", "EditorIcons"),
                            TTR("New Script..."), FILE_NEW_SCRIPT);
  tree_popup->add_icon_item(get_icon("Object", "EditorIcons"),
                            TTR("New Resource..."), FILE_NEW_RESOURCE);
  tree_popup->set_position(tree->get_global_position() + p_pos);
  tree_popup->popup();
}

void FileSystemDock::_tree_empty_selected() { tree->deselect_all(); }
...
void FileSystemDock::_file_list_rmb_pressed(const Vector2 &p_pos) {
  ...
  file_list_popup->add_icon_item(get_icon("Folder", "EditorIcons"),
                                 TTR("New Folder..."), FILE_NEW_FOLDER);
  file_list_popup->add_icon_item(get_icon("PackedScene", "EditorIcons"),
                                 TTR("New Scene..."), FILE_NEW_SCENE);
  file_list_popup->add_icon_item(get_icon("Script", "EditorIcons"),
                                 TTR("New Script..."), FILE_NEW_SCRIPT);
  file_list_popup->add_icon_item(get_icon("Object", "EditorIcons"),
                                 TTR("New Resource..."), FILE_NEW_RESOURCE);
  file_list_popup->add_separator();
  file_list_popup->add_icon_item(get_icon("Filesystem", "EditorIcons"),
...
void FileSystemDock::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("_file_list_thumbnail_done"),
  ...
  ClassDB::bind_method(D_METHOD("get_drag_data_fw"),
                       &FileSystemDock::get_drag_data_fw);
  ClassDB::bind_method(D_METHOD("can_drop_data_fw"),
                       &FileSystemDock::can_drop_data_fw);
  ClassDB::bind_method(D_METHOD("drop_data_fw"), &FileSystemDock::drop_data_fw);
  ClassDB::bind_method(D_METHOD("navigate_to_path"),
...
FileSystemDock::FileSystemDock(EditorNode *p_editor) {
  ...
  ED_SHORTCUT("filesystem_dock/copy_path", TTR("Copy Path"),
              KEY_MASK_CMD | KEY_C);
  ED_SHORTCUT("filesystem_dock/duplicate", TTR("Duplicate..."),
              KEY_MASK_CMD | KEY_D);
  ...
  button_reload->connect("pressed",
                         callable_mp(this, &FileSystemDock::_rescan));
  ...
  button_toggle_display_mode->connect(
      "toggled", callable_mp(this, &FileSystemDock::_toggle_split_mode));
  ...
  tree_search_box->connect("text_changed",
                           callable_mp(this, &FileSystemDock::_search_changed),
  ...
  tree->connect("item_activated",
                callable_mp(this, &FileSystemDock::_tree_activate_file));
  tree->connect("multi_selected",
                callable_mp(this, &FileSystemDock::_tree_multi_selected));
  tree->connect("item_rmb_selected",
                callable_mp(this, &FileSystemDock::_tree_rmb_select));
  tree->connect("empty_rmb",
                callable_mp(this, &FileSystemDock::_tree_rmb_empty));
  tree->connect("nothing_selected",
                callable_mp(this, &FileSystemDock::_tree_empty_selected));
  tree->connect("gui_input",
                callable_mp(this, &FileSystemDock::_tree_gui_input));
  ...
  file_list_search_box->connect(
      "text_changed", callable_mp(this, &FileSystemDock::_search_changed),
  ...
  files->connect("item_rmb_selected",
                 callable_mp(this, &FileSystemDock::_file_list_rmb_select));
  files->connect("gui_input",
                 callable_mp(this, &FileSystemDock::_file_list_gui_input));
  files->connect("multi_selected",
                 callable_mp(this, &FileSystemDock::_file_multi_selected));
  files->connect("rmb_clicked",
                 callable_mp(this, &FileSystemDock::_file_list_rmb_pressed));
  ...
  remove_dialog->connect("file_removed",
                         callable_mp(this, &FileSystemDock::_file_removed));
  remove_dialog->connect("folder_removed",
                         callable_mp(this, &FileSystemDock::_folder_removed));
  ...
  move_dialog->connect(
      "dir_selected",
      callable_mp(this, &FileSystemDock::_move_operation_confirm),
      make_binds(false));
  ...
  rename_dialog->connect(
      "confirmed",
      callable_mp(this, &FileSystemDock::_rename_operation_confirm));

  overwrite_dialog = memnew(ConfirmationDialog);
  overwrite_dialog->set_text(TTR(
      "There is already file or folder with the same name in this location."));
  overwrite_dialog->get_ok()->set_text(TTR("Overwrite"));
  add_child(overwrite_dialog);
  overwrite_dialog->connect(
      "confirmed", callable_mp(this, &FileSystemDock::_move_with_overwrite));
  ...
  duplicate_dialog->connect(
      "confirmed",
      callable_mp(this, &FileSystemDock::_duplicate_operation_confirm));
  ...
  make_dir_dialog->connect(
      "confirmed", callable_mp(this, &FileSystemDock::_make_dir_confirm));
  ...
  make_scene_dialog->connect(
      "confirmed", callable_mp(this, &FileSystemDock::_make_scene_confirm));
  ...
  new_resource_dialog->connect(
      "create", callable_mp(this, &FileSystemDock::_resource_created));
