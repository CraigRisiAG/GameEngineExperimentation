...
/**
 * @file export.cpp
 * @brief JavaScript/WebAssembly export platform for the Godot Editor.
 * 
 * This module implements the export functionality for JavaScript/WebAssembly (WebAssembly) targets.
 * It provides an editor export platform that allows projects to be exported as HTML5 web applications.
 * 
 * Key Components:
 * 
 * - EditorHTTPServer: A lightweight HTTP server implementation that serves exported web content
 *   during development. It handles HTTP GET requests for various file types (HTML, JS, WASM, PCK, PNG)
 *   and serves them with appropriate MIME types.
 * 
 * - EditorExportPlatformJavaScript: Main export platform class that:
 *   * Manages export presets and configuration options
 *   * Exports Godot projects to HTML5/WebAssembly format
 *   * Handles custom HTML shell templates
 *   * Manages boot splash images and favicon generation
 *   * Runs exported projects in a browser via local HTTP server
 *   * Validates export templates and project configuration
 * 
 * Features:
 * - Support for both debug and release export templates
 * - Custom HTML shell support for branded web exports
 * - VRAM texture compression options (S3TC for desktop, ETC/ETC2 for mobile)
 * - Integrated HTTP server for testing exports locally
 * - Customizable HTML head includes and boot splash images
 * - Favicon support for better web page presentation
 * 
 * Export Process:
 * 1. Validates export configuration and required templates
 * 2. Extracts and processes template files from ZIP archives
 * 3. Applies project-specific settings to HTML template
 * 4. Generates and saves all necessary web files
 * 5. Optionally launches in default browser via HTTP server
 */
