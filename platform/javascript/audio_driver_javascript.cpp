...
/**
 * @file audio_driver_javascript.cpp
 * @brief JavaScript/Emscripten audio driver implementation for web platforms.
 * 
 * This file implements audio input/output functionality for WebAudio API using Emscripten.
 * It manages audio context creation, buffer processing, and microphone input capture.
 * 
 * Key Features:
 * - Audio mixing and processing through internal buffer
 * - WebAudio ScriptProcessor integration
 * - Microphone input capture and processing
 * - Audio context resumption for user interaction requirements
 * 
 * The driver uses Emscripten's EM_ASM macros to interface with JavaScript WebAudio API.
 * It maintains a singleton instance for callback access from C-to-JavaScript bridges.
 */
