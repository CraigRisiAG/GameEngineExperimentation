...
/**
 * @brief Calls a Java method on an instance or class with the given arguments.
 *
 * Attempts to find and invoke a matching overloaded Java method by name and
 * argument types. Performs type validation and conversion between Variant and
 * JNI types.
 *
 * @param p_instance Pointer to the JavaObject instance, or NULL for static
 * methods
 * @param p_method The name of the method to call
 * @param p_args Array of Variant pointers containing the method arguments
 * @param p_argcount Number of arguments in p_args
 * @param r_error Reference to a CallError struct that will be populated with
 * error information
 * @param ret Reference to a Variant that will contain the return value
 *
 * @return true if a matching method was found and called, false otherwise
 *
 * @details
 * - Iterates through all overloaded versions of the method to find a matching
 * signature
 * - Validates argument count and types before attempting to call
 * - Converts Variant arguments to appropriate JNI types (jint, jfloat, jstring,
 * etc.)
 * - Handles both primitive types and boxed Java number types
 * - Supports arrays of primitives and objects
 * - Cleans up all allocated JNI local references after the call
 * - Sets r_error.error to CALL_OK on success, or an appropriate CallError code
 * on failure
 */
bool JavaClass::_call_method(JavaObject *p_instance, const StringName &p_method,
                             const Variant **p_args, int p_argcount,
                             Callable::CallError &r_error, Variant &ret);
...
bool JavaClass::_call_method(JavaObject *p_instance, const StringName &p_method,
          ...
          if (!ref.is_null()) {
            if (Object::cast_to<JavaObject>(ref.ptr())) {
              ...
              jclass c = env->FindClass(
                  E->get().param_sigs[i].operator String().utf8().get_data());
  ...
  for (int i = 0; i < method->param_types.size(); i++) {
    ...
    case ARG_ARRAY_BIT | ARG_TYPE_STRING: {
      ...
      jobjectArray a = env->NewObjectArray(
          arr.size(), env->FindClass("java/lang/String"), NULL);
      for (int j = 0; j < arr.size(); j++) {
  ...
  case ARG_TYPE_LONG: {
    ...
    if (method->_static) {
      ...
      ret = (int64_t)env->CallLongMethodA(p_instance->instance, method->method,
                                          argv);
    }
...
Variant JavaClass::call(const StringName &p_method, const Variant **p_args,
...
Variant JavaObject::call(const StringName &p_method, const Variant **p_args,
                         int p_argcount, Callable::CallError &r_error) {
...
bool JavaClassWrapper::_get_type_sig(JNIEnv *env, jobject obj, uint32_t &sig,
                                     String &strsig) {
...
bool JavaClass::_convert_object_to_variant(JNIEnv *env, jobject obj,
                                           Variant &var, uint32_t p_sig) {
  ...
  case ARG_TYPE_BOOLEAN | ARG_NUMBER_CLASS_BIT: {
    ...
    var = env->CallBooleanMethod(
        obj, JavaClassWrapper::singleton->Boolean_booleanValue);
    return true;
  } break;
  ...
  case ARG_TYPE_CHAR | ARG_NUMBER_CLASS_BIT: {
    ...
    var = env->CallCharMethod(
        obj, JavaClassWrapper::singleton->Character_characterValue);
    return true;

  } break;
  case ARG_TYPE_SHORT | ARG_NUMBER_CLASS_BIT: {

    var = env->CallShortMethod(obj,
                               JavaClassWrapper::singleton->Short_shortValue);
    return true;

  } break;
  case ARG_TYPE_INT | ARG_NUMBER_CLASS_BIT: {

    var = env->CallIntMethod(obj,
                             JavaClassWrapper::singleton->Integer_integerValue);
    return true;

  } break;
  case ARG_TYPE_LONG | ARG_NUMBER_CLASS_BIT: {

    var = (int64_t)env->CallLongMethod(
        obj, JavaClassWrapper::singleton->Long_longValue);
    return true;

  } break;
  case ARG_TYPE_FLOAT | ARG_NUMBER_CLASS_BIT: {

    var = env->CallFloatMethod(obj,
                               JavaClassWrapper::singleton->Float_floatValue);
    return true;

  } break;
  case ARG_TYPE_DOUBLE | ARG_NUMBER_CLASS_BIT: {

    var = env->CallDoubleMethod(
        obj, JavaClassWrapper::singleton->Double_doubleValue);
    return true;
  } break;
  case ARG_TYPE_STRING: {
  ...
  case ARG_NUMBER_CLASS_BIT | ARG_ARRAY_BIT | ARG_TYPE_BOOLEAN: {
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (!o)
        ...
        bool val = env->CallBooleanMethod(
            o, JavaClassWrapper::singleton->Boolean_booleanValue);
        ret.push_back(val);
      }
  ...
  case ARG_NUMBER_CLASS_BIT | ARG_ARRAY_BIT | ARG_TYPE_BYTE: {
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (!o)
        ...
        int val =
            env->CallByteMethod(o, JavaClassWrapper::singleton->Byte_byteValue);
        ret.push_back(val);
      }
      env->DeleteLocalRef(o);
    }
  ...
  case ARG_NUMBER_CLASS_BIT | ARG_ARRAY_BIT | ARG_TYPE_CHAR: {
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (!o)
        ...
        int val = env->CallCharMethod(
            o, JavaClassWrapper::singleton->Character_characterValue);
        ret.push_back(val);
      }
      env->DeleteLocalRef(o);
    }
  ...
  case ARG_NUMBER_CLASS_BIT | ARG_ARRAY_BIT | ARG_TYPE_SHORT: {
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (!o)
        ...
        int val = env->CallShortMethod(
            o, JavaClassWrapper::singleton->Short_shortValue);
        ret.push_back(val);
      }
      env->DeleteLocalRef(o);
    }
  ...
  case ARG_NUMBER_CLASS_BIT | ARG_ARRAY_BIT | ARG_TYPE_INT: {
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (!o)
        ...
        int val = env->CallIntMethod(
            o, JavaClassWrapper::singleton->Integer_integerValue);
        ret.push_back(val);
      }
      env->DeleteLocalRef(o);
    }
  ...
  case ARG_NUMBER_CLASS_BIT | ARG_ARRAY_BIT | ARG_TYPE_LONG: {
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (!o)
        ...
        int64_t val =
            env->CallLongMethod(o, JavaClassWrapper::singleton->Long_longValue);
        ret.push_back(val);
      }
      env->DeleteLocalRef(o);
    }
  ...
  case ARG_NUMBER_CLASS_BIT | ARG_ARRAY_BIT | ARG_TYPE_FLOAT: {
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (!o)
        ...
        float val = env->CallFloatMethod(
            o, JavaClassWrapper::singleton->Float_floatValue);
        ret.push_back(val);
      }
      env->DeleteLocalRef(o);
    }
  ...
  case ARG_NUMBER_CLASS_BIT | ARG_ARRAY_BIT | ARG_TYPE_DOUBLE: {
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (!o)
        ...
        double val = env->CallDoubleMethod(
            o, JavaClassWrapper::singleton->Double_doubleValue);
        ret.push_back(val);
      }
      env->DeleteLocalRef(o);
    }
...
Ref<JavaClass> JavaClassWrapper::wrap(const String &p_class) {
  ...
  // env->GetMethodID(bclass,"getDeclaredMethods",
  // "()[Ljava/lang/reflect/Method;");

  // ERR_FAIL_COND_V(!getDeclaredMethods,Ref<JavaClass>());

  jobjectArray methods =
      (jobjectArray)env->CallObjectMethod(bclass, getDeclaredMethods);
  ...
  for (int i = 0; i < count; i++) {
    ...
    jobjectArray param_types =
        (jobjectArray)env->CallObjectMethod(obj, getParameterTypes);
    ...
    if (!valid) {
      print_line("Method can't be bound (unsupported arguments): " + p_class +
                 "::" + str_method);
      env->DeleteLocalRef(obj);
    ...
    if (!_get_type_sig(env, return_type, sig, strsig)) {
      print_line("Method can't be bound (unsupported return type): " + p_class +
                 "::" + str_method);
      env->DeleteLocalRef(obj);
      env->DeleteLocalRef(param_types);
    ...
    for (List<JavaClass::MethodInfo>::Element *E =
             java_class->methods[str_method].front();
         E; E = E->next()) {
      ...
      for (int j = 0; j < E->get().param_types.size(); j++) {
        ...
        float existing_l;
        JavaClass::_convert_to_variant_type(E->get().param_types[j], existing,
                                            existing_l);
    ...
    if (!discard) {
      if (mi._static)
        mi.method = env->GetStaticMethodID(bclass, str_method.utf8().get_data(),
                                           signature.utf8().get_data());
      else
        mi.method = env->GetMethodID(bclass, str_method.utf8().get_data(),
                                     signature.utf8().get_data());
...
JavaClassWrapper::JavaClassWrapper(jobject p_activity) {
  ...
  findClass = env->GetMethodID(classLoaderClass, "loadClass",
                               "(Ljava/lang/String;)Ljava/lang/Class;");

  jclass bclass = env->FindClass("java/lang/Class");
  getDeclaredMethods = env->GetMethodID(bclass, "getDeclaredMethods",
                                        "()[Ljava/lang/reflect/Method;");
  getFields =
      env->GetMethodID(bclass, "getFields", "()[Ljava/lang/reflect/Field;");
  Class_getName = env->GetMethodID(bclass, "getName", "()Ljava/lang/String;");
  //
  bclass = env->FindClass("java/lang/reflect/Method");
  getParameterTypes =
      env->GetMethodID(bclass, "getParameterTypes", "()[Ljava/lang/Class;");
  getReturnType =
      env->GetMethodID(bclass, "getReturnType", "()Ljava/lang/Class;");
  ...
  Field_get =
      env->GetMethodID(bclass, "get", "(Ljava/lang/Object;)Ljava/lang/Object;");
  // each
