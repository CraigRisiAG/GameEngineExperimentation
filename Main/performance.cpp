 ...
 * @brief Performance monitoring system for tracking engine metrics
 *
 * This file implements a Performance singleton class that provides real-time
 * monitoring of various engine performance metrics including:
 * - Timing information (FPS, process time, physics process time)
 * - Memory usage (static memory, max usage, message buffer)
 * - Object counts (total objects, resources, nodes, orphan nodes)
 * - Rendering statistics (objects drawn, vertices, material/shader changes,
 * draw calls, video memory)
 * - Physics simulation data (2D and 3D active objects, collision pairs, island
 * counts)
 * - Audio latency information
 *
 * @class Performance
 * @details
 * - Provides methods to retrieve monitor values by type via get_monitor()
 * - Maintains human-readable monitor names through get_monitor_name()
 * - Categorizes monitors by type (quantity, time, memory) via
 * get_monitor_type()
 * - Tracks process and physics process times for performance profiling
 * - Integrates with various engine subsystems (Memory, ObjectDB, VisualServer,
 * PhysicsServer, AudioServer)
 * - Implemented as a singleton pattern for global access to performance data
 * - Binds enum constants and methods to the script API through _bind_methods()
 *
 * @method get_monitor() - Retrieves float value for specified monitor type
 * @method get_monitor_name() - Returns string name of monitor metric
 * @method get_monitor_type() - Returns the type category of a monitor
 * @method set_process_time() - Updates main process execution time
 * @method set_physics_process_time() - Updates physics simulation execution
 * time
 * @method _get_node_count() - Helper to count total nodes in active scene tree
 */
...
void Performance::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("get_monitor", "monitor"),
...
float Performance::_get_node_count() const {
  _update_node_count_cache();
  return cached_node_count;
}

void Performance::_update_node_count_cache() const {
  uint64_t current_frame = Engine::get_singleton()->get_frame_id();

  // Only update cache once per frame
  if (cache_frame_id != current_frame) {
    MainLoop *ml = OS::get_singleton()->get_main_loop();
    SceneTree *sml = Object::cast_to<SceneTree>(ml);
    if (!sml) {
      cached_node_count = 0;
    } else {
      cached_node_count = sml->get_node_count();
    }
    cache_frame_id = current_frame;
  }
  ...
  void Performance::set_process_time(float p_pt) { _process_time = p_pt; }
