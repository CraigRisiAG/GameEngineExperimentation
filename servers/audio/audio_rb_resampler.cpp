...
bool AudioRBResampler::mix(AudioFrame *p_dest, int p_frames) {
...

  {
    ...
    // Create fadeout effect for the end of stream (note that it can be because
    // of slow writer)
...
Error AudioRBResampler::setup(int p_channels, int p_src_mix_rate,
                              int p_target_mix_rate, int p_buffer_msec,
                              int p_minbuff_needed) {

  ERR_FAIL_COND_V(p_channels != 1 && p_channels != 2 && p_channels != 4 &&
                      p_channels != 6,
  ...
  if (rb && (uint32_t(desired_rb_bits) != rb_bits ||
