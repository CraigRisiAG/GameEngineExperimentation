...
public:
  ...
  _FORCE_INLINE_ bool is_ready() const { return rb != NULL; }

  _FORCE_INLINE_ int get_total() const { return rb_len - 1; }
  ...
  Error setup(int p_channels, int p_src_mix_rate, int p_target_mix_rate,
              int p_buffer_msec, int p_minbuff_needed = -1);
  void clear();
