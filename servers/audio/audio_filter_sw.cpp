...
void AudioFilterSW::set_mode(Mode p_mode) { mode = p_mode; }
void AudioFilterSW::set_cutoff(float p_cutoff) { cutoff = p_cutoff; }
void AudioFilterSW::set_resonance(float p_resonance) {

  resonance = p_resonance;
}

void AudioFilterSW::set_gain(float p_gain) { gain = p_gain; }
...
void AudioFilterSW::prepare_coefficients(Coeffs *p_coeffs) {
  ...
  case BANDLIMIT: {
    ...
    double bandwidth =
        (Math::log(centercutoff) - Math::log(hicutoff)) / Math::log((double)2);
    omega = 2.0 * Math_PI * centercutoff / sampling_rate;
    alpha = Math::sin(omega) * Math::sinh(Math::log((double)2) / 2 * bandwidth *
  ...
  case LOWSHELF: {
    ...
    p_coeffs->b0 =
        tmpgain * ((tmpgain + 1.0) - (tmpgain - 1.0) * cos_v + beta * sin_v);
...
AudioFilterSW::Processor::Processor() { set_filter(NULL); }
...
void AudioFilterSW::Processor::process(float *p_samples, int p_amount,
                                       int p_stride, bool p_interpolate) {
