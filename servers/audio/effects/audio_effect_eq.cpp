...
void AudioEffectEQInstance::process(const AudioFrame *p_src_frames,
...
void AudioEffectEQ::_get_property_list(List<PropertyInfo> *p_list) const {
  ...
  for (int i = 0; i < band_names.size(); i++) {
    ...
    p_list->push_back(PropertyInfo(Variant::FLOAT, band_names[i],
...
void AudioEffectEQ::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("set_band_gain_db", "band_idx", "volume_db"),
                       &AudioEffectEQ::set_band_gain_db);
  ClassDB::bind_method(D_METHOD("get_band_gain_db", "band_idx"),
                       &AudioEffectEQ::get_band_gain_db);
  ClassDB::bind_method(D_METHOD("get_band_count"),
