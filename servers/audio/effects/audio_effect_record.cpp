...
void AudioEffectRecordInstance::process(const AudioFrame *p_src_frames,
...
bool AudioEffectRecordInstance::process_silence() const { return true; }
...
void AudioEffectRecordInstance::init() {
  ...
  AudioServer::get_singleton()->add_update_callback(
      &AudioEffectRecordInstance::_update, this);
#else
...
AudioEffectRecordInstance::~AudioEffectRecordInstance() { finish(); }
...
void AudioEffectRecord::set_recording_active(bool p_record) {
  if (p_record) {
    if (current_instance == 0) {
      WARN_PRINT("Recording should not be set as active before Godot has "
                 "initialized.");
      recording_active = false;
      return;
    }
...
bool AudioEffectRecord::is_recording_active() const { return recording_active; }
...
Ref<AudioStreamSample> AudioEffectRecord::get_recording() const {
  ...
  if (dst_format == AudioStreamSample::FORMAT_8_BITS) {
    ...
    for (int i = 0; i < data_size; i++) {
      int16_t v =
          CLAMP(current_instance->recording_data[i] * 32768, -32768, 32767);
...
void AudioEffectRecord::_bind_methods() {
  ClassDB::bind_method(D_METHOD("set_recording_active", "record"),
                       &AudioEffectRecord::set_recording_active);
  ClassDB::bind_method(D_METHOD("is_recording_active"),
  ...
  ClassDB::bind_method(D_METHOD("get_recording"),
