...
const float Reverb::comb_tunings[MAX_COMBS] = {
    ...
    0.025306122448979593f, 0.026938775510204082f, 0.028956916099773241f,
    0.03074829931972789f,  0.032244897959183672f, 0.03380952380952381f,
    0.035306122448979592f, 0.036666666666666667f};

const float Reverb::allpass_tunings[MAX_ALLPASS] = {
    // freeverb allpass tunings
    0.0051020408163265302f, 0.007732426303854875f, 0.01f,
...
void Reverb::process(float *p_src, float *p_dst, int p_frames) {
  ...
  for (int i = 0; i < p_frames; i++) {
    ...
    float in =
        undenormalise(echo_buffer[read_pos] * params.predelay_fb + p_src[i]);
  ...
  for (int i = 0; i < MAX_COMBS; i++) {
    ...
    int size_limit = c.size - lrintf((float)c.extra_spread_frames *
                                     (1.0 - params.extra_spread));
...
#define PROCESS_ALLPASS(m_ap) 	\
  ...
  */
  ...
  for (int i = 0; i < MAX_ALLPASS; i++) {
    ...
    AllPass &a = allpass[i];
    int size_limit = a.size - lrintf((float)a.extra_spread_frames *
                                     (1.0 - params.extra_spread));
...
void Reverb::set_wet(float p_wet) { params.wet = p_wet; }

void Reverb::set_dry(float p_dry) { params.dry = p_dry; }

void Reverb::set_predelay(float p_predelay) { params.predelay = p_predelay; }
...
void Reverb::update_parameters() {
  ...
  for (int i = 0; i < MAX_COMBS; i++) {
    ...
    float auxdmp =
        params.damp / 2.0 + 0.5; // only half the range (0.5 .. 1.0  is enough)
    auxdmp *= auxdmp;

    c.damp =
        expf(-2.0 * Math_PI * auxdmp * 10000 / params.mix_rate); // 0 .. 10khz
  }
}
