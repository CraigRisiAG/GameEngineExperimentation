...
template <int S>
void AudioEffectFilterInstance::_process_filter(const AudioFrame *p_src_frames,
...
void AudioEffectFilterInstance::process(const AudioFrame *p_src_frames,
                                        AudioFrame *p_dst_frames,
                                        int p_frame_count) {
...
void AudioEffectFilter::set_cutoff(float p_freq) { cutoff = p_freq; }

float AudioEffectFilter::get_cutoff() const { return cutoff; }

void AudioEffectFilter::set_resonance(float p_amount) { resonance = p_amount; }
float AudioEffectFilter::get_resonance() const { return resonance; }

void AudioEffectFilter::set_gain(float p_amount) { gain = p_amount; }
float AudioEffectFilter::get_gain() const { return gain; }
...
void AudioEffectFilter::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("set_cutoff", "freq"),
                       &AudioEffectFilter::set_cutoff);
  ClassDB::bind_method(D_METHOD("get_cutoff"), &AudioEffectFilter::get_cutoff);

  ClassDB::bind_method(D_METHOD("set_resonance", "amount"),
                       &AudioEffectFilter::set_resonance);
  ClassDB::bind_method(D_METHOD("get_resonance"),
                       &AudioEffectFilter::get_resonance);

  ClassDB::bind_method(D_METHOD("set_gain", "amount"),
                       &AudioEffectFilter::set_gain);
  ClassDB::bind_method(D_METHOD("get_gain"), &AudioEffectFilter::get_gain);

  ClassDB::bind_method(D_METHOD("set_db", "amount"),
                       &AudioEffectFilter::set_db);
  ClassDB::bind_method(D_METHOD("get_db"), &AudioEffectFilter::get_db);

  ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "cutoff_hz", PROPERTY_HINT_RANGE,
                            "1,20500,1"),
               "set_cutoff", "get_cutoff");
  ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "resonance", PROPERTY_HINT_RANGE,
                            "0,1,0.01"),
               "set_resonance", "get_resonance");
  ADD_PROPERTY(
      PropertyInfo(Variant::FLOAT, "gain", PROPERTY_HINT_RANGE, "0,4,0.01"),
      "set_gain", "get_gain");
  ADD_PROPERTY(PropertyInfo(Variant::INT, "db", PROPERTY_HINT_ENUM,
