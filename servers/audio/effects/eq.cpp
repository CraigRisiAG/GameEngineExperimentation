...
/* Helper */
static int solve_quadratic(double a, double b, double c, double *r1,
                           double *r2) {
...
#define BAND_LOG(m_f) (log((m_f)) / log(2.))
  ...
  for (int i = 0; i < band.size(); i++) {
    ...
    double c2b =
        2.0 * side_gain2 * POW2(cos(th_l)) + side_gain2 * POW2(cos(th)) -
        2.0 * side_gain2 * cos(th_l) * cos(th) - side_gain2 + POW2(sin(th_l));
    ...
    // uninitialized use. Both variables are properly set in _solve_quadratic
    // before use, or we continue if roots == 0.
...
#define PUSH_BANDS(m_bands)                                                    \
  ...
  case PRESET_10_BANDS: {
    static const double bands[] = {31.25, 62.5, 125, 250, 500,
  ...
  case PRESET_21_BANDS: {
    ...
    static const double bands[] = {22,   32,  44,   63,  90,   125,  175,
                                   250,  350, 500,  700, 1e3,  1400, 2e3,
                                   2800, 4e3, 5600, 8e3, 11e3, 16e3, 22e3};
    PUSH_BANDS(21);
...
int EQ::get_band_count() const { return band.size(); }
