...
void AudioEffectSpectrumAnalyzerInstance::process(
    const AudioFrame *p_src_frames, AudioFrame *p_dst_frames,
    int p_frame_count) {
  ...
  while (p_frame_count) {
    ...
    for (int i = 0; i < to_fill; i++) { // left and right buffers
      float window =
          -0.5 * Math::cos(2.0 * Math_PI * (double)i / (double)to_fill) + 0.5;
      fftw[(i + temporal_fft_pos) * 2] = window * p_src_frames[i].l;
      fftw[(i + temporal_fft_pos) * 2 + 1] = 0;
      fftw[(i + temporal_fft_pos + fft_size * 2) * 2] =
          window * p_src_frames[i].r;
    ...
    if (temporal_fft_pos == fft_size * 2) {
      ...
      AudioFrame *hw =
          (AudioFrame *)fft_history[next].ptr(); // do not use write, avoid cow

      for (int i = 0; i < fft_size; i++) {
        // abs(vec)/fft_size normalizes each frequency
        float window = 1.0; //-.5 * Math::cos(2. * Math_PI * (double)i /
                            //(double)fft_size) + .5;
        hw[i].l = window * Vector2(fftw[i * 2], fftw[i * 2 + 1]).length() /
                  float(fft_size);
        hw[i].r =
            window *
            Vector2(fftw[fft_size * 4 + i * 2], fftw[fft_size * 4 + i * 2 + 1])
                .length() /
            float(fft_size);
      }
  ...
  double remainer_sec =
      (temporal_fft_pos / mix_rate); // subtract remainder from mix time
...
void AudioEffectSpectrumAnalyzerInstance::_bind_methods() {
  ...
  ClassDB::bind_method(
      D_METHOD("get_magnitude_for_frequency_range", "from_hz", "to_hz", "mode"),
      &AudioEffectSpectrumAnalyzerInstance::get_magnitude_for_frequency_range,
      DEFVAL(MAGNITUDE_MAX));
...
Ref<AudioEffectInstance> AudioEffectSpectrumAnalyzer::instance() {
  ...
  ins->temporal_fft.resize(
      ins->fft_size *
      8); // x2 stereo, x2 amount of samples for freqs, x2 for input
...
void AudioEffectSpectrumAnalyzer::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("set_tap_back_pos", "seconds"),
                       &AudioEffectSpectrumAnalyzer::set_tap_back_pos);
  ClassDB::bind_method(D_METHOD("get_tap_back_pos"),
  ...
  ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "buffer_length",
                            PROPERTY_HINT_RANGE, "0.1,4,0.1"),
               "set_buffer_length", "get_buffer_length");
  ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "tap_back_pos", PROPERTY_HINT_RANGE,
                            "0.1,4,0.1"),
               "set_tap_back_pos", "get_tap_back_pos");
