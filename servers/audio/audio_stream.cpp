...
void AudioStreamPlaybackResampled::mix(AudioFrame *p_buffer, float p_rate_scale,
                                       int p_frames) {
  ...
  uint64_t mix_increment =
      uint64_t(((get_stream_sampling_rate() * p_rate_scale) /
  ...
  for (int i = 0; i < p_frames; i++) {
    ...
    // this used to be moved to a LUT for greater performance, but nowadays CPU
    // speed is generally faster than memory.
...
float AudioStreamMicrophone::get_length() const { return 0; }

void AudioStreamMicrophone::_bind_methods() {}

AudioStreamMicrophone::AudioStreamMicrophone() {}

void AudioStreamPlaybackMicrophone::_mix_internal(AudioFrame *p_buffer,
                                                  int p_frames) {
  ...
  if (input_ofs > input_position &&
    ...
    print_verbose(String(get_class_name()) +
                  " buffer underrun: input_position=" + itos(input_position) +
                  " input_ofs=" + itos(input_ofs) +
...
void AudioStreamPlaybackMicrophone::mix(AudioFrame *p_buffer,
                                        float p_rate_scale, int p_frames) {
...
void AudioStreamPlaybackMicrophone::start(float p_from_pos) {
  ...
  if (!GLOBAL_GET("audio/enable_audio_input")) {
    WARN_PRINT("Need to enable Project settings > Audio > Enable Audio Input "
...
bool AudioStreamPlaybackMicrophone::is_playing() const { return active; }

int AudioStreamPlaybackMicrophone::get_loop_count() const { return 0; }

float AudioStreamPlaybackMicrophone::get_playback_position() const { return 0; }
...
void AudioStreamRandomPitch::set_audio_stream(
  ...
  if (audio_stream.is_valid()) {
    for (Set<AudioStreamPlaybackRandomPitch *>::Element *E = playbacks.front();
         E; E = E->next()) {
...
float AudioStreamRandomPitch::get_random_pitch() const { return random_pitch; }
...
void AudioStreamRandomPitch::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("set_audio_stream", "stream"),
  ...
  ClassDB::bind_method(D_METHOD("set_random_pitch", "scale"),
                       &AudioStreamRandomPitch::set_random_pitch);
  ClassDB::bind_method(D_METHOD("get_random_pitch"),
                       &AudioStreamRandomPitch::get_random_pitch);

  ADD_PROPERTY(PropertyInfo(Variant::OBJECT, "audio_stream",
                            PROPERTY_HINT_RESOURCE_TYPE, "AudioStream"),
               "set_audio_stream", "get_audio_stream");
  ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "random_pitch", PROPERTY_HINT_RANGE,
                            "1,16,0.01"),
               "set_random_pitch", "get_random_pitch");
}

AudioStreamRandomPitch::AudioStreamRandomPitch() { random_pitch = 1.1; }
...
void AudioStreamPlaybackRandomPitch::mix(AudioFrame *p_buffer,
                                         float p_rate_scale, int p_frames) {
  if (playing.is_valid()) {
