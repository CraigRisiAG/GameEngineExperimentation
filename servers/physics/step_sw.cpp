...
void StepSW::_populate_island(BodySW *p_body, BodySW **p_island,
  ...
  *p_island = p_body;
  ...
  for (Map<ConstraintSW *, int>::Element *E =
           p_body->get_constraint_map().front();
       E; E = E->next()) {
    ...
    *p_constraint_island = c;
    ...
    for (int i = 0; i < c->get_body_count(); i++) {
      ...
      if (b->get_island_step() == _step ||
          b->get_mode() == PhysicsServer::BODY_MODE_STATIC ||
...
void StepSW::_solve_island(ConstraintSW *p_island, int p_iterations,
                           real_t p_delta) {
...
void StepSW::step(SpaceSW *p_space, real_t p_delta, int p_iterations) {
  ...
  { // profile
    ...
    p_space->set_elapsed_time(SpaceSW::ELAPSED_TIME_INTEGRATE_FORCES,
  ...
  while (aml.first()) {
    ...
    p_space->area_remove_from_moved_list(
        (SelfList<AreaSW> *)aml.first()); // faster to remove here
  }

  { // profile
    profile_endtime = OS::get_singleton()->get_ticks_usec();
    p_space->set_elapsed_time(SpaceSW::ELAPSED_TIME_GENERATE_ISLANDS,
                              profile_endtime - profile_begtime);
    profile_begtime = profile_endtime;
  }
  ...
  { // profile
    profile_endtime = OS::get_singleton()->get_ticks_usec();
    p_space->set_elapsed_time(SpaceSW::ELAPSED_TIME_SETUP_CONSTRAINTS,
                              profile_endtime - profile_begtime);
    profile_begtime = profile_endtime;
  }
  ...
  { // profile
    profile_endtime = OS::get_singleton()->get_ticks_usec();
    p_space->set_elapsed_time(SpaceSW::ELAPSED_TIME_SOLVE_CONSTRAINTS,
                              profile_endtime - profile_begtime);
    profile_begtime = profile_endtime;
  }
  ...
  { // profile
    profile_endtime = OS::get_singleton()->get_ticks_usec();
    p_space->set_elapsed_time(SpaceSW::ELAPSED_TIME_INTEGRATE_VELOCITIES,
                              profile_endtime - profile_begtime);
    profile_begtime = profile_endtime;
  }
