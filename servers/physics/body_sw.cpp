...
void BodySW::update_inertias() {
  ...
  case PhysicsServer::BODY_MODE_RIGID: {
    ...
    // update tensor for all shapes, not the best way but should be somehow OK.
    // (inspired from bullet)
    real_t total_area = 0;
    ...
    for (int i = 0; i < get_shape_count(); i++) {
      ...
      // NOTE: we don't take the scale of collision shapes into account when
      ...
      inertia_tensor +=
          shape_inertia_tensor + (Basis() * shape_origin.dot(shape_origin) -
...
PhysicsServer::BodyMode BodySW::get_mode() const { return mode; }

void BodySW::_shapes_changed() { _update_inertia(); }

void BodySW::set_state(PhysicsServer::BodyState p_state,
...
void BodySW::_compute_area_gravity_and_dampenings(const AreaSW *p_area) {
  ...
  if (p_area->is_gravity_point()) {
    if (p_area->get_gravity_distance_scale() > 0) {
      Vector3 v = p_area->get_transform().xform(p_area->get_gravity_vector()) -
                  get_transform().get_origin();
      gravity +=
          v.normalized() *
          (p_area->get_gravity() /
           Math::pow(v.length() * p_area->get_gravity_distance_scale() + 1, 2));
    } else {
      gravity += (p_area->get_transform().xform(p_area->get_gravity_vector()) -
                  get_transform().get_origin())
                     .normalized() *
                 p_area->get_gravity();
    }
...
void BodySW::integrate_forces(real_t p_step) {
  ...
  */
  ...
  if (mode == PhysicsServer::BODY_MODE_KINEMATIC) {
    ...
    Basis rot = new_transform.basis.orthonormalized().transposed() *
                get_transform().basis.orthonormalized();
    Vector3 axis;
    real_t angle;
...
void BodySW::set_force_integration_callback(ObjectID p_id,
...
BodySW::BodySW()
    : CollisionObjectSW(TYPE_BODY), locked_axis(0), active_list(this),
      inertia_update_list(this), direct_state_query_list(this) {
