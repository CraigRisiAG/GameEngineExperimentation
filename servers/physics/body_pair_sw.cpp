...
void BodyPairSW::_contact_added_callback(const Vector3 &p_point_A,
...
void BodyPairSW::contact_added_callback(const Vector3 &p_point_A,
                                        const Vector3 &p_point_B) {
...
bool BodyPairSW::_test_ccd(real_t p_step, BodySW *p_A, int p_shape_A,
                           const Transform &p_xform_A, BodySW *p_B,
                           int p_shape_B, const Transform &p_xform_B) {
  ...
  // motion by motion length support is the worst case collision point, so real
  ...
  Vector3 local_from = from_inv.xform(
      from -
      mnormal * mlen * 0.1); // start from a little inside the bounding box
  Vector3 local_to = from_inv.xform(to);

  Vector3 rpos, rnorm;
  if (!p_B->get_shape(p_shape_B)->intersect_segment(local_from, local_to, rpos,
                                                    rnorm)) {
    return false;
  }

  // shorten the linear velocity so it does not hit, but gets close enough, next
  // frame will hit softly or soft enough
...
bool BodyPairSW::setup(real_t p_step) {
  ...
  if (!A->test_collision_mask(B) || A->has_exception(B->get_self()) ||
      ...
      (A->get_mode() <= PhysicsServer::BODY_MODE_KINEMATIC &&
       ...
       A->get_max_contacts_reported() == 0 &&
       B->get_max_contacts_reported() == 0)) {
    collided = false;
    return false;
  }

  if (A->is_shape_set_as_disabled(shape_A) ||
      B->is_shape_set_as_disabled(shape_B)) {
    collided = false;
    return false;
  }
  ...
  bool collided = CollisionSolverSW::solve_static(
      shape_A_ptr, xform_A, shape_B_ptr, xform_B, _contact_added_callback, this,
      &sep_axis);
  ...
  if (shape_A_ptr->get_custom_bias() || shape_B_ptr->get_custom_bias()) {
    ...
    if (shape_A_ptr->get_custom_bias() == 0)
      ...
      bias = (shape_B_ptr->get_custom_bias() + shape_A_ptr->get_custom_bias()) *
             0.5;
  }
  ...
  for (int i = 0; i < contact_count; i++) {
    ...
    if (A->can_report_contacts()) {
      Vector3 crA =
          A->get_angular_velocity().cross(c.rA) + A->get_linear_velocity();
      A->add_contact(global_A, -c.normal, depth, shape_A, global_B, shape_B,
                     B->get_instance_id(), B->get_self(), crA);
    }

    if (B->can_report_contacts()) {
      Vector3 crB =
          B->get_angular_velocity().cross(c.rB) + B->get_linear_velocity();
      B->add_contact(global_B, c.normal, depth, shape_B, global_A, shape_A,
                     A->get_instance_id(), A->get_self(), crB);
    }
...
void BodyPairSW::solve(real_t p_step) {
  ...
  for (int i = 0; i < contact_count; i++) {
    ...
    if (tvl > MIN_VELOCITY) {
      ...
      real_t t = -tvl / (A->get_inv_mass() + B->get_inv_mass() +
