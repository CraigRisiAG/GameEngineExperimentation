...
BroadPhaseSW::ID BroadPhaseBasic::create(CollisionObjectSW *p_object,
                                         int p_subindex) {
...
void BroadPhaseBasic::remove(ID p_id) {
  ...
  // unpair must be done immediately on removal to avoid potential invalid
  // pointers
  for (Map<PairKey, void *>::Element *F = pair_map.front(); F; F = F->next()) {
    ...
    if (F->key().a == p_id || F->key().b == p_id) {
      ...
      if (unpair_callback) {
        ...
        unpair_callback(elem_A->owner, elem_A->subindex, elem_B->owner,
                        elem_B->subindex, F->get(), unpair_userdata);
      }
...
int BroadPhaseBasic::cull_point(const Vector3 &p_point,
...
int BroadPhaseBasic::cull_segment(const Vector3 &p_from, const Vector3 &p_to,
                                  CollisionObjectSW **p_results,
                                  int p_max_results, int *p_result_indices) {
...
int BroadPhaseBasic::cull_aabb(const AABB &p_aabb,
...
void BroadPhaseBasic::set_pair_callback(PairCallback p_pair_callback,
                                        void *p_userdata) {

  pair_userdata = p_userdata;
  pair_callback = p_pair_callback;
}
void BroadPhaseBasic::set_unpair_callback(UnpairCallback p_unpair_callback,
                                          void *p_userdata) {
...
void BroadPhaseBasic::update() {
  ...
  for (Map<ID, Element>::Element *I = element_map.front(); I; I = I->next()) {
    ...
    for (Map<ID, Element>::Element *J = I->next(); J; J = J->next()) {
      ...
      bool pair_ok = elem_A->aabb.intersects(elem_B->aabb) &&
                     (!elem_A->_static || !elem_B->_static);
      ...
      if (!pair_ok && E) {
        if (unpair_callback)
          unpair_callback(elem_A->owner, elem_A->subindex, elem_B->owner,
                          elem_B->subindex, E->get(), unpair_userdata);
        pair_map.erase(key);
      }
      ...
      if (pair_ok && !E) {
        ...
        void *data = NULL;
        if (pair_callback)
          data = pair_callback(elem_A->owner, elem_A->subindex, elem_B->owner,
                               elem_B->subindex, unpair_userdata);
...
BroadPhaseSW *BroadPhaseBasic::_create() { return memnew(BroadPhaseBasic); }
