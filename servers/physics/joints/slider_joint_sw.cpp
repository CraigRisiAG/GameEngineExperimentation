...
SliderJointSW::SliderJointSW(BodySW *rbA, BodySW *rbB,
...
bool SliderJointSW::setup(real_t p_step) {
  ...
  for (i = 0; i < 3; i++) {
    ...
    memnew_placement(
        &m_jacLin[i],
        JacobianEntrySW(A->get_principal_inertia_axes().transposed(),
                        B->get_principal_inertia_axes().transposed(),
                        m_relPosA - A->get_center_of_mass(),
                        m_relPosB - B->get_center_of_mass(), normalWorld,
                        A->get_inv_inertia(), A->get_inv_mass(),
  ...
  m_kAngle = real_t(1.0) / (A->compute_angular_impulse_denominator(axisA) +
...
void SliderJointSW::solve(real_t p_step) {
  ...
  for (i = 0; i < 3; i++) {
    ...
    real_t softness =
        (i) ? m_softnessOrthoLin
            : (m_solveLinLim ? m_softnessLimLin : m_softnessDirLin);
    real_t restitution =
        (i) ? m_restitutionOrthoLin
            : (m_solveLinLim ? m_restitutionLimLin : m_restitutionDirLin);
    real_t damping = (i) ? m_dampingOrthoLin
                         : (m_solveLinLim ? m_dampingLimLin : m_dampingDirLin);
    // calcutate and apply impulse
    real_t normalImpulse = softness *
                           (restitution * depth / p_step - damping * rel_vel) *
                           m_jacLinDiagABInv[i];
  ...
  if (m_solveAngLim) {
    impulseMag = (angVelB - angVelA).dot(axisA) * m_dampingLimAng +
                 m_angDepth * m_restitutionLimAng / p_step;
    impulseMag *= m_kAngle * m_softnessLimAng;
  } else {
    impulseMag = (angVelB - angVelA).dot(axisA) * m_dampingDirAng +
                 m_angDepth * m_restitutionDirAng / p_step;
...
Vector3 SliderJointSW::getAncorInA(void) {
  ...
  ancorInA = m_realPivotAInW +
             (m_lowerLinLimit + m_upperLinLimit) * real_t(0.5) * m_sliderAxis;
...
void SliderJointSW::set_param(PhysicsServer::SliderJointParam p_param,
                              real_t p_value) {
