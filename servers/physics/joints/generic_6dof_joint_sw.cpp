...
real_t G6DOFRotationalLimitMotorSW::solveAngularLimits(real_t timeStep,
                                                       Vector3 &axis,
                                                       real_t jacDiagABInv,
                                                       BodySW *body0,
                                                       BodySW *body1) {
...
real_t G6DOFTranslationalLimitMotorSW::solveLinearAxis(
    real_t timeStep, real_t jacDiagABInv, BodySW *body1,
    const Vector3 &pointInA, BodySW *body2, const Vector3 &pointInB,
    int limit_index, const Vector3 &axis_normal_on_a,
  ...
  m_accumulatedImpulse[limit_index] = sum > hi   ? real_t(0.)
                                      : sum < lo ? real_t(0.)
                                                 : sum;
...
Generic6DOFJointSW::Generic6DOFJointSW(BodySW *rbA, BodySW *rbB,
...
void Generic6DOFJointSW::buildLinearJacobian(JacobianEntrySW &jacLinear,
  ...
  memnew_placement(
      ...
      JacobianEntrySW(
          ...
          normalWorld, A->get_inv_inertia(), A->get_inv_mass(),
          B->get_inv_inertia(), B->get_inv_mass()));
}

void Generic6DOFJointSW::buildAngularJacobian(JacobianEntrySW &jacAngular,
                                              const Vector3 &jointAxisW) {
  memnew_placement(&jacAngular,
                   JacobianEntrySW(jointAxisW,
                                   A->get_principal_inertia_axes().transposed(),
                                   B->get_principal_inertia_axes().transposed(),
...
void Generic6DOFJointSW::solve(real_t p_timestep) {
  ...
  for (i = 0; i < 3; i++) {
    if (m_linearLimits.enable_limit[i] && m_linearLimits.isLimited(i)) {
      ...
      m_linearLimits.solveLinearAxis(m_timeStep, jacDiagABInv, A, pointInA, B,
                                     pointInB, i, linear_axis, m_AnchorPos);
    }
  }
  ...
  for (i = 0; i < 3; i++) {
    if (m_angularLimits[i].m_enableLimit &&
      ...
      m_angularLimits[i].solveAngularLimits(m_timeStep, angular_axis,
                                            angularJacDiagABInv, A, B);
    }
  }
}

void Generic6DOFJointSW::updateRHS(real_t timeStep) { (void)timeStep; }
...
void Generic6DOFJointSW::set_param(Vector3::Axis p_axis,
...
real_t Generic6DOFJointSW::get_param(
    Vector3::Axis p_axis, PhysicsServer::G6DOFJointAxisParam p_param) const {
...
void Generic6DOFJointSW::set_flag(Vector3::Axis p_axis,
...
bool Generic6DOFJointSW::get_flag(
    Vector3::Axis p_axis, PhysicsServer::G6DOFJointAxisFlag p_flag) const {
