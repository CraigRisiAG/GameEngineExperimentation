...
HingeJointSW::HingeJointSW(BodySW *rbA, BodySW *rbB, const Transform &frameA,
...
HingeJointSW::HingeJointSW(BodySW *rbA, BodySW *rbB, const Vector3 &pivotInA,
                           const Vector3 &pivotInB, const Vector3 &axisInA,
  ...
  m_rbAFrame.basis =
      Basis(rbAxisA1.x, rbAxisA2.x, axisInA.x, rbAxisA1.y, rbAxisA2.y,
            axisInA.y, rbAxisA1.z, rbAxisA2.z, axisInA.z);
  ...
  m_rbBFrame.basis =
      Basis(rbAxisB1.x, rbAxisB2.x, -axisInB.x, rbAxisB1.y, rbAxisB2.y,
            -axisInB.y, rbAxisB1.z, rbAxisB2.z, -axisInB.z);
...
bool HingeJointSW::setup(real_t p_step) {
    ...
    for (int i = 0; i < 3; i++) {
      memnew_placement(
          ...
          JacobianEntrySW(
              ...
              normal[i], A->get_inv_inertia(), A->get_inv_mass(),
              B->get_inv_inertia(), B->get_inv_mass()));
    }
  }
  ...
  memnew_placement(&m_jacAng[0],
                   JacobianEntrySW(jointAxis0,
                                   A->get_principal_inertia_axes().transposed(),
                                   B->get_principal_inertia_axes().transposed(),
                                   A->get_inv_inertia(), B->get_inv_inertia()));

  memnew_placement(&m_jacAng[1],
                   JacobianEntrySW(jointAxis1,
                                   A->get_principal_inertia_axes().transposed(),
                                   B->get_principal_inertia_axes().transposed(),
                                   A->get_inv_inertia(), B->get_inv_inertia()));

  memnew_placement(&m_jacAng[2],
                   JacobianEntrySW(hingeAxisWorld,
                                   A->get_principal_inertia_axes().transposed(),
                                   B->get_principal_inertia_axes().transposed(),
                                   A->get_inv_inertia(), B->get_inv_inertia()));
...
void HingeJointSW::solve(real_t p_step) {
...

  {
    ...
    Vector3 axisA =
        A->get_transform().basis.xform(m_rbAFrame.basis.get_axis(2));
    Vector3 axisB =
        B->get_transform().basis.xform(m_rbBFrame.basis.get_axis(2));
    ...
    Vector3 velrelOrthog = angAorthog - angBorthog;
    {
      ...
      if (m_solveLimit) {
        real_t amplitude =
            ((angVelB - angVelA).dot(axisA) * m_relaxationFactor +
             m_correction * (real_t(1.) / p_step) * m_biasFactor) *
            m_limitSign;
    ...
    if (m_enableAngularMotor) {
      ...
      real_t clippedMotorImpulse = unclippedMotorImpulse > m_maxMotorImpulse
                                       ? m_maxMotorImpulse
...
real_t HingeJointSW::get_hinge_angle() {
  const Vector3 refAxis0 =
      A->get_transform().basis.xform(m_rbAFrame.basis.get_axis(0));
  const Vector3 refAxis1 =
      A->get_transform().basis.xform(m_rbAFrame.basis.get_axis(1));
  const Vector3 swingAxis =
      B->get_transform().basis.xform(m_rbBFrame.basis.get_axis(1));

  return atan2fast(swingAxis.dot(refAxis0), swingAxis.dot(refAxis1));
}

void HingeJointSW::set_param(PhysicsServer::HingeJointParam p_param,
                             real_t p_value) {
...
void HingeJointSW::set_flag(PhysicsServer::HingeJointFlag p_flag,
                            bool p_value) {
