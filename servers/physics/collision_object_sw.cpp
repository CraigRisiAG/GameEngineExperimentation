...
void CollisionObjectSW::add_shape(ShapeSW *p_shape,
...
void CollisionObjectSW::set_shape_transform(int p_index,
                                            const Transform &p_transform) {
...
void CollisionObjectSW::_update_shapes() {
  ...
  for (int i = 0; i < shapes.size(); i++) {
    ...
    s.aabb_cache = s.aabb_cache.grow(
...
void CollisionObjectSW::_update_shapes_with_motion(const Vector3 &p_motion) {
  ...
  for (int i = 0; i < shapes.size(); i++) {
    ...
    shape_aabb = shape_aabb.merge(
        AABB(shape_aabb.position + p_motion, shape_aabb.size)); // use motion
    s.aabb_cache = shape_aabb;
