...
typedef void (*GenerateContactsFunc)(const Vector3 *, int, const Vector3 *, int,
                                     _CollectorCallback *);

static void _generate_contacts_point_point(const Vector3 *p_points_A,
                                           int p_point_count_A,
                                           const Vector3 *p_points_B,
                                           int p_point_count_B,
...
static void _generate_contacts_point_edge(const Vector3 *p_points_A,
                                          int p_point_count_A,
                                          const Vector3 *p_points_B,
                                          int p_point_count_B,
                                          _CollectorCallback *p_callback) {
  ...
  Vector3 closest_B =
      Geometry::get_closest_point_to_segment_uncapped(*p_points_A, p_points_B);
  p_callback->call(*p_points_A, closest_B);
}

static void _generate_contacts_point_face(const Vector3 *p_points_A,
                                          int p_point_count_A,
                                          const Vector3 *p_points_B,
                                          int p_point_count_B,
                                          _CollectorCallback *p_callback) {
  ...
  Vector3 closest_B =
      Plane(p_points_B[0], p_points_B[1], p_points_B[2]).project(*p_points_A);

  p_callback->call(*p_points_A, closest_B);
}

static void _generate_contacts_edge_edge(const Vector3 *p_points_A,
                                         int p_point_count_A,
                                         const Vector3 *p_points_B,
                                         int p_point_count_B,
                                         _CollectorCallback *p_callback) {
  ...
  if (Math::is_zero_approx(rel_A.dot(c))) {
    ...
    real_t dvec[4] = {axis.dot(p_points_A[0]), axis.dot(p_points_A[1]),
                      axis.dot(p_points_B[0]), axis.dot(p_points_B[1])};
  ...
  Vector3 closest_B =
      Geometry::get_closest_point_to_segment_uncapped(closest_A, p_points_B);
  p_callback->call(closest_A, closest_B);
}

static void _generate_contacts_face_face(const Vector3 *p_points_A,
                                         int p_point_count_A,
                                         const Vector3 *p_points_B,
                                         int p_point_count_B,
                                         _CollectorCallback *p_callback) {
  ...
  for (int i = 0; i < p_point_count_B; i++) {
    ...
    Vector3 clip_normal =
        (edge0_B - edge1_B).cross(plane_B.normal).normalized();
    // make a clip plane
  ...
  for (int i = 0; i < clipbuf_len; i++) {
    ...
    */
    ...
    if (p_callback->normal.dot(clipbuf_src[i]) >=
        p_callback->normal.dot(closest_B))
      continue;
...
static void _generate_contacts_from_supports(const Vector3 *p_points_A,
                                             int p_point_count_A,
                                             const Vector3 *p_points_B,
                                             int p_point_count_B,
                                             _CollectorCallback *p_callback) {
  ...
  GenerateContactsFunc contacts_func =
      generate_contacts_func_table[version_A][version_B];
...
public:
  ...
  _FORCE_INLINE_ bool test_axis(const Vector3 &p_axis) {
    ...
    if (Math::abs(axis.x) < CMP_EPSILON && Math::abs(axis.y) < CMP_EPSILON &&
  ...
  _FORCE_INLINE_ void generate_contacts() {
    ...
    int support_count_A;
    shape_A->get_supports(transform_A->basis.xform_inv(-best_axis).normalized(),
                          max_supports, supports_A, support_count_A);
    ...
    int support_count_B;
    shape_B->get_supports(transform_B->basis.xform_inv(best_axis).normalized(),
                          max_supports, supports_B, support_count_B);
    ...
    if (callback->prev_axis)
      *callback->prev_axis = best_axis;
    _generate_contacts_from_supports(supports_A, support_count_A, supports_B,
                                     support_count_B, callback);
  ...
  SeparatorAxisTest(const ShapeA *p_shape_A, const Transform &p_transform_A,
                    const ShapeB *p_shape_B, const Transform &p_transform_B,
                    _CollectorCallback *p_callback, real_t p_margin_A = 0,
                    real_t p_margin_B = 0) {
    best_depth = 1e15;
    shape_A = p_shape_A;
    shape_B = p_shape_B;
...
typedef void (*CollisionFunc)(const ShapeSW *, const Transform &,
                              const ShapeSW *, const Transform &,
                              _CollectorCallback *p_callback, real_t, real_t);

template <bool withMargin>
static void
_collision_sphere_sphere(const ShapeSW *p_a, const Transform &p_transform_a,
                         const ShapeSW *p_b, const Transform &p_transform_b,
                         _CollectorCallback *p_collector, real_t p_margin_a,
                         real_t p_margin_b) {
  ...
  SeparatorAxisTest<SphereShapeSW, SphereShapeSW, withMargin> separator(
      sphere_A, p_transform_a, sphere_B, p_transform_b, p_collector, p_margin_a,
      p_margin_b);
  ...
  if (!separator.test_axis(
          (p_transform_a.origin - p_transform_b.origin).normalized()))
    return;
...
template <bool withMargin>
static void
_collision_sphere_box(const ShapeSW *p_a, const Transform &p_transform_a,
                      const ShapeSW *p_b, const Transform &p_transform_b,
                      _CollectorCallback *p_collector, real_t p_margin_a,
                      real_t p_margin_b) {
  ...
  SeparatorAxisTest<SphereShapeSW, BoxShapeSW, withMargin> separator(
      sphere_A, p_transform_a, box_B, p_transform_b, p_collector, p_margin_a,
      p_margin_b);
  ...
  Vector3 cpoint = p_transform_b.xform(Vector3(
      ...
      (cnormal.x < 0) ? -box_B->get_half_extents().x
                      : box_B->get_half_extents().x,
      (cnormal.y < 0) ? -box_B->get_half_extents().y
                      : box_B->get_half_extents().y,
      (cnormal.z < 0) ? -box_B->get_half_extents().z
                      : box_B->get_half_extents().z));
  ...
  for (int i = 0; i < 3; i++) {
    ...
    Vector3 axis = point_axis.cross(p_transform_b.basis.get_axis(i))
                       .cross(p_transform_b.basis.get_axis(i))
                       .normalized();
...
template <bool withMargin>
static void
_collision_sphere_capsule(const ShapeSW *p_a, const Transform &p_transform_a,
                          const ShapeSW *p_b, const Transform &p_transform_b,
                          _CollectorCallback *p_collector, real_t p_margin_a,
                          real_t p_margin_b) {
  ...
  SeparatorAxisTest<SphereShapeSW, CapsuleShapeSW, withMargin> separator(
      sphere_A, p_transform_a, capsule_B, p_transform_b, p_collector,
  ...
  Vector3 capsule_axis =
      p_transform_b.basis.get_axis(2) * (capsule_B->get_height() * 0.5);

  Vector3 capsule_ball_1 = p_transform_b.origin + capsule_axis;

  if (!separator.test_axis(
          (capsule_ball_1 - p_transform_a.origin).normalized()))
    return;
  ...
  if (!separator.test_axis(
          (capsule_ball_2 - p_transform_a.origin).normalized()))
    return;
...
template <bool withMargin>
static void
_collision_sphere_cylinder(const ShapeSW *p_a, const Transform &p_transform_a,
                           const ShapeSW *p_b, const Transform &p_transform_b,
                           _CollectorCallback *p_collector, real_t p_margin_a,
                           real_t p_margin_b) {}

template <bool withMargin>
static void _collision_sphere_convex_polygon(
    const ShapeSW *p_a, const Transform &p_transform_a, const ShapeSW *p_b,
    const Transform &p_transform_b, _CollectorCallback *p_collector,
    real_t p_margin_a, real_t p_margin_b) {

  const SphereShapeSW *sphere_A = static_cast<const SphereShapeSW *>(p_a);
  const ConvexPolygonShapeSW *convex_polygon_B =
      static_cast<const ConvexPolygonShapeSW *>(p_b);

  SeparatorAxisTest<SphereShapeSW, ConvexPolygonShapeSW, withMargin> separator(
      sphere_A, p_transform_a, convex_polygon_B, p_transform_b, p_collector,
      p_margin_a, p_margin_b);
...
template <bool withMargin>
static void
_collision_sphere_face(const ShapeSW *p_a, const Transform &p_transform_a,
                       const ShapeSW *p_b, const Transform &p_transform_b,
                       _CollectorCallback *p_collector, real_t p_margin_a,
                       real_t p_margin_b) {
  ...
  SeparatorAxisTest<SphereShapeSW, FaceShapeSW, withMargin> separator(
      sphere_A, p_transform_a, face_B, p_transform_b, p_collector, p_margin_a,
      p_margin_b);
  ...
  if (!separator.test_axis(
          (vertex[0] - vertex[2]).cross(vertex[0] - vertex[1]).normalized()))
    return;
...
template <bool withMargin>
static void
_collision_box_box(const ShapeSW *p_a, const Transform &p_transform_a,
                   const ShapeSW *p_b, const Transform &p_transform_b,
                   _CollectorCallback *p_collector, real_t p_margin_a,
                   real_t p_margin_b) {
  ...
  SeparatorAxisTest<BoxShapeSW, BoxShapeSW, withMargin> separator(
      box_A, p_transform_a, box_B, p_transform_b, p_collector, p_margin_a,
      p_margin_b);
  ...
  for (int i = 0; i < 3; i++) {
    ...
    for (int j = 0; j < 3; j++) {
      ...
      Vector3 axis = p_transform_a.basis.get_axis(i).cross(
          p_transform_b.basis.get_axis(j));
  ...
  if (withMargin) {
    ...
    Vector3 support_a = p_transform_a.xform(Vector3(
        ...
        (cnormal_a.x < 0) ? -box_A->get_half_extents().x
                          : box_A->get_half_extents().x,
        (cnormal_a.y < 0) ? -box_A->get_half_extents().y
                          : box_A->get_half_extents().y,
        (cnormal_a.z < 0) ? -box_A->get_half_extents().z
                          : box_A->get_half_extents().z));

    Vector3 cnormal_b = p_transform_b.basis.xform_inv(-ab_vec);

    Vector3 support_b = p_transform_b.xform(Vector3(

        (cnormal_b.x < 0) ? -box_B->get_half_extents().x
                          : box_B->get_half_extents().x,
        (cnormal_b.y < 0) ? -box_B->get_half_extents().y
                          : box_B->get_half_extents().y,
        (cnormal_b.z < 0) ? -box_B->get_half_extents().z
                          : box_B->get_half_extents().z));
    ...
    for (int i = 0; i < 3; i++) {
      ...
      if (!separator.test_axis(
              axis_ab.cross(axis_a).cross(axis_a).normalized()))
        return;
      ...
      if (!separator.test_axis(
              axis_ab.cross(axis_b).cross(axis_b).normalized()))
        return;
    }
  }
...
template <bool withMargin>
static void
_collision_box_capsule(const ShapeSW *p_a, const Transform &p_transform_a,
                       const ShapeSW *p_b, const Transform &p_transform_b,
                       _CollectorCallback *p_collector, real_t p_margin_a,
                       real_t p_margin_b) {
  ...
  SeparatorAxisTest<BoxShapeSW, CapsuleShapeSW, withMargin> separator(
      box_A, p_transform_a, capsule_B, p_transform_b, p_collector, p_margin_a,
      p_margin_b);
  ...
  for (int i = 0; i < 2; i++) {
    ...
    Vector3 sphere_pos =
        p_transform_b.origin + ((i == 0) ? capsule_axis : -capsule_axis);

    Vector3 cnormal = p_transform_a.xform_inv(sphere_pos);

    Vector3 cpoint = p_transform_a.xform(Vector3(

        (cnormal.x < 0) ? -box_A->get_half_extents().x
                        : box_A->get_half_extents().x,
        (cnormal.y < 0) ? -box_A->get_half_extents().y
                        : box_A->get_half_extents().y,
        (cnormal.z < 0) ? -box_A->get_half_extents().z
                        : box_A->get_half_extents().z));
    ...
    for (int j = 0; j < 3; j++) {
      ...
      Vector3 axis = point_axis.cross(p_transform_a.basis.get_axis(j))
                         .cross(p_transform_a.basis.get_axis(j))
                         .normalized();
...
template <bool withMargin>
static void
_collision_box_cylinder(const ShapeSW *p_a, const Transform &p_transform_a,
                        const ShapeSW *p_b, const Transform &p_transform_b,
                        _CollectorCallback *p_collector, real_t p_margin_a,
                        real_t p_margin_b) {}

template <bool withMargin>
static void _collision_box_convex_polygon(
    const ShapeSW *p_a, const Transform &p_transform_a, const ShapeSW *p_b,
    const Transform &p_transform_b, _CollectorCallback *p_collector,
    real_t p_margin_a, real_t p_margin_b) {
  ...
  SeparatorAxisTest<BoxShapeSW, ConvexPolygonShapeSW, withMargin> separator(
      box_A, p_transform_a, convex_polygon_B, p_transform_b, p_collector,
      p_margin_a, p_margin_b);
  ...
  for (int i = 0; i < 3; i++) {
    ...
    for (int j = 0; j < edge_count; j++) {
      ...
      Vector3 e2 = p_transform_b.basis.xform(vertices[edges[j].a]) -
                   p_transform_b.basis.xform(vertices[edges[j].b]);
      ...
      for (int j = 0; j < 2; j++) {
        for (int k = 0; k < 2; k++) {
          ...
          for (int e = 0; e < edge_count; e++) {
            ...
            if (!separator.test_axis(
                    (point - p2).cross(n).cross(n).normalized()))
              return;
          }
        }
      }
...
template <bool withMargin>
static void
_collision_box_face(const ShapeSW *p_a, const Transform &p_transform_a,
                    const ShapeSW *p_b, const Transform &p_transform_b,
                    _CollectorCallback *p_collector, real_t p_margin_a,
                    real_t p_margin_b) {
  ...
  SeparatorAxisTest<BoxShapeSW, FaceShapeSW, withMargin> separator(
      box_A, p_transform_a, face_B, p_transform_b, p_collector, p_margin_a,
      p_margin_b);
  ...
  if (withMargin) {
    ...
    // convex edges and box points, there has to be a way to speed up this (get
    // closest point?)
    for (int i = 0; i < 2; i++) {
      for (int j = 0; j < 2; j++) {
        for (int k = 0; k < 2; k++) {
...
template <bool withMargin>
static void
_collision_capsule_capsule(const ShapeSW *p_a, const Transform &p_transform_a,
                           const ShapeSW *p_b, const Transform &p_transform_b,
                           _CollectorCallback *p_collector, real_t p_margin_a,
                           real_t p_margin_b) {
  ...
  SeparatorAxisTest<CapsuleShapeSW, CapsuleShapeSW, withMargin> separator(
      capsule_A, p_transform_a, capsule_B, p_transform_b, p_collector,
      p_margin_a, p_margin_b);
  ...
  Vector3 capsule_A_axis =
      p_transform_a.basis.get_axis(2) * (capsule_A->get_height() * 0.5);
  Vector3 capsule_B_axis =
      p_transform_b.basis.get_axis(2) * (capsule_B->get_height() * 0.5);
  ...
  if (!separator.test_axis((capsule_A_ball_1 - capsule_B_ball_1)
                               .cross(capsule_A_axis)
                               .cross(capsule_A_axis)
                               .normalized()))
    return;

  if (!separator.test_axis((capsule_A_ball_1 - capsule_B_ball_2)
                               .cross(capsule_A_axis)
                               .cross(capsule_A_axis)
                               .normalized()))
    return;

  if (!separator.test_axis((capsule_B_ball_1 - capsule_A_ball_1)
                               .cross(capsule_B_axis)
                               .cross(capsule_B_axis)
                               .normalized()))
    return;

  if (!separator.test_axis((capsule_B_ball_1 - capsule_A_ball_2)
                               .cross(capsule_B_axis)
                               .cross(capsule_B_axis)
                               .normalized()))
    return;
...
template <bool withMargin>
static void
_collision_capsule_cylinder(const ShapeSW *p_a, const Transform &p_transform_a,
                            const ShapeSW *p_b, const Transform &p_transform_b,
                            _CollectorCallback *p_collector, real_t p_margin_a,
                            real_t p_margin_b) {}

template <bool withMargin>
static void _collision_capsule_convex_polygon(
    const ShapeSW *p_a, const Transform &p_transform_a, const ShapeSW *p_b,
    const Transform &p_transform_b, _CollectorCallback *p_collector,
    real_t p_margin_a, real_t p_margin_b) {
  ...
  SeparatorAxisTest<CapsuleShapeSW, ConvexPolygonShapeSW, withMargin> separator(
      capsule_A, p_transform_a, convex_polygon_B, p_transform_b, p_collector,
      p_margin_a, p_margin_b);
  ...
  for (int i = 0; i < edge_count; i++) {
    ...
    // cylinder
    Vector3 edge_axis = p_transform_b.basis.xform(vertices[edges[i].a]) -
                        p_transform_b.basis.xform(vertices[edges[i].b]);
    Vector3 axis =
        edge_axis.cross(p_transform_a.basis.get_axis(2)).normalized();
  ...
  for (int i = 0; i < 2; i++) {
    ...
    Vector3 sphere_pos =
        p_transform_a.origin + ((i == 0) ? capsule_axis : -capsule_axis);

    for (int j = 0; j < edge_count; j++) {

      Vector3 n1 = sphere_pos - p_transform_b.xform(vertices[edges[j].a]);
      Vector3 n2 = p_transform_b.basis.xform(vertices[edges[j].a]) -
                   p_transform_b.basis.xform(vertices[edges[j].b]);
...
template <bool withMargin>
static void
_collision_capsule_face(const ShapeSW *p_a, const Transform &p_transform_a,
                        const ShapeSW *p_b, const Transform &p_transform_b,
                        _CollectorCallback *p_collector, real_t p_margin_a,
                        real_t p_margin_b) {
  ...
  SeparatorAxisTest<CapsuleShapeSW, FaceShapeSW, withMargin> separator(
      capsule_A, p_transform_a, face_B, p_transform_b, p_collector, p_margin_a,
      p_margin_b);
  ...
  for (int i = 0; i < 3; i++) {
    ...
    if (!separator.test_axis((p_transform_a.origin - vertex[i])
                                 .cross(capsule_axis)
                                 .cross(capsule_axis)
                                 .normalized()))
      return;
    ...
    for (int j = 0; j < 2; j++) {
      ...
      // point-spheres
      Vector3 sphere_pos =
          p_transform_a.origin + ((j == 0) ? capsule_axis : -capsule_axis);
...
template <bool withMargin>
static void
_collision_cylinder_cylinder(const ShapeSW *p_a, const Transform &p_transform_a,
                             const ShapeSW *p_b, const Transform &p_transform_b,
                             _CollectorCallback *p_collector, real_t p_margin_a,
                             real_t p_margin_b) {}

template <bool withMargin>
static void _collision_cylinder_convex_polygon(
    const ShapeSW *p_a, const Transform &p_transform_a, const ShapeSW *p_b,
    const Transform &p_transform_b, _CollectorCallback *p_collector,
    real_t p_margin_a, real_t p_margin_b) {}
...
template <bool withMargin>
static void
_collision_cylinder_face(const ShapeSW *p_a, const Transform &p_transform_a,
                         const ShapeSW *p_b, const Transform &p_transform_b,
                         _CollectorCallback *p_collector, real_t p_margin_a,
                         real_t p_margin_b) {}
...
static void _collision_convex_polygon_convex_polygon(
  ...
  const ConvexPolygonShapeSW *convex_polygon_A =
      static_cast<const ConvexPolygonShapeSW *>(p_a);
  const ConvexPolygonShapeSW *convex_polygon_B =
      static_cast<const ConvexPolygonShapeSW *>(p_b);

  SeparatorAxisTest<ConvexPolygonShapeSW, ConvexPolygonShapeSW, withMargin>
      separator(convex_polygon_A, p_transform_a, convex_polygon_B,
                p_transform_b, p_collector, p_margin_a, p_margin_b);
  ...
  for (int i = 0; i < face_count_A; i++) {
    ...
    // Vector3 axis = p_transform_a.basis.xform( faces_A[i].plane.normal
    // ).normalized();
  ...
  for (int i = 0; i < face_count_B; i++) {
    ...
    // Vector3 axis = p_transform_b.basis.xform( faces_B[i].plane.normal
    // ).normalized();
  ...
  for (int i = 0; i < edge_count_A; i++) {
    ...
    Vector3 e1 = p_transform_a.basis.xform(vertices_A[edges_A[i].a]) -
                 p_transform_a.basis.xform(vertices_A[edges_A[i].b]);

    for (int j = 0; j < edge_count_B; j++) {

      Vector3 e2 = p_transform_b.basis.xform(vertices_B[edges_B[j].a]) -
                   p_transform_b.basis.xform(vertices_B[edges_B[j].b]);
  ...
  if (withMargin) {
    ...
    for (int i = 0; i < vertex_count_A; i++) {
      ...
      for (int j = 0; j < vertex_count_B; j++) {
        ...
        if (!separator.test_axis(
                (va - p_transform_b.xform(vertices_B[j])).normalized()))
          return;
      }
    }
    // edge-vertex (shell)
...
template <bool withMargin>
static void _collision_convex_polygon_face(
    const ShapeSW *p_a, const Transform &p_transform_a, const ShapeSW *p_b,
    const Transform &p_transform_b, _CollectorCallback *p_collector,
    real_t p_margin_a, real_t p_margin_b) {
  ...
  SeparatorAxisTest<ConvexPolygonShapeSW, FaceShapeSW, withMargin> separator(
      convex_polygon_A, p_transform_a, face_B, p_transform_b, p_collector,
      p_margin_a, p_margin_b);
  ...
  for (int i = 0; i < face_count; i++) {
    ...
    Vector3 axis =
        p_transform_a.basis.xform(faces[i].plane.normal).normalized();
  ...
  for (int i = 0; i < edge_count; i++) {
    ...
    Vector3 e1 = p_transform_a.xform(vertices[edges[i].a]) -
                 p_transform_a.xform(vertices[edges[i].b]);
...
bool sat_calculate_penetration(
    ...
    bool p_swap, Vector3 *r_prev_axis, real_t p_margin_a, real_t p_margin_b) {
  ...
  static const CollisionFunc collision_table[6][6] = {
      {_collision_sphere_sphere<false>, _collision_sphere_box<false>,
       _collision_sphere_capsule<false>, _collision_sphere_cylinder<false>,
       _collision_sphere_convex_polygon<false>, _collision_sphere_face<false>},
      {0, _collision_box_box<false>, _collision_box_capsule<false>,
       _collision_box_cylinder<false>, _collision_box_convex_polygon<false>,
       _collision_box_face<false>},
      {0, 0, _collision_capsule_capsule<false>,
       _collision_capsule_cylinder<false>,
       _collision_capsule_convex_polygon<false>,
       _collision_capsule_face<false>},
      {0, 0, 0, _collision_cylinder_cylinder<false>,
       _collision_cylinder_convex_polygon<false>,
       _collision_cylinder_face<false>},
      {0, 0, 0, 0, _collision_convex_polygon_convex_polygon<false>,
       _collision_convex_polygon_face<false>},
      {0, 0, 0, 0, 0, 0},
  };

  static const CollisionFunc collision_table_margin[6][6] = {
      {_collision_sphere_sphere<true>, _collision_sphere_box<true>,
       _collision_sphere_capsule<true>, _collision_sphere_cylinder<true>,
       _collision_sphere_convex_polygon<true>, _collision_sphere_face<true>},
      {0, _collision_box_box<true>, _collision_box_capsule<true>,
       _collision_box_cylinder<true>, _collision_box_convex_polygon<true>,
       _collision_box_face<true>},
      {0, 0, _collision_capsule_capsule<true>,
       _collision_capsule_cylinder<true>,
       _collision_capsule_convex_polygon<true>, _collision_capsule_face<true>},
      {0, 0, 0, _collision_cylinder_cylinder<true>,
       _collision_cylinder_convex_polygon<true>,
       _collision_cylinder_face<true>},
      {0, 0, 0, 0, _collision_convex_polygon_convex_polygon<true>,
  ...
  collision_func(A, *transform_A, B, *transform_B, &callback, margin_A,
                 margin_B);
