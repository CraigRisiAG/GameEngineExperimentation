...
bool AreaPairSW::setup(real_t p_step) {
  ...
  if (area->is_shape_set_as_disabled(area_shape) ||
      body->is_shape_set_as_disabled(body_shape)) {
    result = false;
  } else if (area->test_collision_mask(body) &&
...
AreaPairSW::AreaPairSW(BodySW *p_body, int p_body_shape, AreaSW *p_area,
                       int p_area_shape) {
...
bool Area2PairSW::setup(real_t p_step) {
  ...
  bool result = false;
  if (area_a->is_shape_set_as_disabled(shape_a) ||
      area_b->is_shape_set_as_disabled(shape_b)) {
    result = false;
  } else if (area_a->test_collision_mask(area_b) &&
             CollisionSolverSW::solve_static(
...
Area2PairSW::Area2PairSW(AreaSW *p_area_a, int p_shape_a, AreaSW *p_area_b,
                         int p_shape_b) {
