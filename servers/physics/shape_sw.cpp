...
#define _FACE_IS_VALID_SUPPORT_THRESHOLD 0.9998
  ...
  aabb = p_aabb;
  configured = true;
  for (Map<ShapeOwnerSW *, int>::Element *E = owners.front(); E;
       E = E->next()) {
...
const Map<ShapeOwnerSW *, int> &ShapeSW::get_owners() const { return owners; }
...
ShapeSW::~ShapeSW() { ERR_FAIL_COND(owners.size()); }

Plane PlaneShapeSW::get_plane() const { return plane; }

void PlaneShapeSW::project_range(const Vector3 &p_normal,
                                 const Transform &p_transform, real_t &r_min,
                                 real_t &r_max) const {
...
bool PlaneShapeSW::intersect_segment(const Vector3 &p_begin,
                                     const Vector3 &p_end, Vector3 &r_result,
...
void PlaneShapeSW::_setup(const Plane &p_plane) {
  ...
  plane = p_plane;
  configure(
      AABB(Vector3(-1e4, -1e4, -1e4), Vector3(1e4 * 2, 1e4 * 2, 1e4 * 2)));
}

void PlaneShapeSW::set_data(const Variant &p_data) { _setup(p_data); }

Variant PlaneShapeSW::get_data() const { return plane; }

PlaneShapeSW::PlaneShapeSW() {}

//

real_t RayShapeSW::get_length() const { return length; }

bool RayShapeSW::get_slips_on_slope() const { return slips_on_slope; }

void RayShapeSW::project_range(const Vector3 &p_normal,
                               const Transform &p_transform, real_t &r_min,
                               real_t &r_max) const {
...
void RayShapeSW::get_supports(const Vector3 &p_normal, int p_max,
                              Vector3 *r_supports, int &r_amount) const {
...
bool RayShapeSW::intersect_segment(const Vector3 &p_begin, const Vector3 &p_end,
                                   Vector3 &r_result, Vector3 &r_normal) const {
...
Vector3 RayShapeSW::get_closest_point_to(const Vector3 &p_point) const {
  ...
  Vector3 s[2] = {Vector3(0, 0, 0), Vector3(0, 0, length)};
...
real_t SphereShapeSW::get_radius() const { return radius; }

void SphereShapeSW::project_range(const Vector3 &p_normal,
                                  const Transform &p_transform, real_t &r_min,
                                  real_t &r_max) const {
...
void SphereShapeSW::get_supports(const Vector3 &p_normal, int p_max,
                                 Vector3 *r_supports, int &r_amount) const {
...
bool SphereShapeSW::intersect_segment(const Vector3 &p_begin,
                                      const Vector3 &p_end, Vector3 &r_result,
                                      Vector3 &r_normal) const {

  return Geometry::segment_intersects_sphere(p_begin, p_end, Vector3(), radius,
                                             &r_result, &r_normal);
}
...
void SphereShapeSW::_setup(real_t p_radius) {
  ...
  radius = p_radius;
  configure(AABB(Vector3(-radius, -radius, -radius),
                 Vector3(radius * 2.0, radius * 2.0, radius * 2.0)));
}

void SphereShapeSW::set_data(const Variant &p_data) { _setup(p_data); }

Variant SphereShapeSW::get_data() const { return radius; }

SphereShapeSW::SphereShapeSW() { radius = 0; }

/********** BOX *************/

void BoxShapeSW::project_range(const Vector3 &p_normal,
                               const Transform &p_transform, real_t &r_min,
                               real_t &r_max) const {
...
Vector3 BoxShapeSW::get_support(const Vector3 &p_normal) const {
  ...
  Vector3 point((p_normal.x < 0) ? -half_extents.x : half_extents.x,
...
void BoxShapeSW::get_supports(const Vector3 &p_normal, int p_max,
                              Vector3 *r_supports, int &r_amount) const {
...
bool BoxShapeSW::intersect_segment(const Vector3 &p_begin, const Vector3 &p_end,
                                   Vector3 &r_result, Vector3 &r_normal) const {
...
bool BoxShapeSW::intersect_point(const Vector3 &p_point) const {
  ...
  return (Math::abs(p_point.x) < half_extents.x &&
          Math::abs(p_point.y) < half_extents.y &&
          Math::abs(p_point.z) < half_extents.z);
}
...
Vector3 BoxShapeSW::get_moment_of_inertia(real_t p_mass) const {
  ...
  return Vector3((p_mass / 3.0) * (ly * ly + lz * lz),
...
void BoxShapeSW::set_data(const Variant &p_data) { _setup(p_data); }

Variant BoxShapeSW::get_data() const { return half_extents; }

BoxShapeSW::BoxShapeSW() {}

/********** CAPSULE *************/

void CapsuleShapeSW::project_range(const Vector3 &p_normal,
                                   const Transform &p_transform, real_t &r_min,
                                   real_t &r_max) const {
...
void CapsuleShapeSW::get_supports(const Vector3 &p_normal, int p_max,
                                  Vector3 *r_supports, int &r_amount) const {
...
bool CapsuleShapeSW::intersect_segment(const Vector3 &p_begin,
                                       const Vector3 &p_end, Vector3 &r_result,
                                       Vector3 &r_normal) const {
  ...
  collided = Geometry::segment_intersects_cylinder(p_begin, p_end, height,
                                                   radius, &auxres, &auxn);
  ...
  collided = Geometry::segment_intersects_sphere(
      p_begin, p_end, Vector3(0, 0, height * 0.5), radius, &auxres, &auxn);
...
void CapsuleShapeSW::_setup(real_t p_height, real_t p_radius) {
  ...
  height = p_height;
  radius = p_radius;
  configure(AABB(Vector3(-radius, -radius, -height * 0.5 - radius),
                 Vector3(radius * 2, radius * 2, height + radius * 2.0)));
}
...
CapsuleShapeSW::CapsuleShapeSW() { height = radius = 0; }

/********** CONVEX POLYGON *************/

void ConvexPolygonShapeSW::project_range(const Vector3 &p_normal,
                                         const Transform &p_transform,
                                         real_t &r_min, real_t &r_max) const {
...
void ConvexPolygonShapeSW::get_supports(const Vector3 &p_normal, int p_max,
                                        Vector3 *r_supports,
                                        int &r_amount) const {
  ...
  for (int i = 0; i < fc; i++) {
    ...
    if (faces[i].plane.normal.dot(p_normal) >
        _FACE_IS_VALID_SUPPORT_THRESHOLD) {
  ...
  for (int i = 0; i < ec; i++) {
    ...
    real_t dot = (vertices[edges[i].a] - vertices[edges[i].b])
                     .normalized()
                     .dot(p_normal);
    dot = ABS(dot);
    if (dot < _EDGE_IS_VALID_SUPPORT_THRESHOLD &&
        (edges[i].a == vtx || edges[i].b == vtx)) {
...
bool ConvexPolygonShapeSW::intersect_segment(const Vector3 &p_begin,
                                             const Vector3 &p_end,
                                             Vector3 &r_result,
                                             Vector3 &r_normal) const {
...
Vector3
ConvexPolygonShapeSW::get_closest_point_to(const Vector3 &p_point) const {
  ...
  for (int i = 0; i < ec; i++) {
    ...
    Vector3 s[2] = {vertices[edges[i].a], vertices[edges[i].b]};
...
void ConvexPolygonShapeSW::set_data(const Variant &p_data) { _setup(p_data); }

Variant ConvexPolygonShapeSW::get_data() const { return mesh.vertices; }

ConvexPolygonShapeSW::ConvexPolygonShapeSW() {}

/********** FACE POLYGON *************/

void FaceShapeSW::project_range(const Vector3 &p_normal,
                                const Transform &p_transform, real_t &r_min,
                                real_t &r_max) const {
...
void FaceShapeSW::get_supports(const Vector3 &p_normal, int p_max,
                               Vector3 *r_supports, int &r_amount) const {
...
bool FaceShapeSW::intersect_segment(const Vector3 &p_begin,
                                    const Vector3 &p_end, Vector3 &r_result,
                                    Vector3 &r_normal) const {

  bool c = Geometry::segment_intersects_triangle(
      p_begin, p_end, vertex[0], vertex[1], vertex[2], &r_result);
  if (c) {
...
FaceShapeSW::FaceShapeSW() { configure(AABB()); }
...
void ConcavePolygonShapeSW::project_range(const Vector3 &p_normal,
                                          const Transform &p_transform,
                                          real_t &r_min, real_t &r_max) const {
...
void ConcavePolygonShapeSW::_cull_segment(int p_idx,
                                          _SegmentCullParams *p_params) const {
  ...
  */
  ...
  if (bvh->face_index >= 0) {
    ...
    if (Geometry::segment_intersects_triangle(p_params->from, p_params->to,
...
bool ConcavePolygonShapeSW::intersect_segment(const Vector3 &p_begin,
                                              const Vector3 &p_end,
                                              Vector3 &r_result,
                                              Vector3 &r_normal) const {
...
Vector3
ConcavePolygonShapeSW::get_closest_point_to(const Vector3 &p_point) const {
...
void ConcavePolygonShapeSW::cull(const AABB &p_local_aabb, Callback p_callback,
                                 void *p_userdata) const {
...
struct _VolumeSW_BVH_CompareX {
  ...
  _FORCE_INLINE_ bool operator()(const _VolumeSW_BVH_Element &a,
                                 const _VolumeSW_BVH_Element &b) const {
...
_VolumeSW_BVH *_volume_sw_build_bvh(_VolumeSW_BVH_Element *p_elements,
                                    int p_size, int &count) {
...
void ConcavePolygonShapeSW::_fill_bvh(_VolumeSW_BVH *p_bvh_tree,
                                      BVH *p_bvh_array, int &p_idx) {
...
void ConcavePolygonShapeSW::_setup(Vector<Vector3> p_faces) {
  ...
  for (int i = 0; i < src_face_count; i++) {
    ...
    bvh_arrayw[i].center =
        bvh_arrayw[i].aabb.position + bvh_arrayw[i].aabb.size * 0.5;
  ...
  int count = 0;
  _VolumeSW_BVH *bvh_tree =
      _volume_sw_build_bvh(bvh_arrayw, src_face_count, count);
...
void ConcavePolygonShapeSW::set_data(const Variant &p_data) { _setup(p_data); }

Variant ConcavePolygonShapeSW::get_data() const { return get_faces(); }

ConcavePolygonShapeSW::ConcavePolygonShapeSW() {}

/* HEIGHT MAP SHAPE */

Vector<real_t> HeightMapShapeSW::get_heights() const { return heights; }
int HeightMapShapeSW::get_width() const { return width; }
int HeightMapShapeSW::get_depth() const { return depth; }
real_t HeightMapShapeSW::get_cell_size() const { return cell_size; }

void HeightMapShapeSW::project_range(const Vector3 &p_normal,
                                     const Transform &p_transform,
                                     real_t &r_min, real_t &r_max) const {

  // not very useful, but not very used either
  p_transform.xform(get_aabb())
      .project_range_in_plane(Plane(p_normal, 0), r_min, r_max);
}
...
bool HeightMapShapeSW::intersect_segment(const Vector3 &p_begin,
                                         const Vector3 &p_end, Vector3 &r_point,
                                         Vector3 &r_normal) const {
...
void HeightMapShapeSW::cull(const AABB &p_local_aabb, Callback p_callback,
                            void *p_userdata) const {}
...
void HeightMapShapeSW::_setup(Vector<real_t> p_heights, int p_width,
                              int p_depth, real_t p_cell_size) {
