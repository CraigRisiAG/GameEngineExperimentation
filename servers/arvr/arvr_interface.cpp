...
#include "arvr_interface.h"
  ...
  ClassDB::bind_method(D_METHOD("set_is_primary", "enable"),
                       &ARVRInterface::set_is_primary);

  ClassDB::bind_method(D_METHOD("is_initialized"),
                       &ARVRInterface::is_initialized);
  ClassDB::bind_method(D_METHOD("set_is_initialized", "initialized"),
  ...
  ClassDB::bind_method(D_METHOD("get_tracking_status"),
                       &ARVRInterface::get_tracking_status);

  ClassDB::bind_method(D_METHOD("get_render_targetsize"),
                       &ARVRInterface::get_render_targetsize);
  ClassDB::bind_method(D_METHOD("is_stereo"), &ARVRInterface::is_stereo);

  ADD_GROUP("Interface", "interface_");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "interface_is_primary"),
               "set_is_primary", "is_primary");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "interface_is_initialized"),
               "set_is_initialized", "is_initialized");

  // we don't have any properties specific to VR yet....

  // but we do have properties specific to AR....
  ClassDB::bind_method(D_METHOD("get_anchor_detection_is_enabled"),
                       &ARVRInterface::get_anchor_detection_is_enabled);
  ClassDB::bind_method(D_METHOD("set_anchor_detection_is_enabled", "enable"),
  ...
  ADD_GROUP("AR", "ar_");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "ar_is_anchor_detection_enabled"),
...
StringName ARVRInterface::get_name() const { return "Unknown"; };
...
ARVRInterface::ARVRInterface() { tracking_state = ARVR_UNKNOWN_TRACKING; };
...
unsigned int
ARVRInterface::get_external_texture_for_eye(ARVRInterface::Eyes p_eye) {
  return 0;
};

/** these will only be implemented on AR interfaces, so we want dummies for VR
 * **/
bool ARVRInterface::get_anchor_detection_is_enabled() const { return false; };
