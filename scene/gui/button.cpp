  ...
  case NOTIFICATION_DRAW: {
    ...
    case DRAW_HOVER_PRESSED: {
      ...
      if (has_stylebox("hover_pressed") &&
          has_stylebox_override("hover_pressed")) {
    ...
    if (!_icon.is_null()) {
      ...
      if (_internal_margin[MARGIN_LEFT] > 0) {
        icon_ofs_region =
            _internal_margin[MARGIN_LEFT] + get_constant("hseparation");
      }
      ...
      if (expand_icon) {
        ...
        float icon_width =
            _icon->get_width() * _size.height / _icon->get_height();
        ...
        icon_region =
            Rect2(style->get_offset() +
                      Point2(icon_ofs_region, (_size.height - icon_height) / 2),
                  Size2(icon_width, icon_height));
      } else {
        icon_region =
            Rect2(style->get_offset() +
                      Point2(icon_ofs_region,
                             Math::floor((valign - _icon->get_height()) / 2.0)),
                  _icon->get_size());
      }
    }
    ...
    Point2 icon_ofs =
        !_icon.is_null()
            ? Point2(icon_region.size.width + get_constant("hseparation"), 0)
            : Point2();
    int text_clip =
    ...
    Point2 text_ofs =
        (size - style->get_minimum_size() - icon_ofs -
         font->get_string_size(xl_text) -
         Point2(_internal_margin[MARGIN_RIGHT] - _internal_margin[MARGIN_LEFT],
                0)) /
        2.0;
    ...
    case ALIGN_RIGHT: {
      if (_internal_margin[MARGIN_RIGHT] > 0) {
        text_ofs.x = size.x - style->get_margin(MARGIN_RIGHT) -
                     font->get_string_size(xl_text).x -
                     _internal_margin[MARGIN_RIGHT] -
                     get_constant("hseparation");
      } else {
        text_ofs.x = size.x - style->get_margin(MARGIN_RIGHT) -
                     font->get_string_size(xl_text).x;
    ...
    font->draw(ci, text_ofs.floor(), xl_text, color,
               clip_text ? text_clip : -1);

    if (!_icon.is_null() && icon_region.size.width > 0) {
      draw_texture_rect_region(_icon, icon_region,
                               Rect2(Point2(), _icon->get_size()), color_icon);
    }
  } break;
  }
}
...
Ref<Texture2D> Button::get_icon() const { return icon; }
...
Button::TextAlign Button::get_text_align() const { return align; }

void Button::_bind_methods() {

  ClassDB::bind_method(D_METHOD("set_text", "text"), &Button::set_text);
  ClassDB::bind_method(D_METHOD("get_text"), &Button::get_text);
  ClassDB::bind_method(D_METHOD("set_button_icon", "texture"),
                       &Button::set_icon);
  ...
  ClassDB::bind_method(D_METHOD("set_clip_text", "enabled"),
                       &Button::set_clip_text);
  ClassDB::bind_method(D_METHOD("get_clip_text"), &Button::get_clip_text);
  ClassDB::bind_method(D_METHOD("set_text_align", "align"),
  ...
  ADD_PROPERTY(PropertyInfo(Variant::INT, "align", PROPERTY_HINT_ENUM,
                            "Left,Center,Right"),
               "set_text_align", "get_text_align");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "expand_icon"), "set_expand_icon",
               "is_expand_icon");
}
