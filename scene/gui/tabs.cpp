...
Size2 Tabs::get_minimum_size() const {
  ...
  Size2 ms(0, MAX(MAX(tab_bg->get_minimum_size().height,
                      tab_fg->get_minimum_size().height),
                  tab_disabled->get_minimum_size().height) +
                  font->get_height());
  ...
  for (int i = 0; i < tabs.size(); i++) {
    ...
    if (tabs[i].right_button.is_valid()) {
      ...
      ms.width += bms.width;
      ms.height =
          MAX(bms.height + tab_bg->get_minimum_size().height, ms.height);
    }
...
void Tabs::_gui_input(const Ref<InputEvent> &p_event) {
  ...
  if (mb.is_valid()) {
    ...
    if (mb->is_pressed() &&
        (mb->get_button_index() == BUTTON_LEFT ||
         (select_with_rmb && mb->get_button_index() == BUTTON_RIGHT))) {
      ...
      for (int i = 0; i < tabs.size(); i++) {
        ...
        if (pos.x >= tabs[i].ofs_cache &&
            pos.x < tabs[i].ofs_cache + tabs[i].size_cache) {
          if (!tabs[i].disabled) {
            found = i;
          }
          break;
...
void Tabs::_notification(int p_what) {
  ...
  case NOTIFICATION_DRAW: {
    ...
    int limit =
        get_size().width - incr->get_size().width - decr->get_size().width;
    ...
    for (int i = 0; i < tabs.size(); i++) {
      ...
      if (icon.is_valid()) {
        ...
        icon->draw(ci, Point2i(w, sb->get_margin(MARGIN_TOP) +
                                      ((sb_rect.size.y - sb_ms.y) -
                                       icon->get_height()) /
                                          2));
        if (tabs[i].text != "")
      ...
      font->draw(
          ci,
          Point2i(w, sb->get_margin(MARGIN_TOP) +
                         ((sb_rect.size.y - sb_ms.y) - font->get_height()) / 2 +
                         font->get_ascent()),
      ...
      if (tabs[i].right_button.is_valid()) {
        ...
        rb_rect.position.y =
            sb->get_margin(MARGIN_TOP) +
            ((sb_rect.size.y - sb_ms.y) - (rb_rect.size.y)) / 2;
        ...
        rb->draw(ci,
                 Point2i(w + style->get_margin(MARGIN_LEFT),
                         rb_rect.position.y + style->get_margin(MARGIN_TOP)));
        w += rb->get_width();
      ...
      if (cb_displaypolicy == CLOSE_BUTTON_SHOW_ALWAYS ||
        ...
        cb_rect.position.y =
            sb->get_margin(MARGIN_TOP) +
            ((sb_rect.size.y - sb_ms.y) - (cb_rect.size.y)) / 2;
        ...
        cb->draw(ci,
                 Point2i(w + style->get_margin(MARGIN_LEFT),
                         cb_rect.position.y + style->get_margin(MARGIN_TOP)));
        w += cb->get_width();
    ...
    if (offset > 0 || missing_right) {
      ...
      if (offset > 0)
        draw_texture(highlight_arrow == 0 ? decr_hl : decr,
                     Point2(limit, vofs));
      else
        draw_texture(decr, Point2(limit, vofs), Color(1, 1, 1, 0.5));

      if (missing_right)
        draw_texture(highlight_arrow == 1 ? incr_hl : incr,
                     Point2(limit + decr->get_size().width, vofs));
      else
        draw_texture(incr, Point2(limit + decr->get_size().width, vofs),
                     Color(1, 1, 1, 0.5));
...
int Tabs::get_tab_count() const { return tabs.size(); }
...
int Tabs::get_current_tab() const { return current; }

int Tabs::get_hovered_tab() const { return hover; }

int Tabs::get_tab_offset() const { return offset; }

bool Tabs::get_offset_buttons_visible() const { return buttons_visible; }
...
void Tabs::set_tab_right_button(int p_tab,
                                const Ref<Texture2D> &p_right_button) {
...
void Tabs::_update_cache() {
  ...
  for (int i = 0; i < tabs.size(); i++) {
    ...
    tabs.write[i].size_text =
        Math::ceil(font->get_string_size(tabs[i].xl_text).width);
  ...
  for (int i = 0; i < tabs.size(); i++) {
    ...
    if (min_width > 0 && mw > limit && i != current) {
      if (lsize > m_width) {
        slen = m_width -
               (sb->get_margin(MARGIN_LEFT) + sb->get_margin(MARGIN_RIGHT));
        if (tabs[i].icon.is_valid()) {
          slen -= tabs[i].icon->get_width();
          slen -= get_constant("hseparation");
        }
        if (cb_displaypolicy == CLOSE_BUTTON_SHOW_ALWAYS ||
            (cb_displaypolicy == CLOSE_BUTTON_SHOW_ACTIVE_ONLY &&
             i == current)) {
          Ref<Texture2D> cb = get_icon("close");
          slen -= cb->get_width();
          slen -= get_constant("hseparation");
...
bool Tabs::can_drop_data(const Point2 &p_point, const Variant &p_data) const {
  ...
  if (String(d["type"]) == "tab_element") {
    ...
    if (from_path == to_path) {
      ...
      if (from_tabs &&
          from_tabs->get_tabs_rearrange_group() == get_tabs_rearrange_group()) {
        return true;
      }
    }
  }
...
Tabs::TabAlign Tabs::get_tab_align() const { return tab_align; }
...
Rect2 Tabs::get_tab_rect(int p_tab) const {
  ...
  return Rect2(tabs[p_tab].ofs_cache, 0, tabs[p_tab].size_cache,
               get_size().height);
}
...
void Tabs::set_min_width(int p_width) { min_width = p_width; }

void Tabs::set_scrolling_enabled(bool p_enabled) {
  scrolling_enabled = p_enabled;
}

bool Tabs::get_scrolling_enabled() const { return scrolling_enabled; }
...
int Tabs::get_tabs_rearrange_group() const { return tabs_rearrange_group; }

void Tabs::set_select_with_rmb(bool p_enabled) { select_with_rmb = p_enabled; }

bool Tabs::get_select_with_rmb() const { return select_with_rmb; }
...
void Tabs::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("set_current_tab", "tab_idx"),
                       &Tabs::set_current_tab);
  ClassDB::bind_method(D_METHOD("get_current_tab"), &Tabs::get_current_tab);
  ClassDB::bind_method(D_METHOD("set_tab_title", "tab_idx", "title"),
                       &Tabs::set_tab_title);
  ClassDB::bind_method(D_METHOD("get_tab_title", "tab_idx"),
                       &Tabs::get_tab_title);
  ClassDB::bind_method(D_METHOD("set_tab_icon", "tab_idx", "icon"),
                       &Tabs::set_tab_icon);
  ClassDB::bind_method(D_METHOD("get_tab_icon", "tab_idx"),
                       &Tabs::get_tab_icon);
  ClassDB::bind_method(D_METHOD("set_tab_disabled", "tab_idx", "disabled"),
                       &Tabs::set_tab_disabled);
  ClassDB::bind_method(D_METHOD("get_tab_disabled", "tab_idx"),
                       &Tabs::get_tab_disabled);
  ClassDB::bind_method(D_METHOD("remove_tab", "tab_idx"), &Tabs::remove_tab);
  ClassDB::bind_method(D_METHOD("add_tab", "title", "icon"), &Tabs::add_tab,
                       DEFVAL(""), DEFVAL(Ref<Texture2D>()));
  ClassDB::bind_method(D_METHOD("set_tab_align", "align"),
                       &Tabs::set_tab_align);
  ...
  ClassDB::bind_method(D_METHOD("get_offset_buttons_visible"),
                       &Tabs::get_offset_buttons_visible);
  ClassDB::bind_method(D_METHOD("ensure_tab_visible", "idx"),
                       &Tabs::ensure_tab_visible);
  ClassDB::bind_method(D_METHOD("get_tab_rect", "tab_idx"),
                       &Tabs::get_tab_rect);
  ClassDB::bind_method(D_METHOD("move_tab", "from", "to"), &Tabs::move_tab);
  ClassDB::bind_method(D_METHOD("set_tab_close_display_policy", "policy"),
                       &Tabs::set_tab_close_display_policy);
  ClassDB::bind_method(D_METHOD("get_tab_close_display_policy"),
                       &Tabs::get_tab_close_display_policy);
  ClassDB::bind_method(D_METHOD("set_scrolling_enabled", "enabled"),
                       &Tabs::set_scrolling_enabled);
  ClassDB::bind_method(D_METHOD("get_scrolling_enabled"),
                       &Tabs::get_scrolling_enabled);
  ClassDB::bind_method(D_METHOD("set_drag_to_rearrange_enabled", "enabled"),
                       &Tabs::set_drag_to_rearrange_enabled);
  ClassDB::bind_method(D_METHOD("get_drag_to_rearrange_enabled"),
                       &Tabs::get_drag_to_rearrange_enabled);
  ClassDB::bind_method(D_METHOD("set_tabs_rearrange_group", "group_id"),
                       &Tabs::set_tabs_rearrange_group);
  ClassDB::bind_method(D_METHOD("get_tabs_rearrange_group"),
                       &Tabs::get_tabs_rearrange_group);

  ClassDB::bind_method(D_METHOD("set_select_with_rmb", "enabled"),
                       &Tabs::set_select_with_rmb);
  ClassDB::bind_method(D_METHOD("get_select_with_rmb"),
                       &Tabs::get_select_with_rmb);

  ADD_SIGNAL(MethodInfo("tab_changed", PropertyInfo(Variant::INT, "tab")));
  ADD_SIGNAL(
      MethodInfo("right_button_pressed", PropertyInfo(Variant::INT, "tab")));
  ADD_SIGNAL(MethodInfo("tab_close", PropertyInfo(Variant::INT, "tab")));
  ADD_SIGNAL(MethodInfo("tab_hover", PropertyInfo(Variant::INT, "tab")));
  ADD_SIGNAL(MethodInfo("reposition_active_tab_request",
                        PropertyInfo(Variant::INT, "idx_to")));
  ADD_SIGNAL(MethodInfo("tab_clicked", PropertyInfo(Variant::INT, "tab")));

  ADD_PROPERTY(PropertyInfo(Variant::INT, "current_tab", PROPERTY_HINT_RANGE,
                            "-1,4096,1", PROPERTY_USAGE_EDITOR),
               "set_current_tab", "get_current_tab");
  ADD_PROPERTY(PropertyInfo(Variant::INT, "tab_align", PROPERTY_HINT_ENUM,
                            "Left,Center,Right"),
               "set_tab_align", "get_tab_align");
  ADD_PROPERTY(PropertyInfo(Variant::INT, "tab_close_display_policy",
                            PROPERTY_HINT_ENUM,
                            "Show Never,Show Active Only,Show Always"),
               "set_tab_close_display_policy", "get_tab_close_display_policy");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "scrolling_enabled"),
               "set_scrolling_enabled", "get_scrolling_enabled");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "drag_to_rearrange_enabled"),
