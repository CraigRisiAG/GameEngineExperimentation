...
public:
  enum TreeCellMode {
    ...
    CELL_MODE_CUSTOM, ///< Contains a custom value, show a string, and an edit
                      ///< button
  };
...
private:
  ...
  struct Cell {
    ...
    void draw_icon(const RID &p_where, const Point2 &p_pos,
                   const Size2 &p_size = Size2(),
                   const Color &p_color = Color()) const;
  };
...
protected:
  ...
  Variant _call_recursive_bind(const Variant **p_args, int p_argcount,
...
public:
  ...
  void add_button(int p_column, const Ref<Texture2D> &p_button, int p_id = -1,
                  bool p_disabled = false, const String &p_tooltip = "");
  ...
  void set_range_config(int p_column, double p_min, double p_max, double p_step,
                        bool p_exp = false);
  void get_range_config(int p_column, double &r_min, double &r_max,
                        double &r_step) const;
  ...
  void set_custom_draw(int p_column, Object *p_object,
  ...
  void call_recursive(const StringName &p_method, const Variant **p_args,
...
public:
  enum SelectMode { SELECT_SINGLE, SELECT_ROW, SELECT_MULTI };
...
private:
  ...
  // void draw_item_text(String p_text,const Ref<Texture2D>& p_icon,int
  // p_icon_max_w,bool p_tool,Rect2i p_rect,const Color& p_color);
  void draw_item_rect(const TreeItem::Cell &p_cell, const Rect2i &p_rect,
                      const Color &p_color, const Color &p_icon_color);
  int draw_item(const Point2i &p_pos, const Point2 &p_draw_ofs,
                const Size2 &p_draw_size, TreeItem *p_item);
  void select_single_item(TreeItem *p_selected, TreeItem *p_current, int p_col,
                          TreeItem *p_prev = NULL, bool *r_in_range = NULL,
                          bool p_force_deselect = false);
  int propagate_mouse_event(const Point2i &p_pos, int x_ofs, int y_ofs,
                            bool p_doubleclick, TreeItem *p_item, int p_button,
  ...
  TreeItem *_search_item_text(TreeItem *p_at, const String &p_find, int *r_col,
                              bool p_selectable, bool p_backwards = false);

  TreeItem *_find_item_at_pos(TreeItem *p_item, const Point2 &p_pos,
...
public:
  ...
  // and wraps around.
  TreeItem *search_item_text(const String &p_find, int *r_col = NULL,
