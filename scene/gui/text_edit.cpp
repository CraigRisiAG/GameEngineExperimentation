...
inline bool _is_symbol(CharType c) { return is_symbol(c); }

static bool _is_text_char(CharType c) { return !is_symbol(c); }

static bool _is_whitespace(CharType c) { return c == '\t' || c == ' '; }
...
static bool _is_number(CharType c) { return (c >= '0' && c <= '9'); }
...
void TextEdit::Text::set_font(const Ref<Font> &p_font) { font = p_font; }
...
const Map<int, TextEdit::Text::ColorRegionInfo> &
TextEdit::Text::get_color_region_info(int p_line) const {
...
void TextEdit::Text::remove(int p_at) { text.remove(p_at); }
...
void TextEdit::_update_scrollbars() {
  ...
  v_scroll->set_begin(Point2(size.width - vmin.width,
                             cache.style_normal->get_margin(MARGIN_TOP)));
  v_scroll->set_end(Point2(
      size.width, size.height - cache.style_normal->get_margin(MARGIN_TOP) -
...
void TextEdit::_click_selection_held() {
  ...
  // not a huge problem. I'm unsure if there's an actual fix that doesn't have a
  // ton of side effects.
...
void TextEdit::_update_minimap_click() {
  ...
  if (row >= get_first_visible_line() &&
      (row < get_last_visible_line() || row >= (text.size() - 1))) {
...
void TextEdit::_notification(int p_what) {
  ...
  case NOTIFICATION_INTERNAL_PHYSICS_PROCESS: {
    if (scrolling && get_v_scroll() != target_v_scroll) {
      ...
      double vel =
          ((target_y / dist) * ((minimap_clicked) ? 3000 : v_scroll_speed)) *
  ...
  case NOTIFICATION_DRAW: {
    if (first_draw) {
      // Size may not be the final one, so attempts to ensure cursor was visible
      // may have failed.
...

    {
      ...
      if (line_numbers) {
        ...
        cache.line_number_w =
            (cache.line_number_w + 1) * cache.font->get_char_size('0').width;
      } else {
        cache.line_number_w = 0;
      }
    }
    ...
    VisualServer::get_singleton()->canvas_item_set_clip(get_canvas_item(),
                                                        true);
    ...
    if (syntax_coloring) {
      if (cache.background_color.a > 0.01) {
        VisualServer::get_singleton()->canvas_item_add_rect(
            ci, Rect2(Point2i(), get_size()), cache.background_color);
      }
    }
    ...
    if (line_length_guidelines) {
      ...
      if (hard_x > xmargin_beg && hard_x < xmargin_end) {
        VisualServer::get_singleton()->canvas_item_add_line(
            ci, Point2(hard_x, 0), Point2(hard_x, size.height),
            cache.line_length_guideline_color);
      }

      // Draw a "Soft" line length guideline, less visible than the hard line
      // length guideline. It's usually set to a lower column compared to the
      // hard line length guideline. Only drawn if its column differs from the
      ...
      if (hard_x != soft_x && soft_x > xmargin_beg && soft_x < xmargin_end) {
        VisualServer::get_singleton()->canvas_item_add_line(
            ci, Point2(soft_x, 0), Point2(soft_x, size.height),
      ...
      if (cursor.column > 0) {
        ...
        if (closec != 0) {
          ...
          for (int i = cursor.line; i >= 0; i--) {
            ...
            int from =
                i == cursor.line ? cursor.column - 2 : text[i].length() - 1;
    ...
    if (draw_minimap) {
      ...
      // draw the minimap
      Color viewport_color = (cache.background_color.get_v() < 0.5)
                                 ? Color(1, 1, 1, 0.1)
                                 : Color(0, 0, 0, 0.1);
      VisualServer::get_singleton()->canvas_item_add_rect(
          ci,
          Rect2((xmargin_end + 2), viewport_offset_y, cache.minimap_width,
                viewport_height),
          viewport_color);
      for (int i = 0; i < minimap_draw_amount; i++) {
        ...
        for (int line_wrap_index = 0; line_wrap_index < line_wrap_amount + 1;
             line_wrap_index++) {
          ...
          int indent_px =
              line_wrap_index != 0 ? get_indent_level(minimap_line) : 0;
          ...
          if (minimap_line == cursor.line &&
              cursor_wrap_index == line_wrap_index && highlight_current_line) {
            VisualServer::get_singleton()->canvas_item_add_rect(
                ci, Rect2((xmargin_end + 2), i * 3, cache.minimap_width, 2),
          ...
          for (int j = 0; j < str.length(); j++) {
            ...
            if (!is_whitespace) {
              ...
              if (j < str.length() - 1 && color == previous_color &&
                  !out_of_bounds) {
                continue;
              }

              // If we've changed colour we are at the start of a new section,
              // therefore we need to go back to the end of the previous section
              // to draw it, we'll also add the character back on.
            ...
            if (characters > 0) {
              ...
              // take one for zero indexing, and if we hit whitespace / the end
              // of a word.
              int chars =
                  MAX(0, (j - (characters - 1)) - (is_whitespace ? 1 : 0)) + 1;
              ...
              VisualServer::get_singleton()->canvas_item_add_rect(
                  ...
                  Rect2(Point2(char_x_ofs, minimap_line_height * i),
                        Point2(minimap_char_size.x * characters,
                               minimap_char_size.y)),
                  previous_color);
            }
    ...
    for (int i = 0; i < draw_amount; i++) {
      ...
      for (int line_wrap_index = 0; line_wrap_index < line_wrap_amount + 1;
        ...
        int indent_px =
            line_wrap_index != 0
                ? get_indent_level(line) * cache.font->get_char_size(' ').width
                : 0;
        if (indent_px >= wrap_at) {
          indent_px = 0;
        }
        ...
        int ofs_y =
            (i * get_row_height() + cache.line_spacing / 2) + ofs_readonly;
        ...
        if (!search_text.empty())
          search_text_col =
              _get_column_pos_of_word(search_text, str, search_flags, 0);
        ...
        if (select_identifiers_enabled && highlighted_word.length() != 0) {
          if (_is_char(highlighted_word[0])) {
            highlighted_word_col = _get_column_pos_of_word(
                highlighted_word, fullstr,
                SEARCH_MATCH_CASE | SEARCH_WHOLE_WORDS, 0);
          }
        }
        ...
        if (text.is_marked(line)) {
          VisualServer::get_singleton()->canvas_item_add_rect(
              ci,
              Rect2(xmargin_beg + ofs_x, ofs_y, xmargin_end - xmargin_beg,
                    get_row_height()),
              cache.mark_color);
        }
        ...
        if (str.length() == 0) {
          ...
          if (line == cursor.line && cursor_wrap_index == line_wrap_index &&
              highlight_current_line) {
            VisualServer::get_singleton()->canvas_item_add_rect(
                ci, Rect2(ofs_x, ofs_y, xmargin_end, get_row_height()),
                cache.current_line_color);
          }

          // Give visual indication of empty selected line.
          if (selection.active && line >= selection.from_line &&
              line <= selection.to_line && char_margin >= xmargin_beg) {
            int char_w = cache.font->get_char_size(' ').width;
            VisualServer::get_singleton()->canvas_item_add_rect(
                ci, Rect2(xmargin_beg + ofs_x, ofs_y, char_w, get_row_height()),
                cache.selection_color);
          }
        } else {
          // If it has text, then draw current line marker in the margin, as
          // line number etc will draw over it, draw the rest of line marker
          // later.
          if (line == cursor.line && cursor_wrap_index == line_wrap_index &&
              highlight_current_line) {
            VisualServer::get_singleton()->canvas_item_add_rect(
                ci, Rect2(0, ofs_y, xmargin_beg + ofs_x, get_row_height()),
                cache.current_line_color);
          }
        }
        ...
        if (line_wrap_index == 0) {
          ...
          if (text.is_bookmark(line)) {
            if (draw_bookmark_gutter) {
              ...
              VisualServer::get_singleton()->canvas_item_add_circle(
                  ci,
                  Point2(cache.style_normal->get_margin(MARGIN_LEFT) +
                             horizontal_gap - 2 + marker_radius / 2,
                         ofs_y + vertical_gap + marker_radius / 2),
                  marker_radius,
                  Color(cache.bookmark_color.r, cache.bookmark_color.g,
          ...
          if (text.is_breakpoint(line)) {
            if (draw_breakpoint_gutter) {
              ...
              int marker_width =
                  cache.breakpoint_gutter_width - (horizontal_gap * 2);
              // No transparency on marker.
              VisualServer::get_singleton()->canvas_item_add_rect(
                  ci,
                  Rect2(cache.style_normal->get_margin(MARGIN_LEFT) +
                            horizontal_gap - 2,
                        ofs_y + vertical_gap, marker_width, marker_height),
                  Color(cache.breakpoint_color.r, cache.breakpoint_color.g,
          ...
          if (draw_info_gutter && text.has_info_icon(line)) {
            ...
            int gutter_left = cache.style_normal->get_margin(MARGIN_LEFT) +
          ...
          if (executing_line == line) {
            if (draw_breakpoint_gutter) {
              ...
              int horizontal_gap = (cache.breakpoint_gutter_width * 30) / 100;
              int marker_height =
                  get_row_height() - (vertical_gap * 2) + icon_extra_size;
              int marker_width = cache.breakpoint_gutter_width -
                                 (horizontal_gap * 2) + icon_extra_size;
              cache.executing_icon->draw_rect(
                  ci,
                  Rect2(cache.style_normal->get_margin(MARGIN_LEFT) +
                            horizontal_gap - 2 - icon_extra_size / 2,
                        ofs_y + vertical_gap - icon_extra_size / 2,
                        marker_width, marker_height),
                  false,
                  Color(cache.executing_line_color.r,
          ...
          if (draw_fold_gutter) {
            ...
            if (is_folded(line)) {
              int xofs =
                  horizontal_gap - (cache.can_fold_icon->get_width()) / 2;
              int yofs =
                  (get_row_height() - cache.folded_icon->get_height()) / 2;
              cache.folded_icon->draw(
                  ci, Point2(gutter_left + xofs + ofs_x, ofs_y + yofs),
                  cache.code_folding_color);
            } else if (can_fold(line)) {
              int xofs =
                  -cache.can_fold_icon->get_width() / 2 - horizontal_gap + 3;
              int yofs =
                  (get_row_height() - cache.can_fold_icon->get_height()) / 2;
          ...
          if (cache.line_number_w) {
            int yofs =
                ofs_y + (get_row_height() - cache.font->get_height()) / 2;
            ...
            cache.font->draw(
                ...
                fc,
                text.is_safe(line) ? cache.safe_line_number_color
        ...
        for (; j < str.length(); j++) {
            ...
            if (line == cursor.line && cursor_wrap_index == line_wrap_index &&
              ...
              if (j == str.length() - 1) {
                ...
                VisualServer::get_singleton()->canvas_item_add_rect(
                    ci,
                    Rect2(xmargin_beg + ofs_x, ofs_y,
                          (char_ofs + char_margin) - (xmargin_beg + ofs_x),
                          get_row_height()),
                    cache.current_line_color);
              }
            }
          ...
          if (search_text_col != -1) {
            ...
            if (j >= search_text_col + search_text.length())
              search_text_col =
                  _get_column_pos_of_word(search_text, str, search_flags, j);

            in_search_result = j >= search_text_col &&
                               j < search_text_col + search_text.length();

            if (in_search_result) {
              VisualServer::get_singleton()->canvas_item_add_rect(
                  ci,
                  Rect2(Point2i(char_ofs + char_margin, ofs_y),
          ...
          bool in_selection =
              (selection.active && line >= selection.from_line &&
               ...
               (line > selection.from_line ||
                last_wrap_column + j >= selection.from_column) &&
               (line < selection.to_line ||
          ...
          if (line == cursor.line && cursor_wrap_index == line_wrap_index &&
            ...
            if (line_wrap_index != 0 && j == 0) {
              VisualServer::get_singleton()->canvas_item_add_rect(
                  ci,
                  Rect2(char_ofs + char_margin + ofs_x - indent_px, ofs_y,
                        indent_px, get_row_height()),
                  cache.current_line_color);
            }
          ...
          if (in_search_result) {
            Color border_color =
                (line == search_result_line && j >= search_result_col &&
            ...
            VisualServer::get_singleton()->canvas_item_add_rect(
                ci,
                Rect2(Point2i(char_ofs + char_margin + ofs_x,
            ...
            if (j == search_text_col + search_text.length() - 1)
              VisualServer::get_singleton()->canvas_item_add_rect(
                  ci,
                  Rect2(Point2i(char_ofs + char_margin + char_w + ofs_x - 1,
                                ofs_y),
                        Size2i(1, get_row_height())),
                  border_color);
          }
            ...
            if (highlighted_text_col != -1) {
              ...
              if (j > highlighted_text_col + highlighted_text.length()) {
                highlighted_text_col = _get_column_pos_of_word(
                    ...
                    SEARCH_MATCH_CASE | SEARCH_WHOLE_WORDS, j);
              }
              ...
              // If this is the original highlighted text we don't want to
              // highlight it again.
              if (cursor.line == line && cursor_wrap_index == line_wrap_index &&
          ...
          if (highlighted_word_col != -1) {
            if (j + last_wrap_column >
                highlighted_word_col + highlighted_word.length()) {
              highlighted_word_col = _get_column_pos_of_word(
                  highlighted_word, fullstr,
          ...
          if (brace_matching_enabled) {
            ...
            if ((brace_open_match_line == line &&
                 brace_open_match_column == last_wrap_column + j) ||
                (cursor.column == last_wrap_column + j && cursor.line == line &&
              ...
              drawer.draw_char(
                  ci, Point2i(char_ofs + char_margin + ofs_x, yofs + ascent),
                  '_', str[j + 1],
            ...
            if ((brace_close_match_line == line &&
                 brace_close_match_column == last_wrap_column + j) ||
                (cursor.column == last_wrap_column + j + 1 &&
                 cursor.line == line && cursor_wrap_index == line_wrap_index &&
          ...
          if (cursor.column == last_wrap_column + j && cursor.line == line &&
              cursor_wrap_index == line_wrap_index) {
            ...
            if (ime_text.length() > 0) {
              ...
              while (true) {
                ...
                int im_char_width =
                    cache.font->get_char_size(cchar, next).width;
                ...
                if (selected) {
                  VisualServer::get_singleton()->canvas_item_add_rect(
                      ci,
                      Rect2(Point2(char_ofs + char_margin,
                ...
                drawer.draw_char(
                    ci, Point2(char_ofs + char_margin + ofs_x, ofs_y + ascent),
                    cchar, next, color);
              ...
              if (draw_caret) {
                if (insert_mode) {
...
#endif
                  VisualServer::get_singleton()->canvas_item_add_rect(
                      ci, Rect2(cursor_pos, Size2i(caret_w, caret_h)),
                      cache.caret_color);
                } else {
#ifdef TOOLS_ENABLED
                  ...
                  VisualServer::get_singleton()->canvas_item_add_rect(
                      ci,
                      Rect2(cursor_pos,
                            Size2i(caret_w, cache.font->get_height())),
                      cache.caret_color);
                }
              }
            }
          ...
          if (cursor.column == last_wrap_column + j && cursor.line == line &&
              cursor_wrap_index == line_wrap_index && block_caret &&
              draw_caret && !insert_mode) {
          ...
          if (str[j] >= 32) {
            ...
            if (underlined) {
              ...
              draw_rect(Rect2(char_ofs + char_margin + ofs_x, yofs + ascent + 2,
                              w, line_width),
                        in_selection && override_selected_font_color
                            ? cache.font_color_selected
                            : color);
            ...
            cache.tab_icon->draw(
                ci, Point2(char_ofs + char_margin + ofs_x, ofs_y + yofs),
                in_selection && override_selected_font_color
                    ? cache.font_color_selected
                    : color);
          }
          ...
          if (line_wrap_index == line_wrap_amount && j == str.length() - 1 &&
              is_folded(line)) {
            int yofs =
                (get_row_height() - cache.folded_eol_icon->get_height()) / 2;
            int xofs = cache.folded_eol_icon->get_width() / 2;
            Color eol_color = cache.code_folding_color;
            eol_color.a = 1;
            cache.folded_eol_icon->draw(
                ci, Point2(char_ofs + char_margin + xofs + ofs_x, ofs_y + yofs),
                eol_color);
          }
        }
...
#endif
                VisualServer::get_singleton()->canvas_item_add_rect(
                    ci, Rect2(cursor_pos, Size2i(char_w, caret_h)),
                    cache.caret_color);
              } else {
                int char_w = cache.font->get_char_size(' ').width;
#ifdef TOOLS_ENABLED
    ...
    if (completion_active && completion_options.size() > 0) {
      ...
      int cmax_width = get_constant("completion_max_width") *
      ...
      if (completion_options_size < 50) {
        for (int i = 0; i < completion_options_size; i++) {
          int w2 =
              MIN(cache.font->get_string_size(completion_options[i].display).x,
                  cmax_width);
          if (w2 > w)
            w = w2;
        }
      ...
      if (cursor_pos.y + get_row_height() + th > get_size().height) {
        completion_rect.position.y = cursor_pos.y - th -
                                     (cache.line_spacing / 2.0f) -
                                     cursor_insert_offset_y;
      } else {
        completion_rect.position.y = cursor_pos.y + cache.font->get_height() +
                                     (cache.line_spacing / 2.0f) +
                                     csb->get_offset().y -
                                     cursor_insert_offset_y;
      ...
      draw_style_box(csb, Rect2(completion_rect.position - csb->get_offset(),
      ...
      if (cache.completion_background_color.a > 0.01) {
        VisualServer::get_singleton()->canvas_item_add_rect(
            ci,
            Rect2(completion_rect.position,
                  completion_rect.size + Size2(scrollw, 0)),
            cache.completion_background_color);
      }
      int line_from = CLAMP(completion_index - lines / 2, 0,
                            completion_options_size - lines);
      VisualServer::get_singleton()->canvas_item_add_rect(
          ci,
          Rect2(Point2(completion_rect.position.x,
                       completion_rect.position.y +
                           (completion_index - line_from) * get_row_height()),
                Size2(completion_rect.size.width, get_row_height())),
          cache.completion_selected_color);
      draw_rect(Rect2(completion_rect.position +
                          Vector2(icon_area_size.x + icon_hsep, 0),
                      Size2(MIN(nofs, completion_rect.size.width -
      ...
      for (int i = 0; i < lines; i++) {
        ...
        Point2 title_pos(completion_rect.position.x,
        ...
        Rect2 icon_area(completion_rect.position.x,
                        completion_rect.position.y + i * get_row_height(),
                        icon_area_size.width, icon_area_size.height);
        if (icon.is_valid()) {
          const real_t max_scale = 0.7f;
          const real_t side = max_scale * icon_area.size.width;
          real_t scale =
              MIN(side / icon->get_width(), side / icon->get_height());
          Size2 icon_size = icon->get_size() * scale;
          draw_texture_rect(
              icon, Rect2(icon_area.position + (icon_area.size - icon_size) / 2,
                          icon_size));
        }

        title_pos.x = icon_area.position.x + icon_area.size.width + icon_hsep;
        draw_string(
            cache.font, title_pos, completion_options[l].display, text_color,
            completion_rect.size.width - (icon_area_size.x + icon_hsep));
      }
      ...
      if (scrollw) {
        ...
        draw_rect(Rect2(completion_rect.position.x + completion_rect.size.width,
                        completion_rect.position.y + o * completion_rect.size.y,
                        scrollw, completion_rect.size.y * r),
                  scrollc);
      }
    ...
    if (show_hint) {
      ...
      for (int i = 0; i < sc; i++) {
        ...
        if (i == 0) {
          offset =
              font->get_string_size(l.substr(0, l.find(String::chr(0xFFFF)))).x;
        } else {
      ...
      Point2 hint_ofs =
          Vector2(completion_hint_offset, cursor_pos.y) + callhint_offset;
      ...
      for (int i = 0; i < sc; i++) {
        ...
        if (l.find(String::chr(0xFFFF)) != -1) {
          begin =
              font->get_string_size(l.substr(0, l.find(String::chr(0xFFFF)))).x;
          end = font->get_string_size(l.substr(0, l.rfind(String::chr(0xFFFF))))
                    .x;
        }
        ...
        draw_string(font, round_ofs, l.replace(String::chr(0xFFFF), ""),
                    font_color);
        if (end > 0) {
          Vector2 b = hint_ofs + sb->get_offset() +
                      Vector2(begin, font->get_height() +
                                         font->get_height() * i + spacing - 1);
  ...
  case NOTIFICATION_FOCUS_ENTER: {
    ...
    OS::get_singleton()->set_ime_active(true);
    Point2 cursor_pos =
        Point2(cursor_get_column(), cursor_get_line()) * get_row_height();
    OS::get_singleton()->set_ime_position(get_global_position() + cursor_pos);
...
void TextEdit::_consume_pair_symbol(CharType ch) {
  ...
  if (is_selection_active()) {
    ...
    _insert_text(get_selection_to_line(),
                 get_selection_to_column() + to_col_offset, ch_single_pair,
                 &new_line, &new_column);
...
void TextEdit::_consume_backspace_for_pair_symbol(int prev_line,
                                                  int prev_column) {
...
void TextEdit::backspace_at_cursor() {
  ...
  if (text.has_info_icon(cursor.line)) {
    set_line_info_icon(prev_line, text.get_info_icon(cursor.line),
  ...
  if (auto_brace_completion_enabled && cursor.column > 0 &&
    ...
    if (cursor.column != 0 && indent_using_spaces) {
      ...
      if (unindent) {
        // We want to remove spaces up to closest indent, or whole indent if
        // cursor is pointing at it.
...
void TextEdit::indent_right() {
  ...
  for (int i = start_line; i <= end_line; i++) {
    ...
    if (indent_using_spaces) {
      ...
      // Since we will add this much spaces we want move whole selection and
      // cursor by this much.
  ...
  if (is_selection_active()) {
    select(selection.from_line, selection.from_column + selection_offset,
           selection.to_line, selection.to_column + selection_offset);
  }
...
void TextEdit::indent_left() {
  ...
  // changing content of line can move cursor and selection on it's own (if new
  // line ends before previous position of either), therefore we just remember
  // initial values and at the end of the operation offset them by number of
  // removed characters.
  ...
  for (int i = start_line; i <= end_line; i++) {
    ...
    if (line_text.begins_with("\t")) {
      ...
      // When unindenting we aim to remove spaces before line that has selection
      // no matter what is selected, so we start of by finding first non
      // whitespace character of line
      int left = _find_first_non_whitespace_column_of_line(line_text);

      // Here we remove only enough spaces to align text to nearest full
      // multiple of indentation_size. In case where selection begins at the
      // start of indentation_size multiple we remove whole indentation level.
...
void TextEdit::_get_mouse_pos(const Point2i &p_mouse, int &r_row,
                              int &r_col) const {
  ...
  if (is_wrap_enabled() || is_hiding_enabled()) {
    ...
    int f_ofs = num_lines_from_rows(first_vis_line, cursor.wrap_ofs,
                                    rows + (1 * SGN(rows)), wrap_index) -
                1;
    if (rows < 0)
  ...
  if (row >= text.size()) {
    ...
    int colx =
        p_mouse.x - (cache.style_normal->get_margin(MARGIN_LEFT) +
                     cache.line_number_w + cache.breakpoint_gutter_width +
                     cache.fold_gutter_width + cache.info_gutter_width);
    colx += cursor.x_ofs;
...
void TextEdit::_get_minimap_mouse_row(const Point2i &p_mouse,
                                      int &r_row) const {
  ...
  int wi;
  int minimap_line =
      (v_scroll->get_max() <= minimap_visible_lines) ? -1 : first_visible_line;
  ...
  if (is_wrap_enabled() || is_hiding_enabled()) {
    ...
    int f_ofs = num_lines_from_rows(minimap_line, cursor.wrap_ofs,
                                    rows + (1 * SGN(rows)), wrap_index) -
                1;
    if (rows < 0) {
...
void TextEdit::_gui_input(const Ref<InputEvent> &p_gui_input) {
  ...
  if (mb.is_valid()) {
    if (completion_active && completion_rect.has_point(mb->get_position())) {
      ...
      if (mb->get_button_index() == BUTTON_LEFT) {
        ...
        completion_index =
            CLAMP(completion_line_ofs +
                      (mb->get_position().y - completion_rect.position.y) /
                          get_row_height(),
                  0, completion_options.size() - 1);
    ...
    if (mb->is_pressed()) {
      ...
      if (mb->get_button_index() == BUTTON_LEFT) {
        ...
        int row, col;
        _get_mouse_pos(Point2i(mb->get_position().x, mb->get_position().y), row,
                       col);
        ...
        if (is_folded(row)) {
          ...
          line_width += cache.style_normal->get_margin(MARGIN_LEFT) +
                        cache.line_number_w + cache.breakpoint_gutter_width +
                        cache.info_gutter_width + cache.fold_gutter_width -
                        cursor.x_ofs;
        ...
        if (!mb->is_doubleclick() &&
            (OS::get_singleton()->get_ticks_msec() - last_dblclk) < 600 &&
      ...
      if (mb->get_button_index() == BUTTON_LEFT) {
        if (mb->get_command() && highlighted_word != String()) {
          int row, col;
          _get_mouse_pos(Point2i(mb->get_position().x, mb->get_position().y),
                         row, col);
  ...
  if (pan_gesture.is_valid()) {
    ...
    h_scroll->set_value(h_scroll->get_value() +
                        pan_gesture->get_delta().x * 100);
  ...
  if (k.is_valid()) {
    ...
    if (completion_active) {
      ...
      if (valid) {
        ...
        if (!k->get_alt()) {
          ...
          if (k->get_keycode() == KEY_KP_ENTER ||
              k->get_keycode() == KEY_ENTER || k->get_keycode() == KEY_TAB) {
          ...
          if (auto_brace_completion_enabled && _is_pair_symbol(chr[0])) {
            ...
            if (insert_mode) {
              ...
              if (cursor.column < get_line(cursor.line).length()) {
                _remove_text(cursor.line, cursor.column, cursor.line,
                             cursor.column + 1);
              }
            }
    ...
    if (k->get_command() && !k->get_shift() && !k->get_alt() &&
        !k->get_metakey() && k->get_keycode() == KEY_INSERT) {
    ...
    if (!readonly && selection.active) {
      ...
      case KEY_HOME:
      case KEY_END:
        // Ignore arrows if any modifiers are held (shift = selecting, others
        // may be used for editor hotkeys).
        if (k->get_command() || k->get_shift() || k->get_alt())
          break;
        unselect = true;
        break;
      ...
      default:
        if (k->get_unicode() >= 32 && !k->get_command() && !k->get_alt() &&
            !k->get_metakey())
          clear = true;
      ...
      if (clear) {
        ...
        selection.active = false;
        update();
        _remove_text(selection.from_line, selection.from_column,
                     selection.to_line, selection.to_column);
    ...
    case KEY_ENTER: {
      ...
      if (auto_indent && !(k->get_command() && k->get_shift())) {
        // Indent once again if previous line will end with ':','{','[','(' and
        // the line is not a comment (i.e. colon/brace precedes current cursor
        // position).
        if (cursor.column > 0) {
          ...
          if (!is_line_comment(cursor.line) && should_indent) {
            ...
            if ((closing_char != 0) &&
                (closing_char == text[cursor.line][cursor.column]) &&
                !k->get_command()) {
              brace_indent = true;
      ...
      if (is_selection_active()) {
        ...
        if (k->get_shift()) {
          ...
          if (cc > 0 && cc <= text[cursor.line].length()) {
            if (text[cursor.line][cc - 1] == '\t') {
              ...
              if (spaces_to_remove > 0) {
                _remove_text(cursor.line, cc - spaces_to_remove, cursor.line,
                             cc);
          ...
          if (indent_using_spaces) {
            ...
            int spaces_to_add =
                _calculate_spaces_till_next_right_indent(cursor.column);
    ...
    case KEY_BACKSPACE: {
      ...
      if (k->get_alt()) {
        ...
        // Check if we are removing a single whitespace, if so remove it and the
        // next char type, else we just remove the whitespace.
        ...
        bool only_char =
            (_is_text_char(text[line][column - 1]) && !only_whitespace);
    ...
    case KEY_LEFT: {
...
#ifdef APPLE_STYLE_KEYS
      if (k->get_command()) {
        // Start at first column (it's slightly faster that way) and look for
        ...
        if (new_cursor_pos == cursor.column) {
          // We're already at the first text character, so move to the very
          // beginning of the line.
          cursor_set_column(0);
        } else {
          // We're somewhere to the right of the first text character; move to
          // the first one.
      ...
      if (k->get_alt()) {
        ...
        if (cursor.line > 0) {
          cursor_set_line(
              cursor.line -
              num_lines_from(CLAMP(cursor.line - 1, 0, text.size() - 1), -1));
          cursor_set_column(text[cursor.line].length());
        }
      } else {
    ...
    case KEY_PAGEUP: {
      ...
      int n_line = cursor.line -
                   num_lines_from_rows(cursor.line, get_cursor_wrap_index(),
                                       -get_visible_rows(), wi) +
                   1;
    ...
    case KEY_SPACE: {
      ...
      if (completion_enabled &&
          k->get_metakey()) { // cmd-space is spotlight shortcut in OSX
#else
...
void TextEdit::_post_shift_selection() {
  ...
  if (selection.active && selection.selecting_mode == Selection::MODE_SHIFT) {
    ...
    select(selection.selecting_line, selection.selecting_column, cursor.line,
           cursor.column);
    update();
  }
...
void TextEdit::_base_insert_text(int p_line, int p_char, const String &p_text,
                                 int &r_end_line, int &r_end_column) {
  ...
  if (shift_first_line) {
    ...
    if (text.has_info_icon(p_line)) {
      text.set_info_icon(p_line + 1, text.get_info_icon(p_line),
...
String TextEdit::_base_get_text(int p_from_line, int p_from_column,
                                int p_to_line, int p_to_column) const {
  ...
  ERR_FAIL_COND_V(p_to_line == p_from_line && p_to_column < p_from_column,
                  String()); // 'from > to'.
...
void TextEdit::_base_remove_text(int p_from_line, int p_from_column,
                                 int p_to_line, int p_to_column) {
  ...
  ERR_FAIL_COND(p_to_line == p_from_line &&
                p_to_column < p_from_column); // 'from > to'.
...
void TextEdit::_remove_text(int p_from_line, int p_from_column, int p_to_line,
                            int p_to_column) {
...
void TextEdit::_generate_context_menu() {
  ...
  if (!readonly) {
    ...
    menu->add_item(
        RTR("Redo"), MENU_REDO,
        is_shortcut_keys_enabled() ? KEY_MASK_CMD | KEY_MASK_SHIFT | KEY_Z : 0);
  }
}
...
void TextEdit::adjust_viewport_to_cursor() {
  ...
  int visible_width =
      get_size().width - cache.style_normal->get_minimum_size().width -
      cache.line_number_w - cache.breakpoint_gutter_width -
      cache.fold_gutter_width - cache.info_gutter_width - cache.minimap_width;
...
void TextEdit::cursor_set_line(int p_row, bool p_adjust_viewport,
                               bool p_can_be_hidden, int p_wrap_index) {
    ...
    if (is_line_hidden(CLAMP(p_row, 0, text.size() - 1))) {
      ...
      if (p_row + move_down <= text.size() - 1 &&
        ...
        if (p_row - move_up > 0 && !is_line_hidden(p_row - move_up)) {
          ...
          WARN_PRINT(("Cursor set to hidden line " + itos(p_row) +
...
int TextEdit::cursor_get_column() const { return cursor.column; }

int TextEdit::cursor_get_line() const { return cursor.line; }

bool TextEdit::cursor_get_blink_enabled() const { return caret_blink_enabled; }
...
bool TextEdit::cursor_is_block_mode() const { return block_caret; }
...
int TextEdit::get_char_pos_for_line(int p_px, int p_line,
                                    int p_wrap_index) const {
...
void TextEdit::insert_text_at_cursor(const String &p_text) {
  ...
  if (selection.active) {
    ...
    _remove_text(selection.from_line, selection.from_column, selection.to_line,
                 selection.to_column);
    selection.active = false;
    selection.selecting_mode = Selection::MODE_NONE;
  }
...
Control::CursorShape TextEdit::get_cursor_shape(const Point2 &p_pos) const {
  ...
  if (p_pos.x < gutter) {
    ...
    // Info icons.
    int gutter_left =
        left_margin + cache.breakpoint_gutter_width + cache.info_gutter_width;
    if (draw_info_gutter &&
    ...
    if (draw_fold_gutter && p_pos.x > gutter_left + cache.line_number_w - 6 &&
        p_pos.x <=
            gutter_left + cache.line_number_w + cache.fold_gutter_width - 3) {
    ...
    if (is_folded(row)) {
      ...
      if (p_pos.x > line_width - 3 &&
          p_pos.x <= line_width + cache.folded_eol_icon->get_width() + 3) {
        return CURSOR_POINTING_HAND;
      }
    }
  }
...
void TextEdit::set_text(String p_text) {
  ...
  if (undo_enabled) {
    ...
    begin_complex_operation();
    _remove_text(0, 0, MAX(0, get_line_count() - 1),
...
bool TextEdit::is_readonly() const { return readonly; }
...
bool TextEdit::is_wrap_enabled() const { return wrap_enabled; }

void TextEdit::set_max_chars(int p_max_chars) { max_chars = p_max_chars; }

int TextEdit::get_max_chars() const { return max_chars; }
...
void TextEdit::_set_syntax_highlighting(
    SyntaxHighlighter *p_syntax_highlighter) {
...
int TextEdit::_is_line_in_region(int p_line) {
  ...
  for (int i = previous_line; i < p_line; i++) {
    ...
    for (const Map<int, Text::ColorRegionInfo>::Element *E = cri_map.front(); E;
         E = E->next()) {
...
Map<int, TextEdit::Text::ColorRegionInfo>
TextEdit::_get_line_color_region_info(int p_line) const {
...
void TextEdit::add_keyword_color(const String &p_keyword,
                                 const Color &p_color) {
...
void TextEdit::add_color_region(const String &p_begin_key,
                                const String &p_end_key, const Color &p_color,
                                bool p_line_only) {
...
void TextEdit::add_member_keyword(const String &p_keyword,
                                  const Color &p_color) {
...
bool TextEdit::is_syntax_coloring_enabled() const { return syntax_coloring; }
...
void TextEdit::cut() {
  ...
  if (!selection.active) {
    ...
    String clipboard =
        _base_get_text(selection.from_line, selection.from_column,
                       selection.to_line, selection.to_column);
    OS::get_singleton()->set_clipboard(clipboard);
    ...
    cursor_set_line(
        selection
            .from_line); // Set afterwards else it causes the view to be offset.
    cursor_set_column(selection.from_column);
...
void TextEdit::copy() {
  ...
  if (!selection.active) {
    ...
    if (text[cursor.line].length() != 0) {
      ...
      String clipboard = _base_get_text(cursor.line, 0, cursor.line,
...
void TextEdit::select(int p_from_line, int p_from_column, int p_to_line,
                      int p_to_column) {
  if (!selecting_enabled)
    return;
...
bool TextEdit::is_selection_active() const { return selection.active; }
...
String TextEdit::get_selection_text() const {
  ...
  return _base_get_text(selection.from_line, selection.from_column,
                        selection.to_line, selection.to_column);
}
...
int TextEdit::_get_column_pos_of_word(const String &p_key,
...
Vector<int> TextEdit::_search_bind(const String &p_key, uint32_t p_search_flags,
                                   int p_from_line, int p_from_column) const {
...
bool TextEdit::search(const String &p_key, uint32_t p_search_flags,
                      int p_from_line, int p_from_column, int &r_line,
                      int &r_column) const {
    ...
    while (true) {
      ...
      if (p_search_flags & SEARCH_BACKWARDS) {
        while ((last_pos = (p_search_flags & SEARCH_MATCH_CASE)
                               ...
                               : text_line.rfindn(p_key, pos_from)) != -1) {
        ...
        while ((last_pos = (p_search_flags & SEARCH_MATCH_CASE)
                               ...
                               : text_line.findn(p_key, pos_from)) != -1) {
          if (last_pos >= from_column) {
            pos = last_pos;
            break;
          }
...
void TextEdit::remove_breakpoints() {
  for (int i = 0; i < text.size(); i++) {
    if (text.is_breakpoint(i))
      /* Should "breakpoint_toggled" be fired when breakpoints are removed this
       * way? */
      text.set_breakpoint(i, false);
  }
}

void TextEdit::set_line_info_icon(int p_line, Ref<Texture2D> p_icon,
                                  String p_info) {
  ERR_FAIL_INDEX(p_line, text.size());
...
int TextEdit::num_lines_from_rows(int p_line_from, int p_wrap_index_from,
                                  int visible_amount, int &wrap_index) const {
...
void TextEdit::_do_text_op(const TextOperation &p_op, bool p_reverse) {
  ...
  if (insert) {
    ...
    int check_line;
    int check_column;
    _base_insert_text(p_op.from_line, p_op.from_column, p_op.text, check_line,
                      check_column);
    ERR_FAIL_COND(check_line != p_op.to_line);     // BUG.
    ERR_FAIL_COND(check_column != p_op.to_column); // BUG.
  } else {

    _base_remove_text(p_op.from_line, p_op.from_column, p_op.to_line,
                      p_op.to_column);
  }
}
...
bool TextEdit::is_indent_using_spaces() const { return indent_using_spaces; }
...
int TextEdit::get_indent_size() { return indent_size; }
...
void TextEdit::set_draw_spaces(bool p_draw) { draw_spaces = p_draw; }
...
bool TextEdit::is_insert_mode() const { return insert_mode; }
...
int TextEdit::get_last_visible_line() const {
  ...
  last_vis_line = first_vis_line +
                  num_lines_from_rows(first_vis_line, cursor.wrap_ofs,
                                      get_visible_rows() + 1, wi) -
                  1;
...
int TextEdit::get_last_visible_line_wrap_index() const {
  ...
  int first_vis_line = get_first_visible_line();
  int wi;
  num_lines_from_rows(first_vis_line, cursor.wrap_ofs, get_visible_rows() + 1,
                      wi);
  return wi;
}
...
void TextEdit::set_v_scroll_speed(float p_speed) { v_scroll_speed = p_speed; }

float TextEdit::get_v_scroll_speed() const { return v_scroll_speed; }

void TextEdit::set_completion(bool p_enabled,
...
void TextEdit::_confirm_completion() {
  ...
  _remove_text(cursor.line, cursor.column - completion_base.length(),
  ...
  if (last_completion_char == '(') {
    ...
    if (next_char == last_completion_char) {
      _base_remove_text(cursor.line, cursor.column - 1, cursor.line,
                        cursor.column);
    ...
    _base_remove_text(cursor.line, cursor.column - 2, cursor.line,
                      cursor.column);
...
void TextEdit::_update_completion_candidates() {
  ...
  for (List<ScriptCodeCompletionOption>::Element *E =
    ...
    if (option.display.length() == 0) {
      ...
      */
      // But is more performant due to being inlined and looping over the
      ...
      if (!*ssq) { // Matched the whole subsequence in s
        if (ssq_last_tgt ==
            &option.display[s.length() - 1]) { // Finished matching in the first
                                               // s.length() characters
          completion_options.push_back(option);
        } else {
          completion_options_subseq.push_back(option);
...
void TextEdit::query_code_comple() {
  ...
  if (!ignored) {
    if (ofs > 0 && (inquote || _is_completable(l[ofs - 1]) ||
                    completion_prefixes.has(String::chr(l[ofs - 1]))))
      emit_signal("request_completion");
    else if (ofs > 1 && l[ofs - 1] == ' ' &&
             completion_prefixes.has(
                 String::chr(l[ofs - 2]))) // Make it work with a space too,
                                           // it's good enough.
      emit_signal("request_completion");
  }
}
...
void TextEdit::code_complete(const List<ScriptCodeCompletionOption> &p_strings,
                             bool p_forced) {
...
String TextEdit::get_tooltip(const Point2 &p_pos) const {
  ...
  if (select_word(s, col, beg, end)) {
    ...
    String tt =
        tooltip_obj->call(tooltip_func, s.substr(beg, end - beg), tooltip_ud);
...
void TextEdit::set_tooltip_request_func(Object *p_obj,
...
bool TextEdit::is_show_line_numbers_enabled() const { return line_numbers; }
...
bool TextEdit::is_drawing_fold_gutter() const { return draw_fold_gutter; }
...
int TextEdit::get_fold_gutter_width() const { return cache.fold_gutter_width; }
...
bool TextEdit::is_drawing_info_gutter() const { return draw_info_gutter; }
...
int TextEdit::get_info_gutter_width() const { return info_gutter_width; }
...
bool TextEdit::is_hiding_enabled() const { return hiding_enabled; }
...
bool TextEdit::is_context_menu_enabled() { return context_menu_enabled; }
...
PopupMenu *TextEdit::get_menu() const { return menu; }

void TextEdit::_bind_methods() {

  ClassDB::bind_method(D_METHOD("_gui_input"), &TextEdit::_gui_input);
  ClassDB::bind_method(D_METHOD("_cursor_changed_emit"),
                       &TextEdit::_cursor_changed_emit);
  ClassDB::bind_method(D_METHOD("_text_changed_emit"),
...
*/
  ...
  ClassDB::bind_method(D_METHOD("insert_text_at_cursor", "text"),
  ...
  ClassDB::bind_method(D_METHOD("center_viewport_to_cursor"),
  ...
  ClassDB::bind_method(D_METHOD("cursor_set_line", "line", "adjust_viewport",
                                "can_be_hidden", "wrap_index"),
                       &TextEdit::cursor_set_line, DEFVAL(true), DEFVAL(true),
                       DEFVAL(0));

  ClassDB::bind_method(D_METHOD("cursor_get_column"),
                       &TextEdit::cursor_get_column);
  ClassDB::bind_method(D_METHOD("cursor_get_line"), &TextEdit::cursor_get_line);
  ClassDB::bind_method(D_METHOD("cursor_set_blink_enabled", "enable"),
                       &TextEdit::cursor_set_blink_enabled);
  ClassDB::bind_method(D_METHOD("cursor_get_blink_enabled"),
                       &TextEdit::cursor_get_blink_enabled);
  ClassDB::bind_method(D_METHOD("cursor_set_blink_speed", "blink_speed"),
                       &TextEdit::cursor_set_blink_speed);
  ClassDB::bind_method(D_METHOD("cursor_get_blink_speed"),
                       &TextEdit::cursor_get_blink_speed);
  ClassDB::bind_method(D_METHOD("cursor_set_block_mode", "enable"),
                       &TextEdit::cursor_set_block_mode);
  ClassDB::bind_method(D_METHOD("cursor_is_block_mode"),
                       &TextEdit::cursor_is_block_mode);

  ClassDB::bind_method(D_METHOD("set_right_click_moves_caret", "enable"),
                       &TextEdit::set_right_click_moves_caret);
  ClassDB::bind_method(D_METHOD("is_right_click_moving_caret"),
  ...
  ClassDB::bind_method(D_METHOD("set_wrap_enabled", "enable"),
                       &TextEdit::set_wrap_enabled);
  ClassDB::bind_method(D_METHOD("is_wrap_enabled"), &TextEdit::is_wrap_enabled);
  ClassDB::bind_method(D_METHOD("set_context_menu_enabled", "enable"),
                       &TextEdit::set_context_menu_enabled);
  ClassDB::bind_method(D_METHOD("is_context_menu_enabled"),
                       &TextEdit::is_context_menu_enabled);
  ClassDB::bind_method(D_METHOD("set_shortcut_keys_enabled", "enable"),
                       &TextEdit::set_shortcut_keys_enabled);
  ClassDB::bind_method(D_METHOD("is_shortcut_keys_enabled"),
                       &TextEdit::is_shortcut_keys_enabled);
  ClassDB::bind_method(D_METHOD("set_selecting_enabled", "enable"),
                       &TextEdit::set_selecting_enabled);
  ClassDB::bind_method(D_METHOD("is_selecting_enabled"),
  ...
  ClassDB::bind_method(D_METHOD("get_selection_from_line"),
                       &TextEdit::get_selection_from_line);
  ClassDB::bind_method(D_METHOD("get_selection_from_column"),
                       &TextEdit::get_selection_from_column);
  ClassDB::bind_method(D_METHOD("get_selection_to_line"),
                       &TextEdit::get_selection_to_line);
  ClassDB::bind_method(D_METHOD("get_selection_to_column"),
                       &TextEdit::get_selection_to_column);
  ClassDB::bind_method(D_METHOD("get_selection_text"),
                       &TextEdit::get_selection_text);
  ClassDB::bind_method(D_METHOD("get_word_under_cursor"),
  ...
  ClassDB::bind_method(D_METHOD("clear_undo_history"),
                       &TextEdit::clear_undo_history);

  ClassDB::bind_method(D_METHOD("set_show_line_numbers", "enable"),
                       &TextEdit::set_show_line_numbers);
  ClassDB::bind_method(D_METHOD("is_show_line_numbers_enabled"),
  ...
  ClassDB::bind_method(D_METHOD("is_drawing_spaces"),
                       &TextEdit::is_drawing_spaces);
  ClassDB::bind_method(D_METHOD("set_breakpoint_gutter_enabled", "enable"),
                       &TextEdit::set_breakpoint_gutter_enabled);
  ClassDB::bind_method(D_METHOD("is_breakpoint_gutter_enabled"),
                       &TextEdit::is_breakpoint_gutter_enabled);
  ClassDB::bind_method(D_METHOD("set_draw_fold_gutter"),
                       &TextEdit::set_draw_fold_gutter);
  ClassDB::bind_method(D_METHOD("is_drawing_fold_gutter"),
                       &TextEdit::is_drawing_fold_gutter);

  ClassDB::bind_method(D_METHOD("set_hiding_enabled", "enable"),
                       &TextEdit::set_hiding_enabled);
  ClassDB::bind_method(D_METHOD("is_hiding_enabled"),
                       &TextEdit::is_hiding_enabled);
  ClassDB::bind_method(D_METHOD("set_line_as_hidden", "line", "enable"),
                       &TextEdit::set_line_as_hidden);
  ClassDB::bind_method(D_METHOD("is_line_hidden", "line"),
                       &TextEdit::is_line_hidden);
  ClassDB::bind_method(D_METHOD("fold_all_lines"), &TextEdit::fold_all_lines);
  ClassDB::bind_method(D_METHOD("unhide_all_lines"),
  ...
  ClassDB::bind_method(D_METHOD("toggle_fold_line", "line"),
  ...
  ClassDB::bind_method(D_METHOD("set_highlight_all_occurrences", "enable"),
                       &TextEdit::set_highlight_all_occurrences);
  ClassDB::bind_method(D_METHOD("is_highlight_all_occurrences_enabled"),
                       &TextEdit::is_highlight_all_occurrences_enabled);

  ClassDB::bind_method(D_METHOD("set_override_selected_font_color", "override"),
  ...
  ClassDB::bind_method(D_METHOD("set_syntax_coloring", "enable"),
                       &TextEdit::set_syntax_coloring);
  ClassDB::bind_method(D_METHOD("is_syntax_coloring_enabled"),
                       &TextEdit::is_syntax_coloring_enabled);

  ClassDB::bind_method(D_METHOD("set_highlight_current_line", "enabled"),
                       &TextEdit::set_highlight_current_line);
  ClassDB::bind_method(D_METHOD("is_highlight_current_line_enabled"),
                       &TextEdit::is_highlight_current_line_enabled);

  ClassDB::bind_method(D_METHOD("set_smooth_scroll_enable", "enable"),
                       &TextEdit::set_smooth_scroll_enabled);
  ClassDB::bind_method(D_METHOD("is_smooth_scroll_enabled"),
                       &TextEdit::is_smooth_scroll_enabled);
  ClassDB::bind_method(D_METHOD("set_v_scroll_speed", "speed"),
                       &TextEdit::set_v_scroll_speed);
  ClassDB::bind_method(D_METHOD("get_v_scroll_speed"),
                       &TextEdit::get_v_scroll_speed);
  ClassDB::bind_method(D_METHOD("set_v_scroll", "value"),
                       &TextEdit::set_v_scroll);
  ClassDB::bind_method(D_METHOD("get_v_scroll"), &TextEdit::get_v_scroll);
  ClassDB::bind_method(D_METHOD("set_h_scroll", "value"),
                       &TextEdit::set_h_scroll);
  ClassDB::bind_method(D_METHOD("get_h_scroll"), &TextEdit::get_h_scroll);

  ClassDB::bind_method(D_METHOD("add_keyword_color", "keyword", "color"),
                       &TextEdit::add_keyword_color);
  ClassDB::bind_method(D_METHOD("has_keyword_color", "keyword"),
                       &TextEdit::has_keyword_color);
  ClassDB::bind_method(D_METHOD("get_keyword_color", "keyword"),
                       &TextEdit::get_keyword_color);
  ClassDB::bind_method(D_METHOD("add_color_region", "begin_key", "end_key",
                                "color", "line_only"),
                       &TextEdit::add_color_region, DEFVAL(false));
  ClassDB::bind_method(D_METHOD("clear_colors"), &TextEdit::clear_colors);
  ClassDB::bind_method(D_METHOD("menu_option", "option"),
                       &TextEdit::menu_option);
  ClassDB::bind_method(D_METHOD("get_menu"), &TextEdit::get_menu);

  ClassDB::bind_method(D_METHOD("get_breakpoints"),
                       &TextEdit::get_breakpoints_array);
  ClassDB::bind_method(D_METHOD("remove_breakpoints"),
                       &TextEdit::remove_breakpoints);

  ClassDB::bind_method(D_METHOD("draw_minimap", "draw"),
                       &TextEdit::set_draw_minimap);
  ClassDB::bind_method(D_METHOD("is_drawing_minimap"),
                       &TextEdit::is_drawing_minimap);
  ClassDB::bind_method(D_METHOD("set_minimap_width", "width"),
                       &TextEdit::set_minimap_width);
  ClassDB::bind_method(D_METHOD("get_minimap_width"),
                       &TextEdit::get_minimap_width);

  ADD_PROPERTY(
      PropertyInfo(Variant::STRING, "text", PROPERTY_HINT_MULTILINE_TEXT),
      "set_text", "get_text");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "readonly"), "set_readonly",
               "is_readonly");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "highlight_current_line"),
               "set_highlight_current_line",
               "is_highlight_current_line_enabled");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "syntax_highlighting"),
               "set_syntax_coloring", "is_syntax_coloring_enabled");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "show_line_numbers"),
               "set_show_line_numbers", "is_show_line_numbers_enabled");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "draw_tabs"), "set_draw_tabs",
               "is_drawing_tabs");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "draw_spaces"), "set_draw_spaces",
               "is_drawing_spaces");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "breakpoint_gutter"),
               "set_breakpoint_gutter_enabled", "is_breakpoint_gutter_enabled");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "fold_gutter"),
               "set_draw_fold_gutter", "is_drawing_fold_gutter");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "highlight_all_occurrences"),
               "set_highlight_all_occurrences",
               "is_highlight_all_occurrences_enabled");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "override_selected_font_color"),
               "set_override_selected_font_color",
               "is_overriding_selected_font_color");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "context_menu_enabled"),
               "set_context_menu_enabled", "is_context_menu_enabled");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "shortcut_keys_enabled"),
               "set_shortcut_keys_enabled", "is_shortcut_keys_enabled");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "selecting_enabled"),
               "set_selecting_enabled", "is_selecting_enabled");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "smooth_scrolling"),
               "set_smooth_scroll_enable", "is_smooth_scroll_enabled");
  ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "v_scroll_speed"),
               "set_v_scroll_speed", "get_v_scroll_speed");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "hiding_enabled"),
               "set_hiding_enabled", "is_hiding_enabled");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "wrap_enabled"), "set_wrap_enabled",
               "is_wrap_enabled");
  ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "scroll_vertical"), "set_v_scroll",
               "get_v_scroll");
  ADD_PROPERTY(PropertyInfo(Variant::INT, "scroll_horizontal"), "set_h_scroll",
               "get_h_scroll");

  ADD_GROUP("Minimap", "minimap_");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "minimap_draw"), "draw_minimap",
               "is_drawing_minimap");
  ADD_PROPERTY(PropertyInfo(Variant::INT, "minimap_width"), "set_minimap_width",
               "get_minimap_width");

  ADD_GROUP("Caret", "caret_");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "caret_block_mode"),
               "cursor_set_block_mode", "cursor_is_block_mode");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "caret_blink"),
               "cursor_set_blink_enabled", "cursor_get_blink_enabled");
  ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "caret_blink_speed",
                            PROPERTY_HINT_RANGE, "0.1,10,0.01"),
               "cursor_set_blink_speed", "cursor_get_blink_speed");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "caret_moving_by_right_click"),
               "set_right_click_moves_caret", "is_right_click_moving_caret");
  ...
  ADD_SIGNAL(MethodInfo(
      "symbol_lookup", PropertyInfo(Variant::STRING, "symbol"),
      PropertyInfo(Variant::INT, "row"), PropertyInfo(Variant::INT, "column")));
  ADD_SIGNAL(MethodInfo("info_clicked", PropertyInfo(Variant::INT, "row"),
                        PropertyInfo(Variant::STRING, "info")));
  ADD_SIGNAL(
      MethodInfo("symbol_validate", PropertyInfo(Variant::STRING, "symbol")));
  ...
  ProjectSettings::get_singleton()->set_custom_property_info(
      ...
      PropertyInfo(Variant::FLOAT, "gui/timers/text_edit_idle_detect_sec",
                   PROPERTY_HINT_RANGE,
...
TextEdit::TextEdit() {
  ...
  h_scroll->connect("value_changed",
                    callable_mp(this, &TextEdit::_scroll_moved));
  v_scroll->connect("value_changed",
                    callable_mp(this, &TextEdit::_scroll_moved));
  ...
  caret_blink_timer->connect("timeout",
                             callable_mp(this, &TextEdit::_toggle_draw_caret));
  ...
  idle_detect->set_wait_time(
      GLOBAL_GET("gui/timers/text_edit_idle_detect_sec"));
  idle_detect->connect("timeout",
                       callable_mp(this, &TextEdit::_push_current_op));

  click_select_held = memnew(Timer);
  add_child(click_select_held);
  click_select_held->set_wait_time(0.05);
  click_select_held->connect(
      "timeout", callable_mp(this, &TextEdit::_click_selection_held));
  ...
  add_child(menu);
  readonly = true; // Initialise to opposite first, so we get past the early-out
                   // in set_readonly.
  set_readonly(false);
...
Map<int, TextEdit::HighlighterInfo>
TextEdit::_get_line_syntax_highlighting(int p_line) {
  ...
  for (int j = 0; j < str.length(); j++) {
      ...
      if (in_region == -1) {
        if (!cri.end) {
          in_region = cri.region;
                 ...
                 !color_regions[cri.region].line_only) { // Ignore otherwise.
    ...
    if (!in_function_name && !in_member_variable && !in_keyword && !is_number &&
        in_word) {
      int k = j;
...
TextEdit *SyntaxHighlighter::get_text_editor() { return text_editor; }
