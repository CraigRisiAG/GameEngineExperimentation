...
void SpinBox::_notification(int p_what) {
  ...
  if (p_what == NOTIFICATION_DRAW) {
    ...
    updown->draw(ci, Point2i(size.width - updown->get_width(),
...
LineEdit::Align SpinBox::get_align() const { return line_edit->get_align(); }
...
void SpinBox::apply() { _text_entered(line_edit->get_text()); }
...
void SpinBox::_bind_methods() {
  ...
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "editable"), "set_editable",
               "is_editable");
  ADD_PROPERTY(PropertyInfo(Variant::STRING, "prefix"), "set_prefix",
               "get_prefix");
  ADD_PROPERTY(PropertyInfo(Variant::STRING, "suffix"), "set_suffix",
               "get_suffix");
}
...
SpinBox::SpinBox() {
  ...
  line_edit->connect("text_entered", callable_mp(this, &SpinBox::_text_entered),
                     Vector<Variant>(), CONNECT_DEFERRED);
  line_edit->connect("focus_exited",
                     callable_mp(this, &SpinBox::_line_edit_focus_exit),
                     Vector<Variant>(), CONNECT_DEFERRED);
  line_edit->connect("gui_input",
                     callable_mp(this, &SpinBox::_line_edit_input));
  drag.enabled = false;
