...
void BoxContainer::_resort() {
  ...
  int sep =
      get_constant("separation"); //,vertical?"VBoxContainer":"HBoxContainer");
  ...
  /** Second, pass sucessively to discard elements that can't be stretched, this
     will run while stretchable elements exist */

  bool has_stretched = false;
  while (
      stretch_ratio_total >
      0) { // first of all, don't even be here if no stretchable objects exist
    ...
    for (int i = 0; i < get_child_count(); i++) {
      ...
      if (msc.will_stretch) { // wants to stretch
        ...
        int final_pixel_size =
            stretch_avail * c->get_stretch_ratio() / stretch_ratio_total;
...
BoxContainer::AlignMode BoxContainer::get_alignment() const { return align; }
...
void BoxContainer::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("add_spacer", "begin"),
...
MarginContainer *VBoxContainer::add_margin_child(const String &p_label,
                                                 Control *p_control,
                                                 bool p_expand) {
