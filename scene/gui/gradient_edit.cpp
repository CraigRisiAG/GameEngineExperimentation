...
void GradientEdit::_show_color_picker() {
  ...
  if (get_global_position().y + get_size().y + minsize.y >
...
void GradientEdit::_gui_input(const Ref<InputEvent> &p_event) {
  ...
  if (k.is_valid() && k->is_pressed() && k->get_keycode() == KEY_DELETE &&
      grabbed != -1) {
  ...
  if (mb.is_valid() && mb->get_button_index() == 1 && mb->is_doubleclick() &&
      mb->is_pressed()) {
  ...
  if (mb.is_valid() && mb->get_button_index() == 1 && mb->is_pressed()) {
    ...
    newPoint.color = prev.color.linear_interpolate(
        next.color,
        (newPoint.offset - prev.offset) / (next.offset - prev.offset));
...
void GradientEdit::_notification(int p_what) {
  ...
  if (p_what == NOTIFICATION_ENTER_TREE) {
    if (!picker->is_connected(
            "color_changed",
            callable_mp(this, &GradientEdit::_color_changed))) {
  ...
  if (p_what == NOTIFICATION_DRAW) {
    ...
    if (w == 0 || h == 0)
      return; // Safety check. We have division by 'h'. And in any case there is
              // nothing to draw with such size
    ...
    if (points.size() == 0)
      ...
      prev.color =
          points[0].color; // Extend color of first point to the beginning.
    ...
    for (int i = -1; i < points.size(); i++) {
      ...
      if (i + 1 == points.size()) {
        if (points.size() == 0)
          next.color =
              Color(0, 0, 0); // Draw black rectangle if we have no points
        else
          next.color = points[i].color; // Extend color of last point to the
                                        // end.
        next.offset = 1;
      } else {
        next = points[i + 1];
    ...
    for (int i = 0; i < points.size(); i++) {
      ...
      draw_line(Vector2(points[i].offset * total_w, 0),
                Vector2(points[i].offset * total_w, h / 2), col);
      Rect2 rect = Rect2(points[i].offset * total_w - POINT_WIDTH / 2, h / 2,
                         POINT_WIDTH, h / 2);
    ...
    if (grabbed != -1) {
      ...
      draw_line(Vector2(total_w + SPACING, 0),
                Vector2(total_w + SPACING + h, h), Color(1, 1, 1, 0.6));
      draw_line(Vector2(total_w + SPACING, h),
                Vector2(total_w + SPACING + h, 0), Color(1, 1, 1, 0.6));
    }
    ...
    if (has_focus()) {
      ...
      draw_line(Vector2(total_w + 1, -1), Vector2(total_w + 1, h + 1),
                Color(1, 1, 1, 0.6));
      draw_line(Vector2(total_w + 1, h + 1), Vector2(-1, h + 1),
                Color(1, 1, 1, 0.6));
...
void GradientEdit::_draw_checker(int x, int y, int w, int h) {
  ...
  uvPoints.push_back(Vector2(x + w * .5f / checker->get_width(),
                             y + h * .5f / checker->get_height()));
  uvPoints.push_back(Vector2(x + w * .5f / checker->get_width(), y));
  draw_polygon(backPoints, colorPoints, uvPoints, checker);
}

Size2 GradientEdit::get_minimum_size() const { return Vector2(0, 16); }
...
void GradientEdit::set_ramp(const Vector<float> &p_offsets,
...
Vector<Gradient::Point> &GradientEdit::get_points() { return points; }
