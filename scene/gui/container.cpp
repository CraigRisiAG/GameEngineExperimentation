...
void Container::add_child_notify(Node *p_child) {
  ...
  control->connect("size_flags_changed",
                   callable_mp(this, &Container::queue_sort));
  control->connect("minimum_size_changed",
                   callable_mp(this, &Container::_child_minsize_changed));
  control->connect("visibility_changed",
                   callable_mp(this, &Container::_child_minsize_changed));
...
void Container::remove_child_notify(Node *p_child) {
  ...
  control->disconnect("size_flags_changed",
                      callable_mp(this, &Container::queue_sort));
  control->disconnect("minimum_size_changed",
                      callable_mp(this, &Container::_child_minsize_changed));
  control->disconnect("visibility_changed",
                      callable_mp(this, &Container::_child_minsize_changed));
...
String Container::get_configuration_warning() const {
  ...
  if (get_class() == "Container" && get_script().is_null()) {
    ...
    warning += TTR("Container by itself serves no purpose unless a script "
...
void Container::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("fit_child_in_rect", "child", "rect"),
