  ...
  if (b.is_valid()) {
    ...
    if (b->is_pressed()) {
      ...
      if (b->get_shift()) {
        ...
        if ((cursor_pos < selection.begin) || (cursor_pos > selection.end) ||
            !selection.enabled) {
...
LineEdit::Align LineEdit::get_align() const { return align; }
...
bool LineEdit::can_drop_data(const Point2 &p_point,
...
void LineEdit::_notification(int p_what) {
  ...
  case NOTIFICATION_ENTER_TREE: {
    if (Engine::get_singleton()->is_editor_hint() &&
      ...
      if (!EditorSettings::get_singleton()->is_connected(
              "settings_changed",
              callable_mp(this, &LineEdit::_editor_settings_changed))) {
  ...
  case NOTIFICATION_DRAW: {
    ...
    case ALIGN_CENTER: {
      ...
      if (window_pos != 0)
        ...
        x_ofs = MAX(style->get_margin(MARGIN_LEFT),
    ...
    Color font_color = is_editable() ? get_color("font_color")
    ...
    if (right_icon.is_valid() || display_clear_icon) {
      ...
      r_icon->draw(
          ci,
          Point2(width - r_icon->get_width() - style->get_margin(MARGIN_RIGHT),
      ...
      if (align == ALIGN_CENTER) {
        if (window_pos == 0) {
          x_ofs = MAX(style->get_margin(MARGIN_LEFT),
                      int(size.width - cached_text_width - r_icon->get_width() -
        ...
        x_ofs =
            MAX(style->get_margin(MARGIN_LEFT),
                x_ofs - r_icon->get_width() - style->get_margin(MARGIN_RIGHT));
      }
    ...
    while (true) {
      ...
      if (char_ofs == cursor_pos) {
        if (ime_text.length() > 0) {
          ...
          while (true) {
            ...
            CharType cchar =
                (pass && !text.empty()) ? secret_character[0] : ime_text[ofs];
            ...
            if (selected) {
              VisualServer::get_singleton()->canvas_item_add_rect(
                  ci,
                  Rect2(Point2(x_ofs, y_ofs + caret_height),
            ...
            drawer.draw_char(ci, Point2(x_ofs, y_ofs + font_ascent), cchar,
                             next, font_color);
      ...
      CharType next =
          (pass && !text.empty()) ? secret_character[0] : t[char_ofs + 1];
      ...
      if (selected)
        VisualServer::get_singleton()->canvas_item_add_rect(
            ci, Rect2(Point2(x_ofs, y_ofs), Size2(char_width, caret_height)),
            selection_color);

      int yofs = y_ofs + (caret_height - font->get_height()) / 2;
      drawer.draw_char(ci, Point2(x_ofs, yofs + font_ascent), cchar, next,
      ...
      if (char_ofs == cursor_pos && draw_caret && !using_placeholder) {
        if (ime_text.length() == 0) {
          ...
          VisualServer::get_singleton()->canvas_item_add_rect(
              ci,
              Rect2(Point2(x_ofs, y_ofs),
                    Size2(Math::round(EDSCALE), caret_height)),
              cursor_color);
#else
          VisualServer::get_singleton()->canvas_item_add_rect(
              ci, Rect2(Point2(x_ofs, y_ofs), Size2(1, caret_height)),
              cursor_color);
#endif
        }
      }
      ...
      if (ime_text.length() > 0) {
        ...
        while (true) {
          ...
          CharType cchar =
              (pass && !text.empty()) ? secret_character[0] : ime_text[ofs];
          CharType next =
              (pass && !text.empty()) ? secret_character[0] : ime_text[ofs + 1];
          int im_char_width = font->get_char_size(cchar, next).width;
          ...
          bool selected =
              ofs >= ime_selection.x && ofs < ime_selection.x + ime_selection.y;
          if (selected) {
            VisualServer::get_singleton()->canvas_item_add_rect(
                ci,
                Rect2(Point2(x_ofs, y_ofs + caret_height),
          ...
          drawer.draw_char(ci, Point2(x_ofs, y_ofs + font_ascent), cchar, next,
                           font_color);
    ...
    if ((char_ofs == cursor_pos || using_placeholder) &&
      ...
      if (ime_text.length() == 0) {
        ...
        VisualServer::get_singleton()->canvas_item_add_rect(
            ci,
            Rect2(Point2(caret_x_ofs, y_ofs),
                  Size2(Math::round(EDSCALE), caret_height)),
            cursor_color);
#else
        VisualServer::get_singleton()->canvas_item_add_rect(
            ci, Rect2(Point2(caret_x_ofs, y_ofs), Size2(1, caret_height)),
            cursor_color);
#endif
      }
    }
    ...
    if (has_focus()) {
      ...
      OS::get_singleton()->set_ime_position(
          get_global_position() +
          Point2(using_placeholder ? 0 : x_ofs, y_ofs + caret_height));
    }
  } break;
  case NOTIFICATION_FOCUS_ENTER: {
    ...
    OS::get_singleton()->set_ime_active(true);
    Point2 cursor_pos =
        Point2(get_cursor_position(), 1) * get_minimum_size().height;
...
void LineEdit::set_cursor_at_pixel_pos(int p_x) {
  ...
  case ALIGN_RIGHT: {
    ...
    pixel_ofs =
        int(size.width - style->get_margin(MARGIN_RIGHT) - (cached_width));
  ...
  while (ofs < text.length()) {
    ...
    if (font != NULL) {
      char_w =
          font->get_char_size(pass ? secret_character[0] : text[ofs]).width;
    }
    pixel_ofs += char_w;
...
void LineEdit::delete_char() {
  ...
  if (font != NULL) {
    cached_width -=
        font->get_char_size(pass ? secret_character[0] : text[cursor_pos - 1])
            .width;
  }
...
void LineEdit::delete_text(int p_from_column, int p_to_column) {
  ...
  if (align == ALIGN_CENTER || align == ALIGN_RIGHT) {
    window_pos = CLAMP(window_pos - (p_to_column - p_from_column), 0,
                       MAX(text.length() - 1, 0));
  }
...
float LineEdit::get_placeholder_alpha() const { return placeholder_alpha; }
  ...
  if (cursor_pos <= window_pos) {
    ...
    if (font.is_valid()) {
      ...
      for (int i = cursor_pos; i >= window_pos; i--) {
        ...
        if (i >= text.length()) {
          // Do not do this, because if the cursor is at the end, its just fine
          // that it takes no space. accum_width = font->get_char_size('
          // ').width;
        } else {
          if (pass) {
            accum_width +=
                font->get_char_size(secret_character[0],
                                    i + 1 < text.length() ? secret_character[0]
                                                          : 0)
                    .width;
          } else {
            accum_width +=
                font->get_char_size(text[i],
...
int LineEdit::get_cursor_position() const { return cursor_pos; }
...
Size2 LineEdit::get_minimum_size() const {
  ...
  if (expand_to_text_length) {
    // Add a space because some fonts are too exact, and because cursor needs a
    // bit more when at the end.
    min_size.width =
...
void LineEdit::set_secret_character(const String &p_string) {
  ...
  // character.
  ERR_FAIL_COND_MSG(p_string.length() != 1,
...
PopupMenu *LineEdit::get_menu() const { return menu; }
...
void LineEdit::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("select", "from", "to"), &LineEdit::select,
                       DEFVAL(0), DEFVAL(-1));
  ...
  ClassDB::bind_method(D_METHOD("set_placeholder", "text"),
                       &LineEdit::set_placeholder);
  ClassDB::bind_method(D_METHOD("get_placeholder"), &LineEdit::get_placeholder);
  ClassDB::bind_method(D_METHOD("set_placeholder_alpha", "alpha"),
                       &LineEdit::set_placeholder_alpha);
  ClassDB::bind_method(D_METHOD("get_placeholder_alpha"),
                       &LineEdit::get_placeholder_alpha);
  ClassDB::bind_method(D_METHOD("set_cursor_position", "position"),
                       &LineEdit::set_cursor_position);
  ClassDB::bind_method(D_METHOD("get_cursor_position"),
                       &LineEdit::get_cursor_position);
  ClassDB::bind_method(D_METHOD("set_expand_to_text_length", "enabled"),
                       &LineEdit::set_expand_to_text_length);
  ClassDB::bind_method(D_METHOD("get_expand_to_text_length"),
                       &LineEdit::get_expand_to_text_length);
  ClassDB::bind_method(D_METHOD("cursor_set_blink_enabled", "enabled"),
  ...
  ClassDB::bind_method(D_METHOD("set_max_length", "chars"),
                       &LineEdit::set_max_length);
  ClassDB::bind_method(D_METHOD("get_max_length"), &LineEdit::get_max_length);
  ClassDB::bind_method(D_METHOD("append_at_cursor", "text"),
                       &LineEdit::append_at_cursor);
  ClassDB::bind_method(D_METHOD("set_editable", "enabled"),
                       &LineEdit::set_editable);
  ClassDB::bind_method(D_METHOD("is_editable"), &LineEdit::is_editable);
  ClassDB::bind_method(D_METHOD("set_secret", "enabled"),
                       &LineEdit::set_secret);
  ClassDB::bind_method(D_METHOD("is_secret"), &LineEdit::is_secret);
  ClassDB::bind_method(D_METHOD("set_secret_character", "character"),
                       &LineEdit::set_secret_character);
  ClassDB::bind_method(D_METHOD("get_secret_character"),
  ...
  ClassDB::bind_method(D_METHOD("set_clear_button_enabled", "enable"),
                       &LineEdit::set_clear_button_enabled);
  ClassDB::bind_method(D_METHOD("is_clear_button_enabled"),
  ...
  ClassDB::bind_method(D_METHOD("set_right_icon", "icon"),
  ...
  ADD_PROPERTY(PropertyInfo(Variant::INT, "align", PROPERTY_HINT_ENUM,
                            "Left,Center,Right,Fill"),
               "set_align", "get_align");
  ADD_PROPERTY(PropertyInfo(Variant::INT, "max_length"), "set_max_length",
               "get_max_length");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "editable"), "set_editable",
               "is_editable");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "secret"), "set_secret",
               "is_secret");
  ADD_PROPERTY(PropertyInfo(Variant::STRING, "secret_character"),
               "set_secret_character", "get_secret_character");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "expand_to_text_length"),
               "set_expand_to_text_length", "get_expand_to_text_length");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "context_menu_enabled"),
               "set_context_menu_enabled", "is_context_menu_enabled");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "clear_button_enabled"),
               "set_clear_button_enabled", "is_clear_button_enabled");
  ...
  ADD_PROPERTY(PropertyInfo(Variant::OBJECT, "right_icon",
                            PROPERTY_HINT_RESOURCE_TYPE, "Texture"),
               "set_right_icon", "get_right_icon");
  ADD_GROUP("Placeholder", "placeholder_");
  ADD_PROPERTY(PropertyInfo(Variant::STRING, "placeholder_text"),
               "set_placeholder", "get_placeholder");
  ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "placeholder_alpha",
                            PROPERTY_HINT_RANGE, "0,1,0.001"),
               "set_placeholder_alpha", "get_placeholder_alpha");
  ...
  ADD_PROPERTY(PropertyInfo(Variant::INT, "caret_position"),
               "set_cursor_position", "get_cursor_position");
}
