...
void SplitContainer::_resort() {
  ...
  if (!collapsed) {
    int clamped_split_offset = CLAMP(
        split_offset, ms_first[axis] - no_offset_middle_sep,
...
void SplitContainer::_notification(int p_what) {
  ...
  case NOTIFICATION_DRAW: {
    ...
    if (vertical)
      draw_texture(tex, Point2i((size.x - tex->get_width()) / 2,
                                middle_sep + (sep - tex->get_height()) / 2));
    else
      draw_texture(tex, Point2i(middle_sep + (sep - tex->get_width()) / 2,
...
void SplitContainer::_gui_input(const Ref<InputEvent> &p_event) {
  ...
  if (mm.is_valid()) {
    ...
    if (vertical)
      mouse_inside_state =
          mm->get_position().y > middle_sep &&
          mm->get_position().y < middle_sep + get_constant("separation");
    else
      mouse_inside_state =
          mm->get_position().x > middle_sep &&
          mm->get_position().x < middle_sep + get_constant("separation");
    ...
    split_offset =
        drag_ofs +
        ((vertical ? mm->get_position().y : mm->get_position().x) - drag_from);
...
Control::CursorShape
SplitContainer::get_cursor_shape(const Point2 &p_pos) const {
...
int SplitContainer::get_split_offset() const { return split_offset; }
...
void SplitContainer::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("set_split_offset", "offset"),
                       &SplitContainer::set_split_offset);
  ClassDB::bind_method(D_METHOD("get_split_offset"),
                       &SplitContainer::get_split_offset);
  ClassDB::bind_method(D_METHOD("clamp_split_offset"),
                       &SplitContainer::clamp_split_offset);

  ClassDB::bind_method(D_METHOD("set_collapsed", "collapsed"),
                       &SplitContainer::set_collapsed);
  ClassDB::bind_method(D_METHOD("is_collapsed"), &SplitContainer::is_collapsed);

  ClassDB::bind_method(D_METHOD("set_dragger_visibility", "mode"),
                       &SplitContainer::set_dragger_visibility);
  ClassDB::bind_method(D_METHOD("get_dragger_visibility"),
                       &SplitContainer::get_dragger_visibility);

  ADD_SIGNAL(MethodInfo("dragged", PropertyInfo(Variant::INT, "offset")));

  ADD_PROPERTY(PropertyInfo(Variant::INT, "split_offset"), "set_split_offset",
               "get_split_offset");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "collapsed"), "set_collapsed",
               "is_collapsed");
  ADD_PROPERTY(PropertyInfo(Variant::INT, "dragger_visibility",
                            PROPERTY_HINT_ENUM,
                            "Visible,Hidden,Hidden & Collapsed"),
               "set_dragger_visibility", "get_dragger_visibility");
