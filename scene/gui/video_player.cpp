...
int VideoPlayer::_audio_mix_callback(void *p_udata, const float *p_data,
                                     int p_frames) {
...
void VideoPlayer::_mix_audio() {
  ...
  if (cc == 1) {
    AudioFrame *target =
        AudioServer::get_singleton()->thread_get_channel_mix_buffer(bus_index,
                                                                    0);
    ...
    for (int k = 0; k < cc; k++) {
      targets[k] = AudioServer::get_singleton()->thread_get_channel_mix_buffer(
          bus_index, k);
...
void VideoPlayer::_notification(int p_notification) {
  ...
  case NOTIFICATION_INTERNAL_PROCESS: {
    ...
    if (stream.is_null() || paused || playback.is_null() ||
        !playback->is_playing())
      return;
    ...
    playback->update(
        delta); // playback->is_playing() returns false in the last video frame
...
bool VideoPlayer::has_expand() const { return expand; }
...
Ref<VideoStream> VideoPlayer::get_stream() const { return stream; };
...
bool VideoPlayer::is_paused() const { return paused; }

void VideoPlayer::set_buffering_msec(int p_msec) { buffering_ms = p_msec; }

int VideoPlayer::get_buffering_msec() const { return buffering_ms; }

void VideoPlayer::set_audio_track(int p_track) { audio_track = p_track; }

int VideoPlayer::get_audio_track() const { return audio_track; }

void VideoPlayer::set_volume(float p_vol) { volume = p_vol; };

float VideoPlayer::get_volume() const { return volume; };
...
void VideoPlayer::set_autoplay(bool p_enable) { autoplay = p_enable; };

bool VideoPlayer::has_autoplay() const { return autoplay; };
...
void VideoPlayer::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("set_stream", "stream"),
  ...
  ClassDB::bind_method(D_METHOD("set_paused", "paused"),
  ...
  ClassDB::bind_method(D_METHOD("set_volume_db", "db"),
                       &VideoPlayer::set_volume_db);
  ClassDB::bind_method(D_METHOD("get_volume_db"), &VideoPlayer::get_volume_db);

  ClassDB::bind_method(D_METHOD("set_audio_track", "track"),
                       &VideoPlayer::set_audio_track);
  ClassDB::bind_method(D_METHOD("get_audio_track"),
  ...
  ClassDB::bind_method(D_METHOD("get_video_texture"),
                       &VideoPlayer::get_video_texture);

  ADD_SIGNAL(MethodInfo("finished"));

  ADD_PROPERTY(
      PropertyInfo(Variant::INT, "audio_track", PROPERTY_HINT_RANGE, "0,128,1"),
      "set_audio_track", "get_audio_track");
  ADD_PROPERTY(PropertyInfo(Variant::OBJECT, "stream",
                            PROPERTY_HINT_RESOURCE_TYPE, "VideoStream"),
               "set_stream", "get_stream");
  // ADD_PROPERTY( PropertyInfo(Variant::BOOL, "stream/loop"), "set_loop",
  // "has_loop") ;
  ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "volume_db", PROPERTY_HINT_RANGE,
                            "-80,24,0.01"),
               "set_volume_db", "get_volume_db");
  ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "volume", PROPERTY_HINT_EXP_RANGE,
                            "0,15,0.01", 0),
               "set_volume", "get_volume");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "autoplay"), "set_autoplay",
               "has_autoplay");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "paused"), "set_paused",
               "is_paused");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "expand"), "set_expand",
               "has_expand");
  ADD_PROPERTY(PropertyInfo(Variant::INT, "buffering_msec", PROPERTY_HINT_RANGE,
                            "10,1000"),
               "set_buffering_msec", "get_buffering_msec");
  ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "stream_position",
                            PROPERTY_HINT_RANGE, "0,1280000,0.1", 0),
               "set_stream_position", "get_stream_position");

  ADD_PROPERTY(
      PropertyInfo(Variant::STRING_NAME, "bus", PROPERTY_HINT_ENUM, ""),
      "set_bus", "get_bus");
}
...
VideoPlayer::~VideoPlayer() {
  ...
  resampler.clear(); // Not necessary here, but make in consistent with other
                     // "stream_player" classes
};
