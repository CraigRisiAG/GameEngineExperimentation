...
void WindowDialog::_fix_size() {
  ...
  // Perhaps this should be called when the viewport resizes as well or windows
  // go out of bounds...
...
void WindowDialog::_gui_input(const Ref<InputEvent> &p_event) {
  ...
  if (mb.is_valid() && mb->get_button_index() == BUTTON_LEFT) {
    ...
    if (mb->is_pressed()) {
      ...
      drag_type =
          _drag_hit_test(Point2(mb->get_position().x, mb->get_position().y));
  ...
  if (mm.is_valid()) {
    ...
    if (drag_type == DRAG_NONE) {
      ...
      if (resizable) {
        int preview_drag_type =
            _drag_hit_test(Point2(mm->get_position().x, mm->get_position().y));
...
void WindowDialog::_notification(int p_what) {
  ...
  case NOTIFICATION_DRAW: {
    ...
    title_font->draw(canvas, Point2(x, y), xl_title, title_color,
  ...
  case NOTIFICATION_ENTER_TREE: {
    ...
    close_button->set_begin(
        Point2(-get_constant("close_h_ofs", "WindowDialog"),
...
String WindowDialog::get_title() const { return title; }

void WindowDialog::set_resizable(bool p_resizable) { resizable = p_resizable; }
bool WindowDialog::get_resizable() const { return resizable; }
...
TextureButton *WindowDialog::get_close_button() { return close_button; }
...
void WindowDialog::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("get_close_button"),
                       &WindowDialog::get_close_button);

  ADD_PROPERTY(PropertyInfo(Variant::STRING, "window_title", PROPERTY_HINT_NONE,
                            "", PROPERTY_USAGE_DEFAULT_INTL),
               "set_title", "get_title");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "resizable", PROPERTY_HINT_NONE, "",
                            PROPERTY_USAGE_DEFAULT_INTL),
...
void AcceptDialog::_text_entered(const String &p_text) { _ok_pressed(); }
...
void AcceptDialog::_close_pressed() { cancel_pressed(); }
...
String AcceptDialog::get_text() const { return label->get_text(); }
...
bool AcceptDialog::has_autowrap() { return label->has_autowrap(); }
...
void AcceptDialog::register_text_enter(Node *p_line_edit) {
  ...
  if (line_edit)
    line_edit->connect("text_entered",
                       callable_mp(this, &AcceptDialog::_text_entered));
}
...
Button *AcceptDialog::add_button(const String &p_text, bool p_right,
  ...
  if (p_action != "") {
    button->connect("pressed", callable_mp(this, &AcceptDialog::_custom_action),
                    varray(p_action));
  }
...
void AcceptDialog::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("set_hide_on_ok", "enabled"),
                       &AcceptDialog::set_hide_on_ok);
  ClassDB::bind_method(D_METHOD("get_hide_on_ok"),
                       &AcceptDialog::get_hide_on_ok);
  ClassDB::bind_method(D_METHOD("add_button", "text", "right", "action"),
                       &AcceptDialog::add_button, DEFVAL(false), DEFVAL(""));
  ClassDB::bind_method(D_METHOD("add_cancel", "name"),
                       &AcceptDialog::add_cancel);
  ClassDB::bind_method(D_METHOD("register_text_enter", "line_edit"),
                       &AcceptDialog::register_text_enter);
  ClassDB::bind_method(D_METHOD("set_text", "text"), &AcceptDialog::set_text);
  ClassDB::bind_method(D_METHOD("get_text"), &AcceptDialog::get_text);
  ClassDB::bind_method(D_METHOD("set_autowrap", "autowrap"),
                       &AcceptDialog::set_autowrap);
  ClassDB::bind_method(D_METHOD("has_autowrap"), &AcceptDialog::has_autowrap);

  ADD_SIGNAL(MethodInfo("confirmed"));
  ADD_SIGNAL(MethodInfo("custom_action",
                        PropertyInfo(Variant::STRING_NAME, "action")));

  ADD_GROUP("Dialog", "dialog");
  ADD_PROPERTY(PropertyInfo(Variant::STRING, "dialog_text",
                            PROPERTY_HINT_MULTILINE_TEXT, "",
                            PROPERTY_USAGE_DEFAULT_INTL),
               "set_text", "get_text");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "dialog_hide_on_ok"),
               "set_hide_on_ok", "get_hide_on_ok");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "dialog_autowrap"), "set_autowrap",
               "has_autowrap");
}

bool AcceptDialog::swap_ok_cancel = false;
void AcceptDialog::set_swap_ok_cancel(bool p_swap) { swap_ok_cancel = p_swap; }
...
Button *ConfirmationDialog::get_cancel() { return cancel; }
