...
bool Label::has_autowrap() const { return autowrap; }
...
int Label::get_line_height() const { return get_font("font")->get_height(); }
...
void Label::_notification(int p_what) {
  ...
  if (p_what == NOTIFICATION_DRAW) {
    ...
    VisualServer::get_singleton()->canvas_item_set_distance_field_mode(
        get_canvas_item(), font.is_valid() && font->is_distance_field_hint());
    ...
    if (lines_visible > 0) {
      ...
      case VALIGN_FILL: {
        vbegin = 0;
        if (lines_visible > 1) {
          vsep = (size.y - (lines_visible * font_h - line_spacing)) /
                 (lines_visible - 1);
        } else {
          vsep = 0;
        }
    ...
    while (wc) {
      ...
      case ALIGN_RIGHT: {
        ...
        x_ofs = int(size.width - style->get_margin(MARGIN_RIGHT) -
      ...
      while (from != to) {
        ...
        if (font_color_shadow.a > 0) {
          ...
          for (int i = 0; i < from->word_len; i++) {
            ...
            if (visible_chars < 0 || chars_total_shadow < visible_chars) {
              ...
              float move =
                  drawer.draw_char(ci, Point2(x_ofs_shadow, y_ofs) + shadow_ofs,
                                   c, n, font_color_shadow);
              if (use_outline) {
                drawer.draw_char(ci,
        ...
        for (int i = 0; i < from->word_len; i++) {
          ...
          if (visible_chars < 0 || chars_total < visible_chars) {
            ...
            x_ofs +=
                drawer.draw_char(ci, Point2(x_ofs, y_ofs), c, n, font_color);
            chars_total++;
          }
        }
        from = from->next;
...
void Label::regenerate_word_cache() {
  ...
  for (int i = 0; i <= xl_text.length(); i++) {
    ...
    bool separatable = (current >= 0x2E08 && current <= 0xFAFF) ||
                       (current >= 0xFE30 && current <= 0xFE4F);
    // current>=33 && (current < 65||current >90) && (current<97||current>122)
    ...
    if ((autowrap && (line_width >= width) &&
      ...
      wc->pixel_width = 0;
      wc->char_pos =
          insert_newline ? WordCache::CHAR_NEWLINE : WordCache::CHAR_WRAPLINE;
  ...
  if (max_lines_visible > 0 && line_count > max_lines_visible) {
    ...
    minsize.height =
        (font->get_height() * line_count) + (line_spacing * (line_count - 1));
  }
...
Label::Align Label::get_align() const { return align; }
...
Label::VAlign Label::get_valign() const { return valign; }
...
float Label::get_percent_visible() const { return percent_visible; }
...
void Label::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("set_autowrap", "enable"),
                       &Label::set_autowrap);
  ClassDB::bind_method(D_METHOD("has_autowrap"), &Label::has_autowrap);
  ClassDB::bind_method(D_METHOD("set_clip_text", "enable"),
                       &Label::set_clip_text);
  ClassDB::bind_method(D_METHOD("is_clipping_text"), &Label::is_clipping_text);
  ClassDB::bind_method(D_METHOD("set_uppercase", "enable"),
                       &Label::set_uppercase);
  ...
  ClassDB::bind_method(D_METHOD("set_lines_skipped", "lines_skipped"),
                       &Label::set_lines_skipped);
  ClassDB::bind_method(D_METHOD("get_lines_skipped"),
                       &Label::get_lines_skipped);
  ClassDB::bind_method(D_METHOD("set_max_lines_visible", "lines_visible"),
                       &Label::set_max_lines_visible);
  ClassDB::bind_method(D_METHOD("get_max_lines_visible"),
  ...
  ADD_PROPERTY(PropertyInfo(Variant::STRING, "text",
  ...
  ADD_PROPERTY(PropertyInfo(Variant::INT, "align", PROPERTY_HINT_ENUM,
               ...
               "set_align", "get_align");
  ADD_PROPERTY(PropertyInfo(Variant::INT, "valign", PROPERTY_HINT_ENUM,
                            "Top,Center,Bottom,Fill"),
               "set_valign", "get_valign");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "autowrap"), "set_autowrap",
               "has_autowrap");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "clip_text"), "set_clip_text",
               "is_clipping_text");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "uppercase"), "set_uppercase",
               "is_uppercase");
  ...
  ADD_PROPERTY(PropertyInfo(Variant::INT, "lines_skipped", PROPERTY_HINT_RANGE,
                            "0,999,1"),
               "set_lines_skipped", "get_lines_skipped");
  ADD_PROPERTY(PropertyInfo(Variant::INT, "max_lines_visible",
                            PROPERTY_HINT_RANGE, "-1,999,1"),
               "set_max_lines_visible", "get_max_lines_visible");
}
