...
void GridContainer::_notification(int p_what) {
  ...
  case NOTIFICATION_SORT_CHILDREN: {
    ...
    Map<int, int> col_minw; // Max of min_width  of all controls in each col
                            // (indexed by col).
    Map<int, int> row_minh; // Max of min_height of all controls in each row
                            // (indexed by row).
    ...
    while (!can_fit && col_expanded.size() > 0) {
      ...
      for (Set<int>::Element *E = col_expanded.front(); E; E = E->next()) {
        ...
        if (can_fit && (remaining_space.width / col_expanded.size()) <
                           col_minw[E->get()]) {
          can_fit = false;
        }
      }
    ...
    while (!can_fit && row_expanded.size() > 0) {
      // Check if all minheight constraints are OK if we use the remaining
      // space.
      can_fit = true;
      ...
      for (Set<int>::Element *E = row_expanded.front(); E; E = E->next()) {
        ...
        if (can_fit && (remaining_space.height / row_expanded.size()) <
                           row_minh[E->get()]) {
          can_fit = false;
        }
      }
    ...
    int col_expand = col_expanded.size() > 0
                         ? remaining_space.width / col_expanded.size()
                         : 0;
    int row_expand = row_expanded.size() > 0
                         ? remaining_space.height / row_expanded.size()
                         : 0;
    ...
    for (int i = 0; i < get_child_count(); i++) {
      ...
      if (col == 0) {
        ...
        if (row > 0)
          row_ofs +=
              (row_expanded.has(row - 1) ? row_expand : row_minh[row - 1]) +
              vsep;
      }

      Point2 p(col_ofs, row_ofs);
      Size2 s(col_expanded.has(col) ? col_expand : col_minw[col],
              row_expanded.has(row) ? row_expand : row_minh[row]);
...
void GridContainer::_bind_methods() {
  ...
  ADD_PROPERTY(
      PropertyInfo(Variant::INT, "columns", PROPERTY_HINT_RANGE, "1,1024,1"),
...
GridContainer::GridContainer() { columns = 1; }
