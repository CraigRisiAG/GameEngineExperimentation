...
void Popup::_fix_size() {
  ...
  Point2 window_size =
      get_viewport_rect().size - get_viewport_transform().get_origin();
...
void Popup::set_as_minsize() {
  ...
  for (int i = 0; i < get_child_count(); i++) {
    ...
    for (int j = 0; j < 2; j++) {
      ...
      minsize[j] +=
          margin_begin * (ANCHOR_END - anchor_begin) + margin_end * anchor_end;
    }
...
void Popup::popup_centered_clamped(const Size2 &p_size,
...
void Popup::popup(const Rect2 &p_bounds) { _popup(p_bounds); }
  ...
  if (!p_bounds.has_no_area()) {
    ...
    if (p_centered && p_bounds.size != get_size()) {
      set_position(p_bounds.position -
                   ((get_size() - p_bounds.size) / 2.0).floor());
    } else {
      set_position(p_bounds.position);
    }
  }
...
void Popup::set_exclusive(bool p_exclusive) { exclusive = p_exclusive; }

bool Popup::is_exclusive() const { return exclusive; }

void Popup::_bind_methods() {

  ClassDB::bind_method(D_METHOD("set_as_minsize"), &Popup::set_as_minsize);
  ClassDB::bind_method(D_METHOD("popup_centered", "size"),
                       &Popup::popup_centered, DEFVAL(Size2()));
  ClassDB::bind_method(D_METHOD("popup_centered_ratio", "ratio"),
                       &Popup::popup_centered_ratio, DEFVAL(0.75));
  ClassDB::bind_method(D_METHOD("popup_centered_minsize", "minsize"),
                       &Popup::popup_centered_minsize, DEFVAL(Size2()));
  ClassDB::bind_method(
      D_METHOD("popup_centered_clamped", "size", "fallback_ratio"),
      &Popup::popup_centered_clamped, DEFVAL(Size2()), DEFVAL(0.75));
  ClassDB::bind_method(D_METHOD("popup", "bounds"), &Popup::popup,
                       DEFVAL(Rect2()));
  ClassDB::bind_method(D_METHOD("set_exclusive", "enable"),
                       &Popup::set_exclusive);
  ClassDB::bind_method(D_METHOD("is_exclusive"), &Popup::is_exclusive);
  ADD_SIGNAL(MethodInfo("about_to_show"));
  ADD_SIGNAL(MethodInfo("popup_hide"));
  ADD_GROUP("Popup", "popup_");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "popup_exclusive"), "set_exclusive",
               "is_exclusive");
...
String Popup::get_configuration_warning() const {
  ...
  if (is_visible_in_tree()) {
    return TTR("Popups will hide by default unless you call popup() or any of "
               "the popup*() functions. Making them visible for editing is "
               "fine, but they will hide upon running.");
  }
