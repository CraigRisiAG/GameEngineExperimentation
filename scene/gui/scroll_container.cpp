...
bool ScrollContainer::clips_input() const { return true; }
...
void ScrollContainer::_gui_input(const Ref<InputEvent> &p_gui_input) {
  ...
  if (mb.is_valid()) {
    ...
    if (mb->get_button_index() == BUTTON_WHEEL_LEFT && mb->is_pressed()) {
      if (h_scroll->is_visible_in_tree()) {
        h_scroll->set_value(h_scroll->get_value() -
                            h_scroll->get_page() * mb->get_factor() / 8);
      }
    }
  ...
  if (mm.is_valid()) {
    ...
    if (drag_touching && !drag_touching_deaccel) {
      ...
      if (beyond_deadzone || (scroll_h && Math::abs(drag_accum.x) > deadzone) ||
          (scroll_v && Math::abs(drag_accum.y) > deadzone)) {
        if (!beyond_deadzone) {
          propagate_notification(NOTIFICATION_SCROLL_BEGIN);
          emit_signal("scroll_started");

          beyond_deadzone = true;
          // resetting drag_accum here ensures smooth scrolling after reaching
          // deadzone
          drag_accum = -motion;
        }
  ...
  if (pan_gesture.is_valid()) {
    ...
    if (h_scroll->is_visible_in_tree()) {
      h_scroll->set_value(h_scroll->get_value() +
                          h_scroll->get_page() * pan_gesture->get_delta().x /
                              8);
    }
    if (v_scroll->is_visible_in_tree()) {
      v_scroll->set_value(v_scroll->get_value() +
                          v_scroll->get_page() * pan_gesture->get_delta().y /
                              8);
    }
  }
...
void ScrollContainer::_ensure_focused_visible(Control *p_control) {
  ...
  if (is_a_parent_of(p_control)) {
    ...
    float diff = MAX(MIN(other_rect.position.y, global_rect.position.y),
                     other_rect.position.y + other_rect.size.y -
                         global_rect.size.y + bottom_margin);
    set_v_scroll(get_v_scroll() + (diff - global_rect.position.y));
    diff = MAX(MIN(other_rect.position.x, global_rect.position.x),
...
void ScrollContainer::_notification(int p_what) {
  ...
  if (p_what == NOTIFICATION_READY) {
    ...
    get_viewport()->connect(
        "gui_focus_changed",
        callable_mp(this, &ScrollContainer::_ensure_focused_visible));
  }
  ...
  if (p_what == NOTIFICATION_SORT_CHILDREN) {
    ...
    if (h_scroll->is_visible_in_tree() &&
        h_scroll->get_parent() ==
            this) // scrolls may have been moved out for reasons
    ...
    for (int i = 0; i < get_child_count(); i++) {
      ...
      if (!scroll_h || (!h_scroll->is_visible_in_tree() &&
                        c->get_h_size_flags() & SIZE_EXPAND)) {
        r.position.x = 0;
      ...
      if (!scroll_v || (!v_scroll->is_visible_in_tree() &&
                        c->get_v_size_flags() & SIZE_EXPAND)) {
        r.position.y = 0;
...
bool ScrollContainer::is_h_scroll_enabled() const { return scroll_h; }
...
bool ScrollContainer::is_v_scroll_enabled() const { return scroll_v; }
...
String ScrollContainer::get_configuration_warning() const {
  ...
  if (found != 1)
    return TTR("ScrollContainer is intended to work with a single child "
               "control.\nUse a container as child (VBox, HBox, etc.), or a "
               "Control and set the custom minimum size manually.");
  else
    return "";
}

HScrollBar *ScrollContainer::get_h_scrollbar() { return h_scroll; }

VScrollBar *ScrollContainer::get_v_scrollbar() { return v_scroll; }

void ScrollContainer::_bind_methods() {

  ClassDB::bind_method(D_METHOD("_gui_input"), &ScrollContainer::_gui_input);
  ClassDB::bind_method(D_METHOD("set_enable_h_scroll", "enable"),
                       &ScrollContainer::set_enable_h_scroll);
  ClassDB::bind_method(D_METHOD("is_h_scroll_enabled"),
                       &ScrollContainer::is_h_scroll_enabled);
  ClassDB::bind_method(D_METHOD("set_enable_v_scroll", "enable"),
                       &ScrollContainer::set_enable_v_scroll);
  ClassDB::bind_method(D_METHOD("is_v_scroll_enabled"),
                       &ScrollContainer::is_v_scroll_enabled);
  ClassDB::bind_method(D_METHOD("_update_scrollbar_position"),
                       &ScrollContainer::_update_scrollbar_position);
  ClassDB::bind_method(D_METHOD("set_h_scroll", "value"),
                       &ScrollContainer::set_h_scroll);
  ClassDB::bind_method(D_METHOD("get_h_scroll"),
                       &ScrollContainer::get_h_scroll);
  ClassDB::bind_method(D_METHOD("set_v_scroll", "value"),
                       &ScrollContainer::set_v_scroll);
  ClassDB::bind_method(D_METHOD("get_v_scroll"),
                       &ScrollContainer::get_v_scroll);
  ClassDB::bind_method(D_METHOD("set_deadzone", "deadzone"),
                       &ScrollContainer::set_deadzone);
  ClassDB::bind_method(D_METHOD("get_deadzone"),
                       &ScrollContainer::get_deadzone);
  ClassDB::bind_method(D_METHOD("set_follow_focus", "enabled"),
                       &ScrollContainer::set_follow_focus);
  ClassDB::bind_method(D_METHOD("is_following_focus"),
                       &ScrollContainer::is_following_focus);

  ClassDB::bind_method(D_METHOD("get_h_scrollbar"),
                       &ScrollContainer::get_h_scrollbar);
  ClassDB::bind_method(D_METHOD("get_v_scrollbar"),
                       &ScrollContainer::get_v_scrollbar);

  ADD_SIGNAL(MethodInfo("scroll_started"));
  ADD_SIGNAL(MethodInfo("scroll_ended"));

  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "follow_focus"), "set_follow_focus",
               "is_following_focus");

  ADD_GROUP("Scroll", "scroll_");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "scroll_horizontal_enabled"),
               "set_enable_h_scroll", "is_h_scroll_enabled");
  ADD_PROPERTY(PropertyInfo(Variant::INT, "scroll_horizontal"), "set_h_scroll",
               "get_h_scroll");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "scroll_vertical_enabled"),
               "set_enable_v_scroll", "is_v_scroll_enabled");
  ADD_PROPERTY(PropertyInfo(Variant::INT, "scroll_vertical"), "set_v_scroll",
               "get_v_scroll");
  ADD_PROPERTY(PropertyInfo(Variant::INT, "scroll_deadzone"), "set_deadzone",
               "get_deadzone");
...
ScrollContainer::ScrollContainer() {
  ...
  h_scroll->connect("value_changed",
                    callable_mp(this, &ScrollContainer::_scroll_moved));
