...
int RichTextLabel::_process_line(
    ItemFrame *p_frame, const Vector2 &p_ofs, int &y, int p_width, int p_line,
    ProcessMode p_mode, const Ref<Font> &p_base_font, const Color &p_base_color,
    const Color &p_font_color_shadow, bool p_shadow_as_outline,
    const Point2 &shadow_ofs, const Point2i &p_click_pos, Item **r_click_item,
    int *r_click_char, bool *r_outside, int p_char_count) {
  ...
  // line height should be the font height for the first time, this ensures that
  // an empty line will never have zero height and successive newlines are
  // displayed
  ...
  {                                                                            \
    if (p_mode != PROCESS_CACHE) {                                             \
      ...
      case ALIGN_RIGHT:                                                        \
        ...
        break;                                                                 \
      case ALIGN_FILL:                                                         \
        l.offset_caches.push_back(line_wrapped ? ((p_width - margin) - used)   \
                                               : 0);                           \
        break;                                                                 \
      }                                                                        \
    ...
    if (p_mode == PROCESS_POINTER && r_click_item &&                           \
        p_click_pos.y >= p_ofs.y + y && p_click_pos.y <= p_ofs.y + y + lh &&   \
        p_click_pos.x < p_ofs.x + wofs) {                                      \
      if (r_outside)                                                           \
        *r_outside = true;                                                     \
      *r_click_item = it;                                                      \
      *r_click_char = rchar;                                                   \
...
#define ENSURE_WIDTH(m_width)                                                  \
  ...
  if (wofs - backtrack + m_width > p_width) {                                  \
    ...
    const bool x_in_range =                                                    \
        (p_click_pos.x > p_ofs.x + wofs) &&                                    \
        (!p_frame->cell || p_click_pos.x < p_ofs.x + p_width);                 \
    if (p_mode == PROCESS_POINTER && r_click_item &&                           \
        p_click_pos.y >= p_ofs.y + y && p_click_pos.y <= p_ofs.y + y + lh &&   \
        x_in_range) {                                                          \
      if (r_outside)                                                           \
...
#define ADVANCE(m_width)                                                       \
  {                                                                            \
    if (p_mode == PROCESS_POINTER && r_click_item &&                           \
        p_click_pos.y >= p_ofs.y + y && p_click_pos.y <= p_ofs.y + y + lh &&   \
        p_click_pos.x >= p_ofs.x + wofs &&                                     \
        p_click_pos.x < p_ofs.x + wofs + m_width) {                            \
      if (r_outside)                                                           \
        *r_outside = false;                                                    \
      *r_click_item = it;                                                      \
      *r_click_char = rchar;                                                   \
  ...
  while (it) {
    ...
    case ITEM_INDENT: {
      ...
      if (it != l.from) {
        ...
        int indent =
            indent_it->level * tab_size * cfont->get_char_size(' ').width;
        margin += indent;
        begin += indent;
        wofs += indent;
      }
...

        {
            ...
            if (p_mode == PROCESS_POINTER && r_click_char &&
              ...
              if (selection.active) {
                ...
                if ((text->index > selection.from->index ||
                     (text->index == selection.from->index &&
              ...
              if (fade) {
                if (c_item_offset >= fade->starting_index) {
                  faded_visibility -=
                      (float)(c_item_offset - fade->starting_index) /
                      (float)fade->length;
                  faded_visibility =
                      faded_visibility < 0.0f ? 0.0f : faded_visibility;
                }
              ...
              bool visible =
                  ...
                  ((p_char_count < visible_characters &&
                    YRANGE_VISIBLE(y + lh - line_descent - line_ascent,
              ...
              for (int j = 0; j < fx_stack.size(); j++) {
                ...
                if (item_fx->type == ITEM_CUSTOMFX && custom_fx_ok) {
                  ...
                  if (!custom_effect.is_null()) {
                    ...
                    bool effect_status =
                        custom_effect->_process_effect_impl(charfx);
                  ...
                  uint64_t char_current_rand =
                      item_shake->offset_random(c_item_offset);
                  uint64_t char_previous_rand =
                      item_shake->offset_previous_random(c_item_offset);
                  uint64_t max_rand = 2147483647;
                  double current_offset =
                      Math::range_lerp(char_current_rand % max_rand, 0,
                                       max_rand, 0.0f, 2.f * (float)Math_PI);
                  double previous_offset =
                      Math::range_lerp(char_previous_rand % max_rand, 0,
                                       max_rand, 0.0f, 2.f * (float)Math_PI);
                  double n_time = (double)(item_shake->elapsed_time /
                                           (0.5f / item_shake->rate));
                  n_time = (n_time > 1.0) ? 1.0 : n_time;
                  fx_offset +=
                      Point2(Math::lerp(Math::sin(previous_offset),
                                        Math::sin(current_offset), n_time),
                             Math::lerp(Math::cos(previous_offset),
                                        Math::cos(current_offset), n_time)) *
                      (float)item_shake->strength / 10.0f;
                } else if (item_fx->type == ITEM_WAVE) {
                  ItemWave *item_wave = static_cast<ItemWave *>(item_fx);

                  double value =
                      Math::sin(item_wave->frequency * item_wave->elapsed_time +
                                ((p_ofs.x + pofs) / 50)) *
                  ...
                  double torn_x = Math::sin(item_tornado->frequency *
                                                item_tornado->elapsed_time +
                                            ((p_ofs.x + pofs) / 50)) *
                                  (item_tornado->radius);
                  double torn_y = Math::cos(item_tornado->frequency *
                                                item_tornado->elapsed_time +
                                            ((p_ofs.x + pofs) / 50)) *
                                  (item_tornado->radius);
                  ...
                  fx_color = fx_color.from_hsv(item_rainbow->frequency *
                                                   (item_rainbow->elapsed_time +
                                                    ((p_ofs.x + pofs) / 50)),
                                               item_rainbow->saturation,
                                               item_rainbow->value, fx_color.a);
                }
              }
              ...
              if (visible) {
                ...
                if (selected) {
                  ...
                  draw_rect(Rect2(p_ofs.x + pofs, p_ofs.y + y, cw, lh),
                            selection_bg);
                }
                ...
                if (p_font_color_shadow.a > 0) {
                  ...
                  font->draw_char(ci,
                                  Point2(x_ofs_shadow, y_ofs_shadow) +
                                      shadow_ofs + fx_offset,
                                  fx_char, c[i + 1], p_font_color_shadow);
                ...
                if (selected) {
                  drawer.draw_char(
                      ci,
                      p_ofs + Point2(align_ofs + pofs, y + lh - line_descent),
                      fx_char, c[i + 1],
                      override_selected_font_color ? selection_fg : fx_color);
                } else {
                  cw = drawer.draw_char(
                      ci,
                      p_ofs + Point2(align_ofs + pofs, y + lh - line_descent) +
                          fx_offset,
                      fx_char, c[i + 1], fx_color);
                }
          ...
          if (underline) {
            ...
            VS::get_singleton()->canvas_item_add_line(
                ci, p_ofs + Point2(align_ofs + wofs, uy),
                p_ofs + Point2(align_ofs + wofs + w, uy), uc, underline_width);
            ...
            VS::get_singleton()->canvas_item_add_line(
                ci, p_ofs + Point2(align_ofs + wofs, uy),
                p_ofs + Point2(align_ofs + wofs + w, uy), uc,
                strikethrough_width);
          }
        }
    ...
    case ITEM_IMAGE: {
      ...
      bool visible =
          visible_characters < 0 ||
          (p_char_count < visible_characters &&
           YRANGE_VISIBLE(y + lh - font->get_descent() - img->size.height,
                          img->size.height));
      if (visible)
        line_is_blank = false;
      ...
      if (p_mode == PROCESS_DRAW && visible) {
        img->image->draw_rect(
            ci, Rect2(p_ofs + Point2(align_ofs + wofs, y + lh -
                                                           font->get_descent() -
                                                           img->size.height),
                      img->size));
      }
    ...
    case ITEM_TABLE: {
      ...
      Point2 shadow_ofs2(get_constant("shadow_offset_x"),
      ...
      if (p_mode == PROCESS_CACHE) {
        ...
        const int available_width =
            p_width - hseparation * (table->columns.size() - 1) - wofs;

        for (List<Item *>::Element *E = table->subitems.front(); E;
             E = E->next()) {
          ERR_CONTINUE(E->get()->type !=
                       ITEM_FRAME); // children should all be frames
          ...
          for (int i = 0; i < frame->lines.size(); i++) {
            ...
            _process_line(frame, Point2(), ly, available_width, i,
                          PROCESS_CACHE, cfont, Color(), font_color_shadow,
                          use_outline, shadow_ofs2);
        ...
        for (int i = 0; i < table->columns.size(); i++) {
          ...
          if (table->columns[i].expand && total_ratio > 0)
            table->columns.write[i].width +=
                table->columns[i].expand_ratio * remaining_width / total_ratio;
          ...
          if (remaining_width > 0 && total_ratio > 0) {
            for (int i = 0; i < table->columns.size(); i++) {
              if (table->columns[i].expand) {
                ...
                if (dif > 0) {
                  int slice = table->columns[i].expand_ratio * remaining_width /
                              total_ratio;
        ...
        for (List<Item *>::Element *E = table->subitems.front(); E;
          ...
          for (int i = 0; i < frame->lines.size(); i++) {
            ...
            int ly = 0;
            _process_line(frame, Point2(), ly, table->columns[column].width, i,
                          PROCESS_CACHE, cfont, Color(), font_color_shadow,
                          use_outline, shadow_ofs2);
      ...
      for (List<Item *>::Element *E = table->subitems.front(); E;
        ...
        int lines_h =
            ...
            (frame->lines[0].height_accum_cache - frame->lines[0].height_cache);
        int lines_ofs = p_ofs.y + offset.y + draw_ofs.y;

        bool visible =
            lines_ofs < get_size().height && lines_ofs + lines_h >= 0;
        if (visible)
          line_is_blank = false;
        ...
        for (int i = 0; i < frame->lines.size(); i++) {
          ...
          if (visible) {
            if (p_mode == PROCESS_DRAW) {
              nonblank_line_count += _process_line(
                  frame, p_ofs + offset + draw_ofs + Vector2(0, yofs), ly,
                  table->columns[column].width, i, PROCESS_DRAW, cfont, ccolor,
                  font_color_shadow, use_outline, shadow_ofs2);
            } else if (p_mode == PROCESS_POINTER) {
              _process_line(frame, p_ofs + offset + draw_ofs + Vector2(0, yofs),
                            ly, table->columns[column].width, i,
                            PROCESS_POINTER, cfont, ccolor, font_color_shadow,
                            use_outline, shadow_ofs2, p_click_pos, r_click_item,
                            r_click_char, r_outside);
          ...
          if (p_mode == PROCESS_CACHE) {
            frame->lines.write[i].height_accum_cache =
                offset.y + draw_ofs.y + frame->lines[i].height_cache;
          }
        }
...
void RichTextLabel::_notification(int p_what) {
  ...
  case NOTIFICATION_DRAW: {
    ...
    Color font_color_shadow = get_color("font_color_shadow");
    bool use_outline = get_constant("shadow_as_outline");
    Point2 shadow_ofs(get_constant("shadow_offset_x"),
                      get_constant("shadow_offset_y"));
    ...
    while (y < size.height && from_line < main->lines.size()) {
      ...
      visible_line_count += _process_line(
          main, text_rect.get_position(), y,
          text_rect.get_size().width - scroll_w, from_line, PROCESS_DRAW,
          base_font, base_color, font_color_shadow, use_outline, shadow_ofs,
          Point2i(), NULL, NULL, NULL, total_chars);
      total_chars += main->lines[from_line].char_count;
...
void RichTextLabel::_find_click(ItemFrame *p_frame, const Point2i &p_click,
                                Item **r_click_item, int *r_click_char,
                                bool *r_outside) {
  ...
  while (y < text_rect.get_size().height && from_line < p_frame->lines.size()) {
    ...
    _process_line(p_frame, text_rect.get_position(), y,
                  text_rect.get_size().width - scroll_w, from_line,
                  PROCESS_POINTER, base_font, base_color, font_color_shadow,
                  use_outline, shadow_ofs, p_click, r_click_item, r_click_char,
                  r_outside);
    if (r_click_item && *r_click_item)
      return;
    from_line++;
...
Control::CursorShape
RichTextLabel::get_cursor_shape(const Point2 &p_pos) const {
...
void RichTextLabel::_gui_input(Ref<InputEvent> p_event) {
  ...
  if (b.is_valid()) {
    ...
    if (b->get_button_index() == BUTTON_WHEEL_UP) {
      if (scroll_active)
        vscroll->set_value(vscroll->get_value() -
...
void RichTextLabel::_fetch_item_fx_stack(Item *p_item,
...
bool RichTextLabel::_find_layout_subitem(Item *from, Item *to) {
  ...
  if (from && from != to) {
    if (from->type != ITEM_FONT && from->type != ITEM_COLOR &&
        from->type != ITEM_UNDERLINE && from->type != ITEM_STRIKETHROUGH)
      return true;
...
void RichTextLabel::_validate_line_caches(ItemFrame *p_frame) {
  ...
  for (int i = p_frame->first_invalid_line; i < p_frame->lines.size(); i++) {
    ...
    int y = 0;
    _process_line(p_frame, text_rect.get_position(), y,
                  text_rect.get_size().width - scroll_w, i, PROCESS_CACHE,
                  base_font, Color(), font_color_shadow, use_outline,
                  shadow_ofs);
...
void RichTextLabel::add_image(const Ref<Texture2D> &p_image, const int p_width,
                              const int p_height) {
...
void RichTextLabel::push_wave(float p_frequency = 1.0f,
...
void RichTextLabel::push_tornado(float p_frequency = 1.0f,
...
void RichTextLabel::push_rainbow(float p_saturation, float p_value,
                                 float p_frequency) {
...
void RichTextLabel::push_customfx(Ref<RichTextEffect> p_custom_effect,
...
void RichTextLabel::set_table_column_expand(int p_column, bool p_expand,
                                            int p_ratio) {
...
void RichTextLabel::set_offset(int p_pixel) { vscroll->set_value(p_pixel); }
...
void RichTextLabel::scroll_to_line(int p_line) {
  ...
  _validate_line_caches(main);
  vscroll->set_value(main->lines[p_line].height_accum_cache -
  ...
  while (item) {
    ...
    if (item->type == ITEM_TEXT) {
      ...
      if (item == selection.from && item == selection.to) {
        text += itext.substr(selection.from_char,
...
String RichTextLabel::get_bbcode() const { return bbcode; }
...
float RichTextLabel::get_percent_visible() const { return percent_visible; }
...
void RichTextLabel::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("add_image", "image", "width", "height"),
                       &RichTextLabel::add_image, DEFVAL(0), DEFVAL(0));
  ClassDB::bind_method(D_METHOD("newline"), &RichTextLabel::add_newline);
  ClassDB::bind_method(D_METHOD("remove_line", "line"),
                       &RichTextLabel::remove_line);
  ClassDB::bind_method(D_METHOD("push_font", "font"),
  ...
  ClassDB::bind_method(D_METHOD("push_bold_italics"),
  ...
  ClassDB::bind_method(D_METHOD("push_color", "color"),
                       &RichTextLabel::push_color);
  ClassDB::bind_method(D_METHOD("push_align", "align"),
                       &RichTextLabel::push_align);
  ClassDB::bind_method(D_METHOD("push_indent", "level"),
                       &RichTextLabel::push_indent);
  ClassDB::bind_method(D_METHOD("push_list", "type"),
  ...
  ClassDB::bind_method(D_METHOD("push_strikethrough"),
                       &RichTextLabel::push_strikethrough);
  ClassDB::bind_method(D_METHOD("push_table", "columns"),
  ...
  ClassDB::bind_method(D_METHOD("set_meta_underline", "enable"),
                       &RichTextLabel::set_meta_underline);
  ClassDB::bind_method(D_METHOD("is_meta_underlined"),
  ...
  ClassDB::bind_method(D_METHOD("set_scroll_active", "active"),
                       &RichTextLabel::set_scroll_active);
  ClassDB::bind_method(D_METHOD("is_scroll_active"),
                       &RichTextLabel::is_scroll_active);

  ClassDB::bind_method(D_METHOD("set_scroll_follow", "follow"),
                       &RichTextLabel::set_scroll_follow);
  ClassDB::bind_method(D_METHOD("is_scroll_following"),
                       &RichTextLabel::is_scroll_following);

  ClassDB::bind_method(D_METHOD("get_v_scroll"), &RichTextLabel::get_v_scroll);

  ClassDB::bind_method(D_METHOD("scroll_to_line", "line"),
                       &RichTextLabel::scroll_to_line);

  ClassDB::bind_method(D_METHOD("set_tab_size", "spaces"),
                       &RichTextLabel::set_tab_size);
  ClassDB::bind_method(D_METHOD("get_tab_size"), &RichTextLabel::get_tab_size);

  ClassDB::bind_method(D_METHOD("set_selection_enabled", "enabled"),
                       &RichTextLabel::set_selection_enabled);
  ClassDB::bind_method(D_METHOD("is_selection_enabled"),
                       &RichTextLabel::is_selection_enabled);

  ClassDB::bind_method(D_METHOD("parse_bbcode", "bbcode"),
                       &RichTextLabel::parse_bbcode);
  ClassDB::bind_method(D_METHOD("append_bbcode", "bbcode"),
                       &RichTextLabel::append_bbcode);

  ClassDB::bind_method(D_METHOD("set_bbcode", "text"),
                       &RichTextLabel::set_bbcode);
  ClassDB::bind_method(D_METHOD("get_bbcode"), &RichTextLabel::get_bbcode);

  ClassDB::bind_method(D_METHOD("set_visible_characters", "amount"),
                       &RichTextLabel::set_visible_characters);
  ClassDB::bind_method(D_METHOD("get_visible_characters"),
                       &RichTextLabel::get_visible_characters);

  ClassDB::bind_method(D_METHOD("set_percent_visible", "percent_visible"),
  ...
  ClassDB::bind_method(D_METHOD("set_use_bbcode", "enable"),
                       &RichTextLabel::set_use_bbcode);
  ClassDB::bind_method(D_METHOD("is_using_bbcode"),
                       &RichTextLabel::is_using_bbcode);

  ClassDB::bind_method(D_METHOD("get_line_count"),
                       &RichTextLabel::get_line_count);
  ClassDB::bind_method(D_METHOD("get_visible_line_count"),
                       &RichTextLabel::get_visible_line_count);

  ClassDB::bind_method(D_METHOD("get_content_height"),
                       &RichTextLabel::get_content_height);

  ClassDB::bind_method(D_METHOD("parse_expressions_for_values", "expressions"),
  ...
  ClassDB::bind_method(D_METHOD("install_effect", "effect"),
                       &RichTextLabel::install_effect);

  ADD_GROUP("BBCode", "bbcode_");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "bbcode_enabled"), "set_use_bbcode",
               "is_using_bbcode");
  ADD_PROPERTY(PropertyInfo(Variant::STRING, "bbcode_text",
                            PROPERTY_HINT_MULTILINE_TEXT),
               "set_bbcode", "get_bbcode");

  ADD_PROPERTY(PropertyInfo(Variant::INT, "visible_characters",
                            PROPERTY_HINT_RANGE, "-1,128000,1"),
               "set_visible_characters", "get_visible_characters");
  ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "percent_visible",
                            PROPERTY_HINT_RANGE, "0,1,0.001"),
               "set_percent_visible", "get_percent_visible");

  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "meta_underlined"),
               "set_meta_underline", "is_meta_underlined");
  ADD_PROPERTY(
  ...
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "scroll_active"),
               "set_scroll_active", "is_scroll_active");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "scroll_following"),
               "set_scroll_follow", "is_scroll_following");

  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "selection_enabled"),
               "set_selection_enabled", "is_selection_enabled");
  ...
  ADD_PROPERTY(
      PropertyInfo(Variant::ARRAY, "custom_effects",
                   PROPERTY_HINT_RESOURCE_TYPE, "17/17:RichTextEffect",
                   (PROPERTY_USAGE_DEFAULT | PROPERTY_USAGE_SCRIPT_VARIABLE),
                   "RichTextEffect"),
      "set_effects", "get_effects");

  ADD_SIGNAL(MethodInfo("meta_clicked",
                        PropertyInfo(Variant::NIL, "meta", PROPERTY_HINT_NONE,
                                     "", PROPERTY_USAGE_NIL_IS_VARIANT)));
  ADD_SIGNAL(MethodInfo("meta_hover_started",
                        PropertyInfo(Variant::NIL, "meta", PROPERTY_HINT_NONE,
                                     "", PROPERTY_USAGE_NIL_IS_VARIANT)));
  ADD_SIGNAL(MethodInfo("meta_hover_ended",
                        PropertyInfo(Variant::NIL, "meta", PROPERTY_HINT_NONE,
                                     "", PROPERTY_USAGE_NIL_IS_VARIANT)));
...
Size2 RichTextLabel::get_minimum_size() const {
  ...
  if (fixed_width != -1) {
    ...
    return Size2(fixed_width,
                 const_cast<RichTextLabel *>(this)->get_content_height());
  }
...
Ref<RichTextEffect>
RichTextLabel::_get_custom_effect_by_code(String p_bbcode_identifier) {
  for (int i = 0; i < custom_effects.size(); i++) {
...
RichTextLabel::RichTextLabel() {
  ...
  vscroll->connect("value_changed",
                   callable_mp(this, &RichTextLabel::_scroll_changed));
  vscroll->set_step(1);
  vscroll->hide();
  current_idx = 1;
  use_bbcode = false;
...
RichTextLabel::~RichTextLabel() { memdelete(main); }
