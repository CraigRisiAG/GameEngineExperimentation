...
void TextureRect::_notification(int p_what) {
  ...
  if (p_what == NOTIFICATION_DRAW) {
    ...
    case STRETCH_KEEP_ASPECT_COVERED: {
      ...
      float scale = scale_size.width > scale_size.height ? scale_size.width
                                                         : scale_size.height;
...
void TextureRect::_bind_methods() {
  ...
  ADD_PROPERTY(
      PropertyInfo(
          Variant::INT, "stretch_mode", PROPERTY_HINT_ENUM,
  ...
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "flip_h"), "set_flip_h",
               "is_flipped_h");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "flip_v"), "set_flip_v",
               "is_flipped_v");
...
void TextureRect::set_texture(const Ref<Texture2D> &p_tex) {
  ...
  if (texture.is_valid()) {
    texture->disconnect(CoreStringNames::get_singleton()->changed,
                        callable_mp(this, &TextureRect::_texture_changed));
  }

  texture = p_tex;

  if (texture.is_valid()) {
    texture->connect(CoreStringNames::get_singleton()->changed,
                     callable_mp(this, &TextureRect::_texture_changed));
  }
...
bool TextureRect::has_expand() const { return expand; }
