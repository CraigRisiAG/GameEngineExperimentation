...
Size2 CheckBox::get_icon_size() const {
  ...
  if (!unchecked.is_null())
    tex_size = Size2(MAX(tex_size.width, unchecked->get_width()),
                     MAX(tex_size.height, unchecked->get_height()));
  if (!radio_checked.is_null())
    tex_size = Size2(MAX(tex_size.width, radio_checked->get_width()),
                     MAX(tex_size.height, radio_checked->get_height()));
  if (!radio_unchecked.is_null())
    tex_size = Size2(MAX(tex_size.width, radio_unchecked->get_width()),
...
Size2 CheckBox::get_minimum_size() const {
  ...
  minsize.height =
      MAX(minsize.height, tex_size.height + sb->get_margin(MARGIN_TOP) +
...
void CheckBox::_notification(int p_what) {
  ...
  if (p_what == NOTIFICATION_THEME_CHANGED) {
    ...
    Ref<Texture2D> on =
        Control::get_icon(is_radio() ? "radio_checked" : "checked");
    Ref<Texture2D> off =
        Control::get_icon(is_radio() ? "radio_unchecked" : "unchecked");
    Ref<StyleBox> sb = get_stylebox("normal");
...
bool CheckBox::is_radio() { return get_button_group().is_valid(); }
