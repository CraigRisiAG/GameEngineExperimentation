...
void TabContainer::_gui_input(const Ref<InputEvent> &p_event) {
  ...
  if (mb.is_valid() && mb->is_pressed() &&
    ...
    if (popup && pos.x > size.width - menu->get_width()) {
      ...
      popup_pos.x +=
          size.width * get_global_transform().get_scale().x -
          popup->get_size().width * popup->get_global_transform().get_scale().x;
  ...
  case NOTIFICATION_DRAW: {
    ...
    for (int i = 0; i < tabs.size(); i++) {
      ...
      if (all_tabs_width > header_width) {
        // Not all tabs are visible at the same time - reserve space for
        // navigation buttons.
    ...
    // Draw the tab area.
    panel->draw(canvas, Rect2(0, header_height, size.width,
    ...
    for (int i = 0; i < tab_widths.size(); i++) {
      ...
      String text = control->has_meta("_tab_name")
                        ? String(tr(String(control->get_meta("_tab_name"))))
                        : String(tr(control->get_name()));

      int x_content = tab_rect.position.x + tab_style->get_margin(MARGIN_LEFT);
      int top_margin = tab_style->get_margin(MARGIN_TOP);
      int y_center =
          top_margin + (tab_rect.size.y - tab_style->get_minimum_size().y) / 2;
      ...
      // Draw the tab text.
      Point2i text_pos(x_content, y_center - (font->get_height() / 2) +
                                      font->get_ascent());
    ...
    if (popup) {
      x -= menu->get_width();
      if (menu_hovered)
        menu_hl->draw(get_canvas_item(),
                      Size2(x, (header_height - menu_hl->get_height()) / 2));
      else
        menu->draw(get_canvas_item(),
                   Size2(x, (header_height - menu->get_height()) / 2));
    }
    ...
    if (buttons_visible_cache) {
      ...
      if (last_tab_cache < tabs.size() - 1) {
        draw_texture(highlight_arrow == 1 ? increment_hl : increment,
                     Point2(x, (header_height - increment->get_height()) / 2));
      } else {
        draw_texture(increment,
                     Point2(x, (header_height - increment->get_height()) / 2),
                     Color(1, 1, 1, 0.5));
      }
      ...
      if (first_tab_cache > 0) {
        draw_texture(highlight_arrow == 0 ? decrement_hl : decrement,
                     Point2(x, (header_height - decrement->get_height()) / 2));
      } else {
        draw_texture(decrement,
                     Point2(x, (header_height - decrement->get_height()) / 2),
                     Color(1, 1, 1, 0.5));
      }
    }
...
void TabContainer::_child_renamed_callback() { update(); }
...
void TabContainer::add_child_notify(Node *p_child) {
  ...
  c->set_margin(Margin(MARGIN_TOP), c->get_margin(Margin(MARGIN_TOP)) +
                                        sb->get_margin(Margin(MARGIN_TOP)));
  c->set_margin(Margin(MARGIN_LEFT), c->get_margin(Margin(MARGIN_LEFT)) +
                                         sb->get_margin(Margin(MARGIN_LEFT)));
  c->set_margin(Margin(MARGIN_RIGHT), c->get_margin(Margin(MARGIN_RIGHT)) -
                                          sb->get_margin(Margin(MARGIN_RIGHT)));
  c->set_margin(Margin(MARGIN_BOTTOM),
                c->get_margin(Margin(MARGIN_BOTTOM)) -
                    sb->get_margin(Margin(MARGIN_BOTTOM)));

  update();
  p_child->connect("renamed",
                   callable_mp(this, &TabContainer::_child_renamed_callback));
  if (first && is_inside_tree())
    emit_signal("tab_changed", current);
}

int TabContainer::get_tab_count() const { return _get_tabs().size(); }
...
void TabContainer::set_current_tab(int p_current) {
  ...
  for (int i = 0; i < tabs.size(); i++) {
    ...
    if (i == current) {
      ...
      c->set_margin(Margin(MARGIN_TOP), c->get_margin(Margin(MARGIN_TOP)) +
                                            sb->get_margin(Margin(MARGIN_TOP)));
      c->set_margin(Margin(MARGIN_LEFT),
                    c->get_margin(Margin(MARGIN_LEFT)) +
                        sb->get_margin(Margin(MARGIN_LEFT)));
      c->set_margin(Margin(MARGIN_RIGHT),
                    c->get_margin(Margin(MARGIN_RIGHT)) -
                        sb->get_margin(Margin(MARGIN_RIGHT)));
      c->set_margin(Margin(MARGIN_BOTTOM),
                    c->get_margin(Margin(MARGIN_BOTTOM)) -
                        sb->get_margin(Margin(MARGIN_BOTTOM)));
...
int TabContainer::get_current_tab() const { return current; }

int TabContainer::get_previous_tab() const { return previous; }
...
bool TabContainer::can_drop_data(const Point2 &p_point,
  ...
  if (String(d["type"]) == "tabc_element") {
    ...
    if (from_path == to_path) {
      ...
      if (from_tabc &&
          from_tabc->get_tabs_rearrange_group() == get_tabs_rearrange_group()) {
        return true;
      }
    }
  }
...
TabContainer::TabAlign TabContainer::get_tab_align() const { return align; }
...
Size2 TabContainer::get_minimum_size() const {
  ...
  if (tabs_visible) {
    ms.y += MAX(MAX(tab_bg->get_minimum_size().y, tab_fg->get_minimum_size().y),
                tab_disabled->get_minimum_size().y);
...
Popup *TabContainer::get_popup() const { return popup; }
...
void TabContainer::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("get_current_tab"),
                       &TabContainer::get_current_tab);
  ClassDB::bind_method(D_METHOD("get_previous_tab"),
                       &TabContainer::get_previous_tab);
  ClassDB::bind_method(D_METHOD("get_current_tab_control"),
                       &TabContainer::get_current_tab_control);
  ClassDB::bind_method(D_METHOD("get_tab_control", "idx"),
  ...
  ClassDB::bind_method(D_METHOD("set_tabs_visible", "visible"),
                       &TabContainer::set_tabs_visible);
  ClassDB::bind_method(D_METHOD("are_tabs_visible"),
  ...
  ClassDB::bind_method(D_METHOD("set_popup", "popup"),
  ...
  ClassDB::bind_method(D_METHOD("set_use_hidden_tabs_for_min_size", "enabled"),
                       &TabContainer::set_use_hidden_tabs_for_min_size);
  ClassDB::bind_method(D_METHOD("get_use_hidden_tabs_for_min_size"),
                       &TabContainer::get_use_hidden_tabs_for_min_size);

  ClassDB::bind_method(D_METHOD("_on_theme_changed"),
                       &TabContainer::_on_theme_changed);
  ClassDB::bind_method(D_METHOD("_update_current_tab"),
                       &TabContainer::_update_current_tab);
  ...
  ADD_PROPERTY(PropertyInfo(Variant::INT, "current_tab", PROPERTY_HINT_RANGE,
               ...
               "set_current_tab", "get_current_tab");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "tabs_visible"), "set_tabs_visible",
               "are_tabs_visible");
  ...
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "use_hidden_tabs_for_min_size"),
