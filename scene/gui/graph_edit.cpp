...
Error GraphEdit::connect_node(const StringName &p_from, int p_from_port,
                              const StringName &p_to, int p_to_port) {
...
bool GraphEdit::is_node_connected(const StringName &p_from, int p_from_port,
                                  const StringName &p_to, int p_to_port) {
...
void GraphEdit::disconnect_node(const StringName &p_from, int p_from_port,
                                const StringName &p_to, int p_to_port) {
...
bool GraphEdit::clips_input() const { return true; }
...
void GraphEdit::_scroll_moved(double) {
  ...
  if (!setting_scroll_ofs) { // in godot, signals on change value are avoided as
                             // a convention
...
void GraphEdit::_update_scroll() {
  ...
  h_scroll->set_anchor_and_margin(MARGIN_RIGHT, ANCHOR_END,
                                  v_scroll->is_visible() ? -vmin.width : 0);
  v_scroll->set_anchor_and_margin(MARGIN_BOTTOM, ANCHOR_END,
...
void GraphEdit::add_child_notify(Node *p_child) {
  ...
  if (gn) {
    gn->set_scale(Vector2(zoom, zoom));
    gn->connect("offset_changed",
                callable_mp(this, &GraphEdit::_graph_node_moved), varray(gn));
    gn->connect("raise_request",
                callable_mp(this, &GraphEdit::_graph_node_raised), varray(gn));
    gn->connect(
        "item_rect_changed",
        callable_mp((CanvasItem *)connections_layer, &CanvasItem::update));
...
void GraphEdit::remove_child_notify(Node *p_child) {
  ...
  GraphNode *gn = Object::cast_to<GraphNode>(p_child);
  if (gn) {
    gn->disconnect("offset_changed",
                   callable_mp(this, &GraphEdit::_graph_node_moved));
    gn->disconnect("raise_request",
                   callable_mp(this, &GraphEdit::_graph_node_raised));
  }
}
  ...
  if (p_what == NOTIFICATION_DRAW) {
    ...
    if (is_using_snap()) {
      ...
      for (int i = from.x; i < from.x + len.x; i++) {
        ...
        draw_line(Vector2(base_ofs, 0), Vector2(base_ofs, get_size().height),
                  color);
      }
      ...
      for (int i = from.y; i < from.y + len.y; i++) {
        ...
        draw_line(Vector2(0, base_ofs), Vector2(get_size().width, base_ofs),
                  color);
      }
    }
  }
    ...
    for (int i = get_child_count() - 1; i >= 0; i--) {
            ...
            for (List<Connection>::Element *E = connections.front(); E;
              ...
              if (E->get().from == gn->get_name() && E->get().from_port == j) {
                ...
                if (Object::cast_to<GraphNode>(to)) {
                  ...
                  connecting_type =
                      Object::cast_to<GraphNode>(to)->get_connection_input_type(
                          E->get().to_port);
                  connecting_color =
                      Object::cast_to<GraphNode>(to)
                          ->get_connection_input_color(E->get().to_port);
                  connecting_target = false;
                  connecting_to = pos;
                  just_disconnected = true;

                  emit_signal("disconnection_request", E->get().from,
                              E->get().from_port, E->get().to,
                              E->get().to_port);
      ...
      for (int j = 0; j < gn->get_connection_input_count(); j++) {
        ...
        if (is_in_hot_zone(pos, mpos)) {
          ...
          if (right_disconnects || valid_right_disconnect_types.has(
                ...
                if (Object::cast_to<GraphNode>(fr)) {
                  ...
                  connecting_type =
                      Object::cast_to<GraphNode>(fr)
                          ->get_connection_output_type(E->get().from_port);
                  connecting_color =
                      Object::cast_to<GraphNode>(fr)
                          ->get_connection_output_color(E->get().from_port);
                  connecting_target = false;
                  connecting_to = pos;
                  just_disconnected = true;
      ...
      if (!connecting_out) {
        for (int j = 0; j < gn->get_connection_output_count(); j++) {
          ...
          if ((type == connecting_type ||
               valid_connection_types.has(ConnType(type, connecting_type))) &&
              is_in_hot_zone(pos, mpos)) {
...
bool GraphEdit::_check_clickable_control(Control *p_control,
                                         const Vector2 &pos) {
...
bool GraphEdit::is_in_hot_zone(const Vector2 &pos, const Vector2 &p_mouse_pos) {
  if (!Rect2(pos.x - port_grab_distance_horizontal,
             pos.y - port_grab_distance_vertical,
             port_grab_distance_horizontal * 2, port_grab_distance_vertical * 2)
  ...
  for (int i = 0; i < get_child_count(); i++) {
    ...
    if (rect.has_point(p_mouse_pos)) {
      ...
      for (int j = 0; j < child->get_child_count(); j++) {
        ...
        if (_check_clickable_control(subchild,
...
static _FORCE_INLINE_ Vector2 _bezier_interp(real_t t, Vector2 start,
                                             Vector2 control_1,
                                             Vector2 control_2, Vector2 end) {
...
void GraphEdit::_bake_segment2d(Vector<Vector2> &points, Vector<Color> &colors,
                                float p_begin, float p_end, const Vector2 &p_a,
                                const Vector2 &p_out, const Vector2 &p_b,
                                const Vector2 &p_in, int p_depth,
                                int p_min_depth, int p_max_depth, float p_tol,
                                const Color &p_color, const Color &p_to_color,
                                int &lines) const {
  ...
  if (p_depth >= p_min_depth && (dp < p_tol || p_depth >= p_max_depth)) {
    ...
    _bake_segment2d(points, colors, p_begin, mp, p_a, p_out, p_b, p_in,
                    p_depth + 1, p_min_depth, p_max_depth, p_tol, p_color,
                    p_to_color, lines);
    _bake_segment2d(points, colors, mp, p_end, p_a, p_out, p_b, p_in,
                    p_depth + 1, p_min_depth, p_max_depth, p_tol, p_color,
                    p_to_color, lines);
  }
}

void GraphEdit::_draw_cos_line(CanvasItem *p_where, const Vector2 &p_from,
                               const Vector2 &p_to, const Color &p_color,
  ...
  _bake_segment2d(points, colors, 0, 1, p_from, c1, p_to, c2, 0, 3, 9, 3,
                  p_color, p_to_color, lines);
...
void GraphEdit::_top_layer_draw() {
  ...
  if (box_selecting) {
    ...
    top_layer->draw_rect(box_selecting_rect, get_color("selection_stroke"),
                         false);
  }
}
...
void GraphEdit::_gui_input(const Ref<InputEvent> &p_ev) {
  ...
  if (mm.is_valid() && dragging) {
    ...
    just_selected = true;
    // TODO: Remove local mouse pos hack if/when InputEventMouseMotion is fixed
    // to support floats
  ...
  if (magnify_gesture.is_valid()) {
    ...
    set_zoom_custom(zoom * magnify_gesture->get_factor(),
...
void GraphEdit::set_connection_activity(const StringName &p_from,
                                        int p_from_port, const StringName &p_to,
                                        int p_to_port, float p_activity) {
...
void GraphEdit::set_zoom_custom(float p_zoom, const Vector2 &p_center) {
  ...
  Vector2 sbofs =
      (Vector2(h_scroll->get_value(), v_scroll->get_value()) + p_center) / zoom;
...
void GraphEdit::_zoom_minus() { set_zoom(zoom / ZOOM_SCALE); }
void GraphEdit::_zoom_reset() { set_zoom(1); }

void GraphEdit::_zoom_plus() { set_zoom(zoom * ZOOM_SCALE); }
...
bool GraphEdit::is_using_snap() const { return snap_button->is_pressed(); }

int GraphEdit::get_snap() const { return snap_amount->get_value(); }
...
void GraphEdit::_snap_toggled() { update(); }

void GraphEdit::_snap_value_changed(double) { update(); }

HBoxContainer *GraphEdit::get_zoom_hbox() { return zoom_hb; }
...
void GraphEdit::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("set_connection_activity", "from", "from_port",
                                "to", "to_port", "amount"),
                       &GraphEdit::set_connection_activity);
  ClassDB::bind_method(D_METHOD("get_connection_list"),
                       &GraphEdit::_get_connection_list);
  ClassDB::bind_method(D_METHOD("clear_connections"),
                       &GraphEdit::clear_connections);
  ClassDB::bind_method(D_METHOD("get_scroll_ofs"), &GraphEdit::get_scroll_ofs);
  ClassDB::bind_method(D_METHOD("set_scroll_ofs", "ofs"),
                       &GraphEdit::set_scroll_ofs);

  ClassDB::bind_method(D_METHOD("add_valid_right_disconnect_type", "type"),
                       &GraphEdit::add_valid_right_disconnect_type);
  ClassDB::bind_method(D_METHOD("remove_valid_right_disconnect_type", "type"),
                       &GraphEdit::remove_valid_right_disconnect_type);
  ClassDB::bind_method(D_METHOD("add_valid_left_disconnect_type", "type"),
                       &GraphEdit::add_valid_left_disconnect_type);
  ClassDB::bind_method(D_METHOD("remove_valid_left_disconnect_type", "type"),
  ...
  ClassDB::bind_method(D_METHOD("set_use_snap", "enable"),
                       &GraphEdit::set_use_snap);
  ClassDB::bind_method(D_METHOD("is_using_snap"), &GraphEdit::is_using_snap);

  ClassDB::bind_method(D_METHOD("set_right_disconnects", "enable"),
                       &GraphEdit::set_right_disconnects);
  ClassDB::bind_method(D_METHOD("is_right_disconnects_enabled"),
                       &GraphEdit::is_right_disconnects_enabled);

  ClassDB::bind_method(D_METHOD("_gui_input"), &GraphEdit::_gui_input);
  ClassDB::bind_method(D_METHOD("_update_scroll_offset"),
                       &GraphEdit::_update_scroll_offset);

  ClassDB::bind_method(D_METHOD("get_zoom_hbox"), &GraphEdit::get_zoom_hbox);

  ClassDB::bind_method(D_METHOD("set_selected", "node"),
                       &GraphEdit::set_selected);

  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "right_disconnects"),
               "set_right_disconnects", "is_right_disconnects_enabled");
  ADD_PROPERTY(PropertyInfo(Variant::VECTOR2, "scroll_offset"),
               "set_scroll_ofs", "get_scroll_ofs");
  ADD_PROPERTY(PropertyInfo(Variant::INT, "snap_distance"), "set_snap",
               "get_snap");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "use_snap"), "set_use_snap",
               "is_using_snap");
  ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "zoom"), "set_zoom", "get_zoom");

  ADD_SIGNAL(MethodInfo("connection_request",
                        PropertyInfo(Variant::STRING_NAME, "from"),
                        PropertyInfo(Variant::INT, "from_slot"),
                        PropertyInfo(Variant::STRING_NAME, "to"),
                        PropertyInfo(Variant::INT, "to_slot")));
  ADD_SIGNAL(MethodInfo("disconnection_request",
                        PropertyInfo(Variant::STRING_NAME, "from"),
                        PropertyInfo(Variant::INT, "from_slot"),
                        PropertyInfo(Variant::STRING_NAME, "to"),
                        PropertyInfo(Variant::INT, "to_slot")));
  ...
  ADD_SIGNAL(MethodInfo("connection_to_empty",
                        PropertyInfo(Variant::STRING_NAME, "from"),
                        PropertyInfo(Variant::INT, "from_slot"),
                        PropertyInfo(Variant::VECTOR2, "release_position")));
  ADD_SIGNAL(MethodInfo("connection_from_empty",
                        PropertyInfo(Variant::STRING_NAME, "to"),
                        PropertyInfo(Variant::INT, "to_slot"),
                        PropertyInfo(Variant::VECTOR2, "release_position")));
  ...
  ADD_SIGNAL(MethodInfo("scroll_offset_changed",
...
GraphEdit::GraphEdit() {
  ...
  top_layer->connect("gui_input",
                     callable_mp(this, &GraphEdit::_top_layer_input));

  connections_layer = memnew(Control);
  add_child(connections_layer);
  connections_layer->connect(
      "draw", callable_mp(this, &GraphEdit::_connections_layer_draw));
  connections_layer->set_name("CLAYER");
  connections_layer->set_disable_visibility_clip(
      true); // so it can draw freely and be offset
  ...
  h_scroll->connect("value_changed",
                    callable_mp(this, &GraphEdit::_scroll_moved));
  ...
  snap_amount->connect("value_changed",
                       callable_mp(this, &GraphEdit::_snap_value_changed));
