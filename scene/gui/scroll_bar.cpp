...
void ScrollBar::_gui_input(Ref<InputEvent> p_event) {
  ...
  if (b.is_valid()) {
    ...
    if (b->is_pressed()) {
      ...
      double ofs =
          orientation == VERTICAL ? b->get_position().y : b->get_position().x;
      Ref<Texture2D> decr = get_icon("decrement");
      Ref<Texture2D> incr = get_icon("increment");

      double decr_size =
          orientation == VERTICAL ? decr->get_height() : decr->get_width();
      double incr_size =
          orientation == VERTICAL ? incr->get_height() : incr->get_width();
      ...
      if (ofs < grabber_ofs) {
        ...
        if (scrolling) {
          target_scroll = CLAMP(target_scroll - get_page(), get_min(),
                                get_max() - get_page());
        } else {
          target_scroll = CLAMP(get_value() - get_page(), get_min(),
                                get_max() - get_page());
        }
  ...
  if (m.is_valid()) {
    ...
    if (drag.active) {
      ...
      double ofs =
          orientation == VERTICAL ? m->get_position().y : m->get_position().x;
      Ref<Texture2D> decr = get_icon("decrement");
...
void ScrollBar::_notification(int p_what) {
  ...
  if (p_what == NOTIFICATION_DRAW) {
    ...
    Ref<Texture2D> decr = highlight == HIGHLIGHT_DECR
                              ? get_icon("decrement_highlight")
                              : get_icon("decrement");
    Ref<Texture2D> incr = highlight == HIGHLIGHT_INCR
                              ? get_icon("increment_highlight")
                              : get_icon("increment");
    Ref<StyleBox> bg =
        has_focus() ? get_stylebox("scroll_focus") : get_stylebox("scroll");
  ...
  if (p_what == NOTIFICATION_ENTER_TREE) {
    ...
    if (drag_node) {
      drag_node->connect("gui_input",
                         callable_mp(this, &ScrollBar::_drag_node_input));
      drag_node->connect("tree_exiting",
                         callable_mp(this, &ScrollBar::_drag_node_exit),
                         varray(), CONNECT_ONESHOT);
    }
  }
  if (p_what == NOTIFICATION_EXIT_TREE) {

    if (drag_node) {
      drag_node->disconnect("gui_input",
                            callable_mp(this, &ScrollBar::_drag_node_input));
      drag_node->disconnect("tree_exiting",
                            callable_mp(this, &ScrollBar::_drag_node_exit));
    }
  ...
  if (p_what == NOTIFICATION_INTERNAL_PHYSICS_PROCESS) {
    ...
    if (scrolling) {
      ...
      if (drag_node_touching_deaccel) {
        ...
        Vector2 pos = Vector2(orientation == HORIZONTAL ? get_value() : 0,
...
Size2 ScrollBar::get_minimum_size() const {
  ...
  if (orientation == VERTICAL) {
    ...
    minsize.width = MAX(incr->get_size().width,
                        (bg->get_minimum_size() + bg->get_center_size()).width);
  ...
  if (orientation == HORIZONTAL) {
    ...
    minsize.height =
        MAX(incr->get_size().height,
            (bg->get_center_size() + bg->get_minimum_size()).height);
...
void ScrollBar::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("set_custom_step", "step"),
                       &ScrollBar::set_custom_step);
  ClassDB::bind_method(D_METHOD("get_custom_step"),
                       &ScrollBar::get_custom_step);

  ADD_SIGNAL(MethodInfo("scrolling"));

  ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "custom_step", PROPERTY_HINT_RANGE,
                            "-1,4096"),
               "set_custom_step", "get_custom_step");
}
