...
void Control::_edit_set_rotation(float p_rotation) { set_rotation(p_rotation); }

float Control::_edit_get_rotation() const { return get_rotation(); }
...
void Control::_edit_set_pivot(const Point2 &p_pivot) {
  ...
  Vector2 move = Vector2((cos(data.rotation) - 1.0) * delta_pivot.x -
...
Point2 Control::_edit_get_pivot() const { return get_pivot_offset(); }
  ...
  if (p_value.get_type() == Variant::NIL) {
    ...
    if (name.begins_with("custom_icons/")) {
      ...
      if (data.icon_override.has(dname)) {
        data.icon_override[dname]->disconnect(
            "changed", callable_mp(this, &Control::_override_changed));
      }
      ...
      if (data.shader_override.has(dname)) {
        data.shader_override[dname]->disconnect(
            "changed", callable_mp(this, &Control::_override_changed));
      }
      ...
      if (data.style_override.has(dname)) {
        data.style_override[dname]->disconnect(
            "changed", callable_mp(this, &Control::_override_changed));
      }
      ...
      if (data.font_override.has(dname)) {
        data.font_override[dname]->disconnect(
            "changed", callable_mp(this, &Control::_override_changed));
      }
...
void Control::_update_minimum_size() {
  ...
  if (minsize.x > data.size_cache.x || minsize.y > data.size_cache.y) {
    _size_changed();
  }
...
bool Control::_get(const StringName &p_name, Variant &r_ret) const {
  ...
  if (sname.begins_with("custom_icons/")) {
    ...
    r_ret = data.shader_override.has(name) ? Variant(data.shader_override[name])
                                           : Variant();
  } else if (sname.begins_with("custom_styles/")) {
    String name = sname.get_slicec('/', 1);

    r_ret = data.style_override.has(name) ? Variant(data.style_override[name])
                                          : Variant();
    ...
    String name = sname.get_slicec('/', 1);
    r_ret = data.color_override.has(name) ? Variant(data.color_override[name])
                                          : Variant();
...
void Control::_get_property_list(List<PropertyInfo> *p_list) const {
  ...
  /* Using the default theme since the properties below are meant for editor
  only if (data.theme.is_valid()) {
...

  {
    ...
    for (List<StringName>::Element *E = names.front(); E; E = E->next()) {
      ...
      p_list->push_back(
          PropertyInfo(Variant::OBJECT, "custom_icons/" + E->get(),
                       PROPERTY_HINT_RESOURCE_TYPE, "Texture2D", hint));
    }
  }
  {
    List<StringName> names;
    ...
    for (List<StringName>::Element *E = names.front(); E; E = E->next()) {
      ...
      p_list->push_back(PropertyInfo(
          Variant::OBJECT, "custom_shaders/" + E->get(),
          PROPERTY_HINT_RESOURCE_TYPE, "Shader,VisualShader", hint));
    }
  }
  {
    List<StringName> names;
    ...
    for (List<StringName>::Element *E = names.front(); E; E = E->next()) {
      ...
      p_list->push_back(
          PropertyInfo(Variant::OBJECT, "custom_styles/" + E->get(),
                       PROPERTY_HINT_RESOURCE_TYPE, "StyleBox", hint));
    }
  }
  {
    List<StringName> names;
    ...
    for (List<StringName>::Element *E = names.front(); E; E = E->next()) {
      ...
      p_list->push_back(
          PropertyInfo(Variant::OBJECT, "custom_fonts/" + E->get(),
                       PROPERTY_HINT_RESOURCE_TYPE, "Font", hint));
    }
  }
  {
    List<StringName> names;
    ...
    for (List<StringName>::Element *E = names.front(); E; E = E->next()) {
      ...
      p_list->push_back(PropertyInfo(Variant::COLOR,
                                     "custom_colors/" + E->get(),
                                     PROPERTY_HINT_NONE, "", hint));
    }
  }
  {
    List<StringName> names;
    ...
    for (List<StringName>::Element *E = names.front(); E; E = E->next()) {
      ...
      p_list->push_back(
          PropertyInfo(Variant::INT, "custom_constants/" + E->get(),
                       PROPERTY_HINT_RANGE, "-16384,16384", hint));
    }
  }
}

Control *Control::get_parent_control() const { return data.parent; }

void Control::_resize(const Size2 &p_size) { _size_changed(); }
...
void Control::add_child_notify(Node *p_child) {
  ...
  if (child_c->data.theme.is_null() && data.theme_owner) {
    _propagate_theme_changed(
        child_c, data.theme_owner); // need to propagate here, since many
                                    // controls may require setting up stuff
  }
}
...
void Control::_notification(int p_notification) {
  ...
  case NOTIFICATION_ENTER_CANVAS: {
    ...
    if (is_set_as_toplevel()) {
      ...
      if (data.parent_canvas_item) {
        ...
        data.parent_canvas_item->connect(
            "item_rect_changed", callable_mp(this, &Control::_size_changed));
      } else {
        // connect viewport
        get_viewport()->connect("size_changed",
                                callable_mp(this, &Control::_size_changed));
      }
    }
  ...
  case NOTIFICATION_EXIT_CANVAS: {
    ...
    if (data.parent_canvas_item) {
      ...
      data.parent_canvas_item->disconnect(
          "item_rect_changed", callable_mp(this, &Control::_size_changed));
      data.parent_canvas_item = NULL;
    } else if (!is_set_as_toplevel()) {
      // disconnect viewport
      get_viewport()->disconnect("size_changed",
                                 callable_mp(this, &Control::_size_changed));
    }
  ...
  case NOTIFICATION_MOVED_IN_PARENT: {
    // some parents need to know the order of the childrens to draw (like
  ...
  case NOTIFICATION_DRAW: {
    ...
    VisualServer::get_singleton()->canvas_item_set_custom_rect(
        get_canvas_item(), !data.disable_visibility_clip,
...
bool Control::has_point(const Point2 &p_point) const {
  ...
  if (get_script_instance()) {
    ...
    Variant ret = get_script_instance()->call(
        SceneStringNames::get_singleton()->has_point, &p, 1, ce);
...
Variant Control::get_drag_data(const Point2 &p_point) {
  ...
  if (get_script_instance()) {
    ...
    Callable::CallError ce;
    Variant ret = get_script_instance()->call(
        SceneStringNames::get_singleton()->get_drag_data, &p, 1, ce);
    if (ce.error == Callable::CallError::CALL_OK)
      return ret;
  }
...
bool Control::can_drop_data(const Point2 &p_point,
  ...
  if (get_script_instance()) {
    ...
    Callable::CallError ce;
    Variant ret = get_script_instance()->call(
        SceneStringNames::get_singleton()->can_drop_data, p, 2, ce);
    if (ce.error == Callable::CallError::CALL_OK)
      return ret;
  }
...
void Control::drop_data(const Point2 &p_point, const Variant &p_data) {
  ...
  if (get_script_instance()) {
    ...
    Callable::CallError ce;
    Variant ret = get_script_instance()->call(
        SceneStringNames::get_singleton()->drop_data, p, 2, ce);
    if (ce.error == Callable::CallError::CALL_OK)
      return;
  }
}
...
uint64_t Control::get_modal_frame() const { return data.modal_frame; }
...
Size2 Control::get_minimum_size() const {
  ...
  if (si) {
    ...
    Callable::CallError ce;
    Variant s = si->call(SceneStringNames::get_singleton()->_get_minimum_size,
                         NULL, 0, ce);
    if (ce.error == Callable::CallError::CALL_OK)
      return s;
  }
...
Ref<Texture2D> Control::get_icon(const StringName &p_name,
                                 const StringName &p_type) const {
...
Ref<Shader> Control::get_shader(const StringName &p_name,
                                const StringName &p_type) const {
  if (p_type == StringName() || p_type == get_class_name()) {
...
Ref<StyleBox> Control::get_stylebox(const StringName &p_name,
                                    const StringName &p_type) const {
...
Ref<Font> Control::get_font(const StringName &p_name,
                            const StringName &p_type) const {
...
Color Control::get_color(const StringName &p_name,
                         const StringName &p_type) const {
...
int Control::get_constant(const StringName &p_name,
                          const StringName &p_type) const {
...
bool Control::has_icon(const StringName &p_name,
                       const StringName &p_type) const {
...
bool Control::has_shader(const StringName &p_name,
                         const StringName &p_type) const {
...
bool Control::has_stylebox(const StringName &p_name,
                           const StringName &p_type) const {
...
bool Control::has_font(const StringName &p_name,
                       const StringName &p_type) const {
...
bool Control::has_color(const StringName &p_name,
                        const StringName &p_type) const {
...
bool Control::has_constant(const StringName &p_name,
                           const StringName &p_type) const {
...
void Control::set_anchor_and_margin(Margin p_margin, float p_anchor,
                                    float p_pos, bool p_push_opposite_anchor) {
...
void Control::set_margins_preset(LayoutPreset p_preset,
                                 LayoutPresetMode p_resize_mode, int p_margin) {
  ...
  case PRESET_HCENTER_WIDE:
  case PRESET_WIDE:
    data.margin[0] = parent_rect.size.x * (0.0 - data.anchor[0]) + p_margin +
  ...
  case PRESET_CENTER_RIGHT:
  case PRESET_RIGHT_WIDE:
    data.margin[0] = parent_rect.size.x * (1.0 - data.anchor[0]) - new_size.x -
  ...
  case PRESET_VCENTER_WIDE:
  case PRESET_WIDE:
    data.margin[1] = parent_rect.size.y * (0.0 - data.anchor[1]) + p_margin +
  ...
  case PRESET_CENTER_BOTTOM:
  case PRESET_BOTTOM_WIDE:
    data.margin[1] = parent_rect.size.y * (1.0 - data.anchor[1]) - new_size.y -
  ...
  case PRESET_CENTER_LEFT:
  case PRESET_LEFT_WIDE:
    data.margin[2] = parent_rect.size.x * (0.0 - data.anchor[2]) + new_size.x +
                     p_margin + parent_rect.position.x;
    break;
  ...
  case PRESET_HCENTER_WIDE:
  case PRESET_WIDE:
    data.margin[2] = parent_rect.size.x * (1.0 - data.anchor[2]) - p_margin +
                     parent_rect.position.x;
    break;
  }
  ...
  case PRESET_CENTER_TOP:
  case PRESET_TOP_WIDE:
    data.margin[3] = parent_rect.size.y * (0.0 - data.anchor[3]) + new_size.y +
                     p_margin + parent_rect.position.y;
    break;
  ...
  case PRESET_VCENTER_WIDE:
  case PRESET_WIDE:
    data.margin[3] = parent_rect.size.y * (1.0 - data.anchor[3]) - p_margin +
                     parent_rect.position.y;
    break;
  }
...
void Control::set_anchors_and_margins_preset(LayoutPreset p_preset,
...
Size2 Control::get_end() const { return Size2(data.margin[2], data.margin[3]); }
...
void Control::_compute_anchors(Rect2 p_rect, const float p_margins[4],
                               float (&r_anchors)[4]) {
...
void Control::_compute_margins(Rect2 p_rect, const float p_anchors[4],
                               float (&r_margins)[4]) {
...
void Control::_set_position(const Size2 &p_point) { set_position(p_point); }
...
void Control::_set_size(const Size2 &p_size) { set_size(p_size); }
...
Size2 Control::get_position() const { return data.pos_cache; }

Size2 Control::get_size() const { return data.size_cache; }
...
Rect2 Control::get_rect() const { return Rect2(get_position(), get_size()); }
...
void Control::add_icon_override(const StringName &p_name,
                                const Ref<Texture2D> &p_icon) {

  if (data.icon_override.has(p_name)) {
    data.icon_override[p_name]->disconnect(
        "changed", callable_mp(this, &Control::_override_changed));
  }
  ...
  if (p_icon.is_null()) {
    ...
    if (data.icon_override[p_name].is_valid()) {
      data.icon_override[p_name]->connect(
          "changed", callable_mp(this, &Control::_override_changed),
          Vector<Variant>(), CONNECT_REFERENCE_COUNTED);
    }
  }
  notification(NOTIFICATION_THEME_CHANGED);
}

void Control::add_shader_override(const StringName &p_name,
                                  const Ref<Shader> &p_shader) {

  if (data.shader_override.has(p_name)) {
    data.shader_override[p_name]->disconnect(
        "changed", callable_mp(this, &Control::_override_changed));
  }
...
void Control::add_style_override(const StringName &p_name,
                                 const Ref<StyleBox> &p_style) {

  if (data.style_override.has(p_name)) {
    data.style_override[p_name]->disconnect(
        "changed", callable_mp(this, &Control::_override_changed));
  }
...
void Control::add_font_override(const StringName &p_name,
                                const Ref<Font> &p_font) {

  if (data.font_override.has(p_name)) {
    data.font_override[p_name]->disconnect(
        "changed", callable_mp(this, &Control::_override_changed));
  }
...
void Control::add_color_override(const StringName &p_name,
                                 const Color &p_color) {
...
Control::FocusMode Control::get_focus_mode() const { return data.focus_mode; }
...
void Control::grab_focus() {
  ...
  if (data.focus_mode == FOCUS_NONE) {
    WARN_PRINT("This control can't grab focus. Use set_focus_mode() to allow a "
...
void Control::_modal_stack_remove() {
  ...
  get_viewport()->_gui_remove_from_modal_stack(element,
...
void Control::_theme_changed() { _propagate_theme_changed(this, this, false); }
...
void Control::set_theme(const Ref<Theme> &p_theme) {
  ...
  if (data.theme.is_valid()) {
    data.theme->connect("changed", callable_mp(this, &Control::_theme_changed),
...
Ref<Theme> Control::get_theme() const { return data.theme; }
...
String Control::get_tooltip(const Point2 &p_pos) const { return data.tooltip; }
...
String Control::_get_tooltip() const { return data.tooltip; }

void Control::set_focus_neighbour(Margin p_margin,
...
NodePath Control::get_focus_next() const { return data.focus_next; }
...
NodePath Control::get_focus_previous() const { return data.focus_prev; }
...
Control *Control::_get_focus_neighbour(Margin p_margin, int p_count) {
  ...
  const Vector2 dir[4] = {Vector2(-1, 0), Vector2(0, -1), Vector2(1, 0),
                          Vector2(0, 1)};
...
void Control::_window_find_focus_neighbour(const Vector2 &p_dir, Node *p_at,
                                           const Point2 *p_points, float p_min,
    ...
    if (min > (p_min - CMP_EPSILON)) {
      ...
      for (int i = 0; i < 4; i++) {
        ...
        for (int j = 0; j < 4; j++) {
          ...
          Vector2 pa, pb;
          float d = Geometry::get_closest_points_between_segments(la, lb, fa,
                                                                  fb, pa, pb);
          // float d =
  ...
  for (int i = 0; i < p_at->get_child_count(); i++) {
    ...
    _window_find_focus_neighbour(p_dir, p_at->get_child(i), p_points, p_min,
                                 r_closest_dist, r_closest);
  }
}
...
int Control::get_h_size_flags() const { return data.h_size_flags; }
...
float Control::get_stretch_ratio() const { return data.expand; }
...
int Control::get_v_size_flags() const { return data.v_size_flags; }
...
float Control::get_rotation() const { return data.rotation; }
...
Vector2 Control::get_pivot_offset() const { return data.pivot_offset; }
...
Vector2 Control::get_scale() const { return data.scale; }
...
void Control::get_argument_options(const StringName &p_function, int p_idx,
...
String Control::get_configuration_warning() const {
  ...
  if (data.mouse_filter == MOUSE_FILTER_IGNORE && data.tooltip != "") {
    ...
    warning += TTR("The Hint Tooltip won't be displayed as the control's Mouse "
...
bool Control::is_clipping_contents() { return data.clip_contents; }
...
void Control::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("_update_minimum_size"),
                       &Control::_update_minimum_size);

  ClassDB::bind_method(D_METHOD("accept_event"), &Control::accept_event);
  ClassDB::bind_method(D_METHOD("get_minimum_size"),
                       &Control::get_minimum_size);
  ClassDB::bind_method(D_METHOD("get_combined_minimum_size"),
                       &Control::get_combined_minimum_size);
  ClassDB::bind_method(D_METHOD("set_anchors_preset", "preset", "keep_margins"),
                       &Control::set_anchors_preset, DEFVAL(false));
  ClassDB::bind_method(
      D_METHOD("set_margins_preset", "preset", "resize_mode", "margin"),
      &Control::set_margins_preset, DEFVAL(PRESET_MODE_MINSIZE), DEFVAL(0));
  ClassDB::bind_method(D_METHOD("set_anchors_and_margins_preset", "preset",
                                "resize_mode", "margin"),
                       &Control::set_anchors_and_margins_preset,
                       DEFVAL(PRESET_MODE_MINSIZE), DEFVAL(0));
  ClassDB::bind_method(D_METHOD("_set_anchor", "margin", "anchor"),
                       &Control::_set_anchor);
  ClassDB::bind_method(D_METHOD("set_anchor", "margin", "anchor", "keep_margin",
                                "push_opposite_anchor"),
                       &Control::set_anchor, DEFVAL(false), DEFVAL(true));
  ClassDB::bind_method(D_METHOD("get_anchor", "margin"), &Control::get_anchor);
  ClassDB::bind_method(D_METHOD("set_margin", "margin", "offset"),
                       &Control::set_margin);
  ClassDB::bind_method(D_METHOD("set_anchor_and_margin", "margin", "anchor",
                                "offset", "push_opposite_anchor"),
                       &Control::set_anchor_and_margin, DEFVAL(false));
  ...
  ClassDB::bind_method(D_METHOD("set_position", "position", "keep_margins"),
                       &Control::set_position, DEFVAL(false));
  ClassDB::bind_method(D_METHOD("_set_position", "margin"),
                       &Control::_set_position);
  ClassDB::bind_method(D_METHOD("set_size", "size", "keep_margins"),
                       &Control::set_size, DEFVAL(false));
  ClassDB::bind_method(D_METHOD("_set_size", "size"), &Control::_set_size);
  ClassDB::bind_method(D_METHOD("set_custom_minimum_size", "size"),
                       &Control::set_custom_minimum_size);
  ClassDB::bind_method(
      D_METHOD("set_global_position", "position", "keep_margins"),
      &Control::set_global_position, DEFVAL(false));
  ClassDB::bind_method(D_METHOD("_set_global_position", "position"),
                       &Control::_set_global_position);
  ClassDB::bind_method(D_METHOD("set_rotation", "radians"),
                       &Control::set_rotation);
  ClassDB::bind_method(D_METHOD("set_rotation_degrees", "degrees"),
                       &Control::set_rotation_degrees);
  ClassDB::bind_method(D_METHOD("set_scale", "scale"), &Control::set_scale);
  ClassDB::bind_method(D_METHOD("set_pivot_offset", "pivot_offset"),
  ...
  ClassDB::bind_method(D_METHOD("get_rotation_degrees"),
                       &Control::get_rotation_degrees);
  ClassDB::bind_method(D_METHOD("get_scale"), &Control::get_scale);
  ClassDB::bind_method(D_METHOD("get_pivot_offset"),
                       &Control::get_pivot_offset);
  ClassDB::bind_method(D_METHOD("get_custom_minimum_size"),
                       &Control::get_custom_minimum_size);
  ClassDB::bind_method(D_METHOD("get_parent_area_size"),
  ...
  ClassDB::bind_method(D_METHOD("show_modal", "exclusive"),
                       &Control::show_modal, DEFVAL(false));
  ClassDB::bind_method(D_METHOD("set_focus_mode", "mode"),
  ...
  ClassDB::bind_method(D_METHOD("set_h_size_flags", "flags"),
                       &Control::set_h_size_flags);
  ClassDB::bind_method(D_METHOD("get_h_size_flags"),
                       &Control::get_h_size_flags);

  ClassDB::bind_method(D_METHOD("set_stretch_ratio", "ratio"),
                       &Control::set_stretch_ratio);
  ClassDB::bind_method(D_METHOD("get_stretch_ratio"),
                       &Control::get_stretch_ratio);

  ClassDB::bind_method(D_METHOD("set_v_size_flags", "flags"),
                       &Control::set_v_size_flags);
  ClassDB::bind_method(D_METHOD("get_v_size_flags"),
  ...
  ClassDB::bind_method(D_METHOD("add_icon_override", "name", "texture"),
                       &Control::add_icon_override);
  ClassDB::bind_method(D_METHOD("add_shader_override", "name", "shader"),
                       &Control::add_shader_override);
  ClassDB::bind_method(D_METHOD("add_stylebox_override", "name", "stylebox"),
                       &Control::add_style_override);
  ClassDB::bind_method(D_METHOD("add_font_override", "name", "font"),
                       &Control::add_font_override);
  ClassDB::bind_method(D_METHOD("add_color_override", "name", "color"),
                       &Control::add_color_override);
  ClassDB::bind_method(D_METHOD("add_constant_override", "name", "constant"),
                       &Control::add_constant_override);

  ClassDB::bind_method(D_METHOD("get_icon", "name", "type"), &Control::get_icon,
                       DEFVAL(""));
  ClassDB::bind_method(D_METHOD("get_stylebox", "name", "type"),
                       &Control::get_stylebox, DEFVAL(""));
  ClassDB::bind_method(D_METHOD("get_font", "name", "type"), &Control::get_font,
                       DEFVAL(""));
  ClassDB::bind_method(D_METHOD("get_color", "name", "type"),
                       &Control::get_color, DEFVAL(""));
  ClassDB::bind_method(D_METHOD("get_constant", "name", "type"),
                       &Control::get_constant, DEFVAL(""));

  ClassDB::bind_method(D_METHOD("has_icon_override", "name"),
                       &Control::has_icon_override);
  ClassDB::bind_method(D_METHOD("has_shader_override", "name"),
                       &Control::has_shader_override);
  ClassDB::bind_method(D_METHOD("has_stylebox_override", "name"),
                       &Control::has_stylebox_override);
  ClassDB::bind_method(D_METHOD("has_font_override", "name"),
                       &Control::has_font_override);
  ClassDB::bind_method(D_METHOD("has_color_override", "name"),
                       &Control::has_color_override);
  ClassDB::bind_method(D_METHOD("has_constant_override", "name"),
                       &Control::has_constant_override);

  ClassDB::bind_method(D_METHOD("has_icon", "name", "type"), &Control::has_icon,
                       DEFVAL(""));
  ClassDB::bind_method(D_METHOD("has_stylebox", "name", "type"),
                       &Control::has_stylebox, DEFVAL(""));
  ClassDB::bind_method(D_METHOD("has_font", "name", "type"), &Control::has_font,
                       DEFVAL(""));
  ClassDB::bind_method(D_METHOD("has_color", "name", "type"),
                       &Control::has_color, DEFVAL(""));
  ClassDB::bind_method(D_METHOD("has_constant", "name", "type"),
                       &Control::has_constant, DEFVAL(""));

  ClassDB::bind_method(D_METHOD("get_parent_control"),
                       &Control::get_parent_control);

  ClassDB::bind_method(D_METHOD("set_h_grow_direction", "direction"),
                       &Control::set_h_grow_direction);
  ClassDB::bind_method(D_METHOD("get_h_grow_direction"),
                       &Control::get_h_grow_direction);

  ClassDB::bind_method(D_METHOD("set_v_grow_direction", "direction"),
                       &Control::set_v_grow_direction);
  ClassDB::bind_method(D_METHOD("get_v_grow_direction"),
                       &Control::get_v_grow_direction);

  ClassDB::bind_method(D_METHOD("set_tooltip", "tooltip"),
                       &Control::set_tooltip);
  ClassDB::bind_method(D_METHOD("get_tooltip", "at_position"),
                       &Control::get_tooltip, DEFVAL(Point2()));
  ClassDB::bind_method(D_METHOD("_get_tooltip"), &Control::_get_tooltip);

  ClassDB::bind_method(D_METHOD("set_default_cursor_shape", "shape"),
                       &Control::set_default_cursor_shape);
  ClassDB::bind_method(D_METHOD("get_default_cursor_shape"),
                       &Control::get_default_cursor_shape);
  ClassDB::bind_method(D_METHOD("get_cursor_shape", "position"),
                       &Control::get_cursor_shape, DEFVAL(Point2()));

  ClassDB::bind_method(D_METHOD("set_focus_neighbour", "margin", "neighbour"),
                       &Control::set_focus_neighbour);
  ClassDB::bind_method(D_METHOD("get_focus_neighbour", "margin"),
                       &Control::get_focus_neighbour);

  ClassDB::bind_method(D_METHOD("set_focus_next", "next"),
                       &Control::set_focus_next);
  ClassDB::bind_method(D_METHOD("get_focus_next"), &Control::get_focus_next);

  ClassDB::bind_method(D_METHOD("set_focus_previous", "previous"),
                       &Control::set_focus_previous);
  ClassDB::bind_method(D_METHOD("get_focus_previous"),
                       &Control::get_focus_previous);

  ClassDB::bind_method(D_METHOD("force_drag", "data", "preview"),
                       &Control::force_drag);

  ClassDB::bind_method(D_METHOD("set_mouse_filter", "filter"),
                       &Control::set_mouse_filter);
  ClassDB::bind_method(D_METHOD("get_mouse_filter"),
                       &Control::get_mouse_filter);

  ClassDB::bind_method(D_METHOD("set_clip_contents", "enable"),
                       &Control::set_clip_contents);
  ClassDB::bind_method(D_METHOD("is_clipping_contents"),
                       &Control::is_clipping_contents);

  ClassDB::bind_method(D_METHOD("grab_click_focus"),
                       &Control::grab_click_focus);

  ClassDB::bind_method(D_METHOD("set_drag_forwarding", "target"),
                       &Control::set_drag_forwarding);
  ClassDB::bind_method(D_METHOD("set_drag_preview", "control"),
                       &Control::set_drag_preview);

  ClassDB::bind_method(D_METHOD("warp_mouse", "to_position"),
                       &Control::warp_mouse);

  ClassDB::bind_method(D_METHOD("minimum_size_changed"),
                       &Control::minimum_size_changed);

  BIND_VMETHOD(MethodInfo(
      "_gui_input", PropertyInfo(Variant::OBJECT, "event",
                                 PROPERTY_HINT_RESOURCE_TYPE, "InputEvent")));
  ...
  BIND_VMETHOD(MethodInfo(Variant::BOOL, "can_drop_data",
                          PropertyInfo(Variant::VECTOR2, "position"),
                          PropertyInfo(Variant::NIL, "data")));
  BIND_VMETHOD(MethodInfo("drop_data",
                          PropertyInfo(Variant::VECTOR2, "position"),
                          PropertyInfo(Variant::NIL, "data")));
  BIND_VMETHOD(MethodInfo(Variant::OBJECT, "_make_custom_tooltip",
                          PropertyInfo(Variant::STRING, "for_text")));
  BIND_VMETHOD(MethodInfo(Variant::BOOL, "_clips_input"));

  ADD_GROUP("Anchor", "anchor_");
  ADD_PROPERTYI(PropertyInfo(Variant::FLOAT, "anchor_left", PROPERTY_HINT_RANGE,
                             "0,1,0.001,or_lesser,or_greater"),
                "_set_anchor", "get_anchor", MARGIN_LEFT);
  ADD_PROPERTYI(PropertyInfo(Variant::FLOAT, "anchor_top", PROPERTY_HINT_RANGE,
                             "0,1,0.001,or_lesser,or_greater"),
                "_set_anchor", "get_anchor", MARGIN_TOP);
  ADD_PROPERTYI(PropertyInfo(Variant::FLOAT, "anchor_right",
                             PROPERTY_HINT_RANGE,
                             "0,1,0.001,or_lesser,or_greater"),
                "_set_anchor", "get_anchor", MARGIN_RIGHT);
  ADD_PROPERTYI(PropertyInfo(Variant::FLOAT, "anchor_bottom",
                             PROPERTY_HINT_RANGE,
                             "0,1,0.001,or_lesser,or_greater"),
                "_set_anchor", "get_anchor", MARGIN_BOTTOM);

  ADD_GROUP("Margin", "margin_");
  ADD_PROPERTYI(PropertyInfo(Variant::INT, "margin_left", PROPERTY_HINT_RANGE,
                             "-4096,4096"),
                "set_margin", "get_margin", MARGIN_LEFT);
  ADD_PROPERTYI(PropertyInfo(Variant::INT, "margin_top", PROPERTY_HINT_RANGE,
                             "-4096,4096"),
                "set_margin", "get_margin", MARGIN_TOP);
  ADD_PROPERTYI(PropertyInfo(Variant::INT, "margin_right", PROPERTY_HINT_RANGE,
                             "-4096,4096"),
                "set_margin", "get_margin", MARGIN_RIGHT);
  ADD_PROPERTYI(PropertyInfo(Variant::INT, "margin_bottom", PROPERTY_HINT_RANGE,
                             "-4096,4096"),
                "set_margin", "get_margin", MARGIN_BOTTOM);

  ADD_GROUP("Grow Direction", "grow_");
  ADD_PROPERTY(PropertyInfo(Variant::INT, "grow_horizontal", PROPERTY_HINT_ENUM,
                            "Begin,End,Both"),
               "set_h_grow_direction", "get_h_grow_direction");
  ADD_PROPERTY(PropertyInfo(Variant::INT, "grow_vertical", PROPERTY_HINT_ENUM,
                            "Begin,End,Both"),
  ...
  ADD_PROPERTY(PropertyInfo(Variant::VECTOR2, "rect_global_position",
                            PROPERTY_HINT_NONE, "", 0),
               "_set_global_position", "get_global_position");
  ADD_PROPERTY(PropertyInfo(Variant::VECTOR2, "rect_size", PROPERTY_HINT_NONE,
                            "", PROPERTY_USAGE_EDITOR),
               "_set_size", "get_size");
  ADD_PROPERTY(PropertyInfo(Variant::VECTOR2, "rect_min_size"),
               "set_custom_minimum_size", "get_custom_minimum_size");
  ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "rect_rotation",
                            PROPERTY_HINT_RANGE,
                            "-360,360,0.1,or_lesser,or_greater"),
               "set_rotation_degrees", "get_rotation_degrees");
  ADD_PROPERTY(PropertyInfo(Variant::VECTOR2, "rect_scale"), "set_scale",
               "get_scale");
  ADD_PROPERTY(PropertyInfo(Variant::VECTOR2, "rect_pivot_offset"),
               "set_pivot_offset", "get_pivot_offset");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "rect_clip_content"),
               "set_clip_contents", "is_clipping_contents");

  ADD_GROUP("Hint", "hint_");
  ADD_PROPERTY(PropertyInfo(Variant::STRING, "hint_tooltip",
                            PROPERTY_HINT_MULTILINE_TEXT),
               "set_tooltip", "_get_tooltip");

  ADD_GROUP("Focus", "focus_");
  ADD_PROPERTYI(PropertyInfo(Variant::NODE_PATH, "focus_neighbour_left",
                             PROPERTY_HINT_NODE_PATH_VALID_TYPES, "Control"),
                "set_focus_neighbour", "get_focus_neighbour", MARGIN_LEFT);
  ADD_PROPERTYI(PropertyInfo(Variant::NODE_PATH, "focus_neighbour_top",
                             PROPERTY_HINT_NODE_PATH_VALID_TYPES, "Control"),
                "set_focus_neighbour", "get_focus_neighbour", MARGIN_TOP);
  ADD_PROPERTYI(PropertyInfo(Variant::NODE_PATH, "focus_neighbour_right",
                             PROPERTY_HINT_NODE_PATH_VALID_TYPES, "Control"),
                "set_focus_neighbour", "get_focus_neighbour", MARGIN_RIGHT);
  ADD_PROPERTYI(PropertyInfo(Variant::NODE_PATH, "focus_neighbour_bottom",
                             PROPERTY_HINT_NODE_PATH_VALID_TYPES, "Control"),
                "set_focus_neighbour", "get_focus_neighbour", MARGIN_BOTTOM);
  ADD_PROPERTY(PropertyInfo(Variant::NODE_PATH, "focus_next",
                            PROPERTY_HINT_NODE_PATH_VALID_TYPES, "Control"),
               "set_focus_next", "get_focus_next");
  ADD_PROPERTY(PropertyInfo(Variant::NODE_PATH, "focus_previous",
                            PROPERTY_HINT_NODE_PATH_VALID_TYPES, "Control"),
               "set_focus_previous", "get_focus_previous");
  ADD_PROPERTY(PropertyInfo(Variant::INT, "focus_mode", PROPERTY_HINT_ENUM,
                            "None,Click,All"),
               "set_focus_mode", "get_focus_mode");

  ADD_GROUP("Mouse", "mouse_");
  ADD_PROPERTY(PropertyInfo(Variant::INT, "mouse_filter", PROPERTY_HINT_ENUM,
                            "Stop,Pass,Ignore"),
  ...
  ADD_PROPERTY(PropertyInfo(Variant::INT, "size_flags_horizontal",
                            PROPERTY_HINT_FLAGS,
                            "Fill,Expand,Shrink Center,Shrink End"),
               "set_h_size_flags", "get_h_size_flags");
  ADD_PROPERTY(PropertyInfo(Variant::INT, "size_flags_vertical",
                            PROPERTY_HINT_FLAGS,
                            "Fill,Expand,Shrink Center,Shrink End"),
               "set_v_size_flags", "get_v_size_flags");
  ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "size_flags_stretch_ratio",
                            PROPERTY_HINT_RANGE, "0,128,0.01"),
               "set_stretch_ratio", "get_stretch_ratio");
  ADD_GROUP("Theme", "");
  ADD_PROPERTY(PropertyInfo(Variant::OBJECT, "theme",
                            PROPERTY_HINT_RESOURCE_TYPE, "Theme"),
               "set_theme", "get_theme");
  ADD_GROUP("", "");
  ...
  ADD_SIGNAL(MethodInfo("gui_input", PropertyInfo(Variant::OBJECT, "event",
  ...
  BIND_VMETHOD(MethodInfo(Variant::BOOL, "has_point",
