...
void ColorPicker::_notification(int p_what) {
  ...
  case NOTIFICATION_ENTER_TREE: {
    ...
    if (Engine::get_singleton()->is_editor_hint()) {
      PackedColorArray saved_presets =
          EditorSettings::get_singleton()->get_project_metadata(
              "color_picker", "presets", PackedColorArray());
  ...
  case NOTIFICATION_VISIBILITY_CHANGED: {
    ...
    if (p)
      p->set_size(Size2(
          get_combined_minimum_size().width + get_constant("margin") * 2,
          get_combined_minimum_size().height + get_constant("margin") * 2));
  } break;
...
void ColorPicker::_value_changed(double) {
  ...
  if (hsv_mode_enabled) {
    color.set_hsv(
        scroll[0]->get_value() / 360.0, scroll[1]->get_value() / 100.0,
        scroll[2]->get_value() / 100.0, scroll[3]->get_value() / 255.0);
  } else {
    for (int i = 0; i < 4; i++) {
      color.components[i] =
...
void ColorPicker::add_preset(const Color &p_color) {
  ...
  if (Engine::get_singleton()->is_editor_hint()) {
    ...
    EditorSettings::get_singleton()->set_project_metadata(
        "color_picker", "presets", arr_to_save);
  }
#endif
}
...
bool ColorPicker::is_hsv_mode() const { return hsv_mode_enabled; }
...
bool ColorPicker::is_raw_mode() const { return raw_mode_enabled; }
...
void ColorPicker::_sample_draw() {
  const Rect2 r = Rect2(Point2(), Size2(uv_edit->get_size().width,
  ...
  if (color.r > 1 || color.g > 1 || color.b > 1) {
    // Draw an indicator to denote that the color is "overbright" and can't be
    // displayed accurately in the preview
    sample->draw_texture(get_icon("overbright_indicator", "ColorPicker"),
                         Point2());
  }
}
...
void ColorPicker::_uv_input(const Ref<InputEvent> &p_event) {
  ...
  if (bev.is_valid()) {
    if (bev->is_pressed() && bev->get_button_index() == BUTTON_LEFT) {
      changing_color = true;
      float x =
          CLAMP((float)bev->get_position().x, 0, uv_edit->get_size().width);
      float y =
          CLAMP((float)bev->get_position().y, 0, uv_edit->get_size().height);
...
void ColorPicker::_w_input(const Ref<InputEvent> &p_event) {
  ...
  if (bev.is_valid()) {
    ...
    if (bev->is_pressed() && bev->get_button_index() == BUTTON_LEFT) {
      changing_color = true;
      float y =
          CLAMP((float)bev->get_position().y, 0, w_edit->get_size().height);
...
void ColorPicker::_screen_input(const Ref<InputEvent> &p_event) {
  ...
  if (mev.is_valid()) {
    ...
    if (!r->get_visible_rect().has_point(
            Point2(mev->get_global_position().x, mev->get_global_position().y)))
      return;
    ...
    if (img.is_valid() && !img->empty()) {
      ...
      Vector2 ofs =
          mev->get_global_position() - r->get_visible_rect().get_position();
      Color c =
          img->get_pixel(ofs.x, r->get_visible_rect().size.height - ofs.y);
...
void ColorPicker::_screen_pick_pressed() {
  ...
  if (!screen) {
    ...
    screen->connect("gui_input",
                    callable_mp(this, &ColorPicker::_screen_input));
    // It immediately toggles off in the first press otherwise.
    screen->call_deferred("connect", "hide", Callable(btn_pick, "set_pressed"),
                          varray(false));
  }
  screen->raise();
  screen->show_modal();
...
bool ColorPicker::are_presets_enabled() const { return presets_enabled; }
...
void ColorPicker::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("set_pick_color", "color"),
                       &ColorPicker::set_pick_color);
  ClassDB::bind_method(D_METHOD("get_pick_color"),
  ...
  ClassDB::bind_method(D_METHOD("is_deferred_mode"),
                       &ColorPicker::is_deferred_mode);
  ClassDB::bind_method(D_METHOD("set_edit_alpha", "show"),
                       &ColorPicker::set_edit_alpha);
  ClassDB::bind_method(D_METHOD("is_editing_alpha"),
                       &ColorPicker::is_editing_alpha);
  ClassDB::bind_method(D_METHOD("set_presets_enabled", "enabled"),
                       &ColorPicker::set_presets_enabled);
  ClassDB::bind_method(D_METHOD("are_presets_enabled"),
                       &ColorPicker::are_presets_enabled);
  ClassDB::bind_method(D_METHOD("set_presets_visible", "visible"),
                       &ColorPicker::set_presets_visible);
  ClassDB::bind_method(D_METHOD("are_presets_visible"),
                       &ColorPicker::are_presets_visible);
  ClassDB::bind_method(D_METHOD("add_preset", "color"),
                       &ColorPicker::add_preset);
  ClassDB::bind_method(D_METHOD("erase_preset", "color"),
                       &ColorPicker::erase_preset);
  ClassDB::bind_method(D_METHOD("get_presets"), &ColorPicker::get_presets);

  ADD_PROPERTY(PropertyInfo(Variant::COLOR, "color"), "set_pick_color",
               "get_pick_color");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "edit_alpha"), "set_edit_alpha",
               "is_editing_alpha");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "hsv_mode"), "set_hsv_mode",
               "is_hsv_mode");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "raw_mode"), "set_raw_mode",
               "is_raw_mode");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "deferred_mode"),
               "set_deferred_mode", "is_deferred_mode");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "presets_enabled"),
               "set_presets_enabled", "are_presets_enabled");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "presets_visible"),
               "set_presets_visible", "are_presets_visible");
...
ColorPicker::ColorPicker() : BoxContainer(true) {
  ...
  uv_edit->connect("draw", callable_mp(this, &ColorPicker::_hsv_draw),
                   make_binds(0, uv_edit));
  ...
  w_edit->connect("draw", callable_mp(this, &ColorPicker::_hsv_draw),
                  make_binds(1, w_edit));
  ...
  btn_pick->connect("pressed",
                    callable_mp(this, &ColorPicker::_screen_pick_pressed));
  ...
  for (int i = 0; i < 4; i++) {
    ...
    values[i]->get_line_edit()->connect(
        "focus_entered", callable_mp(this, &ColorPicker::_focus_enter));
    values[i]->get_line_edit()->connect(
        "focus_exited", callable_mp(this, &ColorPicker::_focus_exit));

    scroll[i]->set_min(0);
    scroll[i]->set_page(0);
    scroll[i]->set_h_size_flags(SIZE_EXPAND_FILL);

    scroll[i]->connect("value_changed",
                       callable_mp(this, &ColorPicker::_value_changed));
...
#ifdef TOOLS_ENABLED
    text_type->set_custom_minimum_size(
        Size2(28 * EDSCALE, 0)); // Adjust for the width of the "Script" icon.
#endif
    text_type->connect("pressed",
                       callable_mp(this, &ColorPicker::_text_type_toggled));
  } else {
  ...
  c_text->connect("text_entered",
                  callable_mp(this, &ColorPicker::_html_entered));
  c_text->connect("focus_entered",
                  callable_mp(this, &ColorPicker::_focus_enter));
  c_text->connect("focus_exited",
                  callable_mp(this, &ColorPicker::_html_focus_exit));
  ...
  bt_add_preset->connect("pressed",
                         callable_mp(this, &ColorPicker::_add_preset_pressed));
}
...
void ColorPickerButton::_modal_closed() { emit_signal("popup_closed"); }

void ColorPickerButton::pressed() {

  _update_picker();
  popup->set_position(get_global_position() -
...
void ColorPickerButton::_notification(int p_what) {
  ...
  case NOTIFICATION_DRAW: {
    ...
    const Rect2 r =
        Rect2(normal->get_offset(), get_size() - normal->get_minimum_size());
    ...
    if (color.r > 1 || color.g > 1 || color.b > 1) {
      // Draw an indicator to denote that the color is "overbright" and can't be
      // displayed accurately in the preview
      draw_texture(Control::get_icon("overbright_indicator", "ColorPicker"),
                   normal->get_offset());
    }
  } break;
  case MainLoop::NOTIFICATION_WM_QUIT_REQUEST: {
...
void ColorPickerButton::_update_picker() {
  if (!picker) {
    ...
    picker->connect("color_changed",
                    callable_mp(this, &ColorPickerButton::_color_changed));
    popup->connect("modal_closed",
                   callable_mp(this, &ColorPickerButton::_modal_closed));
    popup->connect("about_to_show",
                   callable_mp((BaseButton *)this, &BaseButton::set_pressed),
                   varray(true));
