...
void Slider::_gui_input(Ref<InputEvent> p_event) {
  ...
  if (mb.is_valid()) {
    if (mb->get_button_index() == BUTTON_LEFT) {
      ...
      if (mb->is_pressed()) {
        Ref<Texture2D> grabber = get_icon(
            mouse_inside || has_focus() ? "grabber_highlight" : "grabber");
  ...
  if (mm.is_valid()) {
    if (grab.active) {
      ...
      if (orientation == VERTICAL)
        motion = -motion;
      float areasize = orientation == VERTICAL
                           ? size.height - grabber->get_size().height
                           : size.width - grabber->get_size().width;
      if (areasize <= 0)
        return;
...
void Slider::_notification(int p_what) {
  ...
  case NOTIFICATION_DRAW: {
    ...
    Ref<Texture2D> grabber =
        get_icon(editable ? ((mouse_inside || has_focus()) ? "grabber_highlight"
                                                           : "grabber")
                          : "grabber_disabled");
    ...
    if (orientation == VERTICAL) {
      ...
      int widget_width =
          style->get_minimum_size().width + style->get_center_size().width;
      float areasize = size.height - grabber->get_size().height;
      style->draw(ci, Rect2i(Point2i(size.width / 2 - widget_width / 2, 0),
                             Size2i(widget_width, size.height)));
      grabber_area->draw(
          ci, Rect2i(Point2i((size.width - widget_width) / 2,
                             size.height - areasize * ratio -
                                 grabber->get_size().height / 2),
                     Size2i(widget_width,
                            areasize * ratio + grabber->get_size().width / 2)));

      if (ticks > 1) {
        int grabber_offset =
            (grabber->get_size().height / 2 - tick->get_height() / 2);
      ...
      grabber->draw(ci, Point2i(size.width / 2 - grabber->get_size().width / 2,
                                size.height - ratio * areasize -
                                    grabber->get_size().height));
    } else {

      int widget_height =
          style->get_minimum_size().height + style->get_center_size().height;
      ...
      grabber_area->draw(
          ci, Rect2i(Point2i(0, (size.height - widget_height) / 2),
                     Size2i(areasize * ratio + grabber->get_size().width / 2,
                            widget_height)));

      if (ticks > 1) {
        int grabber_offset =
            (grabber->get_size().width / 2 - tick->get_width() / 2);
        for (int i = 0; i < ticks; i++) {
          if ((!ticks_on_borders) && ((i == 0) || ((i + 1) == ticks)))
            continue;
          int ofs = (i * areasize / (ticks - 1)) + grabber_offset;
      ...
      grabber->draw(ci,
                    Point2i(ratio * areasize,
                            size.height / 2 - grabber->get_size().height / 2));
    }
...
float Slider::get_custom_step() const { return custom_step; }
...
void Slider::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("get_ticks_on_borders"),
                       &Slider::get_ticks_on_borders);
  ClassDB::bind_method(D_METHOD("set_ticks_on_borders", "ticks_on_border"),
                       &Slider::set_ticks_on_borders);

  ClassDB::bind_method(D_METHOD("set_editable", "editable"),
                       &Slider::set_editable);
  ClassDB::bind_method(D_METHOD("is_editable"), &Slider::is_editable);
  ClassDB::bind_method(D_METHOD("set_scrollable", "scrollable"),
  ...
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "editable"), "set_editable",
               "is_editable");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "scrollable"), "set_scrollable",
               "is_scrollable");
  ADD_PROPERTY(
      PropertyInfo(Variant::INT, "tick_count", PROPERTY_HINT_RANGE, "0,4096,1"),
      "set_ticks", "get_ticks");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "ticks_on_borders"),
               "set_ticks_on_borders", "get_ticks_on_borders");
}
