...
NodePath RootMotionView::get_animation_path() const { return path; }
...
void RootMotionView::_notification(int p_what) {
  ...
  if (p_what == NOTIFICATION_ENTER_TREE) {
    ...
    VS::get_singleton()->immediate_set_material(
        immediate, StandardMaterial3D::get_material_rid_for_2d(
    ...
    if (has_node(path)) {
      ...
      if (tree && tree->is_active() &&
        ...
        if (is_physics_processing_internal() &&
            tree->get_process_mode() == AnimationTree::ANIMATION_PROCESS_IDLE) {
...
AABB RootMotionView::get_aabb() const {
  ...
  return AABB(Vector3(-radius, 0, -radius),
...
void RootMotionView::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("set_animation_path", "path"),
  ...
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "zero_y"), "set_zero_y",
               "get_zero_y");
}
