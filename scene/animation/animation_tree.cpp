...
Variant AnimationNode::get_parameter_default_value(
...
void AnimationNode::set_parameter(const StringName &p_name,
...
Variant AnimationNode::get_parameter(const StringName &p_name) const {
  ...
  ERR_FAIL_COND_V(!state->tree->property_parent_map[base_path].has(p_name),
                  Variant());
...
void AnimationNode::blend_animation(const StringName &p_animation, float p_time,
                                    float p_delta, bool p_seeked,
                                    float p_blend) {
...
float AnimationNode::_pre_process(const StringName &p_base_path,
                                  AnimationNode *p_parent, State *p_state,
                                  float p_time, bool p_seek,
...
float AnimationNode::blend_input(int p_input, float p_time, bool p_seek,
                                 float p_blend, FilterAction p_filter,
                                 bool p_optimize) {
  ...
  if (!blend_tree->has_node(node_name)) {
    ...
    make_invalid(vformat(RTR("Nothing connected to input '%s' of node '%s'."),
                         get_input_name(p_input), name));
    return 0;
  }
  ...
  float ret = _blend_node(
      node_name, blend_tree->get_node_connection_array(node_name), NULL, node,
      p_time, p_seek, p_blend, p_filter, p_optimize, &activity);

  Vector<AnimationTree::Activity> *activity_ptr =
      state->tree->input_activity_map.getptr(base_path);
...
float AnimationNode::blend_node(const StringName &p_sub_path,
                                Ref<AnimationNode> p_node, float p_time,
                                bool p_seek, float p_blend,
                                FilterAction p_filter, bool p_optimize) {

  return _blend_node(p_sub_path, Vector<StringName>(), this, p_node, p_time,
                     p_seek, p_blend, p_filter, p_optimize);
}

float AnimationNode::_blend_node(const StringName &p_subpath,
                                 const Vector<StringName> &p_connections,
  ...
  // this is the slowest part of processing, but as strings process in powers of
  // 2, and the paths always exist, it will not result in that many allocations
  if (p_new_parent) {
  ...
  return p_node->_pre_process(new_path, new_parent, state, p_time, p_seek,
                              p_connections);
}
...
void AnimationNode::_validate_property(PropertyInfo &property) const {
  if (!has_filter() &&
      (property.name == "filter_enabled" || property.name == "filters")) {
    property.usage = 0;
  }
}
...
void AnimationNode::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("get_input_count"),
                       &AnimationNode::get_input_count);
  ClassDB::bind_method(D_METHOD("get_input_name", "input"),
                       &AnimationNode::get_input_name);

  ClassDB::bind_method(D_METHOD("add_input", "name"),
                       &AnimationNode::add_input);
  ClassDB::bind_method(D_METHOD("remove_input", "index"),
                       &AnimationNode::remove_input);

  ClassDB::bind_method(D_METHOD("set_filter_path", "path", "enable"),
                       &AnimationNode::set_filter_path);
  ClassDB::bind_method(D_METHOD("is_path_filtered", "path"),
                       &AnimationNode::is_path_filtered);

  ClassDB::bind_method(D_METHOD("set_filter_enabled", "enable"),
                       &AnimationNode::set_filter_enabled);
  ClassDB::bind_method(D_METHOD("is_filter_enabled"),
                       &AnimationNode::is_filter_enabled);

  ClassDB::bind_method(D_METHOD("_set_filters", "filters"),
                       &AnimationNode::_set_filters);
  ClassDB::bind_method(D_METHOD("_get_filters"), &AnimationNode::_get_filters);

  ClassDB::bind_method(D_METHOD("blend_animation", "animation", "time", "delta",
                                "seeked", "blend"),
                       &AnimationNode::blend_animation);
  ClassDB::bind_method(D_METHOD("blend_node", "name", "node", "time", "seek",
                                "blend", "filter", "optimize"),
                       &AnimationNode::blend_node, DEFVAL(FILTER_IGNORE),
                       DEFVAL(true));
  ClassDB::bind_method(D_METHOD("blend_input", "input_index", "time", "seek",
                                "blend", "filter", "optimize"),
                       &AnimationNode::blend_input, DEFVAL(FILTER_IGNORE),
                       DEFVAL(true));

  ClassDB::bind_method(D_METHOD("set_parameter", "name", "value"),
                       &AnimationNode::set_parameter);
  ClassDB::bind_method(D_METHOD("get_parameter", "name"),
  ...
  BIND_VMETHOD(MethodInfo(Variant::OBJECT, "get_child_by_name",
                          PropertyInfo(Variant::STRING, "name")));
  {
    MethodInfo mi = MethodInfo(Variant::NIL, "get_parameter_default_value",
                               PropertyInfo(Variant::STRING_NAME, "name"));
    mi.return_val.usage = PROPERTY_USAGE_NIL_IS_VARIANT;
    BIND_VMETHOD(mi);
  }
  BIND_VMETHOD(MethodInfo("process", PropertyInfo(Variant::FLOAT, "time"),
                          PropertyInfo(Variant::BOOL, "seek")));
...
void AnimationTree::set_tree_root(const Ref<AnimationNode> &p_root) {
  ...
  if (root.is_valid()) {
    root->disconnect("tree_changed",
                     callable_mp(this, &AnimationTree::_tree_changed));
  }

  root = p_root;

  if (root.is_valid()) {
    root->connect("tree_changed",
                  callable_mp(this, &AnimationTree::_tree_changed));
  }
...
Ref<AnimationNode> AnimationTree::get_tree_root() const { return root; }
...
bool AnimationTree::is_active() const { return active; }
...
void AnimationTree::_node_removed(Node *p_node) { cache_valid = false; }
...
bool AnimationTree::_update_caches(AnimationPlayer *player) {
  ...
  for (List<StringName>::Element *E = sname.front(); E; E = E->next()) {
    ...
    for (int i = 0; i < anim->get_track_count(); i++) {
      ...
      if (track && (track->type != track_type ||
                    ObjectDB::get_instance(track->object_id) == NULL)) {
      ...
      if (!track) {
        ...
        Node *child =
            parent->get_node_and_resource(path, resource, leftover_path);

        if (!child) {
          ERR_PRINT("AnimationTree: '" + String(E->get()) +
                    "', couldn't resolve track:  '" + String(path) + "'");
          continue;
        }

        if (!child->is_connected(
                "tree_exited",
                callable_mp(this, &AnimationTree::_node_removed))) {
          child->connect("tree_exited",
                         callable_mp(this, &AnimationTree::_node_removed),
                         varray(child));
        }
        ...
        case Animation::TYPE_TRANSFORM: {
          ...
          if (!spatial) {
            ERR_PRINT("AnimationTree: '" + String(E->get()) +
                      "', transform track does not point to spatial:  '" +
                      String(path) + "'");
            continue;
          }
          ...
          if (path.get_subname_count() == 1 &&
              Object::cast_to<Skeleton>(spatial)) {
        ...
        case Animation::TYPE_ANIMATION: {
          ...
          track_animation->object_id =
              track_animation->object->get_instance_id();
...
void AnimationTree::_process_graph(float p_delta) {
  ...
  if (!root.is_valid()) {
    ERR_PRINT(
        "AnimationTree: root AnimationNode is not set, disabling playback.");
    set_active(false);
    cache_valid = false;
    return;
  }
  ...
  if (!has_node(animation_player)) {
    ERR_PRINT(
        "AnimationTree: no valid AnimationPlayer path set, disabling playback");
    set_active(false);
    cache_valid = false;
    return;
  }
  ...
  AnimationPlayer *player =
      Object::cast_to<AnimationPlayer>(get_node(animation_player));
    ...
    if (last_animation_player.is_valid()) {
      ...
      if (old_player) {
        old_player->disconnect(
            "caches_cleared", callable_mp(this, &AnimationTree::_clear_caches));
      }
    }

    if (player) {
      player->connect("caches_cleared",
                      callable_mp(this, &AnimationTree::_clear_caches));
    }
  ...
  if (!player) {
    ERR_PRINT("AnimationTree: path points to a node not an AnimationPlayer, "
              "disabling playback");
    set_active(false);
    cache_valid = false;
    return;
...

  {
    ...
    if (started) {
      // if started, seek
      root->_pre_process(
          SceneStringNames::get_singleton()->parameters_base_path, NULL, &state,
          0, true, Vector<StringName>());
      started = false;
    }

    root->_pre_process(SceneStringNames::get_singleton()->parameters_base_path,
                       NULL, &state, p_delta, false, Vector<StringName>());
  }

  if (!state.valid) {
    return; // state is not valid. do nothing.
  }
  // apply value/transform/bezier blends to track caches and execute
  // method/audio/animation tracks

  {
    ...
    for (List<AnimationNode::AnimationState>::Element *E =
             state.animation_states.front();
         E; E = E->next()) {
      ...
      for (int i = 0; i < a->get_track_count(); i++) {
        ...
        case Animation::TYPE_TRANSFORM: {
          ...
          if (track->root_motion) {
            ...
            if (prev_time > time) {
              ...
              Error err = a->transform_track_interpolate(i, prev_time, &loc[0],
                                                         &rot[0], &scale[0]);
              if (err != OK) {
                continue;
              }

              a->transform_track_interpolate(i, a->get_length(), &loc[1],
                                             &rot[1], &scale[1]);

              t->loc += (loc[1] - loc[0]) * blend;
              t->scale += (scale[1] - scale[0]) * blend;
              Quat q = Quat()
                           .slerp(rot[0].normalized().inverse() *
                                      rot[1].normalized(),
                                  blend)
                           .normalized();
            ...
            a->transform_track_interpolate(i, time, &loc[1], &rot[1],
                                           &scale[1]);
            ...
            Quat q =
                Quat()
                    .slerp(rot[0].normalized().inverse() * rot[1].normalized(),
                           blend)
                    .normalized();
            t->rot = (t->rot * q).normalized();
            ...
            Error err =
                a->transform_track_interpolate(i, time, &loc, &rot, &scale);
            ...
            if (t->rot_blend_accum == 0) {
              ...
              t->rot = rot.slerp(t->rot, t->rot_blend_accum / rot_total)
                           .normalized();
        ...
        case Animation::TYPE_VALUE: {
          ...
          if (update_mode == Animation::UPDATE_CONTINUOUS ||
              update_mode ==
                  Animation::UPDATE_CAPTURE) { // delta == 0 means seek
        ...
        case Animation::TYPE_METHOD: {
          ...
          for (List<int>::Element *F = indices.front(); F; F = F->next()) {
            ...
            if (can_call) {
              t->object->call_deferred(method, s >= 1 ? params[0] : Variant(),
          ...
          if (seeked) {
            ...
            if (to_play.size()) {
              ...
              if (!loop && time < t->start) {
                ...
                float len = t->start > time
                                ? (a->get_length() - t->start) + time
                                : time - t->start;
        ...
        case Animation::TYPE_ANIMATION: {
          ...
          AnimationPlayer *player2 =
              Object::cast_to<AnimationPlayer>(t->object);
          ...
          if (delta == 0 || seeked) {
            ...
            if (String(anim_name) == "[stop]" ||
                !player2->has_animation(anim_name))
              continue;
            ...
            if (anim->has_loop()) {
              at_anim_pos =
                  Math::fposmod(time - pos, anim->get_length()); // seek to loop
            } else {
            ...
            if (to_play.size()) {
              ...
              StringName anim_name =
                  a->animation_track_get_key_animation(i, idx);
              if (String(anim_name) == "[stop]" ||
                  !player2->has_animation(anim_name)) {
        ...
        if (t->root_motion) {
          ...
          if (t->skeleton && t->bone_idx >= 0) {
            root_motion_transform =
                (t->skeleton->get_bone_rest(t->bone_idx) *
                 root_motion_transform) *
                t->skeleton->get_bone_rest(t->bone_idx).affine_inverse();
          }
...
void AnimationTree::advance(float p_time) { _process_graph(p_time); }

void AnimationTree::_notification(int p_what) {

  if (active && p_what == NOTIFICATION_INTERNAL_PHYSICS_PROCESS &&
      process_mode == ANIMATION_PROCESS_PHYSICS) {
    _process_graph(get_physics_process_delta_time());
  }

  if (active && p_what == NOTIFICATION_INTERNAL_PROCESS &&
  ...
  if (p_what == NOTIFICATION_EXIT_TREE) {
    ...
    if (last_animation_player.is_valid()) {
      ...
      if (player) {
        player->disconnect("caches_cleared",
                           callable_mp(this, &AnimationTree::_clear_caches));
      }
    }
...
bool AnimationTree::is_state_invalid() const { return !state.valid; }
String AnimationTree::get_invalid_state_reason() const {

  return state.invalid_reasons;
}

uint64_t AnimationTree::get_last_process_pass() const { return process_pass; }
...
String AnimationTree::get_configuration_warning() const {
  ...
  if (!player) {
    ...
    warning += TTR("Path set for AnimationPlayer does not lead to an "
...
void AnimationTree::_update_properties_for_node(const String &p_base_path,
  ...
  if (node->get_input_count() && !input_activity_map.has(p_base_path)) {
    ...
    input_activity_map_get[String(p_base_path)
                               .substr(0, String(p_base_path).length() - 1)] =
  ...
  for (List<AnimationNode::ChildNode>::Element *E = children.front(); E;
       E = E->next()) {
    _update_properties_for_node(p_base_path + E->get().name + "/",
                                E->get().node);
  }
}
...
void AnimationTree::_update_properties() {
  ...
  if (root.is_valid()) {
    _update_properties_for_node(
        SceneStringNames::get_singleton()->parameters_base_path, root);
  }
...
void AnimationTree::rename_parameter(const String &p_base,
                                     const String &p_new_base) {
...
float AnimationTree::get_connection_activity(const StringName &p_path,
                                             int p_connection) const {
...
void AnimationTree::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("set_tree_root", "root"),
                       &AnimationTree::set_tree_root);
  ClassDB::bind_method(D_METHOD("get_tree_root"),
                       &AnimationTree::get_tree_root);

  ClassDB::bind_method(D_METHOD("set_process_mode", "mode"),
                       &AnimationTree::set_process_mode);
  ClassDB::bind_method(D_METHOD("get_process_mode"),
                       &AnimationTree::get_process_mode);

  ClassDB::bind_method(D_METHOD("set_animation_player", "root"),
                       &AnimationTree::set_animation_player);
  ClassDB::bind_method(D_METHOD("get_animation_player"),
                       &AnimationTree::get_animation_player);

  ClassDB::bind_method(D_METHOD("set_root_motion_track", "path"),
                       &AnimationTree::set_root_motion_track);
  ClassDB::bind_method(D_METHOD("get_root_motion_track"),
                       &AnimationTree::get_root_motion_track);

  ClassDB::bind_method(D_METHOD("get_root_motion_transform"),
                       &AnimationTree::get_root_motion_transform);

  ClassDB::bind_method(D_METHOD("_update_properties"),
                       &AnimationTree::_update_properties);

  ClassDB::bind_method(D_METHOD("rename_parameter", "old_name", "new_name"),
                       &AnimationTree::rename_parameter);

  ClassDB::bind_method(D_METHOD("advance", "delta"), &AnimationTree::advance);

  ADD_PROPERTY(PropertyInfo(Variant::OBJECT, "tree_root",
                            PROPERTY_HINT_RESOURCE_TYPE, "AnimationRootNode"),
               "set_tree_root", "get_tree_root");
  ADD_PROPERTY(PropertyInfo(Variant::NODE_PATH, "anim_player",
  ...
  ADD_PROPERTY(PropertyInfo(Variant::NODE_PATH, "root_motion_track"),
               "set_root_motion_track", "get_root_motion_track");
