...
public:
  enum FilterAction { FILTER_IGNORE, FILTER_PASS, FILTER_STOP, FILTER_BLEND };
  ...
  float _pre_process(const StringName &p_base_path, AnimationNode *p_parent,
                     State *p_state, float p_time, bool p_seek,
  ...
  float _blend_node(const StringName &p_subpath,
                    const Vector<StringName> &p_connections,
                    AnimationNode *p_new_parent, Ref<AnimationNode> p_node,
                    float p_time, bool p_seek, float p_blend,
                    FilterAction p_filter = FILTER_IGNORE,
                    bool p_optimize = true, float *r_max = NULL);

protected:
  void blend_animation(const StringName &p_animation, float p_time,
                       float p_delta, bool p_seeked, float p_blend);
  float blend_node(const StringName &p_sub_path, Ref<AnimationNode> p_node,
                   float p_time, bool p_seek, float p_blend,
                   FilterAction p_filter = FILTER_IGNORE,
                   bool p_optimize = true);
  float blend_input(int p_input, float p_time, bool p_seek, float p_blend,
                    FilterAction p_filter = FILTER_IGNORE,
                    bool p_optimize = true);
...
public:
  ...
  virtual Variant
  get_parameter_default_value(const StringName &p_parameter) const;
...
private:
  ...
  void _update_properties_for_node(const String &p_base_path,
                                   Ref<AnimationNode> node);
...
public:
  ...
  float get_connection_activity(const StringName &p_path,
