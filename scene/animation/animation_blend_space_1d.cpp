...
void AnimationNodeBlendSpace1D::get_parameter_list(
    List<PropertyInfo> *r_list) const {
  r_list->push_back(PropertyInfo(Variant::FLOAT, blend_position));
}
Variant AnimationNodeBlendSpace1D::get_parameter_default_value(
    const StringName &p_parameter) const {
  return 0;
}

Ref<AnimationNode>
AnimationNodeBlendSpace1D::get_child_by_name(const StringName &p_name) {
  return get_blend_point_node(p_name.operator String().to_int());
}

void AnimationNodeBlendSpace1D::_validate_property(
...
void AnimationNodeBlendSpace1D::_tree_changed() { emit_signal("tree_changed"); }

void AnimationNodeBlendSpace1D::_bind_methods() {
  ClassDB::bind_method(D_METHOD("add_blend_point", "node", "pos", "at_index"),
                       &AnimationNodeBlendSpace1D::add_blend_point, DEFVAL(-1));
  ...
  ClassDB::bind_method(D_METHOD("set_value_label", "text"),
                       &AnimationNodeBlendSpace1D::set_value_label);
  ClassDB::bind_method(D_METHOD("get_value_label"),
  ...
  for (int i = 0; i < MAX_BLEND_POINTS; i++) {
    ...
    ADD_PROPERTYI(
        PropertyInfo(Variant::FLOAT, "blend_point_" + itos(i) + "/pos",
                     PROPERTY_HINT_NONE, "",
                     PROPERTY_USAGE_NOEDITOR | PROPERTY_USAGE_INTERNAL),
        "set_blend_point_position", "get_blend_point_position", i);
  }

  ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "min_space", PROPERTY_HINT_NONE, "",
                            PROPERTY_USAGE_NOEDITOR),
               "set_min_space", "get_min_space");
  ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "max_space", PROPERTY_HINT_NONE, "",
                            PROPERTY_USAGE_NOEDITOR),
               "set_max_space", "get_max_space");
  ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "snap", PROPERTY_HINT_NONE, "",
                            PROPERTY_USAGE_NOEDITOR),
               "set_snap", "get_snap");
  ADD_PROPERTY(PropertyInfo(Variant::STRING, "value_label", PROPERTY_HINT_NONE,
                            "", PROPERTY_USAGE_NOEDITOR),
               "set_value_label", "get_value_label");
}

void AnimationNodeBlendSpace1D::get_child_nodes(
...
void AnimationNodeBlendSpace1D::add_blend_point(
  ...
  blend_points[p_at_index].node->connect(
      "tree_changed",
      callable_mp(this, &AnimationNodeBlendSpace1D::_tree_changed), varray(),
...
void AnimationNodeBlendSpace1D::set_blend_point_position(int p_point,
                                                         float p_position) {
...
void AnimationNodeBlendSpace1D::set_blend_point_node(
  ...
  if (blend_points[p_point].node.is_valid()) {
    blend_points[p_point].node->disconnect(
        "tree_changed",
        callable_mp(this, &AnimationNodeBlendSpace1D::_tree_changed));
  }
...
AnimationNodeBlendSpace1D::get_blend_point_node(int p_point) const {
...
void AnimationNodeBlendSpace1D::_add_blend_point(
...
float AnimationNodeBlendSpace1D::process(float p_time, bool p_seek) {
  ...
  if (blend_points_used == 1) {
    ...
    return blend_node(blend_points[0].name, blend_points[0].node, p_time,
                      p_seek, 1.0, FILTER_IGNORE, false);
  }
