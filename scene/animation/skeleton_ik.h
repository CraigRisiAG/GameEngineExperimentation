...
class FabrikInverseKinematic {
  ...
  struct ChainItem {
    ...
    ChainItem() : parent_item(NULL), bone(-1), pb(NULL), length(0) {}
  ...
  struct ChainTip {
    ...
    ChainTip() : chain_item(NULL), end_effector(NULL) {}
...
public:
  struct Task {
    ...
    Task()
        : skeleton(NULL), min_distance(0.01), max_iterations(10),
          root_bone(-1) {}
  };
...
public:
  static Task *create_simple_task(Skeleton *p_sk, BoneId root_bone,
                                  BoneId tip_bone,
  ...
  static void make_goal(Task *p_task, const Transform &p_inverse_transf,
