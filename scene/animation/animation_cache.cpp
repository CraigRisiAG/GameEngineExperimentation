...
void AnimationCache::_animation_changed() { _clear_cache(); }

void AnimationCache::_clear_cache() {

  while (connected_nodes.size()) {

    connected_nodes.front()->get()->disconnect(
        "tree_exiting", callable_mp(this, &AnimationCache::_node_exit_tree));
  ...
  for (int i = 0; i < animation->get_track_count(); i++) {
    ...
    if (animation->track_get_type(i) == Animation::TYPE_TRANSFORM) {
      ...
      if (np.get_subname_count() > 1) {
        path_cache.push_back(Path());
        ERR_CONTINUE_MSG(animation->track_get_type(i) ==
                             Animation::TYPE_TRANSFORM,
                         "Transform tracks can't have a subpath '" + np + "'.");
      }
      ...
      if (!sp) {
        ...
        path_cache.push_back(Path());
        ERR_CONTINUE_MSG(!sp,
                         "Transform track not of type Spatial '" + np + "'.");
      }
      ...
      if (np.get_subname_count() == 1) {
        ...
        if (!sk) {
          ...
          ERR_CONTINUE_MSG(
              !sk,
              "Property defined in Transform track, but not a Skeleton! '" +
                  np + "'.");
        }
        ...
        if (idx == -1) {
          path_cache.push_back(Path());
          ERR_CONTINUE_MSG(idx == -1, "Property defined in Transform track, "
                                      "but not a Skeleton Bone! '" +
                                          np + "'.");
        }
    ...
    if (animation->track_get_type(i) == Animation::TYPE_VALUE) {
      ...
      if (np.get_subname_count() == 0) {
        ...
        path_cache.push_back(Path());
        ERR_CONTINUE_MSG(np.get_subname_count() == 0,
                         "Value Track lacks property: " + np + ".");
      }

    } else if (animation->track_get_type(i) == Animation::TYPE_METHOD) {

      if (path.subpath.size() !=
          0) { // Trying to call a method of a non-resource

        path_cache.push_back(Path());
        ERR_CONTINUE_MSG(path.subpath.size() != 0,
                         "Method Track has property: " + np + ".");
      }
    }
    ...
    if (!connected_nodes.has(path.node)) {
      ...
      path.node->connect("tree_exiting",
                         callable_mp(this, &AnimationCache::_node_exit_tree),
                         Node::make_binds(path.node), CONNECT_ONESHOT);
    }
  }
...
void AnimationCache::set_track_transform(int p_idx,
...
void AnimationCache::call_track(int p_idx, const StringName &p_method,
...
void AnimationCache::set_all(float p_time, float p_delta) {
  ...
  for (int i = 0; i < tc; i++) {
    ...
    case Animation::TYPE_VALUE: {
      ...
      if (animation->value_track_get_update_mode(i) ==
              Animation::UPDATE_CONTINUOUS ||
          (animation->value_track_get_update_mode(i) ==
...
void AnimationCache::set_animation(const Ref<Animation> &p_animation) {
  ...
  if (animation.is_valid())
    animation->disconnect(
        "changed", callable_mp(this, &AnimationCache::_animation_changed));

  animation = p_animation;

  if (animation.is_valid())
    animation->connect("changed",
                       callable_mp(this, &AnimationCache::_animation_changed));
}
