...
FabrikInverseKinematic::ChainItem *
FabrikInverseKinematic::ChainItem::find_child(const BoneId p_bone_id) {
...
FabrikInverseKinematic::ChainItem *
FabrikInverseKinematic::ChainItem::add_child(const BoneId p_bone_id) {
...
bool FabrikInverseKinematic::build_chain(Task *p_task,
  ...
  chain.chain_root.pb =
      p_task->skeleton->get_physical_bone(chain.chain_root.bone);
  ...
  for (int x = p_task->end_effectors.size() - 1; 0 <= x; --x) {
    ...
    sub_chain_size = 0;
    // Picks all IDs that composing a single chain in reverse order (except the
    // root)
    ...
    for (int i = sub_chain_size - 1; 0 <= i; --i) {
      ...
      if (!child_ci) {
        ...
        if (child_ci->parent_item) {
          child_ci->length =
              (child_ci->current_pos - child_ci->parent_item->current_pos)
                  .length();
        }
      }
    ...
    if (p_force_simple_chain) {
      ...
      //since the solver of multi tip (end effector) 	is not yet created. 	Remove
      //this code when this is done
      break;
    }
  }
  return true;
...
void FabrikInverseKinematic::update_chain(const Skeleton *p_sk,
  ...
  p_chain_item->initial_transform =
      p_sk->get_bone_global_pose(p_chain_item->bone);
...
void FabrikInverseKinematic::solve_simple_backwards(Chain &r_chain,
                                                    bool p_solve_magnet) {
  ...
  while (sub_chain_tip) {
    ...
    if (sub_chain_tip->parent_item) {
      ...
      const Vector3 look_parent(
          (sub_chain_tip->parent_item->current_pos - sub_chain_tip->current_pos)
              .normalized());
...
void FabrikInverseKinematic::solve_simple_forwards(Chain &r_chain,
                                                   bool p_solve_magnet) {
...
FabrikInverseKinematic::Task *
FabrikInverseKinematic::create_simple_task(Skeleton *p_sk, BoneId root_bone,
                                           BoneId tip_bone,
...
void FabrikInverseKinematic::make_goal(Task *p_task,
  ...
  if (blending_delta >= 0.99f) {
    ...
    const Transform end_effector_pose(p_task->skeleton->get_bone_global_pose(
        p_task->end_effectors.write[0].tip_bone));
...
void FabrikInverseKinematic::solve(Task *p_task, real_t blending_delta,
                                   bool override_tip_basis, bool p_use_magnet,
  ...
  make_goal(
      p_task,
      p_task->skeleton->get_global_transform().affine_inverse().scaled(
          p_task->skeleton->get_global_transform().get_basis().get_scale()),
      blending_delta);
  ...
  while (ci) {
    ...
    p_task->skeleton->set_bone_global_pose_override(ci->bone, new_bone_pose,
                                                    1.0, true);
...
void SkeletonIK::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("set_root_bone", "root_bone"),
                       &SkeletonIK::set_root_bone);
  ClassDB::bind_method(D_METHOD("get_root_bone"), &SkeletonIK::get_root_bone);

  ClassDB::bind_method(D_METHOD("set_tip_bone", "tip_bone"),
                       &SkeletonIK::set_tip_bone);
  ClassDB::bind_method(D_METHOD("get_tip_bone"), &SkeletonIK::get_tip_bone);

  ClassDB::bind_method(D_METHOD("set_interpolation", "interpolation"),
                       &SkeletonIK::set_interpolation);
  ClassDB::bind_method(D_METHOD("get_interpolation"),
                       &SkeletonIK::get_interpolation);

  ClassDB::bind_method(D_METHOD("set_target_transform", "target"),
  ...
  ClassDB::bind_method(D_METHOD("set_override_tip_basis", "override"),
                       &SkeletonIK::set_override_tip_basis);
  ClassDB::bind_method(D_METHOD("is_override_tip_basis"),
                       &SkeletonIK::is_override_tip_basis);

  ClassDB::bind_method(D_METHOD("set_use_magnet", "use"),
                       &SkeletonIK::set_use_magnet);
  ClassDB::bind_method(D_METHOD("is_using_magnet"),
                       &SkeletonIK::is_using_magnet);

  ClassDB::bind_method(D_METHOD("set_magnet_position", "local_position"),
                       &SkeletonIK::set_magnet_position);
  ClassDB::bind_method(D_METHOD("get_magnet_position"),
                       &SkeletonIK::get_magnet_position);

  ClassDB::bind_method(D_METHOD("get_parent_skeleton"),
                       &SkeletonIK::get_parent_skeleton);
  ClassDB::bind_method(D_METHOD("is_running"), &SkeletonIK::is_running);

  ClassDB::bind_method(D_METHOD("set_min_distance", "min_distance"),
                       &SkeletonIK::set_min_distance);
  ClassDB::bind_method(D_METHOD("get_min_distance"),
                       &SkeletonIK::get_min_distance);

  ClassDB::bind_method(D_METHOD("set_max_iterations", "iterations"),
                       &SkeletonIK::set_max_iterations);
  ClassDB::bind_method(D_METHOD("get_max_iterations"),
                       &SkeletonIK::get_max_iterations);

  ClassDB::bind_method(D_METHOD("start", "one_time"), &SkeletonIK::start,
                       DEFVAL(false));
  ClassDB::bind_method(D_METHOD("stop"), &SkeletonIK::stop);

  ADD_PROPERTY(PropertyInfo(Variant::STRING_NAME, "root_bone"), "set_root_bone",
               "get_root_bone");
  ADD_PROPERTY(PropertyInfo(Variant::STRING_NAME, "tip_bone"), "set_tip_bone",
               "get_tip_bone");
  ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "interpolation",
                            PROPERTY_HINT_RANGE, "0,1,0.001"),
               "set_interpolation", "get_interpolation");
  ADD_PROPERTY(PropertyInfo(Variant::TRANSFORM, "target"),
               "set_target_transform", "get_target_transform");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "override_tip_basis"),
               "set_override_tip_basis", "is_override_tip_basis");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "use_magnet"), "set_use_magnet",
               "is_using_magnet");
  ADD_PROPERTY(PropertyInfo(Variant::VECTOR3, "magnet"), "set_magnet_position",
               "get_magnet_position");
  ADD_PROPERTY(PropertyInfo(Variant::NODE_PATH, "target_node"),
               "set_target_node", "get_target_node");
  ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "min_distance"), "set_min_distance",
               "get_min_distance");
  ADD_PROPERTY(PropertyInfo(Variant::INT, "max_iterations"),
...
SkeletonIK::SkeletonIK()
    : interpolation(1), override_tip_basis(true), use_magnet(false),
      min_distance(0.01), max_iterations(10), skeleton(NULL),
      target_node_override(NULL), task(NULL) {}
...
StringName SkeletonIK::get_root_bone() const { return root_bone; }
...
StringName SkeletonIK::get_tip_bone() const { return tip_bone; }

void SkeletonIK::set_interpolation(real_t p_interpolation) {
  interpolation = p_interpolation;
}

real_t SkeletonIK::get_interpolation() const { return interpolation; }
...
const Transform &SkeletonIK::get_target_transform() const { return target; }
...
NodePath SkeletonIK::get_target_node() { return target_node_path_override; }

void SkeletonIK::set_override_tip_basis(bool p_override) {
  override_tip_basis = p_override;
}

bool SkeletonIK::is_override_tip_basis() const { return override_tip_basis; }

void SkeletonIK::set_use_magnet(bool p_use) { use_magnet = p_use; }

bool SkeletonIK::is_using_magnet() const { return use_magnet; }
...
bool SkeletonIK::is_running() { return is_processing_internal(); }
...
void SkeletonIK::stop() { set_process_internal(false); }
...
void SkeletonIK::reload_chain() {
  ...
  task = FabrikInverseKinematic::create_simple_task(
      skeleton, skeleton->find_bone(root_bone), skeleton->find_bone(tip_bone),
...
void SkeletonIK::_solve_chain() {
  if (!task)
    return;
  FabrikInverseKinematic::solve(task, interpolation, override_tip_basis,
