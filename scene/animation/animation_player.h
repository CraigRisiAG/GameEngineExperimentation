...
private:
  ...
  struct TrackNodeCache {
    ...
    struct PropertyAnim {
      ...
      PropertyAnim()
          : owner(NULL), special(SP_NONE), object(NULL), accum_pass(0) {}
    };
    ...
    struct BezierAnim {
      ...
      BezierAnim()
          : owner(NULL), bezier_accum(0.0), object(NULL), accum_pass(0) {}
    };

    Map<StringName, BezierAnim> bezier_anim;

    TrackNodeCache()
        : id(0), node(NULL), spatial(NULL), node_2d(NULL), skeleton(NULL),
          bone_idx(-1), accum_pass(0), audio_playing(false), audio_start(0.0),
          audio_len(0.0), animation_playing(false) {}
  };
  ...
  void _animation_process_animation(AnimationData *p_anim, float p_time,
                                    float p_delta, float p_interp,
...
public:
  ...
  Error add_animation(const StringName &p_name,
  ...
  void set_blend_time(const StringName &p_animation1,
                      const StringName &p_animation2, float p_time);
  float get_blend_time(const StringName &p_animation1,
                       const StringName &p_animation2) const;

  void animation_set_next(const StringName &p_animation,
  ...
  void play(const StringName &p_name = StringName(), float p_custom_blend = -1,
  ...
  void clear_caches(); ///< must be called by hand if an animation was modified
                       ///< after added
