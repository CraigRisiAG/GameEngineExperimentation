...
void SceneDebugger::initialize() {
  ...
  EngineDebugger::register_message_capture(
      "scene", EngineDebugger::Capture(NULL, SceneDebugger::parse_message));
#endif
}
...
#ifdef DEBUG_ENABLED
Error SceneDebugger::parse_message(void *p_user, const String &p_msg,
                                   const Array &p_args, bool &r_captured) {
  ...
  if (p_msg == "request_scene_tree") { // Scene tree
    ...
    float far = p_args[4];
    if (is_perspective) {
      scene_tree->get_root()->set_camera_override_perspective(size_or_fov, near,
                                                              far);
    } else {
      scene_tree->get_root()->set_camera_override_orthogonal(size_or_fov, near,
                                                             far);
    }
    ...
    live_editor->_node_call_func(p_args[0], p_args[1], p_args[2], p_args[3],
                                 p_args[4], p_args[5], p_args[6]);

  } else if (p_msg == "live_res_call") {
    ERR_FAIL_COND_V(p_args.size() < 7, ERR_INVALID_DATA);
    live_editor->_res_call_func(p_args[0], p_args[1], p_args[2], p_args[3],
                                p_args[4], p_args[5], p_args[6]);
    ...
    live_editor->_reparent_node_func(p_args[0], p_args[1], p_args[2],
                                     p_args[3]);
  } else {
    r_captured = false;
  }
...
void SceneDebugger::_set_object_property(ObjectID p_id,
...
void SceneDebugger::remove_from_cache(const String &p_filename, Node *p_node) {
  ...
  Map<Node *, Map<ObjectID, Node *>> &remove_list =
      debugger->live_edit_remove_list;
  ...
  for (List<PropertyInfo>::Element *E = pinfo.front(); E; E = E->next()) {
    if (E->get().usage & (PROPERTY_USAGE_EDITOR | PROPERTY_USAGE_CATEGORY)) {
      properties.push_back(
          SceneDebuggerProperty(E->get(), obj->get(E->get().name)));
    }
  }
}

void SceneDebuggerObject::_parse_script_properties(Script *p_script,
  ...
  for (ScriptMemberMap::Element *sm = members.front(); sm; sm = sm->next()) {
    for (Set<StringName>::Element *E = sm->get().front(); E; E = E->next()) {
      ...
      if (p_instance->get(E->get(), m)) {
        String script_path =
            sm->key() == p_script ? "" : sm->key()->get_path().get_file() + "/";
  ...
  for (ScriptConstantsMap::Element *sc = constants.front(); sc;
       sc = sc->next()) {
    for (Map<StringName, Variant>::Element *E = sc->get().front(); E;
         E = E->next()) {
      String script_path =
          sc->key() == p_script ? "" : sc->key()->get_path().get_file() + "/";
      if (E->value().get_type() == Variant::OBJECT) {
        Variant id = ((Object *)E->value())->get_instance_id();
        PropertyInfo pi(id.get_type(), "Constants/" + E->key(),
                        PROPERTY_HINT_OBJECT_ID, "Object");
        properties.push_back(SceneDebuggerProperty(pi, id));
      } else {
        PropertyInfo pi(E->value().get_type(),
                        "Constants/" + script_path + E->key());
...
#define CHECK_TYPE(p_what, p_type)                                             \
  ERR_FAIL_COND(p_what.get_type() != Variant::p_type);
...
SceneDebuggerTree::SceneDebuggerTree(Node *p_root) {
  ...
  while (stack.size()) {
    ...
    nodes.push_back(
        RemoteNode(count, n->get_name(), n->get_class(), n->get_instance_id()));
...
void SceneDebuggerTree::deserialize(const Array &p_arr) {
  ...
  while (p_arr.size() > idx) {
    ...
    nodes.push_back(
        RemoteNode(p_arr[idx], p_arr[idx + 1], p_arr[idx + 2], p_arr[idx + 3]));
    idx += 4;
  }
}
...
LiveEditor *LiveEditor::get_singleton() { return singleton; }
...
void LiveEditor::_node_set_func(int p_id, const StringName &p_prop,
                                const Variant &p_value) {
...
void LiveEditor::_node_set_res_func(int p_id, const StringName &p_prop,
...
void LiveEditor::_node_call_func(int p_id, const StringName &p_method,
                                 VARIANT_ARG_DECLARE) {
  SceneTree *scene_tree = SceneTree::get_singleton();
  if (!scene_tree)
    return;
...
void LiveEditor::_res_set_func(int p_id, const StringName &p_prop,
                               const Variant &p_value) {
...
void LiveEditor::_res_set_res_func(int p_id, const StringName &p_prop,
                                   const String &p_value) {
...
void LiveEditor::_res_call_func(int p_id, const StringName &p_method,
                                VARIANT_ARG_DECLARE) {
...
void LiveEditor::_root_func(const NodePath &p_scene_path,
...
void LiveEditor::_create_node_func(const NodePath &p_parent,
...
void LiveEditor::_instance_node_func(const NodePath &p_parent,
                                     const String &p_path,
                                     const String &p_name) {
  SceneTree *scene_tree = SceneTree::get_singleton();
  if (!scene_tree)
...
void LiveEditor::_remove_and_keep_node_func(const NodePath &p_at,
                                            ObjectID p_keep_id) {
  SceneTree *scene_tree = SceneTree::get_singleton();
  if (!scene_tree)
    return;
...
void LiveEditor::_restore_node_func(ObjectID p_id, const NodePath &p_at,
                                    int p_at_pos) {
  SceneTree *scene_tree = SceneTree::get_singleton();
  if (!scene_tree)
    return;
...
void LiveEditor::_duplicate_node_func(const NodePath &p_at,
  ...
  for (Set<Node *>::Element *F = E->get().front(); F; F = F->next()) {
    ...
    Node *dup = n2->duplicate(Node::DUPLICATE_SIGNALS | Node::DUPLICATE_GROUPS |
...
void LiveEditor::_reparent_node_func(const NodePath &p_at,
                                     const NodePath &p_new_place,
                                     const String &p_new_name, int p_at_pos) {
  SceneTree *scene_tree = SceneTree::get_singleton();
  if (!scene_tree)
    return;
