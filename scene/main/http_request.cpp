...
Error HTTPRequest::_parse_url(const String &p_url) {
  ...
  ERR_FAIL_COND_V_MSG(url.length() < 1, ERR_INVALID_PARAMETER,
                      "URL too short: " + url + ".");
...
Error HTTPRequest::request(const String &p_url,
                           const Vector<String> &p_custom_headers,
                           bool p_ssl_validate_domain,
                           HTTPClient::Method p_method,
                           const String &p_request_data) {

  ERR_FAIL_COND_V(!is_inside_tree(), ERR_UNCONFIGURED);
  ERR_FAIL_COND_V_MSG(requesting, ERR_BUSY,
                      "HTTPRequest is processing a request. Wait for "
                      "completion or cancel it before attempting a new one.");
  ...
  if (use_threads) {
    ...
    err = _request();
    if (err != OK) {
      call_deferred("_request_done", RESULT_CANT_CONNECT, 0,
...
void HTTPRequest::_thread_func(void *p_userdata) {
  ...
  if (err != OK) {
    hr->call_deferred("_request_done", RESULT_CANT_CONNECT, 0,
                      PackedStringArray(), PackedByteArray());
  } else {
...
bool HTTPRequest::_handle_response(bool *ret_value) {
  ...
  if (!client->has_response()) {
    call_deferred("_request_done", RESULT_NO_RESPONSE, 0, PackedStringArray(),
                  PackedByteArray());
    *ret_value = true;
    return true;
  }
  ...
  if (response_code == 301 || response_code == 302) {
    ...
    if (max_redirects >= 0 && redirections >= max_redirects) {
      ...
      call_deferred("_request_done", RESULT_REDIRECT_LIMIT_REACHED,
...
bool HTTPRequest::_update_connection() {
  ...
  case HTTPClient::STATUS_DISCONNECTED: {
    call_deferred("_request_done", RESULT_CANT_CONNECT, 0, PackedStringArray(),
                  PackedByteArray());
  ...
  case HTTPClient::STATUS_CANT_RESOLVE: {
    call_deferred("_request_done", RESULT_CANT_RESOLVE, 0, PackedStringArray(),
                  PackedByteArray());
    return true;
  ...
  case HTTPClient::STATUS_CONNECTED: {
    ...
    if (request_sent) {
      ...
      if (!got_response) {
        ...
        call_deferred("_request_done", RESULT_SUCCESS, response_code,
                      response_headers, PackedByteArray());
        return true;
      }
      if (body_len < 0) {
      ...
      call_deferred("_request_done", RESULT_CHUNKED_BODY_SIZE_MISMATCH,
                    response_code, response_headers, PackedByteArray());
      return true;
      ...
      Error err =
          client->request(method, request_string, headers, request_data);
      if (err != OK) {
        call_deferred("_request_done", RESULT_CONNECTION_ERROR, 0,
                      PackedStringArray(), PackedByteArray());
        return true;
      }
  ...
  case HTTPClient::STATUS_BODY: {
    ...
    if (!got_response) {
      ...
      if (!client->is_response_chunked() &&
          client->get_response_body_length() == 0) {
      ...
      if (body_size_limit >= 0 && body_len > body_size_limit) {
        call_deferred("_request_done", RESULT_BODY_SIZE_LIMIT_EXCEEDED,
                      response_code, response_headers, PackedByteArray());
        return true;
      }
      ...
      if (download_to_file != String()) {
        ...
        if (!file) {
          ...
          call_deferred("_request_done", RESULT_DOWNLOAD_FILE_CANT_OPEN,
                        response_code, response_headers, PackedByteArray());
          return true;
        }
      }
    ...
    if (file) {
      ...
      if (file->get_error() != OK) {
        call_deferred("_request_done", RESULT_DOWNLOAD_FILE_WRITE_ERROR,
                      response_code, response_headers, PackedByteArray());
        return true;
      }
    } else {
  ...
  case HTTPClient::STATUS_SSL_HANDSHAKE_ERROR: {
    call_deferred("_request_done", RESULT_SSL_HANDSHAKE_ERROR, 0,
                  PackedStringArray(), PackedByteArray());
    return true;
  } break;
  }
...
void HTTPRequest::_request_done(int p_status, int p_code,
...
bool HTTPRequest::is_using_threads() const { return use_threads; }
...
int HTTPRequest::get_body_size_limit() const { return body_size_limit; }
...
String HTTPRequest::get_download_file() const { return download_to_file; }
...
void HTTPRequest::set_max_redirects(int p_max) { max_redirects = p_max; }

int HTTPRequest::get_max_redirects() const { return max_redirects; }

int HTTPRequest::get_downloaded_bytes() const { return downloaded; }
int HTTPRequest::get_body_size() const { return body_len; }
...
int HTTPRequest::get_timeout() { return timeout; }

void HTTPRequest::_timeout() {

  cancel_request();
  call_deferred("_request_done", RESULT_TIMEOUT, 0, PackedStringArray(),
                PackedByteArray());
}
...
void HTTPRequest::_bind_methods() {
  ...
  ClassDB::bind_method(
      D_METHOD("request", "url", "custom_headers", "ssl_validate_domain",
               "method", "request_data"),
      &HTTPRequest::request, DEFVAL(PackedStringArray()), DEFVAL(true),
      DEFVAL(HTTPClient::METHOD_GET), DEFVAL(String()));
  ClassDB::bind_method(D_METHOD("cancel_request"),
                       &HTTPRequest::cancel_request);

  ClassDB::bind_method(D_METHOD("get_http_client_status"),
                       &HTTPRequest::get_http_client_status);

  ClassDB::bind_method(D_METHOD("set_use_threads", "enable"),
                       &HTTPRequest::set_use_threads);
  ClassDB::bind_method(D_METHOD("is_using_threads"),
                       &HTTPRequest::is_using_threads);

  ClassDB::bind_method(D_METHOD("set_body_size_limit", "bytes"),
                       &HTTPRequest::set_body_size_limit);
  ClassDB::bind_method(D_METHOD("get_body_size_limit"),
                       &HTTPRequest::get_body_size_limit);

  ClassDB::bind_method(D_METHOD("set_max_redirects", "amount"),
                       &HTTPRequest::set_max_redirects);
  ClassDB::bind_method(D_METHOD("get_max_redirects"),
                       &HTTPRequest::get_max_redirects);

  ClassDB::bind_method(D_METHOD("set_download_file", "path"),
                       &HTTPRequest::set_download_file);
  ClassDB::bind_method(D_METHOD("get_download_file"),
                       &HTTPRequest::get_download_file);

  ClassDB::bind_method(D_METHOD("get_downloaded_bytes"),
                       &HTTPRequest::get_downloaded_bytes);
  ClassDB::bind_method(D_METHOD("get_body_size"), &HTTPRequest::get_body_size);

  ClassDB::bind_method(D_METHOD("_redirect_request"),
                       &HTTPRequest::_redirect_request);
  ClassDB::bind_method(D_METHOD("_request_done"), &HTTPRequest::_request_done);

  ClassDB::bind_method(D_METHOD("set_timeout", "timeout"),
                       &HTTPRequest::set_timeout);
  ClassDB::bind_method(D_METHOD("get_timeout"), &HTTPRequest::get_timeout);

  ClassDB::bind_method(D_METHOD("set_download_chunk_size"),
                       &HTTPRequest::set_download_chunk_size);
  ClassDB::bind_method(D_METHOD("get_download_chunk_size"),
  ...
  ADD_PROPERTY(PropertyInfo(Variant::INT, "download_chunk_size",
                            PROPERTY_HINT_RANGE, "256,16777216"),
               "set_download_chunk_size", "get_download_chunk_size");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "use_threads"), "set_use_threads",
               "is_using_threads");
  ADD_PROPERTY(PropertyInfo(Variant::INT, "body_size_limit",
                            PROPERTY_HINT_RANGE, "-1,2000000000"),
               "set_body_size_limit", "get_body_size_limit");
  ADD_PROPERTY(
      PropertyInfo(Variant::INT, "max_redirects", PROPERTY_HINT_RANGE, "-1,64"),
      "set_max_redirects", "get_max_redirects");
  ADD_PROPERTY(
      PropertyInfo(Variant::INT, "timeout", PROPERTY_HINT_RANGE, "0,86400"),
