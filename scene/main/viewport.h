...
private:
  ...
  struct CameraOverrideData {
    Transform transform;
    enum Projection { PROJECTION_PERSPECTIVE, PROJECTION_ORTHOGONAL };
  ...
  void _collision_object_input_event(CollisionObject *p_object,
                                     Camera *p_camera,
  ...
  Control *_gui_find_control_at_pos(CanvasItem *p_node, const Point2 &p_global,
  ...
  void _gui_remove_from_modal_stack(List<Control *>::Element *MI,
  ...
  void _gui_force_drag(Control *p_base, const Variant &p_data,
                       Control *p_control);
...
public:
  ...
  void set_camera_override_perspective(float p_fovy_degrees, float p_z_near,
                                       float p_z_far);
  void set_camera_override_orthogonal(float p_size, float p_z_near,
                                      float p_z_far);
  ...
  void set_size_override(bool p_enable, const Size2 &p_size = Size2(-1, -1),
  ...
  void set_shadow_atlas_quadrant_subdiv(int p_quadrant,
                                        ShadowAtlasQuadrantSubdiv p_subdiv);
  ShadowAtlasQuadrantSubdiv
  get_shadow_atlas_quadrant_subdiv(int p_quadrant) const;
