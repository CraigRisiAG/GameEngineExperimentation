...
void Node::_notification(int p_notification) {
  ...
  case NOTIFICATION_PROCESS: {
    ...
    if (get_script_instance()) {
      ...
      get_script_instance()->call_multilevel(
          SceneStringNames::get_singleton()->_process, ptr, 1);
    }
  } break;
  case NOTIFICATION_PHYSICS_PROCESS: {
    ...
    if (get_script_instance()) {
      ...
      const Variant *ptr[1] = {&time};
      get_script_instance()->call_multilevel(
          SceneStringNames::get_singleton()->_physics_process, ptr, 1);
    }
  ...
  case NOTIFICATION_ENTER_TREE: {
    ...
    if (data.unhandled_input)
      add_to_group("_vp_unhandled_input" +
                   itos(get_viewport()->get_instance_id()));
    if (data.unhandled_key_input)
      add_to_group("_vp_unhandled_key_input" +
                   itos(get_viewport()->get_instance_id()));
  ...
  case NOTIFICATION_EXIT_TREE: {
    ...
    if (data.unhandled_input)
      remove_from_group("_vp_unhandled_input" +
                        itos(get_viewport()->get_instance_id()));
    if (data.unhandled_key_input)
      remove_from_group("_vp_unhandled_key_input" +
                        itos(get_viewport()->get_instance_id()));
  ...
  case NOTIFICATION_READY: {
    ...
    if (get_script_instance()) {
      ...
      get_script_instance()->call_multilevel_reversed(
          SceneStringNames::get_singleton()->_ready, NULL, 0);
    }
  ...
  case NOTIFICATION_PREDELETE: {
    ...
    while (data.children.size()) {
      ...
      Node *child = data.children[data.children.size() -
                                  1]; // begin from the end because its faster
                                      // and more consistent with creation
      remove_child(child);
      memdelete(child);
    }
...
void Node::_propagate_enter_tree() {
  ...
  if (get_script_instance()) {
    ...
    get_script_instance()->call_multilevel_reversed(
        SceneStringNames::get_singleton()->_enter_tree, NULL, 0);
  }
  ...
  for (int i = 0; i < data.children.size(); i++) {
    ...
    if (!data.children[i]
             ->is_inside_tree()) // could have been added in enter_tree
...
void Node::_propagate_exit_tree() {
  ...
  if (get_script_instance()) {
    ...
    get_script_instance()->call_multilevel(
        SceneStringNames::get_singleton()->_exit_tree, NULL, 0);
  }
...
void Node::move_child(Node *p_child, int p_pos) {
  ...
  ERR_FAIL_INDEX_MSG(p_pos, data.children.size() + 1,
                     "Invalid new child position: " + itos(p_pos) + ".");
  ERR_FAIL_COND_MSG(p_child->data.parent != this,
                    "Child is not a child of this node.");
  ERR_FAIL_COND_MSG(data.blocked > 0,
                    "Parent node is busy setting up children, move_child() "
                    "failed. Consider using call_deferred(\"move_child\") "
                    "instead (or \"popup\" if this is from a popup).");
...
Node::PauseMode Node::get_pause_mode() const { return data.pause_mode; }
...
int Node::get_network_master() const { return data.network_master; }
...
uint16_t Node::rpc_config(const StringName &p_method,
...
uint16_t Node::rset_config(const StringName &p_property,
                           MultiplayerAPI::RPCMode p_mode) {
...
void Node::rpc_id(int p_peer_id, const StringName &p_method,
                  VARIANT_ARG_DECLARE) {
...
void Node::rpc_unreliable_id(int p_peer_id, const StringName &p_method,
                             VARIANT_ARG_DECLARE) {
...
Variant Node::_rpc_bind(const Variant **p_args, int p_argcount,
...
Variant Node::_rpc_id_bind(const Variant **p_args, int p_argcount,
                           Callable::CallError &r_error) {
...
Variant Node::_rpc_unreliable_bind(const Variant **p_args, int p_argcount,
                                   Callable::CallError &r_error) {
...
Variant Node::_rpc_unreliable_id_bind(const Variant **p_args, int p_argcount,
                                      Callable::CallError &r_error) {
...
void Node::rpcp(int p_peer_id, bool p_unreliable, const StringName &p_method,
                const Variant **p_arg, int p_argcount) {
  ERR_FAIL_COND(!is_inside_tree());
  get_multiplayer()->rpcp(this, p_peer_id, p_unreliable, p_method, p_arg,
                          p_argcount);
}

void Node::rsetp(int p_peer_id, bool p_unreliable, const StringName &p_property,
...
void Node::rset_id(int p_peer_id, const StringName &p_property,
                   const Variant &p_value) {

  rsetp(p_peer_id, false, p_property, p_value);
}

void Node::rset_unreliable(const StringName &p_property,
                           const Variant &p_value) {

  rsetp(0, true, p_property, p_value);
}

void Node::rset_unreliable_id(int p_peer_id, const StringName &p_property,
                              const Variant &p_value) {
...
Ref<MultiplayerAPI> Node::get_custom_multiplayer() const { return multiplayer; }
...
Node::get_node_rpc_mode_by_id(const uint16_t p_rpc_method_id) const {
  // Make sure this is a node generated ID.
  if (((1 << 15) & p_rpc_method_id) > 0) {
    int mid = (~(1 << 15)) & p_rpc_method_id;
    if (mid < data.rpc_methods.size())
...
MultiplayerAPI::RPCMode
Node::get_node_rpc_mode(const StringName &p_method) const {
...
StringName
Node::get_node_rset_property(const uint16_t p_rset_property_id) const {
  // Make sure this is a node generated ID.
...
MultiplayerAPI::RPCMode
Node::get_node_rset_mode_by_id(const uint16_t p_rset_property_id) const {
  if (((1 << 15) & p_rset_property_id) > 0) {
    int mid = (~(1 << 15)) & p_rset_property_id;
    if (mid < data.rpc_properties.size())
...
MultiplayerAPI::RPCMode
Node::get_node_rset_mode(const StringName &p_property) const {
...
bool Node::is_processing() const { return data.idle_process; }
...
bool Node::is_processing_internal() const { return data.idle_process_internal; }
...
void Node::_validate_child_name(Node *p_child, bool p_force_human_readable) {
  ...
  if (node_hrcr || p_force_human_readable) {
    ...
    if (p_child->data.name == StringName() ||
        p_child->data.name.operator String()[0] == '@') {
    ...
    if (!unique) {
      ...
      String name =
          "@" + String(p_child->get_name()) + "@" + itos(node_hrcr_count.get());
      p_child->data.name = name;
    }
  }
}
...
void Node::_generate_serial_child_name(const Node *p_child,
...
void Node::add_child(Node *p_child, bool p_legible_unique_name) {
  ...
  ERR_FAIL_NULL(p_child);
  ERR_FAIL_COND_MSG(p_child == this, "Can't add child '" + p_child->get_name() +
                                         "' to itself."); // adding to itself!
  ERR_FAIL_COND_MSG(p_child->data.parent,
                    "Can't add child '" + p_child->get_name() + "' to '" +
                        get_name() + "', already has a parent '" +
...
void Node::add_child_below_node(Node *p_node, Node *p_child,
  ...
  if (is_a_parent_of(p_node)) {
    ...
    WARN_PRINT("Cannot move under node " + p_node->get_name() + " as " +
...
void Node::remove_child(Node *p_child) {
  ...
  ERR_FAIL_COND_MSG(
      data.blocked > 0,
      "Parent node is busy setting up children, remove_node() failed. Consider "
      "using call_deferred(\"remove_child\", child) instead.");
  ...
  if (idx == -1) { // maybe removed while unparenting or something and index was
  ...
  ERR_FAIL_COND_MSG(idx == -1, "Cannot remove child node " +
                                   p_child->get_name() +
                                   " as it is not a child of this node.");
...
int Node::get_child_count() const { return data.children.size(); }
...
Node *Node::get_node_or_null(const NodePath &p_path) const {
  ...
  ERR_FAIL_COND_V_MSG(!data.inside_tree && p_path.is_absolute(), NULL,
                      "Can't use get_node() with absolute paths from outside "
...
Node *Node::find_node(const String &p_mask, bool p_recursive,
                      bool p_owned) const {
...
void Node::_print_tree_pretty(const String &prefix, const bool last) {
  ...
  for (int i = 0; i < data.children.size(); i++) {
    ...
    data.children[i]->_print_tree_pretty(prefix + new_prefix,
                                         i == data.children.size() - 1);
  }
}

void Node::print_tree_pretty() { _print_tree_pretty("", true); }

void Node::print_tree() { _print_tree(this); }
...
void Node::_propagate_deferred_notification(int p_notification,
                                            bool p_reverse) {
  ...
  for (int i = 0; i < data.children.size(); i++) {
    ...
    data.children[i]->_propagate_deferred_notification(p_notification,
                                                       p_reverse);
  }
...
void Node::propagate_call(const StringName &p_method, const Array &p_args,
...
int Node::get_index() const { return data.pos; }
...
String Node::get_filename() const { return data.filename; }
...
void Node::set_editable_instances(
    const HashMap<NodePath, int> &p_editable_instances) {
...
int Node::get_position_in_parent() const { return data.pos; }

Node *Node::_duplicate(int p_flags,
                       Map<const Node *, Node *> *r_duplimap) const {
  ...
  if (instanced) {
    // Since nodes in the instanced hierarchy won't be duplicated explicitly, we
    // need to make an inventory of all the nodes in the tree of the instanced
    // scene in order to transfer the values of the properties

    for (List<const Node *>::Element *N = node_tree.front(); N; N = N->next()) {
      for (int i = 0; i < N->get()->get_child_count(); ++i) {

        Node *descendant = N->get()->get_child(i);
        // Skip nodes not really belonging to the instanced hierarchy; they'll
        // be processed normally later but remember non-instanced nodes that are
        // hidden below instanced ones
        if (descendant->data.owner != this) {
          if (descendant->get_parent() && descendant->get_parent() != this &&
              descendant->get_parent()->data.owner == this &&
...
Node *Node::duplicate_from_editor(Map<const Node *, Node *> &r_duplimap) const {
  ...
  Node *dupe =
      _duplicate(DUPLICATE_SIGNALS | DUPLICATE_GROUPS | DUPLICATE_SCRIPTS |
...
void Node::_duplicate_and_reown(Node *p_new_parent,
                                const Map<Node *, Node *> &p_reown_map) const {
  ...
  if (get_filename() != "") {
    ...
    Object *obj = ClassDB::instance(get_class());
    ERR_FAIL_COND_MSG(
        !obj, "Node: Could not duplicate: " + String(get_class()) + ".");
    node = Object::cast_to<Node>(obj);
    if (!node) {
      memdelete(obj);
...
void Node::_duplicate_signals(const Node *p_original, Node *p_copy) const {
  ...
  if (this != p_original &&
      (get_owner() != p_original && get_owner() != p_original->get_owner()))
    return;
  ...
  for (List<Connection>::Element *E = conns.front(); E; E = E->next()) {
    ...
    if (E->get().flags & CONNECT_PERSIST) {
      ...
      // Attempt to find a path to the duplicate target, if it seems it's not
      // part of the duplicated and not yet parented hierarchy then at least try
      // to connect to the same target as the original

      if (p_copy->has_node(ptarget))
        copytarget = p_copy->get_node(ptarget);

      if (copy && copytarget) {
        const Callable copy_callable =
            Callable(copytarget, E->get().callable.get_method());
        if (!copy->is_connected(E->get().signal.get_name(), copy_callable)) {
          copy->connect(E->get().signal.get_name(), copy_callable,
                        E->get().binds, E->get().flags);
        }
      }
    }
  }
...
Node *Node::duplicate_and_reown(const Map<Node *, Node *> &p_reown_map) const {
  ...
  Object *obj = ClassDB::instance(get_class());
  ERR_FAIL_COND_V_MSG(
      !obj, NULL, "Node: Could not duplicate: " + String(get_class()) + ".");
  ...
  for (List<Connection>::Element *E = cl.front(); E; E = E->next()) {
    ...
    if (c.flags & CONNECT_PERSIST) {
      c.signal.get_object()->disconnect(
          c.signal.get_name(), Callable(this, c.callable.get_method()));
      bool valid = p_new_target->has_method(c.callable.get_method()) ||
                   ...
                   Ref<Script>(p_new_target->get_script())
                       ->has_method(c.callable.get_method());
      ERR_CONTINUE_MSG(!valid, "Attempt to connect signal '" +
                                   c.signal.get_object()->get_class() + "." +
                                   c.signal.get_name() +
                                   "' to nonexistent method '" +
                                   c.callable.get_object()->get_class() + "." +
                                   c.callable.get_method() + "'.");
      c.signal.get_object()->connect(
          c.signal.get_name(), Callable(p_new_target, c.callable.get_method()),
          c.binds, c.flags);
    }
  }
}
...
Node *Node::get_node_and_resource(const NodePath &p_path, RES &r_res,
                                  Vector<StringName> &r_leftover_subpath,
                                  bool p_last_is_property) const {
  ...
  if (p_path.get_subname_count()) {
    ...
    int j = 0;
    // If not p_last_is_property, we shouldn't consider the last one as part of
    // the resource
...
void Node::_set_tree(SceneTree *p_tree) {
  ...
  // ERR_FAIL_COND(p_scene && data.parent && !data.parent->data.scene); //nobug
  // if both are null
  ...
  if (data.tree) {
    ...
    if (!data.parent ||
        data.parent->data.ready_notified) { // No parent (root) or parent ready
...
static void _Node_debug_sn(Object *p_obj) {
  ...
  print_line(itos(p_obj->get_instance_id()) + " - Stray Node: " + path +
             " (Type: " + n->get_class() + ")");
}
#endif // DEBUG_ENABLED

void Node::_print_stray_nodes() { print_stray_nodes(); }
...
static void _add_nodes_to_options(const Node *p_base, const Node *p_node,
...
void Node::get_argument_options(const StringName &p_function, int p_idx,
...
void Node::update_configuration_warning() {
  ...
  if (get_tree()->get_edited_scene_root() &&
    ...
    get_tree()->emit_signal(
        SceneStringNames::get_singleton()->node_configuration_warning_changed,
        this);
  }
#endif
}

bool Node::is_owned_by_parent() const { return data.parent_owned; }

void Node::set_display_folded(bool p_folded) { data.display_folded = p_folded; }

bool Node::is_displayed_folded() const { return data.display_folded; }

void Node::request_ready() { data.ready_first = true; }
...
void Node::_bind_methods() {
  ...
  ProjectSettings::get_singleton()->set_custom_property_info(
      ...
      PropertyInfo(Variant::INT, "node/name_num_separator", PROPERTY_HINT_ENUM,
                   "None,Space,Underscore,Dash"));
  GLOBAL_DEF("node/name_casing", NAME_CASING_PASCAL_CASE);
  ProjectSettings::get_singleton()->set_custom_property_info(
      "node/name_casing",
      PropertyInfo(Variant::INT, "node/name_casing", PROPERTY_HINT_ENUM,
                   "PascalCase,camelCase,snake_case"));

  ClassDB::bind_method(D_METHOD("add_child_below_node", "preceding_node",
                                "node", "legible_unique_name"),
                       &Node::add_child_below_node, DEFVAL(false));

  ClassDB::bind_method(D_METHOD("set_name", "name"), &Node::set_name);
  ClassDB::bind_method(D_METHOD("get_name"), &Node::get_name);
  ClassDB::bind_method(D_METHOD("add_child", "node", "legible_unique_name"),
                       &Node::add_child, DEFVAL(false));
  ...
  ClassDB::bind_method(D_METHOD("get_node_or_null", "path"),
                       &Node::get_node_or_null);
  ClassDB::bind_method(D_METHOD("get_parent"), &Node::get_parent);
  ClassDB::bind_method(D_METHOD("find_node", "mask", "recursive", "owned"),
                       &Node::find_node, DEFVAL(true), DEFVAL(true));
  ClassDB::bind_method(D_METHOD("find_parent", "mask"), &Node::find_parent);
  ClassDB::bind_method(D_METHOD("has_node_and_resource", "path"),
                       &Node::has_node_and_resource);
  ClassDB::bind_method(D_METHOD("get_node_and_resource", "path"),
                       &Node::_get_node_and_resource);

  ClassDB::bind_method(D_METHOD("is_inside_tree"), &Node::is_inside_tree);
  ClassDB::bind_method(D_METHOD("is_a_parent_of", "node"),
                       &Node::is_a_parent_of);
  ClassDB::bind_method(D_METHOD("is_greater_than", "node"),
  ...
  ClassDB::bind_method(D_METHOD("add_to_group", "group", "persistent"),
                       &Node::add_to_group, DEFVAL(false));
  ClassDB::bind_method(D_METHOD("remove_from_group", "group"),
                       &Node::remove_from_group);
  ClassDB::bind_method(D_METHOD("is_in_group", "group"), &Node::is_in_group);
  ClassDB::bind_method(D_METHOD("move_child", "child_node", "to_position"),
                       &Node::move_child);
  ...
  ClassDB::bind_method(D_METHOD("set_filename", "filename"),
                       &Node::set_filename);
  ClassDB::bind_method(D_METHOD("get_filename"), &Node::get_filename);
  ClassDB::bind_method(D_METHOD("propagate_notification", "what"),
                       &Node::propagate_notification);
  ClassDB::bind_method(
      D_METHOD("propagate_call", "method", "args", "parent_first"),
      &Node::propagate_call, DEFVAL(Array()), DEFVAL(false));
  ClassDB::bind_method(D_METHOD("set_physics_process", "enable"),
                       &Node::set_physics_process);
  ClassDB::bind_method(D_METHOD("get_physics_process_delta_time"),
                       &Node::get_physics_process_delta_time);
  ClassDB::bind_method(D_METHOD("is_physics_processing"),
                       &Node::is_physics_processing);
  ClassDB::bind_method(D_METHOD("get_process_delta_time"),
                       &Node::get_process_delta_time);
  ClassDB::bind_method(D_METHOD("set_process", "enable"), &Node::set_process);
  ClassDB::bind_method(D_METHOD("set_process_priority", "priority"),
                       &Node::set_process_priority);
  ClassDB::bind_method(D_METHOD("get_process_priority"),
                       &Node::get_process_priority);
  ClassDB::bind_method(D_METHOD("is_processing"), &Node::is_processing);
  ClassDB::bind_method(D_METHOD("set_process_input", "enable"),
                       &Node::set_process_input);
  ClassDB::bind_method(D_METHOD("is_processing_input"),
                       &Node::is_processing_input);
  ClassDB::bind_method(D_METHOD("set_process_unhandled_input", "enable"),
                       &Node::set_process_unhandled_input);
  ClassDB::bind_method(D_METHOD("is_processing_unhandled_input"),
  ...
  ClassDB::bind_method(D_METHOD("is_processing_unhandled_key_input"),
                       &Node::is_processing_unhandled_key_input);
  ClassDB::bind_method(D_METHOD("set_pause_mode", "mode"),
                       &Node::set_pause_mode);
  ClassDB::bind_method(D_METHOD("get_pause_mode"), &Node::get_pause_mode);
  ClassDB::bind_method(D_METHOD("can_process"), &Node::can_process);
  ClassDB::bind_method(D_METHOD("print_stray_nodes"),
                       &Node::_print_stray_nodes);
  ClassDB::bind_method(D_METHOD("get_position_in_parent"),
                       &Node::get_position_in_parent);

  ClassDB::bind_method(D_METHOD("set_display_folded", "fold"),
                       &Node::set_display_folded);
  ClassDB::bind_method(D_METHOD("is_displayed_folded"),
                       &Node::is_displayed_folded);

  ClassDB::bind_method(D_METHOD("set_process_internal", "enable"),
                       &Node::set_process_internal);
  ClassDB::bind_method(D_METHOD("is_processing_internal"),
                       &Node::is_processing_internal);

  ClassDB::bind_method(D_METHOD("set_physics_process_internal", "enable"),
                       &Node::set_physics_process_internal);
  ClassDB::bind_method(D_METHOD("is_physics_processing_internal"),
                       &Node::is_physics_processing_internal);

  ClassDB::bind_method(D_METHOD("get_tree"), &Node::get_tree);

  ClassDB::bind_method(D_METHOD("duplicate", "flags"), &Node::duplicate,
                       DEFVAL(DUPLICATE_USE_INSTANCING | DUPLICATE_SIGNALS |
                              DUPLICATE_GROUPS | DUPLICATE_SCRIPTS));
  ClassDB::bind_method(D_METHOD("replace_by", "node", "keep_data"),
                       &Node::replace_by, DEFVAL(false));
  ...
  ClassDB::bind_method(D_METHOD("set_network_master", "id", "recursive"),
                       &Node::set_network_master, DEFVAL(true));
  ClassDB::bind_method(D_METHOD("get_network_master"),
                       &Node::get_network_master);

  ClassDB::bind_method(D_METHOD("is_network_master"), &Node::is_network_master);

  ClassDB::bind_method(D_METHOD("get_multiplayer"), &Node::get_multiplayer);
  ClassDB::bind_method(D_METHOD("get_custom_multiplayer"),
                       &Node::get_custom_multiplayer);
  ClassDB::bind_method(D_METHOD("set_custom_multiplayer", "api"),
                       &Node::set_custom_multiplayer);
  ClassDB::bind_method(D_METHOD("rpc_config", "method", "mode"),
                       &Node::rpc_config);
  ClassDB::bind_method(D_METHOD("rset_config", "property", "mode"),
                       &Node::rset_config);

  ClassDB::bind_method(
      D_METHOD("_set_editor_description", "editor_description"),
      &Node::set_editor_description);
  ClassDB::bind_method(D_METHOD("_get_editor_description"),
                       &Node::get_editor_description);
  ADD_PROPERTY(PropertyInfo(Variant::STRING, "editor_description",
                            PROPERTY_HINT_MULTILINE_TEXT, "",
                            PROPERTY_USAGE_EDITOR | PROPERTY_USAGE_INTERNAL),
               "_set_editor_description", "_get_editor_description");

  ClassDB::bind_method(D_METHOD("_set_import_path", "import_path"),
                       &Node::set_import_path);
  ClassDB::bind_method(D_METHOD("_get_import_path"), &Node::get_import_path);
  ADD_PROPERTY(PropertyInfo(Variant::NODE_PATH, "_import_path",
                            PROPERTY_HINT_NONE, "",
                            PROPERTY_USAGE_NOEDITOR | PROPERTY_USAGE_INTERNAL),
               "_set_import_path", "_get_import_path");

  {
    ...
    mi.name = "rpc";
    ClassDB::bind_vararg_method(METHOD_FLAGS_DEFAULT, "rpc", &Node::_rpc_bind,
                                mi);
    mi.name = "rpc_unreliable";
    ClassDB::bind_vararg_method(METHOD_FLAGS_DEFAULT, "rpc_unreliable",
                                &Node::_rpc_unreliable_bind, mi);

    mi.arguments.push_front(PropertyInfo(Variant::INT, "peer_id"));

    mi.name = "rpc_id";
    ClassDB::bind_vararg_method(METHOD_FLAGS_DEFAULT, "rpc_id",
                                &Node::_rpc_id_bind, mi);
    mi.name = "rpc_unreliable_id";
    ClassDB::bind_vararg_method(METHOD_FLAGS_DEFAULT, "rpc_unreliable_id",
                                &Node::_rpc_unreliable_id_bind, mi);
  }

  ClassDB::bind_method(D_METHOD("rset", "property", "value"), &Node::rset);
  ClassDB::bind_method(D_METHOD("rset_id", "peer_id", "property", "value"),
                       &Node::rset_id);
  ClassDB::bind_method(D_METHOD("rset_unreliable", "property", "value"),
                       &Node::rset_unreliable);
  ClassDB::bind_method(
      D_METHOD("rset_unreliable_id", "peer_id", "property", "value"),
      &Node::rset_unreliable_id);

  ClassDB::bind_method(D_METHOD("update_configuration_warning"),
  ...
  ADD_PROPERTY(PropertyInfo(Variant::INT, "pause_mode", PROPERTY_HINT_ENUM,
                            "Inherit,Stop,Process"),
               "set_pause_mode", "get_pause_mode");

  ADD_PROPERTY(
      PropertyInfo(Variant::STRING_NAME, "name", PROPERTY_HINT_NONE, "", 0),
      "set_name", "get_name");
  ADD_PROPERTY(
      PropertyInfo(Variant::STRING, "filename", PROPERTY_HINT_NONE, "", 0),
      "set_filename", "get_filename");
  ADD_PROPERTY(PropertyInfo(Variant::OBJECT, "owner",
                            PROPERTY_HINT_RESOURCE_TYPE, "Node", 0),
               "set_owner", "get_owner");
  ADD_PROPERTY(PropertyInfo(Variant::OBJECT, "multiplayer",
                            PROPERTY_HINT_RESOURCE_TYPE, "MultiplayerAPI", 0),
               "", "get_multiplayer");
  ADD_PROPERTY(PropertyInfo(Variant::OBJECT, "custom_multiplayer",
                            PROPERTY_HINT_RESOURCE_TYPE, "MultiplayerAPI", 0),
               "set_custom_multiplayer", "get_custom_multiplayer");
  ADD_PROPERTY(PropertyInfo(Variant::INT, "process_priority"),
               "set_process_priority", "get_process_priority");

  BIND_VMETHOD(MethodInfo("_process", PropertyInfo(Variant::FLOAT, "delta")));
  BIND_VMETHOD(
      MethodInfo("_physics_process", PropertyInfo(Variant::FLOAT, "delta")));
  BIND_VMETHOD(MethodInfo("_enter_tree"));
  BIND_VMETHOD(MethodInfo("_exit_tree"));
  BIND_VMETHOD(MethodInfo("_ready"));
  BIND_VMETHOD(MethodInfo("_input", PropertyInfo(Variant::OBJECT, "event",
                                                 PROPERTY_HINT_RESOURCE_TYPE,
                                                 "InputEvent")));
  BIND_VMETHOD(
      MethodInfo("_unhandled_input",
                 PropertyInfo(Variant::OBJECT, "event",
                              PROPERTY_HINT_RESOURCE_TYPE, "InputEvent")));
  BIND_VMETHOD(
      MethodInfo("_unhandled_key_input",
                 PropertyInfo(Variant::OBJECT, "event",
                              PROPERTY_HINT_RESOURCE_TYPE, "InputEventKey")));
