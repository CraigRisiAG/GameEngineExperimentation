...
private:
  ...
  struct Data {
    ...
    int pos;
    int depth;
    int blocked; // safeguard that throws an error when attempting to modify the
                 // tree in a harmful way while being traversed.
    StringName name;
    SceneTree *tree;
    bool inside_tree;
    bool ready_notified; // this is a small hack, so if a node is added during
                         // _ready() to the tree, it correctly gets the _ready()
                         // notification
    bool ready_first;
#ifdef TOOLS_ENABLED
    NodePath import_path; // path used when imported, used by scene editors to
                          // keep tracking
#endif
  ...
  void _duplicate_and_reown(Node *p_new_parent,
                            const Map<Node *, Node *> &p_reown_map) const;
  Node *_duplicate(int p_flags,
                   Map<const Node *, Node *> *r_duplimap = NULL) const;

  Array _get_children() const;
  Array _get_groups() const;

  Variant _rpc_bind(const Variant **p_args, int p_argcount,
                    Callable::CallError &r_error);
  Variant _rpc_unreliable_bind(const Variant **p_args, int p_argcount,
                               Callable::CallError &r_error);
  Variant _rpc_id_bind(const Variant **p_args, int p_argcount,
                       Callable::CallError &r_error);
  Variant _rpc_unreliable_id_bind(const Variant **p_args, int p_argcount,
                                  Callable::CallError &r_error);
...
public:
  ...
  void add_child_below_node(Node *p_node, Node *p_child,
  ...
  Node *find_node(const String &p_mask, bool p_recursive = true,
                  bool p_owned = true) const;
  bool has_node_and_resource(const NodePath &p_path) const;
  Node *get_node_and_resource(const NodePath &p_path, RES &r_res,
                              Vector<StringName> &r_leftover_subpath,
                              bool p_last_is_property = true) const;
  ...
  void
  set_editable_instances(const HashMap<NodePath, int> &p_editable_instances);
  ...
  Node *duplicate(int p_flags = DUPLICATE_GROUPS | DUPLICATE_SIGNALS |
                                DUPLICATE_SCRIPTS) const;
  ...
  void set_import_path(
      const NodePath &p_import_path); // path used when imported, used by scene
                                      // editors to keep tracking
  NodePath get_import_path() const;

  bool is_owned_by_parent() const;

  void get_argument_options(const StringName &p_function, int p_idx,
  ...
  uint16_t
  rpc_config(const StringName &p_method,
             MultiplayerAPI::RPCMode p_mode); // config a local method for RPC
  uint16_t rset_config(
      const StringName &p_property,
      MultiplayerAPI::RPCMode p_mode); // config a local property for RPC

  void rpc(const StringName &p_method,
           VARIANT_ARG_LIST); // rpc call, honors RPCMode
  void rpc_unreliable(const StringName &p_method,
                      VARIANT_ARG_LIST); // rpc call, honors RPCMode
  void rpc_id(int p_peer_id, const StringName &p_method,
              VARIANT_ARG_LIST); // rpc call, honors RPCMode
  void rpc_unreliable_id(int p_peer_id, const StringName &p_method,
                         VARIANT_ARG_LIST); // rpc call, honors RPCMode

  void rset(const StringName &p_property,
            const Variant &p_value); // remote set call, honors RPCMode
  void
  rset_unreliable(const StringName &p_property,
                  const Variant &p_value); // remote set call, honors RPCMode
  void rset_id(int p_peer_id, const StringName &p_property,
               const Variant &p_value); // remote set call, honors RPCMode
  void
  rset_unreliable_id(int p_peer_id, const StringName &p_property,
                     const Variant &p_value); // remote set call, honors RPCMode

  void rpcp(int p_peer_id, bool p_unreliable, const StringName &p_method,
            const Variant **p_arg, int p_argcount);
  void rsetp(int p_peer_id, bool p_unreliable, const StringName &p_property,
  ...
  get_node_rpc_mode_by_id(const uint16_t p_rpc_method_id) const;
  ...
  MultiplayerAPI::RPCMode
  get_node_rset_mode_by_id(const uint16_t p_rpc_method_id) const;
  MultiplayerAPI::RPCMode
  get_node_rset_mode(const StringName &p_property) const;
