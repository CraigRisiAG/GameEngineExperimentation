...
void ViewportTexture::setup_local_to_scene() {
  ...
  ERR_FAIL_COND_MSG(
      !vp, "ViewportTexture: Path to node does not point to a viewport.");
...
Ref<Image> ViewportTexture::get_data() const {
  ...
  ERR_FAIL_COND_V_MSG(!vp, Ref<Image>(),
...
void ViewportTexture::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("set_viewport_path_in_scene", "path"),
                       &ViewportTexture::set_viewport_path_in_scene);
  ClassDB::bind_method(D_METHOD("get_viewport_path_in_scene"),
                       &ViewportTexture::get_viewport_path_in_scene);

  ADD_PROPERTY(PropertyInfo(Variant::NODE_PATH, "viewport_path",
                            PROPERTY_HINT_NODE_PATH_VALID_TYPES, "Viewport",
...
void Viewport::_collision_object_input_event(
    CollisionObject *p_object, Camera *p_camera,
    const Ref<InputEvent> &p_input_event, const Vector3 &p_pos,
    const Vector3 &p_normal, int p_shape) {

  Transform object_transform = p_object->get_global_transform();
  Transform camera_transform = p_camera->get_global_transform();
  ObjectID id = p_object->get_instance_id();

  // avoid sending the fake event unnecessarily if nothing really changed in the
  // context
...
void Viewport::_own_world_changed() {
  ...
  if (is_inside_tree()) {
    VisualServer::get_singleton()->viewport_set_scenario(
        viewport, find_world()->get_scenario());
  }
...
void Viewport::_notification(int p_what) {
  ...
  case NOTIFICATION_ENTER_TREE: {
    ...
    if (get_parent()) {
      ...
      VisualServer::get_singleton()->viewport_set_parent_viewport(
          viewport, parent->get_viewport_rid());
    } else {
      parent = NULL;
    }
    ...
    if (get_tree()->is_debugging_collisions_hint()) {
      ...
      VisualServer::get_singleton()->canvas_item_set_parent(
          contact_2d_debug, find_world_2d()->get_canvas());
      // 3D
      ...
      VisualServer::get_singleton()->multimesh_allocate(
          contact_3d_debug_multimesh,
          get_tree()->get_collision_debug_contact_count(),
          VS::MULTIMESH_TRANSFORM_3D, true);
      VisualServer::get_singleton()->multimesh_set_visible_instances(
          contact_3d_debug_multimesh, 0);
      ...
      VisualServer::get_singleton()->instance_set_base(
          contact_3d_debug_instance, contact_3d_debug_multimesh);
      VisualServer::get_singleton()->instance_set_scenario(
          contact_3d_debug_instance, find_world()->get_scenario());
  ...
  case NOTIFICATION_READY: {
    ...
    if (cameras.size() && !camera) {
      // there are cameras but no current camera, pick first in tree and make it
      // current
      Camera *first = NULL;
    ...
    // Enable processing for tooltips, collision debugging, physics object
    // picking, etc.
  ...
  case NOTIFICATION_EXIT_TREE: {
    ...
    //RID());
    VisualServer::get_singleton()->viewport_remove_canvas(viewport,
                                                          current_canvas);
  ...
  case NOTIFICATION_INTERNAL_PHYSICS_PROCESS: {
    ...
    if (get_tree()->is_debugging_collisions_hint() &&
      ...
      VisualServer::get_singleton()->canvas_item_set_draw_index(
          contact_2d_debug, 0xFFFFF); // very high index
      ...
      for (int i = 0; i < point_count; i++) {
        ...
        VisualServer::get_singleton()->canvas_item_add_rect(
            contact_2d_debug, Rect2(points[i] - Vector2(2, 2), Vector2(5, 5)),
            ccol);
      }
    }
    ...
    if (get_tree()->is_debugging_collisions_hint() &&
      ...
      Vector<Vector3> points =
          PhysicsServer::get_singleton()->space_get_contacts(
              find_world()->get_space());
      int point_count = PhysicsServer::get_singleton()->space_get_contact_count(
          find_world()->get_space());

      VS::get_singleton()->multimesh_set_visible_instances(
          contact_3d_debug_multimesh, point_count);
    }

    if (physics_object_picking && (to_screen_rect == Rect2() ||
      ...
      while (physics_picking_events.size()) {
        ...
        if (mb.is_valid()) {
          ...
          if (mb->is_pressed()) {
            ...
            // If touch mouse raised, assume we don't know last mouse pos until
            // new events come
        ...
        if (ss2d) {
          ...
          for (Set<CanvasLayer *>::Element *E = canvas_layers.front(); E;
            ...
            int rc = ss2d->intersect_point_on_canvas(
                point, canvas_layer_id, res, 64, Set<RID>(), 0xFFFFFFFF, true,
                true, true);
            for (int i = 0; i < rc; i++) {
                ...
                if (co) {
                  ...
                  if (is_mouse) {
                    ...
                    if (!F) {
                      ...
                      F->get() = frame;
                      // It was already hovered, so don't send the event if it's
                      // faked
                      if (mm.is_valid() &&
        ...
        if (physics_object_capture.is_valid()) {
          ...
          if (co && camera) {
            _collision_object_input_event(co, camera, ev, Vector3(), Vector3(),
                                          0);
            captured = true;
        ...
        if (captured) {
          ...
          if (last_id.is_valid()) {
            if (ObjectDB::get_instance(last_id) && last_object) {
              // good, exists
              _collision_object_input_event(last_object, camera, ev,
                                            result.position, result.normal,
                                            result.shape);
              if (last_object->get_capture_input_on_drag() && mb.is_valid() &&
                  mb->get_button_index() == 1 && mb->is_pressed()) {
          ...
          if (camera) {
            ...
            if (space) {
              ...
              bool col = space->intersect_ray(from, from + dir * 10000, result,
                                              Set<RID>(), 0xFFFFFFFF, true,
                                              true, true);
              ...
              if (col) {
                ...
                if (co) {
                  ...
                  _collision_object_input_event(co, camera, ev, result.position,
                                                result.normal, result.shape);
                  last_object = co;
...
RID Viewport::get_viewport_rid() const { return viewport; }
...
bool Viewport::use_arvr() { return arvr; }
...
Size2 Viewport::get_size() const { return size; }
...
bool Viewport::is_audio_listener() const { return audio_listener; }
...
bool Viewport::is_audio_listener_2d() const { return audio_listener_2d; }
...
Transform2D Viewport::get_canvas_transform() const { return canvas_transform; }

void Viewport::_update_global_transform() {

  Transform2D sxform = stretch_transform * global_canvas_transform;

  VisualServer::get_singleton()->viewport_set_global_canvas_transform(viewport,
                                                                      sxform);
}
...
bool Viewport::has_transparent_background() const { return transparent_bg; }
...
void Viewport::set_world(const Ref<World> &p_world) {
  ...
  if (own_world.is_valid() && world.is_valid()) {
    world->disconnect(CoreStringNames::get_singleton()->changed,
                      callable_mp(this, &Viewport::_own_world_changed));
  }
  ...
  if (own_world.is_valid()) {
    if (world.is_valid()) {
      own_world = world->duplicate();
      world->connect(CoreStringNames::get_singleton()->changed,
                     callable_mp(this, &Viewport::_own_world_changed));
    } else {
...
Ref<World> Viewport::get_world() const { return world; }

Ref<World2D> Viewport::get_world_2d() const { return world_2d; }
...
Listener *Viewport::get_listener() const { return listener; }

Camera *Viewport::get_camera() const { return camera; }
...
void Viewport::enable_camera_override(bool p_enable) {
  ...
  if (p_enable) {
    VisualServer::get_singleton()->viewport_attach_camera(viewport,
                                                          camera_override.rid);
  } else if (camera) {
    VisualServer::get_singleton()->viewport_attach_camera(viewport,
                                                          camera->get_camera());
...
bool Viewport::is_camera_override_enabled() const { return camera_override; }

void Viewport::set_camera_override_transform(const Transform &p_transform) {
  if (camera_override) {
    camera_override.transform = p_transform;
    VisualServer::get_singleton()->camera_set_transform(camera_override.rid,
                                                        p_transform);
  }
}
...
void Viewport::set_camera_override_perspective(float p_fovy_degrees,
  ...
  if (camera_override) {
    ...
    VisualServer::get_singleton()->camera_set_perspective(
        camera_override.rid, camera_override.fov, camera_override.z_near,
        camera_override.z_far);
  }
}

void Viewport::set_camera_override_orthogonal(float p_size, float p_z_near,
                                              float p_z_far) {
  if (camera_override) {
    ...
    VisualServer::get_singleton()->camera_set_orthogonal(
        camera_override.rid, camera_override.size, camera_override.z_near,
        camera_override.z_far);
  }
}
...
void Viewport::set_size_override(bool p_enable, const Size2 &p_size,
...
Size2 Viewport::get_size_override() const { return size_override_size; }
bool Viewport::is_size_override_enabled() const { return size_override; }
...
void Viewport::set_update_mode(UpdateMode p_mode) {
  ...
  update_mode = p_mode;
  VS::get_singleton()->viewport_set_update_mode(viewport,
                                                VS::ViewportUpdateMode(p_mode));
}
Viewport::UpdateMode Viewport::get_update_mode() const { return update_mode; }

Ref<ViewportTexture> Viewport::get_texture() const { return default_texture; }

void Viewport::set_clear_mode(ClearMode p_mode) {

  clear_mode = p_mode;
  VS::get_singleton()->viewport_set_clear_mode(viewport,
                                               VS::ViewportClearMode(p_mode));
}

Viewport::ClearMode Viewport::get_clear_mode() const { return clear_mode; }
...
int Viewport::get_shadow_atlas_size() const { return shadow_atlas_size; }

void Viewport::set_shadow_atlas_quadrant_subdiv(
    int p_quadrant, ShadowAtlasQuadrantSubdiv p_subdiv) {
  ...
  static const int subdiv[SHADOW_ATLAS_QUADRANT_SUBDIV_MAX] = {0,  1,   4,   16,
                                                               64, 256, 1024};
...
Viewport::get_shadow_atlas_quadrant_subdiv(int p_quadrant) const {
...
Vector2 Viewport::get_mouse_position() const {
  ...
  return (get_final_transform().affine_inverse() * _get_input_pre_xform())
      .xform(Input::get_singleton()->get_mouse_position() -
             _get_window_offset());
}
...
void Viewport::_gui_show_tooltip() {
  ...
  String tooltip = _gui_get_tooltip(
      gui.tooltip,
      gui.tooltip->get_global_transform().xform_inv(gui.tooltip_pos), &which);
  ...
  if (!gui.tooltip_popup) {
    ...
    gui.tooltip_label->set_anchor_and_margin(MARGIN_LEFT, Control::ANCHOR_BEGIN,
                                             ttp->get_margin(MARGIN_LEFT));
    gui.tooltip_label->set_anchor_and_margin(MARGIN_TOP, Control::ANCHOR_BEGIN,
                                             ttp->get_margin(MARGIN_TOP));
    gui.tooltip_label->set_anchor_and_margin(MARGIN_RIGHT, Control::ANCHOR_END,
                                             -ttp->get_margin(MARGIN_RIGHT));
    gui.tooltip_label->set_anchor_and_margin(MARGIN_BOTTOM, Control::ANCHOR_END,
...
void Viewport::_gui_call_input(Control *p_control,
  ...
  bool cant_stop_me_now =
      (mb.is_valid() && (mb->get_button_index() == BUTTON_WHEEL_DOWN ||
  ...
  bool ismouse =
      ev.is_valid() || Object::cast_to<InputEventMouseMotion>(*p_input) != NULL;
  ...
  while (ci) {
    ...
    if (control) {
      ...
      if (control->data.mouse_filter != Control::MOUSE_FILTER_IGNORE) {
        control->emit_signal(SceneStringNames::get_singleton()->gui_input,
                             ev); // signal should be first, so it's possible to
                                  // override an event (and then accept it)
      }
      ...
      if (control->data.mouse_filter != Control::MOUSE_FILTER_IGNORE) {
        control->call_multilevel(SceneStringNames::get_singleton()->_gui_input,
                                 ev);
      }
      ...
      if (gui.key_event_accepted)
        break;
      if (!cant_stop_me_now &&
          control->data.mouse_filter == Control::MOUSE_FILTER_STOP && ismouse)
        break;
    }
...
Control *Viewport::_gui_find_control_at_pos(CanvasItem *p_node,
  ...
  if (c->data.mouse_filter != Control::MOUSE_FILTER_IGNORE &&
      c->has_point(matrix.xform(p_global)) &&
      (!gui.drag_preview ||
       (c != gui.drag_preview && !gui.drag_preview->is_a_parent_of(c)))) {
    r_inv_xform = matrix;
    return c;
  } else
    return NULL;
...
bool Viewport::_gui_drop(Control *p_at_control, Point2 p_at_pos,
                         bool p_just_check) {
...
void Viewport::_gui_input_event(Ref<InputEvent> p_event) {
  ...
  */
    ...
    if (mb->is_pressed()) {
      ...
      if (gui.mouse_focus_mask) {
        ...
        gui.mouse_focus_mask |=
            1 << (mb->get_button_index() - 1); // add the button to the mask
      } else {
        ...
        while (!gui.modal_stack.empty()) {
          ...
          Vector2 pos2 =
              top->get_global_transform_with_canvas().affine_inverse().xform(
                  mpos);
          if (!top->has_point(pos2)) {
            ...
            if (top->data.modal_exclusive ||
              ...
              // alternative, you can't pop out a window the same frame it was
              // made modal (fixes many issues)
            ...
            if (mb->get_button_index() == BUTTON_WHEEL_UP ||
                ...
                mb->get_button_index() == BUTTON_WHEEL_LEFT ||
                mb->get_button_index() == BUTTON_WHEEL_RIGHT) {
              // cancel scroll wheel events, only clicks should trigger focus
              // changes.
              set_input_as_handled();
              return;
            }
      ...
      if (!gui.mouse_focus) {
        // release event is only sent if a mouse focus (previously pressed
        // button) exists
        return;
      }
      ...
      // disable mouse focus if needed before calling input, this makes popups
      // on mouse press event work better, as the release will never be received
      // otherwise
  ...
  if (mm.is_valid()) {
            ...
            if (control) {
              ...
              if (gui.drag_data.get_type() != Variant::NIL) {
                ...
                if (gui.drag_preview != NULL) {
                  ERR_PRINT("Don't set a drag preview and return null data. "
                            "Preview was deleted and drag request ignored.");
                  memdelete(gui.drag_preview);
                  gui.drag_preview = NULL;
                }
                gui.dragging = false;
    ...
    if (gui.drag_data.get_type() == Variant::NIL && over &&
      ...
      if (over != top && !top->is_a_parent_of(over)) {
        ...
        // if there is already a pop-up menu open at the same hierarchical
        // level.
    ...
    if (over != gui.mouse_over) {
      ...
      if (gui.mouse_over) {
        _gui_call_notification(gui.mouse_over,
    ...
    if (!over) {
      OS::get_singleton()->set_cursor_shape(
          (OS::CursorShape)Input::get_singleton()->get_default_cursor_shape());
      return;
    }
    ...
    if (mm->get_button_mask() == 0) {
      ...
      if (gui.tooltip_popup) {
        if (can_tooltip && gui.tooltip) {
          String tooltip = _gui_get_tooltip(
              over, gui.tooltip->get_global_transform().xform_inv(mpos));
          ...
          if (tooltip.length() == 0)
            ...
            if (tooltip == gui.tooltip_label->get_text()) {
              is_tooltip_shown = true;
                     ...
                     String(gui.tooltip_popup->call("get_tooltip_text"))) {
            is_tooltip_shown = true;
          }
        } else
          _gui_cancel_tooltip();
      ...
      if (can_tooltip && !is_tooltip_shown) {
        ...
        gui.tooltip = over;
        gui.tooltip_pos = mpos; //(parent_xform *
                                //get_transform()).affine_inverse().xform(pos);
      ...
      if (over) {
        ...
        if (over->can_process()) {
          ...
          if (over == gui.mouse_focus) {
            ...
            pos =
                over->get_global_transform_with_canvas().affine_inverse().xform(
                    pos);
          }
    ...
    if (over) {
      ...
      if (over->can_process()) {
        ...
        if (over == gui.mouse_focus) {
          ...
          pos = over->get_global_transform_with_canvas().affine_inverse().xform(
              pos);
        }
  ...
  if (mm.is_null() && mb.is_null() && p_event->is_action_type()) {
    ...
    if (gui.key_focus) {
      ...
      if (gui.key_focus->can_process()) {
        gui.key_focus->call_multilevel(
            SceneStringNames::get_singleton()->_gui_input, p_event);
        if (gui.key_focus) // maybe lost it
          gui.key_focus->emit_signal(
              SceneStringNames::get_singleton()->gui_input, p_event);
      }
    ...
    // alt/ctrl/shift+<up/down/left/righ/etc> is handled here when it shouldn't
    // be.
    bool mods = k.is_valid() && (k->get_control() || k->get_alt() ||
                                 k->get_shift() || k->get_metakey());
...
Viewport::_gui_add_subwindow_control(Control *p_control) {

  p_control->connect(
      "visibility_changed",
      callable_mp(this, &Viewport::_subwindow_visibility_changed));
...
void Viewport::_gui_set_root_order_dirty() { gui.roots_order_dirty = true; }
...
void Viewport::_gui_remove_from_modal_stack(List<Control *>::Element *MI,
...
void Viewport::_gui_force_drag(Control *p_base, const Variant &p_data,
                               Control *p_control) {

  ERR_FAIL_COND_MSG(p_data.get_type() == Variant::NIL,
...
void Viewport::_gui_set_drag_preview(Control *p_base, Control *p_control) {
  ...
  p_base->get_root_parent_control()->add_child(
      p_control); // add as child of viewport
  p_control->raise();
...
void Viewport::_gui_control_grab_focus(Control *p_control) {
  ...
  if (gui.key_focus && gui.key_focus == p_control)
    return;
  get_tree()->call_group_flags(SceneTree::GROUP_CALL_REALTIME, "_viewports",
                               "_gui_remove_focus");
...
Control *Viewport::_gui_get_focus_owner() { return gui.key_focus; }
    ...
    for (int i = 0; i < 3; i++) {
      ...
      if (mask & (1 << i)) {
        ...
        gui.mouse_focus->call_multilevel(
            SceneStringNames::get_singleton()->_gui_input, mb);
      }
    }
...
void Viewport::input(const Ref<InputEvent> &p_event) {
  ...
  if (!is_input_handled()) {
    get_tree()->_call_input_pause(
        input_group, "_input",
        p_event); // not a bug, must happen before GUI, order is _input -> gui
                  // input -> _unhandled input
  }
...
void Viewport::unhandled_input(const Ref<InputEvent> &p_event) {
  ...
  get_tree()->_call_input_pause(unhandled_input_group, "_unhandled_input",
                                p_event);
  // call_group(GROUP_CALL_REVERSE|GROUP_CALL_REALTIME|GROUP_CALL_MULIILEVEL,"unhandled_input","_unhandled_input",ev);
  if (!get_tree()->input_handled &&
      Object::cast_to<InputEventKey>(*p_event) != NULL) {
    get_tree()->_call_input_pause(unhandled_key_input_group,
...
bool Viewport::is_using_own_world() const { return own_world.is_valid(); }
...
Rect2 Viewport::get_attach_to_screen_rect() const { return to_screen_rect; }
...
void Viewport::set_use_render_direct_to_screen(bool p_render_direct_to_screen) {
  ...
  VS::get_singleton()->viewport_set_render_direct_to_screen(
      viewport, p_render_direct_to_screen);
}
...
bool Viewport::get_physics_object_picking() { return physics_object_picking; }
...
Vector2 Viewport::get_camera_rect_size() const { return size; }

bool Viewport::gui_has_modal_stack() const { return gui.modal_stack.size(); }

void Viewport::set_disable_input(bool p_disable) { disable_input = p_disable; }

bool Viewport::is_input_disabled() const { return disable_input; }

Variant Viewport::gui_get_drag_data() const { return gui.drag_data; }
...
String Viewport::get_configuration_warning() const {
  ...
  !render_target) {
          ...
          return TTR("This viewport is not set as render target. If you intend
  for it to display its contents directly to the screen, make it a child of a
  Control so it can obtain a size. Otherwise, make it a RenderTarget and assign
  its internal texture to some node for display.");
  }*/
...
void Viewport::gui_reset_canvas_sort_index() { gui.canvas_sort_index = 0; }
int Viewport::gui_get_canvas_sort_index() { return gui.canvas_sort_index++; }
...
Viewport::MSAA Viewport::get_msaa() const { return msaa; }

void Viewport::set_debug_draw(DebugDraw p_debug_draw) {

  debug_draw = p_debug_draw;
  VS::get_singleton()->viewport_set_debug_draw(
      viewport, VS::ViewportDebugDraw(p_debug_draw));
}

Viewport::DebugDraw Viewport::get_debug_draw() const { return debug_draw; }

int Viewport::get_render_info(RenderInfo p_info) {

  return VS::get_singleton()->viewport_get_render_info(
      viewport, VS::ViewportRenderInfo(p_info));
}
...
bool Viewport::gui_is_dragging() const { return gui.dragging; }
...
void Viewport::set_default_canvas_item_texture_filter(
    DefaultCanvasItemTextureFilter p_filter) {
...
Viewport::get_default_canvas_item_texture_filter() const {
  return default_canvas_item_texture_filter;
}

void Viewport::set_default_canvas_item_texture_repeat(
    DefaultCanvasItemTextureRepeat p_repeat) {
...
Viewport::get_default_canvas_item_texture_repeat() const {
...
void Viewport::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("set_use_arvr", "use"),
  ...
  ClassDB::bind_method(D_METHOD("set_world_2d", "world_2d"),
  ...
  ClassDB::bind_method(D_METHOD("set_canvas_transform", "xform"),
                       &Viewport::set_canvas_transform);
  ClassDB::bind_method(D_METHOD("get_canvas_transform"),
                       &Viewport::get_canvas_transform);

  ClassDB::bind_method(D_METHOD("set_global_canvas_transform", "xform"),
  ...
  ClassDB::bind_method(D_METHOD("get_final_transform"),
                       &Viewport::get_final_transform);

  ClassDB::bind_method(D_METHOD("get_visible_rect"),
                       &Viewport::get_visible_rect);
  ClassDB::bind_method(D_METHOD("set_transparent_background", "enable"),
                       &Viewport::set_transparent_background);
  ClassDB::bind_method(D_METHOD("has_transparent_background"),
                       &Viewport::has_transparent_background);

  ClassDB::bind_method(D_METHOD("_vp_input"), &Viewport::_vp_input);
  ClassDB::bind_method(D_METHOD("_vp_input_text", "text"),
                       &Viewport::_vp_input_text);
  ClassDB::bind_method(D_METHOD("_vp_unhandled_input"),
  ...
  ClassDB::bind_method(D_METHOD("get_size_override"),
                       &Viewport::get_size_override);
  ClassDB::bind_method(D_METHOD("is_size_override_enabled"),
                       &Viewport::is_size_override_enabled);
  ClassDB::bind_method(D_METHOD("set_size_override_stretch", "enabled"),
                       &Viewport::set_size_override_stretch);
  ClassDB::bind_method(D_METHOD("is_size_override_stretch_enabled"),
                       &Viewport::is_size_override_stretch_enabled);

  ClassDB::bind_method(D_METHOD("set_clear_mode", "mode"),
                       &Viewport::set_clear_mode);
  ClassDB::bind_method(D_METHOD("get_clear_mode"), &Viewport::get_clear_mode);

  ClassDB::bind_method(D_METHOD("set_update_mode", "mode"),
  ...
  ClassDB::bind_method(D_METHOD("set_debug_draw", "debug_draw"),
                       &Viewport::set_debug_draw);
  ClassDB::bind_method(D_METHOD("get_debug_draw"), &Viewport::get_debug_draw);

  ClassDB::bind_method(D_METHOD("get_render_info", "info"),
                       &Viewport::get_render_info);

  ClassDB::bind_method(D_METHOD("get_texture"), &Viewport::get_texture);

  ClassDB::bind_method(D_METHOD("set_physics_object_picking", "enable"),
                       &Viewport::set_physics_object_picking);
  ClassDB::bind_method(D_METHOD("get_physics_object_picking"),
                       &Viewport::get_physics_object_picking);

  ClassDB::bind_method(D_METHOD("get_viewport_rid"),
                       &Viewport::get_viewport_rid);
  ClassDB::bind_method(D_METHOD("input", "local_event"), &Viewport::input);
  ClassDB::bind_method(D_METHOD("unhandled_input", "local_event"),
                       &Viewport::unhandled_input);

  ClassDB::bind_method(D_METHOD("update_worlds"), &Viewport::update_worlds);

  ClassDB::bind_method(D_METHOD("set_use_own_world", "enable"),
                       &Viewport::set_use_own_world);
  ClassDB::bind_method(D_METHOD("is_using_own_world"),
                       &Viewport::is_using_own_world);

  ClassDB::bind_method(D_METHOD("get_camera"), &Viewport::get_camera);

  ClassDB::bind_method(D_METHOD("set_as_audio_listener", "enable"),
                       &Viewport::set_as_audio_listener);
  ClassDB::bind_method(D_METHOD("is_audio_listener"),
                       &Viewport::is_audio_listener);

  ClassDB::bind_method(D_METHOD("set_as_audio_listener_2d", "enable"),
                       &Viewport::set_as_audio_listener_2d);
  ClassDB::bind_method(D_METHOD("is_audio_listener_2d"),
                       &Viewport::is_audio_listener_2d);
  ClassDB::bind_method(D_METHOD("set_attach_to_screen_rect", "rect"),
                       &Viewport::set_attach_to_screen_rect);
  ClassDB::bind_method(D_METHOD("set_use_render_direct_to_screen", "enable"),
                       &Viewport::set_use_render_direct_to_screen);
  ClassDB::bind_method(D_METHOD("is_using_render_direct_to_screen"),
                       &Viewport::is_using_render_direct_to_screen);

  ClassDB::bind_method(D_METHOD("get_mouse_position"),
                       &Viewport::get_mouse_position);
  ClassDB::bind_method(D_METHOD("warp_mouse", "to_position"),
                       &Viewport::warp_mouse);

  ClassDB::bind_method(D_METHOD("gui_has_modal_stack"),
                       &Viewport::gui_has_modal_stack);
  ClassDB::bind_method(D_METHOD("gui_get_drag_data"),
                       &Viewport::gui_get_drag_data);
  ClassDB::bind_method(D_METHOD("gui_is_dragging"), &Viewport::gui_is_dragging);

  ClassDB::bind_method(D_METHOD("get_modal_stack_top"),
                       &Viewport::get_modal_stack_top);

  ClassDB::bind_method(D_METHOD("set_disable_input", "disable"),
                       &Viewport::set_disable_input);
  ClassDB::bind_method(D_METHOD("is_input_disabled"),
                       &Viewport::is_input_disabled);

  ClassDB::bind_method(D_METHOD("_gui_show_tooltip"),
                       &Viewport::_gui_show_tooltip);
  ClassDB::bind_method(D_METHOD("_gui_remove_focus"),
                       &Viewport::_gui_remove_focus);
  ClassDB::bind_method(D_METHOD("_post_gui_grab_click_focus"),
                       &Viewport::_post_gui_grab_click_focus);

  ClassDB::bind_method(D_METHOD("set_shadow_atlas_size", "size"),
                       &Viewport::set_shadow_atlas_size);
  ClassDB::bind_method(D_METHOD("get_shadow_atlas_size"),
  ...
  ClassDB::bind_method(D_METHOD("is_snap_controls_to_pixels_enabled"),
  ...
  ClassDB::bind_method(D_METHOD("get_shadow_atlas_quadrant_subdiv", "quadrant"),
                       &Viewport::get_shadow_atlas_quadrant_subdiv);

  ClassDB::bind_method(D_METHOD("set_input_as_handled"),
                       &Viewport::set_input_as_handled);
  ClassDB::bind_method(D_METHOD("is_input_handled"),
                       &Viewport::is_input_handled);

  ClassDB::bind_method(D_METHOD("set_handle_input_locally", "enable"),
                       &Viewport::set_handle_input_locally);
  ClassDB::bind_method(D_METHOD("is_handling_input_locally"),
                       &Viewport::is_handling_input_locally);

  ClassDB::bind_method(
      D_METHOD("set_default_canvas_item_texture_filter", "mode"),
  ...
  ClassDB::bind_method(
      D_METHOD("set_default_canvas_item_texture_repeat", "mode"),
      &Viewport::set_default_canvas_item_texture_repeat);
  ClassDB::bind_method(D_METHOD("get_default_canvas_item_texture_repeat"),
                       &Viewport::get_default_canvas_item_texture_repeat);

  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "arvr"), "set_use_arvr", "use_arvr");

  ADD_PROPERTY(PropertyInfo(Variant::VECTOR2, "size"), "set_size", "get_size");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "size_override_stretch"),
               "set_size_override_stretch", "is_size_override_stretch_enabled");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "own_world"), "set_use_own_world",
               "is_using_own_world");
  ADD_PROPERTY(PropertyInfo(Variant::OBJECT, "world",
                            PROPERTY_HINT_RESOURCE_TYPE, "World"),
               "set_world", "get_world");
  ADD_PROPERTY(PropertyInfo(Variant::OBJECT, "world_2d",
                            PROPERTY_HINT_RESOURCE_TYPE, "World2D", 0),
               "set_world_2d", "get_world_2d");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "transparent_bg"),
               "set_transparent_background", "has_transparent_background");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "handle_input_locally"),
               "set_handle_input_locally", "is_handling_input_locally");
  ADD_GROUP("Rendering", "");
  ADD_PROPERTY(PropertyInfo(Variant::INT, "msaa", PROPERTY_HINT_ENUM,
                            "Disabled,2x,4x,8x,16x,AndroidVR 2x,AndroidVR 4x"),
               "set_msaa", "get_msaa");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "render_direct_to_screen"),
               "set_use_render_direct_to_screen",
               "is_using_render_direct_to_screen");
  ADD_PROPERTY(PropertyInfo(Variant::INT, "debug_draw", PROPERTY_HINT_ENUM,
                            "Disabled,Unshaded,Overdraw,Wireframe"),
               "set_debug_draw", "get_debug_draw");
  ADD_GROUP("Render Target", "render_target_");
  ADD_PROPERTY(PropertyInfo(Variant::INT, "render_target_clear_mode",
                            PROPERTY_HINT_ENUM, "Always,Never,Next Frame"),
               "set_clear_mode", "get_clear_mode");
  ADD_PROPERTY(PropertyInfo(Variant::INT, "render_target_update_mode",
                            PROPERTY_HINT_ENUM,
                            "Disabled,Once,When Visible,Always"),
               "set_update_mode", "get_update_mode");
  ADD_GROUP("Canvas Items", "canvas_item_");
  ADD_PROPERTY(PropertyInfo(Variant::INT, "canvas_item_default_texture_filter",
                            PROPERTY_HINT_ENUM,
                            "Nearest,Linear,MipmapLinear,MipmapNearest"),
               "set_default_canvas_item_texture_filter",
               "get_default_canvas_item_texture_filter");
  ADD_PROPERTY(PropertyInfo(Variant::INT, "canvas_item_default_texture_repeat",
                            PROPERTY_HINT_ENUM, "Disabled,Enabled,Mirror"),
               "set_default_canvas_item_texture_repeat",
               "get_default_canvas_item_texture_repeat");
  ADD_GROUP("Audio Listener", "audio_listener_");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "audio_listener_enable_2d"),
               "set_as_audio_listener_2d", "is_audio_listener_2d");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "audio_listener_enable_3d"),
               "set_as_audio_listener", "is_audio_listener");
  ADD_GROUP("Physics", "physics_");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "physics_object_picking"),
               "set_physics_object_picking", "get_physics_object_picking");
  ADD_GROUP("GUI", "gui_");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "gui_disable_input"),
               "set_disable_input", "is_input_disabled");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "gui_snap_controls_to_pixels"),
               "set_snap_controls_to_pixels",
               "is_snap_controls_to_pixels_enabled");
  ADD_GROUP("Shadow Atlas", "shadow_atlas_");
  ADD_PROPERTY(PropertyInfo(Variant::INT, "shadow_atlas_size"),
               "set_shadow_atlas_size", "get_shadow_atlas_size");
  ADD_PROPERTYI(PropertyInfo(Variant::INT, "shadow_atlas_quad_0",
                             PROPERTY_HINT_ENUM,
                             "Disabled,1 Shadow,4 Shadows,16 Shadows,64 "
                             "Shadows,256 Shadows,1024 Shadows"),
                "set_shadow_atlas_quadrant_subdiv",
                "get_shadow_atlas_quadrant_subdiv", 0);
  ADD_PROPERTYI(PropertyInfo(Variant::INT, "shadow_atlas_quad_1",
                             PROPERTY_HINT_ENUM,
                             "Disabled,1 Shadow,4 Shadows,16 Shadows,64 "
                             "Shadows,256 Shadows,1024 Shadows"),
                "set_shadow_atlas_quadrant_subdiv",
                "get_shadow_atlas_quadrant_subdiv", 1);
  ADD_PROPERTYI(PropertyInfo(Variant::INT, "shadow_atlas_quad_2",
                             PROPERTY_HINT_ENUM,
                             "Disabled,1 Shadow,4 Shadows,16 Shadows,64 "
                             "Shadows,256 Shadows,1024 Shadows"),
                "set_shadow_atlas_quadrant_subdiv",
                "get_shadow_atlas_quadrant_subdiv", 2);
  ADD_PROPERTYI(PropertyInfo(Variant::INT, "shadow_atlas_quad_3",
                             PROPERTY_HINT_ENUM,
                             "Disabled,1 Shadow,4 Shadows,16 Shadows,64 "
                             "Shadows,256 Shadows,1024 Shadows"),
                "set_shadow_atlas_quadrant_subdiv",
                "get_shadow_atlas_quadrant_subdiv", 3);
  ADD_PROPERTY(PropertyInfo(Variant::TRANSFORM2D, "canvas_transform",
                            PROPERTY_HINT_NONE, "", 0),
               "set_canvas_transform", "get_canvas_transform");
  ADD_PROPERTY(PropertyInfo(Variant::TRANSFORM2D, "global_canvas_transform",
                            PROPERTY_HINT_NONE, "", 0),
               "set_global_canvas_transform", "get_global_canvas_transform");

  ADD_SIGNAL(MethodInfo("size_changed"));
  ADD_SIGNAL(MethodInfo("gui_focus_changed",
                        PropertyInfo(Variant::OBJECT, "node",
                                     PROPERTY_HINT_RESOURCE_TYPE, "Control")));
...
Viewport::Viewport() {
  ...
  default_texture->proxy =
      VS::get_singleton()->texture_proxy_create(texture_rid);
  ...
  canvas_layers.insert(NULL); // This eases picking code (interpreted as the
                              // canvas of the Viewport)
  arvr = false;
  size_override = false;
  ...
  ProjectSettings::get_singleton()->set_custom_property_info(
      ...
      PropertyInfo(Variant::FLOAT, "gui/timers/tooltip_delay_sec",
                   PROPERTY_HINT_RANGE,
                   "0,5,0.01,or_greater")); // No negative numbers
  ...
  default_canvas_item_texture_filter =
