...
class Texture2D : public Texture {
  ...
  OBJ_SAVE_TYPE(Texture2D); // Saves derived classes with common type so they
                            // can be interchanged.
...
public:
  ...
  virtual void draw(RID p_canvas_item, const Point2 &p_pos,
                    ...
                    const Ref<Texture2D> &p_normal_map = Ref<Texture2D>(),
                    const Ref<Texture2D> &p_specular_map = Ref<Texture2D>(),
                    const Color &p_specular_color_shininess = Color(1, 1, 1, 1),
                    VS::CanvasItemTextureFilter p_texture_filter =
                        VS::CANVAS_ITEM_TEXTURE_FILTER_DEFAULT,
                    VS::CanvasItemTextureRepeat p_texture_repeat =
                        VS::CANVAS_ITEM_TEXTURE_REPEAT_DEFAULT) const;
  virtual void
  draw_rect(RID p_canvas_item, const Rect2 &p_rect, bool p_tile = false,
  ...
  virtual void draw_rect_region(
      ...
      VS::CanvasItemTextureFilter p_texture_filter =
          VS::CANVAS_ITEM_TEXTURE_FILTER_DEFAULT,
      VS::CanvasItemTextureRepeat p_texture_repeat =
          VS::CANVAS_ITEM_TEXTURE_REPEAT_DEFAULT,
      bool p_clip_uv = true) const;
  virtual bool get_rect_region(const Rect2 &p_rect, const Rect2 &p_src_rect,
...
private:
  Error _load_data(const String &p_path, int &tw, int &th, int &tw_custom,
                   int &th_custom, Ref<Image> &image, bool &r_request_3d,
                   bool &r_request_normal, bool &r_request_roughness,
                   int &mipmap_limit, int p_size_limit = 0);
  String path_to_file;
  mutable RID texture;
  Image::Format format;
  int w, h;
  ...
  static void
  _requested_roughness(void *p_ud, const String &p_normal_path,
...
public:
  ...
  typedef void (*TextureFormatRoughnessRequestCallback)(
      const Ref<StreamTexture> &, const String &p_normal_path,
      VS::TextureDetectRoughnessChannel p_roughness_channel);
...
public:
  virtual RES load(const String &p_path, const String &p_original_path = "",
                   Error *r_error = NULL, bool p_use_sub_threads = false,
...
public:
  Texture2DArray() : TextureLayered(VS::TEXTURE_LAYERED_2D_ARRAY) {}
};
...
public:
  Cubemap() : TextureLayered(VS::TEXTURE_LAYERED_CUBEMAP) {}
};
...
public:
  CubemapArray() : TextureLayered(VS::TEXTURE_LAYERED_CUBEMAP_ARRAY) {}
};
