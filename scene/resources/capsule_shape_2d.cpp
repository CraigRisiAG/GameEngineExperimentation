...
Vector<Vector2> CapsuleShape2D::_get_points() const {
  ...
  for (int i = 0; i < 24; i++) {
    Vector2 ofs = Vector2(0, (i > 6 && i <= 18) ? -get_height() * 0.5
                                                : get_height() * 0.5);

    points.push_back(Vector2(Math::sin(i * Math_PI * 2 / 24.0),
...
bool CapsuleShape2D::_edit_is_selected_on_click(const Point2 &p_point,
...
void CapsuleShape2D::_update_shape() {
  ...
  Physics2DServer::get_singleton()->shape_set_data(get_rid(),
...
real_t CapsuleShape2D::get_radius() const { return radius; }
...
real_t CapsuleShape2D::get_height() const { return height; }
