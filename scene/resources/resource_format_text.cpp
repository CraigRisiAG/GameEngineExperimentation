...
Error ResourceLoaderText::_parse_sub_resource_dummy(
    DummyReadData *p_data, VariantParser::Stream *p_stream,
    Ref<Resource> &r_res, int &line, String &r_err_str) {
...
Error ResourceLoaderText::_parse_ext_resource_dummy(
    DummyReadData *p_data, VariantParser::Stream *p_stream,
    Ref<Resource> &r_res, int &line, String &r_err_str) {
...
Error ResourceLoaderText::_parse_sub_resource(VariantParser::Stream *p_stream,
                                              Ref<Resource> &r_res, int &line,
                                              String &r_err_str) {
...
Error ResourceLoaderText::_parse_ext_resource(VariantParser::Stream *p_stream,
                                              Ref<Resource> &r_res, int &line,
                                              String &r_err_str) {
    ...
    if (ext_resources[id].cache.is_valid()) {
      ...
      if (res.is_null()) {
        ...
        if (ResourceLoader::get_abort_on_missing_resources()) {
          ...
          error_text =
              "[ext_resource] referenced nonexistent resource at: " + path;
          _printerr();
          return error;
        } else {
...
Ref<PackedScene>
ResourceLoaderText::_parse_node_tag(VariantParser::ResourceParser &parser) {
  ...
  while (true) {
    ...
    if (next_tag.name == "node") {
      ...
      if (next_tag.fields.has("instance")) {
        ...
        instance =
            packed_scene->get_state()->add_value(next_tag.fields["instance"]);
      ...
      if (next_tag.fields.has("owner")) {
        owner =
            packed_scene->get_state()->add_node_path(next_tag.fields["owner"]);
      } else {
        if (parent != -1 &&
      ...
      int node_id = packed_scene->get_state()->add_node(parent, owner, type,
                                                        name, instance, index);
      ...
      if (next_tag.fields.has("groups")) {
        ...
        for (int i = 0; i < groups.size(); i++) {
          packed_scene->get_state()->add_node_group(
              node_id, packed_scene->get_state()->add_name(groups[i]));
        }
      }
      ...
      while (true) {
        ...
        error = VariantParser::parse_tag_assign_eof(
            &stream, lines, error_text, next_tag, assign, value, &parser);
        ...
        if (assign != String()) {
          ...
          packed_scene->get_state()->add_node_property(node_id, nameidx,
                                                       valueidx);
          // it's assignment
      ...
      packed_scene->get_state()->add_connection(
          ...
          packed_scene->get_state()->add_name(method), flags, bind_ints);
...
Error ResourceLoaderText::load() {
  ...
  while (true) {
    ...
    if (use_sub_threads) {
      ...
      Error err = ResourceLoader::load_threaded_request(
          path, type, use_sub_threads, local_path);
  ...
  while (true) {
    ...
    while (true) {
      ...
      error = VariantParser::parse_tag_assign_eof(&stream, lines, error_text,
                                                  next_tag, assign, value, &rp);
...
ResourceLoaderText::~ResourceLoaderText() { memdelete(f); }

void ResourceLoaderText::get_dependencies(FileAccess *p_f,
...
Error ResourceLoaderText::rename_dependencies(
    FileAccess *p_f, const String &p_path, const Map<String, String> &p_map) {
  ...
  while (true) {
    ...
    if (next_tag.name != "ext_resource") {
      ...
      if (!fw) {
        ...
        if (is_scene) {
          fw->store_line("[gd_scene load_steps=" + itos(resources_total) +
                         " format=" + itos(FORMAT_VERSION) + "]\n");
        } else {
          fw->store_line("[gd_resource type=\"" + res_type +
      ...
      fw->store_line("[ext_resource path=\"" + path + "\" type=\"" + type +
                     "\" id=" + itos(index) + "]");
...
static void bs_save_unicode_string(FileAccess *f, const String &p_string,
...
Error ResourceLoaderText::save_as_binary(FileAccess *p_f,
                                         const String &p_path) {
  ...
  while (next_tag.name == "ext_resource") {
    ...
    dr->set_path(
        "res://dummy" +
        itos(lindex)); // anything is good to detect it for saving as external
    ...
    while (true) {
      ...
      if (assign != String()) {
        ...
        ResourceFormatSaverBinaryInstance::write_variant(
            wf2, value, dummy_read.resource_set, dummy_read.external_resources,
            empty_string_map);
        prop_count++;
...
RES ResourceFormatLoaderText::load(const String &p_path,
...
void ResourceFormatLoaderText::get_dependencies(const String &p_path,
                                                List<String> *p_dependencies,
                                                bool p_add_types) {
...
Error ResourceFormatLoaderText::rename_dependencies(
    const String &p_path, const Map<String, String> &p_map) {
...
Error ResourceFormatLoaderText::convert_file_to_binary(
    const String &p_src_path, const String &p_dst_path) {
...
String
ResourceFormatSaverTextInstance::_write_resources(void *ud,
...
String ResourceFormatSaverTextInstance::_write_resource(const RES &res) {
  ...
  if (external_resources.has(res)) {
    ...
    if (internal_resources.has(res)) {
      ...
      // external resource
      String path = relative_paths ? local_path.path_to_file(res->get_path())
                                   : res->get_path();
      return "Resource( \"" + path + "\" )";
    } else {
      ERR_FAIL_V_MSG(
          "null", "Resource was not pre cached for the resource section, bug?");
      // internal resource
    }
  }
}
  ...
  case Variant::OBJECT: {
    ...
    if (!p_main && (!bundle_resources) && res->get_path().length() &&
      ...
      if (res->get_path() == local_path) {
        ERR_PRINT("Circular reference to resource being saved found: '" +
    ...
    resource_set.insert(
        res); // saved after, so the children it needs are available when loaded
...
Error ResourceFormatSaverTextInstance::save(const String &p_path,
                                            const RES &p_resource,
                                            uint32_t p_flags) {
  ...
  for (int i = 0; i < sorted_er.size(); i++) {
    ...
    f->store_string("[ext_resource path=\"" + p + "\" type=\"" +
    ...
    for (List<PropertyInfo>::Element *PE = property_list.front(); PE;
      ...
      if (PE->get().usage & PROPERTY_USAGE_STORAGE) {
        ...
        Variant default_value =
            ClassDB::class_get_default_property_value(res->get_class(), name);
  ...
  if (packed_scene.is_valid()) {
    ...
    for (int i = 0; i < state->get_node_count(); i++) {
      ...
      if (instance_placeholder != String()) {
        ...
        VariantWriter::write_to_string(instance_placeholder, vars,
                                       _write_resources, this);
        f->store_string(vars);
      }
      ...
      for (int j = 0; j < state->get_node_property_count(i); j++) {
        ...
        String vars;
        VariantWriter::write_to_string(state->get_node_property_value(i, j),
                                       vars, _write_resources, this);

        f->store_string(
            String(state->get_node_property_name(i, j)).property_name_encode() +
            " = " + vars + "\n");
      }
    ...
    for (int i = 0; i < editable_instances.size(); i++) {
      f->store_line("\n[editable path=\"" +
                    editable_instances[i].operator String() + "\"]");
    }
  }
...
Error ResourceFormatSaverText::save(const String &p_path, const RES &p_resource,
                                    uint32_t p_flags) {
...
void ResourceFormatSaverText::get_recognized_extensions(
    const RES &p_resource, List<String> *p_extensions) const {
