...
bool LineShape2D::_edit_is_selected_on_click(const Point2 &p_point,
                                             double p_tolerance) const {

  Vector2 point = get_d() * get_normal();
  Vector2 l[2][2] = {{point - get_normal().tangent() * 100,
...
Vector2 LineShape2D::get_normal() const { return normal; }
real_t LineShape2D::get_d() const { return d; }

void LineShape2D::draw(const RID &p_to_rid, const Color &p_color) {

  Vector2 point = get_d() * get_normal();

  Vector2 l1[2] = {point - get_normal().tangent() * 100,
                   point + get_normal().tangent() * 100};
...
real_t LineShape2D::get_enclosing_radius() const { return d; }

void LineShape2D::_bind_methods() {

  ClassDB::bind_method(D_METHOD("set_normal", "normal"),
  ...
  ADD_PROPERTY(PropertyInfo(Variant::VECTOR2, "normal"), "set_normal",
               "get_normal");
