...
void Shape::add_vertices_to_array(Vector<Vector3> &array,
...
real_t Shape::get_margin() const { return margin; }
  ...
  if (!lines.empty()) {
    ...
    if (st) {
      debug_mesh_cache->surface_set_material(
          0, st->get_debug_collision_material());
    }
  }
...
void Shape::_bind_methods() {
  ...
  ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "margin", PROPERTY_HINT_RANGE,
                            "0.001,10,0.001"),
               "set_margin", "get_margin");
}

Shape::Shape() : margin(0.04) { ERR_PRINT("Constructor must not be called!"); }

Shape::Shape(RID p_shape) : margin(0.04) { shape = p_shape; }

Shape::~Shape() { PhysicsServer::get_singleton()->free(shape); }
