...
bool PolygonPathFinder::_is_point_inside(const Vector2 &p_point) const {
  ...
  for (Set<Edge>::Element *E = edges.front(); E; E = E->next()) {
    ...
    if (Geometry::segment_intersects_segment_2d(a, b, p_point, outside_point,
                                                NULL)) {
      crosses++;
    }
  }
...
void PolygonPathFinder::setup(const Vector<Vector2> &p_points,
  ...
  for (int i = 0; i < p_points.size(); i++) {
    ...
    outside_point.x =
        i == 0 ? p_points[0].x : (MAX(p_points[i].x, outside_point.x));
    outside_point.y =
        i == 0 ? p_points[0].y : (MAX(p_points[i].y, outside_point.y));
  ...
  for (int i = 0; i < point_count; i++) {
    ...
    for (int j = i + 1; j < point_count; j++) {
      ...
      if (!_is_point_inside(
              from * 0.5 +
              to * 0.5)) // connection between points in inside space
        continue;
      ...
      for (Set<Edge>::Element *E = edges.front(); E; E = E->next()) {
        ...
        const Edge &e = E->get();
        if (e.points[0] == i || e.points[1] == i || e.points[0] == j ||
            e.points[1] == j)
          continue;
...
Vector<Vector2> PolygonPathFinder::find_path(const Vector2 &p_from,
                                             const Vector2 &p_to) {
  ...
  // test direct connection
  {
    ...
    for (Set<Edge>::Element *E = edges.front(); E; E = E->next()) {
      ...
      const Edge &e = E->get();
      if (e.points[0] == ignore_from_edge.points[0] &&
          e.points[1] == ignore_from_edge.points[1])
        continue;
      if (e.points[0] == ignore_to_edge.points[0] &&
    ...
    for (Set<Edge>::Element *E = edges.front(); E; E = E->next()) {
      ...
      if (valid_a) {
        ...
        if (e.points[0] != ignore_from_edge.points[1] &&
          ...
          if (Geometry::segment_intersects_segment_2d(a, b, from, points[i].pos,
                                                      NULL)) {
            valid_a = false;
          }
        }
      ...
      if (valid_b) {
        ...
        if (e.points[0] != ignore_to_edge.points[1] &&
          ...
          if (Geometry::segment_intersects_segment_2d(a, b, to, points[i].pos,
                                                      NULL)) {
            valid_b = false;
          }
        }
...
Vector<Vector2>
PolygonPathFinder::get_intersections(const Vector2 &p_from,
                                     const Vector2 &p_to) const {
...
Rect2 PolygonPathFinder::get_bounds() const { return bounds; }
...
void PolygonPathFinder::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("setup", "points", "connections"),
                       &PolygonPathFinder::setup);
  ClassDB::bind_method(D_METHOD("find_path", "from", "to"),
                       &PolygonPathFinder::find_path);
  ClassDB::bind_method(D_METHOD("get_intersections", "from", "to"),
                       &PolygonPathFinder::get_intersections);
  ClassDB::bind_method(D_METHOD("get_closest_point", "point"),
                       &PolygonPathFinder::get_closest_point);
  ClassDB::bind_method(D_METHOD("is_point_inside", "point"),
                       &PolygonPathFinder::is_point_inside);
  ClassDB::bind_method(D_METHOD("set_point_penalty", "idx", "penalty"),
                       &PolygonPathFinder::set_point_penalty);
  ClassDB::bind_method(D_METHOD("get_point_penalty", "idx"),
