...
void BitMap::set_bit_rect(const Rect2 &p_rect, bool p_value) {
  ...
  for (int i = current.position.x; i < current.position.x + current.size.x;
    ...
    for (int j = current.position.y; j < current.position.y + current.size.y;
         j++) {
...
Size2 BitMap::get_size() const { return Size2(width, height); }
...
Vector<Vector2> BitMap::_march_square(const Rect2i &rect,
                                      const Point2i &start) const {
  ...
  do {
    ...
    if (stepx == prevx && stepy == prevy) {
      ...
      _points.push_back(Vector2((float)(curx - rect.position.x),
                                (float)(cury + rect.position.y)));
    }
...
static float perpendicular_distance(const Vector2 &i, const Vector2 &start,
                                    const Vector2 &end) {
  float res;
  float slope;
  float intercept;
...
static Vector<Vector2> reduce(const Vector<Vector2> &points, const Rect2i &rect,
                              float epsilon) {
...
static void fill_bits(const BitMap *p_src, Ref<BitMap> &p_map,
                      const Point2i &p_pos, const Rect2i &rect) {

  // Using a custom stack to work iteratively to avoid stack overflow on big
  // bitmaps
    ...
    for (int i = next_i; i <= pos.x + 1; i++) {
      for (int j = next_j; j <= pos.y + 1; j++) {
        if (popped) {
          ...
          next_j = pos.y;
          popped = false;
          // Skip because an iteration was already executed with current counter
          // values
          continue;
        }
...
Vector<Vector<Vector2>> BitMap::clip_opaque_to_polygons(const Rect2 &p_rect,
    ...
    for (int j = r.position.x; j < r.position.x + r.size.width; j++) {
      ...
      for (int y = i - p_pixels; y <= i + p_pixels; y++) {
        for (int x = j - p_pixels; x <= j + p_pixels; x++) {
          ...
          if ((x < p_rect.position.x) ||
              (x >= p_rect.position.x + p_rect.size.x) ||
              (y < p_rect.position.y) ||
              (y >= p_rect.position.y + p_rect.size.y)) {
...
Array BitMap::_opaque_to_polygons_bind(const Rect2 &p_rect,
                                       float p_epsilon) const {
...
Ref<Image> BitMap::convert_to_image() const {
  ...
  for (int i = 0; i < width; i++) {
    for (int j = 0; j < height; j++) {
      image->set_pixel(i, j,
                       get_bit(Point2(i, j)) ? Color(1, 1, 1) : Color(0, 0, 0));
    }
  }
...
void BitMap::_bind_methods() {
  ...
  ClassDB::bind_method(
      D_METHOD("create_from_image_alpha", "image", "threshold"),
      &BitMap::create_from_image_alpha, DEFVAL(0.1));

  ClassDB::bind_method(D_METHOD("set_bit", "position", "bit"),
                       &BitMap::set_bit);
  ClassDB::bind_method(D_METHOD("get_bit", "position"), &BitMap::get_bit);

  ClassDB::bind_method(D_METHOD("set_bit_rect", "rect", "bit"),
                       &BitMap::set_bit_rect);
  ClassDB::bind_method(D_METHOD("get_true_bit_count"),
  ...
  ClassDB::bind_method(D_METHOD("grow_mask", "pixels", "rect"),
                       &BitMap::grow_mask);
  ClassDB::bind_method(D_METHOD("opaque_to_polygons", "rect", "epsilon"),
                       &BitMap::_opaque_to_polygons_bind, DEFVAL(2.0));
