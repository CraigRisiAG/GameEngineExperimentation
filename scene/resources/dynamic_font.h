...
public:
  ...
  enum Hinting { HINTING_NONE, HINTING_LIGHT, HINTING_NORMAL };
...
class DynamicFontAtSize : public Reference {
  ...
  const Pair<const Character *, DynamicFontAtSize *>
  _find_char_with_font(CharType p_char,
                       const Vector<Ref<DynamicFontAtSize>> &p_fallbacks) const;
  Character _make_outline_char(CharType p_char);
  TexturePosition _find_texture_pos_for_glyph(int p_color_size,
                                              Image::Format p_image_format,
                                              int p_width, int p_height);
  Character _bitmap_to_character(FT_Bitmap bitmap, int yofs, int xofs,
                                 float advance);

  static unsigned long _ft_stream_io(FT_Stream stream, unsigned long offset,
...
public:
  ...
  Size2 get_char_size(CharType p_char, CharType p_next,
                      const Vector<Ref<DynamicFontAtSize>> &p_fallbacks) const;
  ...
  float draw_char(RID p_canvas_item, const Point2 &p_pos, CharType p_char,
                  ...
                  bool p_advance_only = false, bool p_outline = false) const;
...
public:
  enum SpacingType { SPACING_TOP, SPACING_BOTTOM, SPACING_CHAR, SPACING_SPACE };
