...
void Font::draw_halign(RID p_canvas_item, const Point2 &p_pos, HAlign p_align,
                       float p_width, const String &p_text,
...
void Font::draw(RID p_canvas_item, const Point2 &p_pos, const String &p_text,
  ...
  for (int i = 0; i < p_text.length(); i++) {
    ...
    ofs.x +=
        draw_char(p_canvas_item, p_pos + ofs, p_text[i], p_text[i + 1],
                  with_outline ? p_outline_modulate : p_modulate, with_outline);
    ++chars_drawn;
  }
  ...
  if (has_outline()) {
    ...
    for (int i = 0; i < chars_drawn; i++) {
      ofs.x += draw_char(p_canvas_item, p_pos + ofs, p_text[i], p_text[i + 1],
                         p_modulate, false);
    }
  }
}

void Font::update_changes() { emit_changed(); }

void Font::_bind_methods() {

  ClassDB::bind_method(D_METHOD("draw", "canvas_item", "position", "string",
                                "modulate", "clip_w", "outline_modulate"),
                       &Font::draw, DEFVAL(Color(1, 1, 1)), DEFVAL(-1),
                       DEFVAL(Color(1, 1, 1)));
  ...
  ClassDB::bind_method(D_METHOD("is_distance_field_hint"),
                       &Font::is_distance_field_hint);
  ClassDB::bind_method(D_METHOD("get_string_size", "string"),
                       &Font::get_string_size);
  ClassDB::bind_method(D_METHOD("get_wordwrap_string_size", "string", "width"),
                       &Font::get_wordwrap_string_size);
  ClassDB::bind_method(D_METHOD("has_outline"), &Font::has_outline);
  ClassDB::bind_method(D_METHOD("draw_char", "canvas_item", "position", "char",
                                "next", "modulate", "outline"),
                       &Font::draw_char, DEFVAL(-1), DEFVAL(Color(1, 1, 1)),
                       DEFVAL(false));
...
void BitmapFont::_set_chars(const Vector<int> &p_chars) {
  ...
  for (int i = 0; i < chars; i++) {
    ...
    add_char(data[0], data[1], Rect2(data[2], data[3], data[4], data[5]),
             Size2(data[6], data[7]), data[8]);
  }
}
...
Error BitmapFont::create_from_fnt(const String &p_file) {
  ...
  ERR_FAIL_COND_V_MSG(!f, ERR_FILE_NOT_FOUND,
...
void BitmapFont::set_height(float p_height) { height = p_height; }
float BitmapFont::get_height() const { return height; }

void BitmapFont::set_ascent(float p_ascent) { ascent = p_ascent; }
float BitmapFont::get_ascent() const { return ascent; }
float BitmapFont::get_descent() const { return height - ascent; }

void BitmapFont::add_texture(const Ref<Texture2D> &p_texture) {

  ERR_FAIL_COND_MSG(p_texture.is_null(),
                    "It's not a reference to a valid Texture object.");
  textures.push_back(p_texture);
}

int BitmapFont::get_texture_count() const { return textures.size(); };
...
int BitmapFont::get_character_count() const { return char_map.size(); };
...
void BitmapFont::add_char(CharType p_char, int p_texture_idx,
                          const Rect2 &p_rect, const Size2 &p_align,
                          float p_advance) {
...
bool BitmapFont::is_distance_field_hint() const { return distance_field_hint; }
...
Size2 Font::get_wordwrap_string_size(const String &p_string,
                                     float p_width) const {
...
void BitmapFont::set_fallback(const Ref<BitmapFont> &p_fallback) {
  ...
  for (Ref<BitmapFont> fallback_child = p_fallback; fallback_child != NULL;
       fallback_child = fallback_child->get_fallback()) {
    ERR_FAIL_COND_MSG(fallback_child == this,
                      "Can't set as fallback one of its parents to prevent "
...
float BitmapFont::draw_char(RID p_canvas_item, const Point2 &p_pos,
  ...
  if (!c) {
    if (fallback.is_valid())
      return fallback->draw_char(p_canvas_item, p_pos, p_char, p_next,
                                 p_modulate, p_outline);
    return 0;
  }
  ...
  if (!p_outline && c->texture_idx != -1) {
    ...
    VisualServer::get_singleton()->canvas_item_add_texture_rect_region(
        p_canvas_item, Rect2(cpos, c->rect.size),
        textures[c->texture_idx]->get_rid(), c->rect, p_modulate, false, RID(),
        RID(), Color(1, 1, 1, 1), false);
  }
...
void BitmapFont::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("create_from_fnt", "path"),
  ...
  ClassDB::bind_method(D_METHOD("get_kerning_pair", "char_a", "char_b"),
                       &BitmapFont::get_kerning_pair);

  ClassDB::bind_method(D_METHOD("add_texture", "texture"),
                       &BitmapFont::add_texture);
  ClassDB::bind_method(
      D_METHOD("add_char", "character", "texture", "rect", "align", "advance"),
      &BitmapFont::add_char, DEFVAL(Point2()), DEFVAL(-1));

  ClassDB::bind_method(D_METHOD("get_texture_count"),
                       &BitmapFont::get_texture_count);
  ClassDB::bind_method(D_METHOD("get_texture", "idx"),
                       &BitmapFont::get_texture);

  ClassDB::bind_method(D_METHOD("get_char_size", "char", "next"),
                       &BitmapFont::get_char_size, DEFVAL(0));

  ClassDB::bind_method(D_METHOD("set_distance_field_hint", "enable"),
  ...
  ClassDB::bind_method(D_METHOD("set_fallback", "fallback"),
  ...
  ADD_PROPERTY(PropertyInfo(Variant::PACKED_INT32_ARRAY, "chars",
                            PROPERTY_HINT_NONE, "",
                            PROPERTY_USAGE_NOEDITOR | PROPERTY_USAGE_INTERNAL),
               "_set_chars", "_get_chars");
  ADD_PROPERTY(PropertyInfo(Variant::PACKED_INT32_ARRAY, "kernings",
                            PROPERTY_HINT_NONE, "",
                            PROPERTY_USAGE_NOEDITOR | PROPERTY_USAGE_INTERNAL),
               "_set_kernings", "_get_kernings");

  ADD_PROPERTY(
      PropertyInfo(Variant::FLOAT, "height", PROPERTY_HINT_RANGE, "1,1024,1"),
      "set_height", "get_height");
  ADD_PROPERTY(
      PropertyInfo(Variant::FLOAT, "ascent", PROPERTY_HINT_RANGE, "0,1024,1"),
      "set_ascent", "get_ascent");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "distance_field"),
               "set_distance_field_hint", "is_distance_field_hint");
  ADD_PROPERTY(PropertyInfo(Variant::OBJECT, "fallback",
                            PROPERTY_HINT_RESOURCE_TYPE, "BitmapFont"),
               "set_fallback", "get_fallback");
}

BitmapFont::BitmapFont() { clear(); }

BitmapFont::~BitmapFont() { clear(); }

////////////

RES ResourceFormatLoaderBMFont::load(const String &p_path,
...
String
ResourceFormatLoaderBMFont::get_resource_type(const String &p_path) const {
