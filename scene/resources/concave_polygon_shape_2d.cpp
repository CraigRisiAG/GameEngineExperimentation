...
void ConcavePolygonShape2D::draw(const RID &p_to_rid, const Color &p_color) {
  ...
  const Vector2 *r = s.ptr();
  for (int i = 0; i < len; i += 2) {
    VisualServer::get_singleton()->canvas_item_add_line(p_to_rid, r[i],
                                                        r[i + 1], p_color, 2);
  }
}
...
void ConcavePolygonShape2D::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("get_segments"),
                       &ConcavePolygonShape2D::get_segments);

  ADD_PROPERTY(PropertyInfo(Variant::PACKED_VECTOR2_ARRAY, "segments"),
               "set_segments", "get_segments");
}

ConcavePolygonShape2D::ConcavePolygonShape2D()
    : Shape2D(
          Physics2DServer::get_singleton()->concave_polygon_shape_create()) {
