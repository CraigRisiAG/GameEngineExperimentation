...
bool SceneState::can_instance() const { return nodes.size() > 0; }
...
#define NODE_FROM_ID(p_name, p_id)                                             \
  ...
  bool gen_node_path_cache =
      p_edit_state != GEN_EDIT_STATE_DISABLED && node_path_cache.empty();
  ...
  for (int i = 0; i < nc; i++) {
    ...
    if (i > 0) {
      ...
      ERR_FAIL_COND_V_MSG(
          n.parent == -1, NULL,
          vformat("Invalid scene: node %s does not specify its parent node.",
                  snames[n.name]));
      ...
      if (!nparent && (n.parent & FLAG_ID_IS_PATH)) {
        ...
        WARN_PRINT(
            String("Parent path '" + String(node_paths[n.parent & FLAG_MASK]) +
                   "' for node '" + String(snames[n.name]) +
                   "' has vanished when instancing: '" + get_path() + "'.")
                .ascii()
                .get_data());
      }
#endif
    ...
    if (i == 0 && base_scene_idx >= 0) {
      ...
      node = sdata->instance(
          p_edit_state == GEN_EDIT_STATE_DISABLED
              ? PackedScene::GEN_EDIT_STATE_DISABLED
              : PackedScene::GEN_EDIT_STATE_INSTANCE); // only main gets main
                                                       // edit state
      ...
      if (n.instance & FLAG_INSTANCE_IS_PLACEHOLDER) {
        ...
        if (disable_placeholders) {
          ...
          ERR_FAIL_COND_V(!sdata.is_valid(), NULL);
          node = sdata->instance(p_edit_state == GEN_EDIT_STATE_DISABLED
                                     ? PackedScene::GEN_EDIT_STATE_DISABLED
      ...
      // get the node from somewhere, it likely already exists from another
      // instance
      if (parent) {
        node = parent->_get_child_by_name(snames[n.name]);
#ifdef DEBUG_ENABLED
        if (!node) {
          WARN_PRINT(String("Node '" +
                            String(ret_nodes[0]->get_path_to(parent)) + "/" +
                            String(snames[n.name]) +
                            "' was modified from inside an instance, but it "
                            "has vanished.")
                         .ascii()
                         .get_data());
        }
      ...
      if (!Object::cast_to<Node>(obj)) {
        ...
        WARN_PRINT(String("Warning node of type " +
                          snames[n.type].operator String() + " does not exist.")
                       .ascii()
                       .get_data());
    ...
    if (node) {
      ...
      if (nprop_count) {
        ...
        for (int j = 0; j < nprop_count; j++) {
          ...
          if (snames[nprops[j].name] ==
              CoreStringNames::get_singleton()->_script) {
            // work around to avoid old script variables from disappearing,
            // should be the proper fix to:
            ...
            for (List<Pair<StringName, Variant>>::Element *E =
                     old_state.front();
                 E; E = E->next()) {
            ...
            if (value.get_type() == Variant::OBJECT) {
              // handle resources that are local to scene by duplicating them if
              // needed
              Ref<Resource> res = value;
              if (res.is_valid()) {
                if (res->is_local_to_scene()) {

                  Map<Ref<Resource>, Ref<Resource>>::Element *E =
                      resources_local_to_scene.find(res);
                  ...
                  if (E) {
                    ...
                    if (p_edit_state == GEN_EDIT_STATE_MAIN) {
                      ...
                      res->configure_for_local_scene(base,
                                                     resources_local_to_scene);
                      resources_local_to_scene[res] = res;

                    } else {
                      // for instances, a copy must be made
                      Node *base2 = i == 0 ? node : ret_nodes[0];
                      Ref<Resource> local_dupe = res->duplicate_for_local_scene(
                          base2, resources_local_to_scene);
              ...
              value = value.duplicate(
                  true); // Duplicate arrays and dictionaries for the editor
            }
      ...
      if (n.instance >= 0 || n.type != TYPE_INSTANCED || i == 0) {
        // if node was not part of instance, must set its name, parenthood and
        // ownership
        if (i > 0) {
          if (parent) {
  ...
  for (Map<Ref<Resource>, Ref<Resource>>::Element *E =
           resources_local_to_scene.front();
       E; E = E->next()) {
  ...
  for (int i = 0; i < cc; i++) {
    ...
    cfrom->connect(snames[c.signal], Callable(cto, snames[c.method]), binds,
...
static int _nm_get_string(const String &p_string,
...
Error SceneState::_parse_node(
    Node *p_owner, Node *p_node, int p_parent_idx,
    Map<StringName, int> &name_map,
    HashMap<Variant, int, VariantHasher, VariantComparator> &variant_map,
    Map<Node *, int> &node_map, Map<Node *, int> &nodepath_map) {

  // this function handles all the work related to properly packing scenes, be
  // it instanced or inherited. given the complexity of this process, an attempt
  // will be made to properly document it. if you fail to understand something,
  // please ask!
  ...
  // save the child instanced scenes that are chosen as editable, so they can be
  // restored upon load back
  if (p_node != p_owner && p_node->get_filename() != String() &&
      p_owner->is_editable_instance(p_node))
  ...
  // really convoluted condition, but it basically checks that index is only
  // saved when part of an inherited scene OR the node parent is from the edited
  // scene
  if (p_owner->get_scene_inherited_state().is_null() &&
      (p_node == p_owner || (p_node->get_owner() == p_owner &&
                             (p_node->get_parent() == p_owner ||
                              p_node->get_parent()->get_owner() == p_owner)))) {
    // do not save index, because it belongs to saved scene and scene is not
    // inherited
    nd.index = -1;
  } else if (p_node == p_owner) {
    // This (hopefully) happens if the node is a scene root, so its index is
    // irrelevant.
    nd.index = -1;
  } else {
...

  {
    ...
    while (n) {
      ...
      if (n == p_owner) {
        ...
        if (p_node->get_filename() != String() &&
            p_node->get_owner() == p_owner && instanced_by_owner) {
          ...
          if (p_node->get_scene_instance_load_placeholder()) {
            ...
            Ref<PackedScene> instance =
                ResourceLoader::load(p_node->get_filename());
  ...
  for (List<PropertyInfo>::Element *E = plist.front(); E; E = E->next()) {
    ...
    Variant default_value =
        ClassDB::class_get_default_property_value(type, name);
    ...
    if (!isdefault && script.is_valid() &&
        script->get_property_default_value(name, default_value)) {
      isdefault =
          bool(Variant::evaluate(Variant::OP_EQUAL, value, default_value));
    }
    // the version above makes more sense, because it does not rely on
    // placeholder or usage flag in the script, just the default value function.
    ...
    if (pack_state_stack.size()) {
      ...
      for (List<PackState>::Element *F = pack_state_stack.back(); F;
           F = F->prev()) {
      ...
      if (exists) {
        ...
        if (value.get_type() == Variant::FLOAT &&
            original.get_type() == Variant::FLOAT) {
          // this must be done because, as some scenes save as text, there might
          // be a tiny difference in floats due to numerical error
          float a = value;
          float b = original;
          ...
          if (Math::is_equal_approx(a, b))
            continue;
                       ...
                       Variant::evaluate(Variant::OP_EQUAL, value, original))) {
  ...
  for (List<Node::GroupInfo>::Element *E = groups.front(); E; E = E->next()) {
    ...
    instance_state->is_node_in_group(instance_state_node,gi.name)) continue;
    //group was instanced, don't add here
    */

    bool skip = false;
    for (List<PackState>::Element *F = pack_state_stack.front(); F;
         F = F->next()) {
  ...
  // if this node is part of an instanced sub-scene, we can skip storing it if
  // basically no properties changed and no groups were added to it. below
  // condition is true for all nodes of the scene being saved, and ones in
  // subscenes that hold changes

  bool save_node = nd.properties.size() ||
                   nd.groups.size(); // some local properties or groups exist
  save_node = save_node || p_node == p_owner; // owner is always saved
  save_node =
      save_node || (p_node->get_owner() == p_owner &&
                    instanced_by_owner); // part of scene and not instanced
  ...
  for (int i = 0; i < p_node->get_child_count(); i++) {
    ...
    Error err = _parse_node(p_owner, c, parent_node, name_map, variant_map,
                            node_map, nodepath_map);
    if (err)
      return err;
  }
...
Error SceneState::_parse_connections(
    Node *p_owner, Node *p_node, Map<StringName, int> &name_map,
    HashMap<Variant, int, VariantHasher, VariantComparator> &variant_map,
    Map<Node *, int> &node_map, Map<Node *, int> &nodepath_map) {
  ...
  for (List<MethodInfo>::Element *E = _signals.front(); E; E = E->next()) {
    ...
    for (List<Node::Connection>::Element *F = conns.front(); F; F = F->next()) {
      ...
      if (common_parent != p_owner &&
          common_parent->get_filename() == String()) {
      ...
      while (common_parent) {
        ...
        if (ps.is_valid()) {
          ...
          if (ps->has_connection(signal_from, c.signal.get_name(), signal_to,
      ...
      if (exists) { // already exists (comes from instance or inheritance), so
                    // don't save
        continue;
      }
          ...
          if (nl == p_owner) {
            ...
            if (state.is_valid()) {
              ...
              if (from_node >= 0 && to_node >= 0) {
                // this one has state for this node, save
                if (state->is_connection(from_node, c.signal.get_name(),
                                         to_node, c.callable.get_method())) {
                  exists2 = true;
                  break;
                }
              }
            ...
            if (nl->get_filename() != String()) {
              ...
              if (state.is_valid()) {
                int from_node =
                    state->find_node_by_path(nl->get_path_to(p_node));
                int to_node = state->find_node_by_path(nl->get_path_to(target));
...
int SceneState::_find_base_scene_node_remap_key(int p_idx) const {
  ...
  for (Map<int, int>::Element *E = base_scene_node_remap.front(); E;
       E = E->next()) {
    if (E->value() == p_idx) {
      return E->key();
    }
...
Variant SceneState::get_property_value(int p_node, const StringName &p_property,
                                       bool &found) const {
  ...
  if (base_scene_node_remap.has(p_node)) {
    return _get_base_scene_state()->get_property_value(
        base_scene_node_remap[p_node], p_property, found);
  }
...
bool SceneState::is_node_in_group(int p_node, const StringName &p_group) const {
  ...
  if (base_scene_node_remap.has(p_node)) {
    return _get_base_scene_state()->is_node_in_group(
        base_scene_node_remap[p_node], p_group);
  }
...
bool SceneState::is_connection(int p_node, const StringName &p_signal,
                               int p_to_node,
                               const StringName &p_to_method) const {
    ...
    if (signal_idx >= 0 && method_idx >= 0) {
      ...
      for (int i = 0; i < connections.size(); i++) {
        ...
        if (connections[i].from == p_node && connections[i].to == p_to_node &&
            connections[i].signal == signal_idx &&
            connections[i].method == method_idx) {
  ...
  if (base_scene_node_remap.has(p_node) &&
      base_scene_node_remap.has(p_to_node)) {
    return _get_base_scene_state()->is_connection(
        base_scene_node_remap[p_node], p_signal,
        base_scene_node_remap[p_to_node], p_to_method);
  }
...
void SceneState::set_bundled_scene(const Dictionary &p_dictionary) {
  ...
  ERR_FAIL_COND_MSG(version > PACKED_SCENE_VERSION,
                    "Save format version too new.");
  ...
  for (int i = 0; i < nodes.size(); i++) {
    ...
    if (nd.index <
      ...
      name_index |= uint32_t(nd.index + 1)
                    << NAME_INDEX_BITS; // for backwards compatibility, index 0
                                        // is no index
    }
...
bool SceneState::is_node_instance_placeholder(int p_idx) const {
  ...
  return nodes[p_idx].instance >= 0 &&
         (nodes[p_idx].instance & FLAG_INSTANCE_IS_PLACEHOLDER);
}
...
bool SceneState::has_connection(const NodePath &p_node_from,
  ...
  do {
    for (int i = 0; i < ss->connections.size(); i++) {
      ...
      if (np_from == p_node_from && sn_signal == p_signal &&
          np_to == p_node_to && sn_method == p_method) {
        return true;
      }
    }
...
int SceneState::add_node(int p_parent, int p_owner, int p_type, int p_name,
                         int p_instance, int p_index) {
...
void SceneState::add_connection(int p_from, int p_to, int p_signal,
                                int p_method, int p_flags,
...
void SceneState::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("get_node_type", "idx"),
                       &SceneState::get_node_type);
  ClassDB::bind_method(D_METHOD("get_node_name", "idx"),
                       &SceneState::get_node_name);
  ClassDB::bind_method(D_METHOD("get_node_path", "idx", "for_parent"),
                       &SceneState::get_node_path, DEFVAL(false));
  ClassDB::bind_method(D_METHOD("get_node_owner_path", "idx"),
                       &SceneState::get_node_owner_path);
  ClassDB::bind_method(D_METHOD("is_node_instance_placeholder", "idx"),
                       &SceneState::is_node_instance_placeholder);
  ClassDB::bind_method(D_METHOD("get_node_instance_placeholder", "idx"),
                       &SceneState::get_node_instance_placeholder);
  ClassDB::bind_method(D_METHOD("get_node_instance", "idx"),
                       &SceneState::get_node_instance);
  ClassDB::bind_method(D_METHOD("get_node_groups", "idx"),
                       &SceneState::_get_node_groups);
  ClassDB::bind_method(D_METHOD("get_node_index", "idx"),
                       &SceneState::get_node_index);
  ClassDB::bind_method(D_METHOD("get_node_property_count", "idx"),
                       &SceneState::get_node_property_count);
  ClassDB::bind_method(D_METHOD("get_node_property_name", "idx", "prop_idx"),
                       &SceneState::get_node_property_name);
  ClassDB::bind_method(D_METHOD("get_node_property_value", "idx", "prop_idx"),
                       &SceneState::get_node_property_value);
  ClassDB::bind_method(D_METHOD("get_connection_count"),
                       &SceneState::get_connection_count);
  ClassDB::bind_method(D_METHOD("get_connection_source", "idx"),
                       &SceneState::get_connection_source);
  ClassDB::bind_method(D_METHOD("get_connection_signal", "idx"),
                       &SceneState::get_connection_signal);
  ClassDB::bind_method(D_METHOD("get_connection_target", "idx"),
                       &SceneState::get_connection_target);
  ClassDB::bind_method(D_METHOD("get_connection_method", "idx"),
                       &SceneState::get_connection_method);
  ClassDB::bind_method(D_METHOD("get_connection_flags", "idx"),
                       &SceneState::get_connection_flags);
  ClassDB::bind_method(D_METHOD("get_connection_binds", "idx"),
...
bool PackedScene::can_instance() const { return state->can_instance(); }

Node *PackedScene::instance(GenEditState p_edit_state) const {

#ifndef TOOLS_ENABLED
  ERR_FAIL_COND_V_MSG(
      p_edit_state != GEN_EDIT_STATE_DISABLED, NULL,
...
void PackedScene::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("instance", "edit_state"),
                       &PackedScene::instance, DEFVAL(GEN_EDIT_STATE_DISABLED));
  ClassDB::bind_method(D_METHOD("can_instance"), &PackedScene::can_instance);
  ClassDB::bind_method(D_METHOD("_set_bundled_scene"),
                       &PackedScene::_set_bundled_scene);
  ClassDB::bind_method(D_METHOD("_get_bundled_scene"),
                       &PackedScene::_get_bundled_scene);
  ClassDB::bind_method(D_METHOD("get_state"), &PackedScene::get_state);

  ADD_PROPERTY(PropertyInfo(Variant::DICTIONARY, "_bundled"),
               "_set_bundled_scene", "_get_bundled_scene");
