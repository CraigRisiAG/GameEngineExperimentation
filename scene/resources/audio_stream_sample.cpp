...
void AudioStreamPlaybackSample::stop() { active = false; }

bool AudioStreamPlaybackSample::is_playing() const { return active; }

int AudioStreamPlaybackSample::get_loop_count() const { return 0; }
...
void AudioStreamPlaybackSample::do_resample(const Depth *p_src,
                                            AudioFrame *p_dst, int64_t &offset,
                                            int32_t &increment, uint32_t amount,
    ...
    if (is_ima_adpcm) {
      ...
      while (sample_pos > ima_adpcm[0].last_nibble) {
        ...
        for (int i = 0; i < (is_stereo ? 2 : 1); i++) {
          ...
          uint8_t nbb =
              src_ptr[(ima_adpcm[i].last_nibble >> 1) * (is_stereo ? 2 : 1) +
                      i];
...
void AudioStreamPlaybackSample::mix(AudioFrame *p_buffer, float p_rate_scale,
                                    int p_frames) {
  ...
  int64_t begin_limit =
      (base->loop_mode != AudioStreamSample::LOOP_DISABLED) ? loop_begin_fp : 0;
  ...
  while (todo > 0) {
    ...
    case AudioStreamSample::FORMAT_8_BITS: {
      ...
      if (is_stereo)
        do_resample<int8_t, true, false>((int8_t *)data, dst_buff, offset,
                                         increment, target, ima_adpcm);
      else
        do_resample<int8_t, false, false>((int8_t *)data, dst_buff, offset,
                                          increment, target, ima_adpcm);
    } break;
    case AudioStreamSample::FORMAT_16_BITS: {
      if (is_stereo)
        do_resample<int16_t, true, false>((int16_t *)data, dst_buff, offset,
                                          increment, target, ima_adpcm);
      else
        do_resample<int16_t, false, false>((int16_t *)data, dst_buff, offset,
                                           increment, target, ima_adpcm);

    } break;
    case AudioStreamSample::FORMAT_IMA_ADPCM: {
      if (is_stereo)
        do_resample<int8_t, true, true>((int8_t *)data, dst_buff, offset,
                                        increment, target, ima_adpcm);
      else
        do_resample<int8_t, false, true>((int8_t *)data, dst_buff, offset,
                                         increment, target, ima_adpcm);
...
void AudioStreamSample::set_format(Format p_format) { format = p_format; }
...
void AudioStreamSample::set_loop_begin(int p_frame) { loop_begin = p_frame; }
int AudioStreamSample::get_loop_begin() const { return loop_begin; }

void AudioStreamSample::set_loop_end(int p_frame) { loop_end = p_frame; }
int AudioStreamSample::get_loop_end() const { return loop_end; }
...
int AudioStreamSample::get_mix_rate() const { return mix_rate; }
void AudioStreamSample::set_stereo(bool p_enable) { stereo = p_enable; }
bool AudioStreamSample::is_stereo() const { return stereo; }
...
void AudioStreamSample::set_data(const Vector<uint8_t> &p_data) {
  ...
  if (datalen) {
    ...
    data = AudioServer::get_singleton()->audio_data_alloc(
        alloc_len); // alloc with some padding for interpolation
...
Error AudioStreamSample::save_to_wav(const String &p_path) {
  ...
  FileAccessRef file = FileAccess::open(
      file_path, FileAccess::WRITE); // Overrides existing file if present

  ERR_FAIL_COND_V(!file, ERR_FILE_CANT_WRITE);

  // Create WAV Header
  file->store_string("RIFF"); // ChunkID
  file->store_32(sub_chunk_2_size +
                 36); // ChunkSize = 36 + SubChunk2Size (size of entire file
                      // minus the 8 bits for this and previous header)
  ...
  file->store_16(n_channels *
                 byte_pr_sample);     // BlockAlign = NumChannels * BytePrSample
...
String AudioStreamSample::get_stream_name() const { return ""; }
...
void AudioStreamSample::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("set_loop_mode", "loop_mode"),
                       &AudioStreamSample::set_loop_mode);
  ClassDB::bind_method(D_METHOD("get_loop_mode"),
                       &AudioStreamSample::get_loop_mode);

  ClassDB::bind_method(D_METHOD("set_loop_begin", "loop_begin"),
                       &AudioStreamSample::set_loop_begin);
  ClassDB::bind_method(D_METHOD("get_loop_begin"),
                       &AudioStreamSample::get_loop_begin);

  ClassDB::bind_method(D_METHOD("set_loop_end", "loop_end"),
                       &AudioStreamSample::set_loop_end);
  ClassDB::bind_method(D_METHOD("get_loop_end"),
  ...
  ClassDB::bind_method(D_METHOD("save_to_wav", "path"),
  ...
  ADD_PROPERTY(PropertyInfo(Variant::INT, "loop_mode", PROPERTY_HINT_ENUM,
               ...
               "set_loop_mode", "get_loop_mode");
  ADD_PROPERTY(PropertyInfo(Variant::INT, "loop_begin"), "set_loop_begin",
               "get_loop_begin");
  ADD_PROPERTY(PropertyInfo(Variant::INT, "loop_end"), "set_loop_end",
               "get_loop_end");
