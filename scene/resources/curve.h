...
public:
  ...
  enum TangentMode { TANGENT_FREE = 0, TANGENT_LINEAR, TANGENT_MODE_COUNT };
  ...
  struct Point {
    ...
    Point(Vector2 p_pos, real_t p_left = 0, real_t p_right = 0,
  ...
  int add_point(Vector2 p_pos, real_t left_tangent = 0,
                real_t right_tangent = 0, TangentMode left_mode = TANGENT_FREE,
...
class Curve2D : public Resource {
  ...
  void _bake_segment2d(Map<float, Vector2> &r_bake, float p_begin, float p_end,
                       const Vector2 &p_a, const Vector2 &p_out,
                       const Vector2 &p_b, const Vector2 &p_in, int p_depth,
                       int p_max_depth, float p_tol) const;
...
public:
  ...
  void add_point(const Vector2 &p_pos, const Vector2 &p_in = Vector2(),
  ...
  PackedVector2Array
  tessellate(int p_max_stages = 5,
             float p_tolerance = 4) const; // useful for display
...
class Curve3D : public Resource {
  ...
  void _bake_segment3d(Map<float, Vector3> &r_bake, float p_begin, float p_end,
                       const Vector3 &p_a, const Vector3 &p_out,
                       const Vector3 &p_b, const Vector3 &p_in, int p_depth,
                       int p_max_depth, float p_tol) const;
  Dictionary _get_data() const;
  void _set_data(const Dictionary &p_data);
...
public:
  int get_point_count() const;
  void add_point(const Vector3 &p_pos, const Vector3 &p_in = Vector3(),
  ...
  Vector3 interpolate_baked_up_vector(float p_offset,
                                      bool p_apply_tilt = false) const;
