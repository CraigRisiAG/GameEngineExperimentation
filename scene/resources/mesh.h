...
public:
  ...
  enum ArrayFormat {
    ...
    ARRAY_COMPRESS_DEFAULT = ARRAY_COMPRESS_NORMAL | ARRAY_COMPRESS_TANGENT |
        ARRAY_COMPRESS_COLOR | ARRAY_COMPRESS_TEX_UV | ARRAY_COMPRESS_TEX_UV2
  ...
  virtual void surface_set_material(int p_idx,
                                    const Ref<Material> &p_material) = 0;
  ...
  typedef Vector<Vector<Face3>> (*ConvexDecompositionFunc)(
      const Vector<Face3> &);
...
public:
  ...
  void add_surface(
      uint32_t p_format, PrimitiveType p_primitive,
      const Vector<uint8_t> &p_array, int p_vertex_count,
      const Vector<uint8_t> &p_index_array, int p_index_count,
      const AABB &p_aabb,
      const Vector<Vector<uint8_t>> &p_blend_shapes = Vector<Vector<uint8_t>>(),
      const Vector<AABB> &p_bone_aabbs = Vector<AABB>(),
      const Vector<VS::SurfaceData::LOD> &p_lods =
          Vector<VS::SurfaceData::LOD>());
  ...
  void surface_update_region(int p_surface, int p_offset,
  ...
  void surface_set_custom_aabb(int p_idx,
                               const AABB &p_aabb); // only recognized by driver
  ...
  Error lightmap_unwrap(const Transform &p_base_transform = Transform(),
