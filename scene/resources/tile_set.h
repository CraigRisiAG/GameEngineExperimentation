...
public:
  ...
  enum BitmaskMode { BITMASK_2X2, BITMASK_3X3_MINIMAL, BITMASK_3X3 };
  ...
  enum TileMode { SINGLE_TILE, AUTO_TILE, ATLAS_TILE };
  ...
  struct AutotileData {
    ...
    explicit AutotileData()
        : bitmask_mode(BITMASK_2X2), size(64, 64), spacing(0),
          icon_coord(0, 0) {}
  };
...
private:
  struct TileData {
    ...
    explicit TileData()
        : tile_mode(SINGLE_TILE), modulate(1, 1, 1), z_index(0) {}
  };
...
public:
  ...
  void autotile_set_subtile_priority(int p_id, const Vector2 &p_coord,
                                     int p_priority);
  ...
  Vector2
  autotile_get_subtile_for_bitmask(int p_id, uint16_t p_bitmask,
                                   const Node *p_tilemap_node = NULL,
                                   const Vector2 &p_tile_location = Vector2());
  Vector2
  atlastile_get_subtile_by_priority(int p_id, const Node *p_tilemap_node = NULL,
                                    const Vector2 &p_tile_location = Vector2());
  ...
  void tile_set_shape_transform(int p_id, int p_shape_id,
  ...
  void tile_add_shape(int p_id, const Ref<Shape2D> &p_shape,
                      const Transform2D &p_transform, bool p_one_way = false,
  ...
  autotile_get_navigation_polygon(int p_id, const Vector2 &p_coord) const;
  const Map<Vector2, Ref<NavigationPolygon>> &
  autotile_get_navigation_map(int p_id) const;
