...
bool SegmentShape2D::_edit_is_selected_on_click(const Point2 &p_point,
                                                double p_tolerance) const {
...
void SegmentShape2D::draw(const RID &p_to_rid, const Color &p_color) {
  ...
  VisualServer::get_singleton()->canvas_item_add_line(p_to_rid, a, b, p_color,
                                                      3);
}
...
void RayShape2D::draw(const RID &p_to_rid, const Color &p_color) {
  ...
  VS::get_singleton()->canvas_item_add_line(p_to_rid, Vector2(), tip, p_color,
                                            3);
  Vector<Vector2> pts;
  float tsize = 4;
  ...
  VS::get_singleton()->canvas_item_add_primitive(p_to_rid, pts, cols,
                                                 Vector<Point2>(), RID());
}
...
real_t RayShape2D::get_enclosing_radius() const { return length; }
...
void RayShape2D::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("set_slips_on_slope", "active"),
                       &RayShape2D::set_slips_on_slope);
  ClassDB::bind_method(D_METHOD("get_slips_on_slope"),
                       &RayShape2D::get_slips_on_slope);

  ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "length"), "set_length",
               "get_length");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "slips_on_slope"),
               "set_slips_on_slope", "get_slips_on_slope");
}
...
real_t RayShape2D::get_length() const { return length; }
...
bool RayShape2D::get_slips_on_slope() const { return slips_on_slope; }
