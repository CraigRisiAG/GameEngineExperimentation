...
void Theme::_emit_theme_changed() { emit_changed(); }
...
void Theme::_get_property_list(List<PropertyInfo> *p_list) const {
  ...
  while ((key = icon_map.next(key))) {
    ...
    while ((key2 = icon_map[*key].next(key2))) {
      ...
      list.push_back(
          PropertyInfo(Variant::OBJECT, String() + *key + "/icons/" + *key2,
  ...
  while ((key = style_map.next(key))) {
    ...
    while ((key2 = style_map[*key].next(key2))) {
      ...
      list.push_back(
          PropertyInfo(Variant::OBJECT, String() + *key + "/styles/" + *key2,
                       PROPERTY_HINT_RESOURCE_TYPE, "StyleBox",
                       PROPERTY_USAGE_DEFAULT | PROPERTY_USAGE_STORE_IF_NULL));
    }
  }
  ...
  while ((key = font_map.next(key))) {
    ...
    while ((key2 = font_map[*key].next(key2))) {
      ...
      list.push_back(
          PropertyInfo(Variant::OBJECT, String() + *key + "/fonts/" + *key2,
                       PROPERTY_HINT_RESOURCE_TYPE, "Font",
                       PROPERTY_USAGE_DEFAULT | PROPERTY_USAGE_STORE_IF_NULL));
    }
  }
  ...
  while ((key = color_map.next(key))) {
    ...
    while ((key2 = color_map[*key].next(key2))) {
      ...
      list.push_back(
          PropertyInfo(Variant::COLOR, String() + *key + "/colors/" + *key2));
    }
  }
  ...
  while ((key = constant_map.next(key))) {
    ...
    while ((key2 = constant_map[*key].next(key2))) {
      ...
      list.push_back(
          PropertyInfo(Variant::INT, String() + *key + "/constants/" + *key2));
    }
  }
...
void Theme::set_default_theme_font(const Ref<Font> &p_default_font) {
  ...
  if (default_theme_font.is_valid()) {
    default_theme_font->disconnect(
        "changed", callable_mp(this, &Theme::_emit_theme_changed));
  }

  default_theme_font = p_default_font;

  if (default_theme_font.is_valid()) {
    default_theme_font->connect("changed",
...
Ref<Font> Theme::get_default_theme_font() const { return default_theme_font; }
...
Ref<Theme> Theme::get_default() { return default_theme; }
...
Ref<Theme> Theme::get_project_default() { return project_default_theme; }
...
void Theme::set_default_font(const Ref<Font> &p_font) { default_font = p_font; }

void Theme::set_icon(const StringName &p_name, const StringName &p_type,
  ...
  if (icon_map[p_type].has(p_name) && icon_map[p_type][p_name].is_valid()) {
    icon_map[p_type][p_name]->disconnect(
        "changed", callable_mp(this, &Theme::_emit_theme_changed));
  }
  ...
  if (p_icon.is_valid()) {
    icon_map[p_type][p_name]->connect(
        "changed", callable_mp(this, &Theme::_emit_theme_changed), varray(),
...
Ref<Texture2D> Theme::get_icon(const StringName &p_name,
                               const StringName &p_type) const {

  if (icon_map.has(p_type) && icon_map[p_type].has(p_name) &&
      icon_map[p_type][p_name].is_valid()) {
...
void Theme::set_shader(const StringName &p_name, const StringName &p_type,
...
Ref<Shader> Theme::get_shader(const StringName &p_name,
                              const StringName &p_type) const {
  if (shader_map.has(p_type) && shader_map[p_type].has(p_name) &&
      shader_map[p_type][p_name].is_valid()) {
...
bool Theme::has_shader(const StringName &p_name,
                       const StringName &p_type) const {
  return (shader_map.has(p_type) && shader_map[p_type].has(p_name) &&
          shader_map[p_type][p_name].is_valid());
}
...
void Theme::get_shader_list(const StringName &p_type,
                            List<StringName> *p_list) const {
...
void Theme::set_stylebox(const StringName &p_name, const StringName &p_type,
  ...
  if (style_map[p_type].has(p_name) && style_map[p_type][p_name].is_valid()) {
    style_map[p_type][p_name]->disconnect(
        "changed", callable_mp(this, &Theme::_emit_theme_changed));
  }
  ...
  if (p_style.is_valid()) {
    style_map[p_type][p_name]->connect(
        "changed", callable_mp(this, &Theme::_emit_theme_changed), varray(),
        CONNECT_REFERENCE_COUNTED);
  }
...
Ref<StyleBox> Theme::get_stylebox(const StringName &p_name,
                                  const StringName &p_type) const {

  if (style_map.has(p_type) && style_map[p_type].has(p_name) &&
      style_map[p_type][p_name].is_valid()) {
...
bool Theme::has_stylebox(const StringName &p_name,
                         const StringName &p_type) const {
...
void Theme::get_stylebox_list(StringName p_type,
                              List<StringName> *p_list) const {
...
void Theme::set_font(const StringName &p_name, const StringName &p_type,
  ...
  if (font_map[p_type][p_name].is_valid()) {
    font_map[p_type][p_name]->disconnect(
        "changed", callable_mp(this, &Theme::_emit_theme_changed));
  }
  ...
  if (p_font.is_valid()) {
    font_map[p_type][p_name]->connect(
        "changed", callable_mp(this, &Theme::_emit_theme_changed), varray(),
        CONNECT_REFERENCE_COUNTED);
  }
...
Ref<Font> Theme::get_font(const StringName &p_name,
                          const StringName &p_type) const {

  if (font_map.has(p_type) && font_map[p_type].has(p_name) &&
      font_map[p_type][p_name].is_valid())
...
void Theme::set_color(const StringName &p_name, const StringName &p_type,
                      const Color &p_color) {
...
bool Theme::has_color(const StringName &p_name,
                      const StringName &p_type) const {
...
void Theme::set_constant(const StringName &p_name, const StringName &p_type,
                         int p_constant) {

  bool new_value =
      !constant_map.has(p_type) || !constant_map[p_type].has(p_name);
...
int Theme::get_constant(const StringName &p_name,
                        const StringName &p_type) const {
...
bool Theme::has_constant(const StringName &p_name,
                         const StringName &p_type) const {
...
void Theme::get_constant_list(StringName p_type,
                              List<StringName> *p_list) const {
...
void Theme::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("set_icon", "name", "type", "texture"),
                       &Theme::set_icon);
  ...
  ClassDB::bind_method(D_METHOD("clear_icon", "name", "type"),
                       &Theme::clear_icon);
  ClassDB::bind_method(D_METHOD("get_icon_list", "type"),
                       &Theme::_get_icon_list);

  ClassDB::bind_method(D_METHOD("set_stylebox", "name", "type", "texture"),
                       &Theme::set_stylebox);
  ClassDB::bind_method(D_METHOD("get_stylebox", "name", "type"),
                       &Theme::get_stylebox);
  ClassDB::bind_method(D_METHOD("has_stylebox", "name", "type"),
                       &Theme::has_stylebox);
  ClassDB::bind_method(D_METHOD("clear_stylebox", "name", "type"),
                       &Theme::clear_stylebox);
  ClassDB::bind_method(D_METHOD("get_stylebox_list", "type"),
                       &Theme::_get_stylebox_list);
  ClassDB::bind_method(D_METHOD("get_stylebox_types"),
                       &Theme::_get_stylebox_types);

  ClassDB::bind_method(D_METHOD("set_font", "name", "type", "font"),
                       &Theme::set_font);
  ...
  ClassDB::bind_method(D_METHOD("clear_font", "name", "type"),
                       &Theme::clear_font);
  ClassDB::bind_method(D_METHOD("get_font_list", "type"),
                       &Theme::_get_font_list);

  ClassDB::bind_method(D_METHOD("set_color", "name", "type", "color"),
                       &Theme::set_color);
  ClassDB::bind_method(D_METHOD("get_color", "name", "type"),
                       &Theme::get_color);
  ClassDB::bind_method(D_METHOD("has_color", "name", "type"),
                       &Theme::has_color);
  ClassDB::bind_method(D_METHOD("clear_color", "name", "type"),
                       &Theme::clear_color);
  ClassDB::bind_method(D_METHOD("get_color_list", "type"),
                       &Theme::_get_color_list);

  ClassDB::bind_method(D_METHOD("set_constant", "name", "type", "constant"),
                       &Theme::set_constant);
  ClassDB::bind_method(D_METHOD("get_constant", "name", "type"),
                       &Theme::get_constant);
  ClassDB::bind_method(D_METHOD("has_constant", "name", "type"),
                       &Theme::has_constant);
  ClassDB::bind_method(D_METHOD("clear_constant", "name", "type"),
                       &Theme::clear_constant);
  ClassDB::bind_method(D_METHOD("get_constant_list", "type"),
                       &Theme::_get_constant_list);

  ClassDB::bind_method(D_METHOD("clear"), &Theme::clear);

  ClassDB::bind_method(D_METHOD("set_default_font", "font"),
                       &Theme::set_default_theme_font);
  ClassDB::bind_method(D_METHOD("get_default_font"),
                       &Theme::get_default_theme_font);

  ClassDB::bind_method(D_METHOD("get_type_list", "type"),
                       &Theme::_get_type_list);

  ClassDB::bind_method("copy_default_theme", &Theme::copy_default_theme);
  ClassDB::bind_method(D_METHOD("copy_theme", "other"), &Theme::copy_theme);

  ADD_PROPERTY(PropertyInfo(Variant::OBJECT, "default_font",
                            PROPERTY_HINT_RESOURCE_TYPE, "Font"),
