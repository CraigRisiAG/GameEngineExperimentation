...
void Shader::set_default_texture_param(const StringName &p_param,
                                       const Ref<Texture2D> &p_texture) {

  if (p_texture.is_valid()) {
    default_textures[p_param] = p_texture;
    VS::get_singleton()->shader_set_default_texture_param(shader, p_param,
                                                          p_texture->get_rid());
  } else {
...
Ref<Texture2D>
Shader::get_default_texture_param(const StringName &p_param) const {
...
void Shader::get_default_texture_param_list(
    List<StringName> *r_textures) const {

  for (const Map<StringName, Ref<Texture2D>>::Element *E =
           default_textures.front();
       E; E = E->next()) {
...
void Shader::_bind_methods() {
  ...
  ClassDB::bind_method(
      D_METHOD("set_default_texture_param", "param", "texture"),
      &Shader::set_default_texture_param);
  ClassDB::bind_method(D_METHOD("get_default_texture_param", "param"),
  ...
  ADD_PROPERTY(PropertyInfo(Variant::STRING, "code", PROPERTY_HINT_NONE, "",
...
Shader::~Shader() { VisualServer::get_singleton()->free(shader); }
////////////

RES ResourceFormatLoaderShader::load(const String &p_path,
...
void ResourceFormatLoaderShader::get_recognized_extensions(
...
String
ResourceFormatLoaderShader::get_resource_type(const String &p_path) const {
...
Error ResourceFormatSaverShader::save(const String &p_path,
