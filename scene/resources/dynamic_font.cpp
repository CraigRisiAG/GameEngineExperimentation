...
Ref<DynamicFontAtSize>
DynamicFontData::_get_dynamic_font_at_size(CacheID p_cache_id) {
...
void DynamicFontData::set_font_ptr(const uint8_t *p_font_mem,
                                   int p_font_mem_size) {
...
String DynamicFontData::get_font_path() const { return font_path; }
...
void DynamicFontData::_bind_methods() {
  ClassDB::bind_method(D_METHOD("set_antialiased", "antialiased"),
                       &DynamicFontData::set_antialiased);
  ClassDB::bind_method(D_METHOD("is_antialiased"),
                       &DynamicFontData::is_antialiased);
  ClassDB::bind_method(D_METHOD("set_font_path", "path"),
                       &DynamicFontData::set_font_path);
  ClassDB::bind_method(D_METHOD("get_font_path"),
                       &DynamicFontData::get_font_path);
  ClassDB::bind_method(D_METHOD("set_hinting", "mode"),
                       &DynamicFontData::set_hinting);
  ClassDB::bind_method(D_METHOD("get_hinting"), &DynamicFontData::get_hinting);

  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "antialiased"), "set_antialiased",
               "is_antialiased");
  ADD_PROPERTY(PropertyInfo(Variant::INT, "hinting", PROPERTY_HINT_ENUM,
                            "None,Light,Normal"),
               "set_hinting", "get_hinting");

  BIND_ENUM_CONSTANT(HINTING_NONE);
  BIND_ENUM_CONSTANT(HINTING_LIGHT);
  BIND_ENUM_CONSTANT(HINTING_NORMAL);

  ADD_PROPERTY(PropertyInfo(Variant::STRING, "font_path", PROPERTY_HINT_FILE,
                            "*.ttf,*.otf"),
               "set_font_path", "get_font_path");
}
...
Error DynamicFontAtSize::_load() {
  ...
  ERR_FAIL_COND_V_MSG(error != 0, ERR_CANT_CREATE,
                      "Error initializing FreeType.");

  // FT_OPEN_STREAM is extremely slow only on Android.
  if (OS::get_singleton()->get_name() == "Android" && font->font_mem == NULL &&
      font->font_path != String()) {
    // cache font only once for each font->font_path
    if (_fontdata.has(font->font_path)) {

      font->set_font_ptr(_fontdata[font->font_path].ptr(),
                         _fontdata[font->font_path].size());
  ...
  if (FT_HAS_COLOR(face) && face->num_fixed_sizes > 0) {
    ...
    for (int i = 1; i < face->num_fixed_sizes; i++) {
      ...
      if (ndiff < diff) {
        ...
        diff = ndiff;
        scale_color_font =
            float(id.size * oversampling) / face->available_sizes[i].width;
      }
    }
  ...
  descent =
      (-face->size->metrics.descender / 64.0) / oversampling * scale_color_font;
  linegap = 0;
...
float DynamicFontAtSize::get_height() const { return ascent + descent; }

float DynamicFontAtSize::get_ascent() const { return ascent; }
float DynamicFontAtSize::get_descent() const { return descent; }

const Pair<const DynamicFontAtSize::Character *, DynamicFontAtSize *>
DynamicFontAtSize::_find_char_with_font(
    CharType p_char, const Vector<Ref<DynamicFontAtSize>> &p_fallbacks) const {
  ...
  return Pair<const Character *, DynamicFontAtSize *>(
      chr, const_cast<DynamicFontAtSize *>(this));
}

Size2 DynamicFontAtSize::get_char_size(
    CharType p_char, CharType p_next,
    const Vector<Ref<DynamicFontAtSize>> &p_fallbacks) const {
...
float DynamicFontAtSize::draw_char(
    RID p_canvas_item, const Point2 &p_pos, CharType p_char, CharType p_next,
    const Color &p_modulate, const Vector<Ref<DynamicFontAtSize>> &p_fallbacks,
    bool p_advance_only, bool p_outline) const {
  ...
  if (p_outline && !ch->found) {
    ...
    int error = FT_Load_Char(
        face, p_char, FT_HAS_COLOR(face) ? FT_LOAD_COLOR : FT_LOAD_DEFAULT);
    if (!error) {
      error = FT_Render_Glyph(face->glyph, FT_RENDER_MODE_NORMAL);
      if (!error) {
        Character character = Character::not_found();
        character = const_cast<DynamicFontAtSize *>(this)->_bitmap_to_character(
            slot->bitmap, slot->bitmap_top, slot->bitmap_left,
            slot->advance.x / 64.0);
  ...
  if (ch->found) {
    ERR_FAIL_COND_V(
        ch->texture_idx < -1 || ch->texture_idx >= font->textures.size(), 0);
    ...
    if (!p_advance_only && ch->texture_idx != -1) {
      ...
      VisualServer::get_singleton()->canvas_item_add_texture_rect_region(
          p_canvas_item, Rect2(cpos, ch->rect.size), texture, ch->rect_uv,
          modulate, false, RID(), RID(), Color(1, 1, 1, 1), false);
    }
...
unsigned long DynamicFontAtSize::_ft_stream_io(FT_Stream stream,
...
DynamicFontAtSize::TexturePosition
DynamicFontAtSize::_find_texture_pos_for_glyph(int p_color_size,
  ...
  for (int i = 0; i < textures.size(); i++) {
    ...
    if (mw > ct.texture_size || mh > ct.texture_size) // too big for this
                                                      // texture
      continue;
  ...
  if (ret.index == -1) {
...

    {
      ...
      ERR_FAIL_COND_V(texsize * texsize * p_color_size > tex.imgdata.size(),
                      ret);
...
DynamicFontAtSize::Character
DynamicFontAtSize::_bitmap_to_character(FT_Bitmap bitmap, int yofs, int xofs,
                                        float advance) {
  int w = bitmap.width;
  int h = bitmap.rows;
...

  {
    ...
    for (int i = 0; i < h; i++) {
      for (int j = 0; j < w; j++) {
        ...
        default:
          ERR_FAIL_V_MSG(Character::not_found(),
  ...
  // blit to image and texture
  {
    ...
    Ref<Image> img = memnew(Image(tex.texture_size, tex.texture_size, 0,
                                  require_format, tex.imgdata));
  ...
  chr.v_align =
      ascent - (yofs * scale_color_font / oversampling); // + ascent - descent;
...
DynamicFontAtSize::Character
DynamicFontAtSize::_make_outline_char(CharType p_char) {
  Character ret = Character::not_found();

  if (FT_Load_Char(face, p_char,
                   FT_LOAD_NO_BITMAP |
  ...
  FT_Stroker_Set(stroker, (int)(id.outline_size * oversampling * 64.0),
  ...
  ret = _bitmap_to_character(glyph_bitmap->bitmap, glyph_bitmap->top,
                             glyph_bitmap->left, glyph->advance.x / 65536.0);
...
void DynamicFontAtSize::_update_char(CharType p_char) {
  ...
  if (id.outline_size > 0) {
    ...
    error =
        FT_Render_Glyph(face->glyph, font->antialiased ? FT_RENDER_MODE_NORMAL
                                                       : FT_RENDER_MODE_MONO);
    if (!error)
      character =
          _bitmap_to_character(slot->bitmap, slot->bitmap_top,
                               slot->bitmap_left, slot->advance.x / 64.0);
  }
...
Ref<DynamicFontData> DynamicFont::get_font_data() const { return data; }
...
int DynamicFont::get_size() const { return cache_id.size; }
...
Color DynamicFont::get_outline_color() const { return outline_color; }

bool DynamicFontData::is_antialiased() const { return antialiased; }
...
bool DynamicFont::is_distance_field_hint() const { return false; }

bool DynamicFont::has_outline() const {
  return outline_cache_id.outline_size > 0;
}

float DynamicFont::draw_char(RID p_canvas_item, const Point2 &p_pos,
                             CharType p_char, CharType p_next,
                             const Color &p_modulate, bool p_outline) const {
  const Ref<DynamicFontAtSize> &font_at_size =
      p_outline && outline_cache_id.outline_size > 0 ? outline_data_at_size
                                                     : data_at_size;
  ...
  const Vector<Ref<DynamicFontAtSize>> &fallbacks =
      p_outline && outline_cache_id.outline_size > 0
          ? fallback_outline_data_at_size
          : fallback_data_at_size;
  Color color = p_outline && outline_cache_id.outline_size > 0
                    ? p_modulate * outline_color
                    : p_modulate;

  // If requested outline draw, but no outline is present, simply return advance
  // without drawing anything
  bool advance_only = p_outline && outline_cache_id.outline_size == 0;
  return font_at_size->draw_char(p_canvas_item, p_pos, p_char, p_next, color,
                                 fallbacks, advance_only, p_outline) +
         spacing_char;
}
...
void DynamicFont::add_fallback(const Ref<DynamicFontData> &p_data) {
  ...
  fallback_data_at_size.push_back(
...
int DynamicFont::get_fallback_count() const { return fallbacks.size(); }
...
void DynamicFont::_get_property_list(List<PropertyInfo> *p_list) const {
  ...
  for (int i = 0; i < fallbacks.size(); i++) {
    p_list->push_back(PropertyInfo(Variant::OBJECT, "fallback/" + itos(i),
                                   PROPERTY_HINT_RESOURCE_TYPE,
                                   "DynamicFontData"));
  }

  p_list->push_back(
      PropertyInfo(Variant::OBJECT, "fallback/" + itos(fallbacks.size()),
                   PROPERTY_HINT_RESOURCE_TYPE, "DynamicFontData"));
}

void DynamicFont::_bind_methods() {

  ClassDB::bind_method(D_METHOD("set_font_data", "data"),
  ...
  ClassDB::bind_method(D_METHOD("set_outline_size", "size"),
                       &DynamicFont::set_outline_size);
  ClassDB::bind_method(D_METHOD("get_outline_size"),
  ...
  ClassDB::bind_method(D_METHOD("set_spacing", "type", "value"),
                       &DynamicFont::set_spacing);
  ClassDB::bind_method(D_METHOD("get_spacing", "type"),
                       &DynamicFont::get_spacing);

  ClassDB::bind_method(D_METHOD("add_fallback", "data"),
                       &DynamicFont::add_fallback);
  ClassDB::bind_method(D_METHOD("set_fallback", "idx", "data"),
                       &DynamicFont::set_fallback);
  ClassDB::bind_method(D_METHOD("get_fallback", "idx"),
                       &DynamicFont::get_fallback);
  ClassDB::bind_method(D_METHOD("remove_fallback", "idx"),
                       &DynamicFont::remove_fallback);
  ClassDB::bind_method(D_METHOD("get_fallback_count"),
  ...
  ADD_PROPERTY(
      ...
      "set_size", "get_size");
  ADD_PROPERTY(PropertyInfo(Variant::INT, "outline_size", PROPERTY_HINT_RANGE,
                            "0,1024,1"),
               "set_outline_size", "get_outline_size");
  ADD_PROPERTY(PropertyInfo(Variant::COLOR, "outline_color"),
               "set_outline_color", "get_outline_color");
  ADD_GROUP("Extra Spacing", "extra_spacing");
  ADD_PROPERTYI(PropertyInfo(Variant::INT, "extra_spacing_top"), "set_spacing",
                "get_spacing", SPACING_TOP);
  ADD_PROPERTYI(PropertyInfo(Variant::INT, "extra_spacing_bottom"),
                "set_spacing", "get_spacing", SPACING_BOTTOM);
  ADD_PROPERTYI(PropertyInfo(Variant::INT, "extra_spacing_char"), "set_spacing",
                "get_spacing", SPACING_CHAR);
  ADD_PROPERTYI(PropertyInfo(Variant::INT, "extra_spacing_space"),
                "set_spacing", "get_spacing", SPACING_SPACE);
  ADD_GROUP("Font", "");
  ADD_PROPERTY(PropertyInfo(Variant::OBJECT, "font_data",
                            PROPERTY_HINT_RESOURCE_TYPE, "DynamicFontData"),
               "set_font_data", "get_font_data");
...
RES ResourceFormatLoaderDynamicFont::load(const String &p_path,
...
String
ResourceFormatLoaderDynamicFont::get_resource_type(const String &p_path) const {
