...
uint32_t SurfaceTool::VertexHasher::hash(const Vertex &p_vtx) {
  ...
  h = hash_djb2_buffer((const uint8_t *)p_vtx.bones.ptr(),
                       p_vtx.bones.size() * sizeof(int), h);
  h = hash_djb2_buffer((const uint8_t *)p_vtx.weights.ptr(),
                       p_vtx.weights.size() * sizeof(float), h);
  return h;
}
...
void SurfaceTool::add_triangle_fan(const Vector<Vector3> &p_vertices,
...
void SurfaceTool::_create_list(const Ref<Mesh> &p_existing, int p_surface,
                               List<Vertex> *r_vertex, List<int> *r_index,
                               int &lformat) {
...
Vector<SurfaceTool::Vertex>
SurfaceTool::create_vertex_array_from_triangle_arrays(const Array &p_arrays) {
  ...
  for (int i = 0; i < vc; i++) {
    ...
    if (lformat & VS::ARRAY_FORMAT_TANGENT) {
      Plane p(tarr[i * 4 + 0], tarr[i * 4 + 1], tarr[i * 4 + 2],
              tarr[i * 4 + 3]);
      v.tangent = p.normal;
...
void SurfaceTool::_create_list_from_arrays(Array arr, List<Vertex> *r_vertex,
                                           List<int> *r_index, int &lformat) {
...
void SurfaceTool::create_from_blend_shape(const Ref<Mesh> &p_existing,
                                          int p_surface,
...
void SurfaceTool::append_from(const Ref<Mesh> &p_existing, int p_surface,
...
int SurfaceTool::mikktGetNumVerticesOfFace(const SMikkTSpaceContext *pContext,
                                           const int iFace) {

  return 3; // always 3
}
void SurfaceTool::mikktGetPosition(const SMikkTSpaceContext *pContext,
...
void SurfaceTool::mikktGetNormal(const SMikkTSpaceContext *pContext,
...
void SurfaceTool::mikktGetTexCoord(const SMikkTSpaceContext *pContext,
...
void SurfaceTool::mikktSetTSpaceDefault(const SMikkTSpaceContext *pContext,
  ...
  if (vtx != NULL) {
    ...
    vtx->binormal = Vector3(
        -fvBiTangent[0], -fvBiTangent[1],
        -fvBiTangent[2]); // for some reason these are reversed, something with
                          // the coordinate system in Godot
  }
}
...
void SurfaceTool::generate_normals(bool p_flip) {
  ...
  for (List<Vertex>::Element *E = B; E;) {
    ...
    Vector3 normal;
    if (!p_flip)
      normal = Plane(v[0]->get().vertex, v[1]->get().vertex, v[2]->get().vertex)
                   .normal;
    else
      normal = Plane(v[2]->get().vertex, v[1]->get().vertex, v[0]->get().vertex)
                   .normal;
...
void SurfaceTool::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("add_vertex", "vertex"),
                       &SurfaceTool::add_vertex);
  ClassDB::bind_method(D_METHOD("add_color", "color"), &SurfaceTool::add_color);
  ClassDB::bind_method(D_METHOD("add_normal", "normal"),
                       &SurfaceTool::add_normal);
  ClassDB::bind_method(D_METHOD("add_tangent", "tangent"),
  ...
  ClassDB::bind_method(D_METHOD("add_weights", "weights"),
                       &SurfaceTool::add_weights);
  ClassDB::bind_method(D_METHOD("add_smooth_group", "smooth"),
                       &SurfaceTool::add_smooth_group);

  ClassDB::bind_method(D_METHOD("add_triangle_fan", "vertices", "uvs", "colors",
                                "uv2s", "normals", "tangents"),
                       &SurfaceTool::add_triangle_fan,
                       DEFVAL(Vector<Vector2>()), DEFVAL(Vector<Color>()),
                       DEFVAL(Vector<Vector2>()), DEFVAL(Vector<Vector3>()),
                       DEFVAL(Vector<Plane>()));
  ...
  ClassDB::bind_method(D_METHOD("generate_normals", "flip"),
                       &SurfaceTool::generate_normals, DEFVAL(false));
  ClassDB::bind_method(D_METHOD("generate_tangents"),
  ...
  ClassDB::bind_method(D_METHOD("create_from", "existing", "surface"),
                       &SurfaceTool::create_from);
  ClassDB::bind_method(
      D_METHOD("create_from_blend_shape", "existing", "surface", "blend_shape"),
      &SurfaceTool::create_from_blend_shape);
  ClassDB::bind_method(
      D_METHOD("append_from", "existing", "surface", "transform"),
      &SurfaceTool::append_from);
  ClassDB::bind_method(D_METHOD("commit", "existing", "flags"),
                       &SurfaceTool::commit, DEFVAL(Variant()),
                       DEFVAL(Mesh::ARRAY_COMPRESS_DEFAULT));
  ClassDB::bind_method(D_METHOD("commit_to_arrays"),
